...
import { MapPin, Calendar, Clock, Clipboard, UserCog, MoreHorizontal, Check, AlertCircle, Loader2 } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function EventInstancesList({
  ...
  const getPreparationStatusBadge = (status?: string) => {
    ...
    switch(status.toLowerCase()) {
      ...
      case 'ready':
        return <Badge variant="success" className="bg-green-100 text-green-800 hover:bg-green-100">Ready</Badge>;
      default:
        return <Badge>{status}</Badge>;
    }
  };
            ...
            events.map((event) => (
              <TableRow key={event.id}>
                ...
                <TableCell>
                  {event.fieldManager ? (
                    <div className="flex flex-col">
                      <div className="font-medium">{event.fieldManager.name}</div>
                  ...
                  <TooltipProvider>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        {!event.fieldManager && (
                          <DropdownMenuItem onClick={() => onAssignManager(event.id)}>
