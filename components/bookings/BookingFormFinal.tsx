...
export function BookingFormFinal({ onSubmit, onCancel, defaultValues }: BookingFormProps) {
  ...
  const availableActivities = [
    ...
    { value: "promotional_material", label: "Promotional Material Distribution" },
  ...
  const approvedLocations = [
    { id: "loc-001", name: "Green Therapeutics Dispensary", address: "123 Main Street, Los Angeles, CA", capacity: 50, status: "approved" },
    { id: "loc-002", name: "Healing Leaf Collective", address: "45 Market Avenue, San Francisco, CA", capacity: 35, status: "approved" },
    { id: "loc-003", name: "Herbal Wellness Center", address: "78 Highland Drive, Oakland, CA", capacity: 40, status: "approved" },
    { id: "loc-004", name: "Emerald City Cannabis", address: "10 Pacific Boulevard, San Diego, CA", capacity: 60, status: "approved" },
    { id: "loc-005", name: "Coastal Dispensary", address: "22 Ocean View, Santa Barbara, CA", capacity: 30, status: "approved" },
    { id: "loc-006", name: "Nature's Medicine", address: "55 Redwood Lane, Sacramento, CA", capacity: 45, status: "approved" },
    { id: "loc-007", name: "Evergreen Dispensary", address: "33 Valley Road, Phoenix, AZ", capacity: 50, status: "approved" },
    { id: "loc-008", name: "Pure Relief Dispensary", address: "67 Desert View, Tucson, AZ", capacity: 40, status: "approved" },
    { id: "loc-009", name: "Great Lakes Cannabis Co.", address: "12 Lakeshore Drive, Detroit, MI", capacity: 55, status: "approved" },
    { id: "loc-010", name: "Midwest Healing Center", address: "88 River Street, Grand Rapids, MI", capacity: 35, status: "approved" },
  ]
  ...
  const [selectedLocation, setLocationData] = useState<LocationDTO | null>(null)
  ...
  React.useEffect(() => {
    ...
    const subscription = form.watch((value, { name }) => {
      ...
      if (showQuickSettings && activeTab === "details" && 
          (name === "isRecurring" || name === "sendInvites")) {
        // If the user has turned on either option
        if ((name === "isRecurring" && form.getValues("isRecurring")) || 
  ...
  const generateTitle = (): string => {
    ...
    if (brand) {
      // Get the brand label 
      const brandLabel = availableBrands.find(b => b.value === brand)?.label || brand
      generatedTitle = brandLabel
      
      // Add event type if available
      if (eventType) {
        const eventLabel = eventTypes.find(type => type.value === eventType)?.label || eventType
        generatedTitle += ` - ${eventLabel}`
        
        // Add activity type if available
        if (activityType) {
          const activityLabel = activityTypes.find(type => type.value === activityType)?.label || activityType
        ...
        <div className="flex flex-col space-y-2 md:space-y-0 md:flex-row md:items-center md:justify-between">
          <div className="flex-1 group">
            <div className="flex items-center">
              <span className="text-sm font-medium text-muted-foreground mr-2">Event Title:</span>
              <span className="badge bg-primary/10 text-primary px-2 py-0.5 text-xs rounded">Live Preview</span>
            </div>
            <h2 
              ...
              <TabsContent value="details" className="mt-0 space-y-6">
                <div className="grid grid-cols-1 gap-6">
                  ...
                  {isInternalUser && (
                    <div className="space-y-2">
                      ...
                      {form.formState.errors.clientId && (
                        <p className="text-sm font-medium text-destructive">{form.formState.errors.clientId.message}</p>
                      )}
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue placeholder={availableBrands.length === 0 ? "Select a client first" : "Select a brand"} />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    {form.formState.errors.brand && (
                      <p className="text-sm font-medium text-destructive">{form.formState.errors.brand.message}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {form.formState.errors.eventType && (
                      <p className="text-sm font-medium text-destructive">{form.formState.errors.eventType.message}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <LocationSelector 
                      ...
                      onChange={(locationId, locationData) => {
                        ...
                        if (locationData && locationData.status === LocationStatus.REJECTED && locationData.rejectionReason) {
                          toast({
                        ...
                        if (locationData && locationData.status === LocationStatus.PENDING) {
                          toast({
                            title: "Location Pending Approval",
                            description: "You can still submit your booking, but it will require additional approval.",
                            variant: "default"
                          });
                        }
                      }}
                      onCreateLocation={async (locationData) => {
                        ...
                        toast({
                          ...
                          description: "Your location request has been submitted for approval.",
                        });
                    ...
                    <p className="text-sm text-muted-foreground">
                      Select from existing locations or request a new one. Bookings can be submitted with pending locations.
                    </p>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Popover>
                        ...
                        <PopoverContent className="w-auto p-3 border border-border shadow-md bg-background" align="start">
                          {/* 
                          ...
                          <DatePicker
                            ...
                            onChange={(date) => form.setValue("date", date as Date)}
                            inline
                    ...
                    <div className="space-y-2">
                      ...
                      {form.formState.errors.activityType && (
                        <p className="text-sm font-medium text-destructive">{form.formState.errors.activityType.message}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        >
                          ...
                          <SelectContent className="h-80 overflow-y-auto">
                            <div className="p-2 text-xs text-muted-foreground">Local timezone: {Intl.DateTimeFormat().resolvedOptions().timeZone}</div>
                      ...
                      {form.formState.errors.startTime && (
                        <p className="text-sm font-medium text-destructive">{form.formState.errors.startTime.message}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {form.formState.errors.endTime && (
                        <p className="text-sm font-medium text-destructive">{form.formState.errors.endTime.message}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {form.formState.errors.eventDetails && (
                      <p className="text-sm font-medium text-destructive">{form.formState.errors.eventDetails.message}</p>
                    )}
                  </div>
                </div>
              ...
              <TabsContent value="options" className="mt-0 space-y-6">
                <div className="mb-4 p-3 bg-muted/30 rounded-md border border-muted flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground text-sm">Configure recurring events, promotional details, and calendar invites for this booking.</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 gap-6">
                  {/* Promotion Type */}
                  <div className="space-y-2">
                    ...
                    <Select 
                      onValueChange={(value) => form.setValue("promotionType", value)}
                    ...
                    {form.formState.errors.promotionType && (
                      <p className="text-sm font-medium text-destructive">{form.formState.errors.promotionType.message}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {form.formState.errors.promotionDetails && (
                      <p className="text-sm font-medium text-destructive">{form.formState.errors.promotionDetails.message}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="border rounded-lg overflow-hidden">
                      <button 
                        ...
                        onClick={() => form.setValue("isRecurring", !form.watch("isRecurring"))}
                      >
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            "flex items-center justify-center w-8 h-8 rounded-full",
                            form.watch("isRecurring") ? "bg-primary text-primary-foreground" : "bg-muted-foreground/20 text-muted-foreground"
                          )}>
                            <svg 
                              xmlns="http://www.w3.org/2000/svg" 
                              width="16" 
                          ...
                          <div className="text-left">
                            ...
                            <p className="text-muted-foreground text-sm">Set this event to repeat on a schedule</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                          <span className={cn(
                            "text-sm", 
                            form.watch("isRecurring") ? "text-primary font-medium" : "text-muted-foreground"
                          )}>
                            {form.watch("isRecurring") ? "Enabled" : "Disabled"}
                          </span>
                          <div className="transform transition-transform duration-200" 
                            style={{ transform: form.watch("isRecurring") ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                            <svg 
                              xmlns="http://www.w3.org/2000/svg" 
                              width="18" 
                              height="18" 
                      ...
                      {form.watch("isRecurring") && (
                        <div className="p-4 grid grid-cols-2 gap-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <Select
                              value={form.watch("recurringFrequency") || "weekly"}
                          ...
                          <div className="space-y-2">
                            ...
                            <Select
                              value={(form.watch("recurringCount")?.toString() || "4")}
                              onValueChange={(value) => {
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="12">12 events (quarterly)</SelectItem>
                                <SelectItem value="24">24 events (biweekly)</SelectItem>
                                <SelectItem value="52">52 events (weekly)</SelectItem>
                              </SelectContent>
                            </Select>
                          </div>
                          ...
                          <div className="col-span-2 mt-3 p-3 bg-primary/5 rounded-md border border-primary/10">
                            ...
                            <div className="mt-2">
                              <p className="text-sm">
                                Starting on <span className="font-medium">{format(form.watch("date") || new Date(), "MMMM d, yyyy")}</span>, 
                                this will create <span className="font-medium">{form.watch("recurringCount") || 4}</span> events, 
                                {form.watch("recurringFrequency") === "daily" && " one per day"}
                                {form.watch("recurringFrequency") === "weekly" && " one per week"}
                                {form.watch("recurringFrequency") === "biweekly" && " one every two weeks"}
                                {form.watch("recurringFrequency") === "monthly" && " one per month"}
                                .
                              </p>
                              <p className="text-xs text-muted-foreground mt-1">
                                Each event will have the same details (time, location, etc.) but occur on different dates.
                              </p>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="border rounded-lg overflow-hidden">
                      <button 
                        ...
                        onClick={() => form.setValue("sendInvites", !form.watch("sendInvites"))}
                      >
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            "flex items-center justify-center w-8 h-8 rounded-full",
                            form.watch("sendInvites") ? "bg-primary text-primary-foreground" : "bg-muted-foreground/20 text-muted-foreground"
                          )}>
                            <svg 
                              xmlns="http://www.w3.org/2000/svg" 
                              width="16" 
                          ...
                          <div className="text-left">
                            <div className="font-medium">Send Calendar Invites</div>
                            <p className="text-muted-foreground text-sm">Send calendar invitations to all participants</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                          <span className={cn(
                            "text-sm", 
                            form.watch("sendInvites") ? "text-primary font-medium" : "text-muted-foreground"
                          )}>
                            {form.watch("sendInvites") ? "Enabled" : "Disabled"}
                          </span>
                          <div className="transform transition-transform duration-200" 
                            style={{ transform: form.watch("sendInvites") ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                            <svg 
                              xmlns="http://www.w3.org/2000/svg" 
                              width="18" 
                              height="18" 
                      ...
                      {form.watch("sendInvites") && (
                        <div className="p-4 space-y-3">
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                Each person will receive an email with a calendar invitation
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-start gap-2 p-2 bg-primary/5 rounded-sm">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Invitations will include all event details (date, time, location) and will appear in recipients' 
                              calendar applications like Google Calendar, Outlook, or Apple Calendar.
                            </p>
                          </div>
                        </div>
                      )}
