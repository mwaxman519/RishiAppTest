...
import { format, addDays, addWeeks, addMonths, addYears, isAfter, isBefore, isSameDay } from "date-fns";
  ...
  const occurrences = useMemo(() => {
    ...
    if (endDate && !count) {
      ...
      if (pattern.frequency === 'DAILY') {
        const days = Math.round((endDateObj.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
        maxCount = Math.min(Math.ceil(days / pattern.interval), 100);
      } else if (pattern.frequency === 'WEEKLY') {
        const weeks = Math.round((endDateObj.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24 * 7));
        maxCount = Math.min(Math.ceil(weeks / pattern.interval), 100);
      } else if (pattern.frequency === 'MONTHLY') {
        const months = (endDateObj.getFullYear() - startDate.getFullYear()) * 12 + 
      ...
      switch (pattern.frequency) {
        ...
        case 'WEEKLY':
          if (pattern.byDay && pattern.byDay.length > 0) {
            ...
            for (let i = 0; i < 7 * pattern.interval && !foundNextDate; i++) {
              ...
              const dayCode = Object.keys(dayToIndex).find(key => dayToIndex[key] === dayIndex);
        ...
        case 'MONTHLY':
          if (pattern.byMonthDay) {
            ...
            nextDate.setDate(Math.min(pattern.byMonthDay, 
              new Date(nextDate.getFullYear(), nextDate.getMonth() + 1, 0).getDate())); // Adjust for shorter months
          } else {
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="right">
                <p className="max-w-xs">This shows a preview of when events in this series will occur based on your recurrence settings.</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </CardTitle>
        <div className="flex items-center space-x-2">
          ...
          >
            {viewMode === 'list' ? <CalendarIcon className="h-4 w-4" /> : <ListFilter className="h-4 w-4" />}
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        {occurrences.length === 0 ? (
          <div className="flex items-center justify-center h-40 border border-dashed rounded-md">
            <p className="text-muted-foreground">No recurring events to display</p>
          </div>
        ) : (
          <div className="space-y-3">
              ...
              {occurrences.slice(0, visibleCount).map((date, index) => (
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className="font-medium text-sm">{format(date, "EEEE, MMMM d, yyyy")}</p>
                  ...
                  {index === 0 && (
                    <Badge className="bg-primary-foreground text-primary">Start</Badge>
                  )}
                </div>
              ))}
            </div>
            ...
            {occurrences.length > visibleCount && (
              <Button 
                ...
                onClick={() => setVisibleCount(Math.min(visibleCount + 10, occurrences.length))}
              >
                Show more events
              </Button>
            )}
