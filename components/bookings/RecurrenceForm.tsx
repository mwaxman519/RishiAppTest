...
export function RecurrenceForm({ value = {}, onChange, startDate = new Date() }: RecurrenceFormProps) {
  ...
  useEffect(() => {
    const dates: Date[] = []
    const { frequency, interval, weekdays, endType, occurrences, endDate } = formData
    
    if (!frequency) return

    // Number of dates to preview (limited to reasonable number)
    const maxPreviewDates = 10
    const maxDatesToCalculate = endType === "after" && occurrences 
  ...
  useEffect(() => {
    ...
    if (frequency === "daily") {
      ...
      if (weekdays && weekdays.length > 0) {
        const daysText = weekdays.map(day => day.charAt(0).toUpperCase() + day.slice(1)).join(", ")
        preview += ` on ${daysText}`
      }
    } else if (frequency === "monthly") {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(value: "daily" | "weekly" | "monthly") => handleChange('frequency', value)}
              >
                <SelectTrigger>
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={(checked: CheckedState) => {
                          if (checked) {
                            handleChange('weekdays', [...(formData.weekdays || []), day])
                          } else {
                            handleChange(
                              'weekdays',
            ...
            <div className="space-y-3">
              ...
              <RadioGroup
                ...
                onValueChange={(value: "never" | "after" | "on") => handleChange('endType', value)}
                className="flex flex-col space-y-3"
              >
                <div className="flex items-center space-x-3 space-y-0">
                  <RadioGroupItem value="never" id="never" />
                  <FormLabel htmlFor="never" className="font-normal cursor-pointer">
                    Never
                  </FormLabel>
                </div>
                <div className="flex items-center space-x-3">
                  <RadioGroupItem value="after" id="after" />
                  <FormLabel htmlFor="after" className="font-normal cursor-pointer">
                    After
                  </FormLabel>
                  <div className="w-16">
                    <Input
                      ...
                      onChange={(e) => handleChange('occurrences', Number(e.target.value))}
                      className="h-9"
                    />
                  </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <FormLabel htmlFor="on" className="font-normal cursor-pointer">
                    On date
                  </FormLabel>
                  <DatePicker
        ...
        <div className="bg-muted/20 rounded-md p-4 space-y-4">
          ...
          {previewDates.length > 0 && (
            <div>
              <p className="text-sm mb-2 font-medium">Next {previewDates.length} occurrences:</p>
          ...
          <div className="flex items-start space-x-2 text-xs text-muted-foreground mt-2">
            ...
            <p>
              This is a preview of your recurring pattern. The actual events will be created 
