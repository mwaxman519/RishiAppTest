...
async function addSuperAdmins() {
  try {
    ...
    for (const admin of superAdmins) {
      ...
      const userCheck = await client.query('SELECT * FROM users WHERE username = $1', [admin.username]);
      ...
      if (userCheck.rows.length === 0) {
        ...
        `, [userId, admin.username, hashedPassword, admin.email, admin.full_name, 'super_admin', true]);
        
        console.log(`Super admin user ${admin.username} created successfully with ID: ${userId}`);
      } else {
        console.log(`Super admin user ${admin.username} already exists, updating role and password...`);
    ...
    if (organizations.rows.length > 0) {
      ...
      const mattUser = await client.query('SELECT id FROM users WHERE username = $1', ['matt']);
      const ryanUser = await client.query('SELECT id FROM users WHERE username = $1', ['ryan']);
        ...
        for (const org of organizations.rows) {
          ...
          if (mattOrgCheck.rows.length === 0) {
            await client.query(`
              ...
              VALUES ($1, $2, $3, $4)
            `, [mattId, orgId, 'super_admin', orgId === '00000000-0000-0000-0000-000000000001']);
          ...
          if (ryanOrgCheck.rows.length === 0) {
            await client.query(`
              INSERT INTO organization_users (user_id, organization_id, role, is_primary)
              VALUES ($1, $2, $3, $4)
            `, [ryanId, orgId, 'super_admin', orgId === '00000000-0000-0000-0000-000000000001']);
