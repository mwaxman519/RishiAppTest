...
import { webSocketEventSubscriber } from "../services/infrastructure/messaging/webSocketEventSubscriber";
...
function setupWebSocketServer(wss: any) {
  console.log('[WebSocketServer] Initializing WebSocket server for event distribution');
  ...
  wss.on('connection', (ws: any, req: any) => {
    ...
    console.log(`[WebSocketServer] Client connected: ${connectionId} from ${ip}`);
    ...
    if (userId) {
      webSocketEventSubscriber.addConnection(connectionId, ws, userId, organizationId);
    ...
    const pingInterval = setInterval(() => {
      ...
      if (Date.now() - lastHeartbeat > 30000) { // 30 seconds
        ...
        try { 
          ...
          console.error(`[WebSocketServer] Error closing connection: ${err?.message || 'Unknown error'}`);
        }
        clearInterval(pingInterval);
        return;
      }
      ...
      try {
        ...
        console.error(`[WebSocketServer] Error sending ping: ${err?.message || 'Unknown error'}`);
      }
    ...
    ws.addEventListener('message', (event: any) => {
      try {
        ...
        console.log(`[WebSocketServer] Received message from ${connectionId}:`, message.type);
        ...
        if (message.type === 'subscribe' && Array.isArray(message.events)) {
          ...
          console.log(`[WebSocketServer] Client ${connectionId} subscribed to events:`, message.events);
        }
        ...
        if (message.type === 'unsubscribe' && Array.isArray(message.events)) {
          ...
          console.log(`[WebSocketServer] Client ${connectionId} unsubscribed from events:`, message.events);
        }
    ...
    ws.addEventListener('close', () => {
      ...
      // Clean up resources
      clearInterval(pingInterval);
      webSocketEventSubscriber.removeConnection(connectionId, userId, organizationId);
    });
    
    // Handle WebSocket errors
    ws.on('error', (error: Error) => {
      console.error(`[WebSocketServer] Error with client ${connectionId}:`, error);
