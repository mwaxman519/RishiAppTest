...
export class OrganizationCacheManager {
   ...
   */
  set<T>(key: string, value: T, context: OrganizationContext, ttl?: number): void {
    const orgCache = this.getOrCreateOrgCache(context.organizationId);
   ...
   */
  getStats(): { organizationCount: number, totalEntries: number, organizationSizes: Record<number, number> } {
    const stats = {
   ...
   */
  private getOrCreateOrgCache(organizationId: number): Map<string, CacheEntry<any>> {
