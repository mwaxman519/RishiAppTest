...
function getDatabaseUrl() {
  ...
  if (!url) {
    ...
    if (environment === 'production' || environment === 'replit') {
      ...
      console.warn('Using a default DATABASE_URL for development, this is not secure for production');
...
function verifyDatabaseUrl(url) {
  try {
    ...
    if (environment !== 'replit') {
      ...
      if (parsedUrl.username === 'default' || parsedUrl.password === 'default') {
        console.warn('DATABASE_URL contains default credentials, this is not secure for production');
      }
    }
