    ...
    handlers.forEach(handler => {
      ...
      if (matches) {
        matches.forEach(() => {
          ...
          const startIndex = content.indexOf(`export async function ${handler}`);
          ...
          if (patterns.destructuredParams.test(handlerCode) && !patterns.contextParams.test(handlerCode)) {
            issues.usingDestructuredParams.push(`${filePath} (${handler})`);
            fileHasIssues = true;
            issues.errorCount++;
          }
          
          if (patterns.accessViaDestructured.test(handlerCode) && !patterns.accessViaContext.test(handlerCode)) {
...
function printSummary() {
  ...
  if (issues.errorCount === 0) {
    log('✅ All API routes follow the recommended TypeScript patterns', 'green');
  } else {
    log(`❌ Found ${issues.errorCount} issues in ${issues.routeCount} route files`, 'red');
    
    if (issues.missingExplicitReturnType.length > 0) {
      log('\nMissing explicit Promise<NextResponse> return type:', 'yellow');
      issues.missingExplicitReturnType.forEach(file => log(`  - ${file}`, 'red'));
    }
    
    if (issues.usingDestructuredParams.length > 0) {
      log('\nUsing destructured { params } pattern (use context: { params } instead):', 'yellow');
      issues.usingDestructuredParams.forEach(file => log(`  - ${file}`, 'red'));
    }
    
    if (issues.accessingViaDestructured.length > 0) {
      log('\nAccessing params directly (use context.params instead):', 'yellow');
      issues.accessingViaDestructured.forEach(file => log(`  - ${file}`, 'red'));
    }
    
    log('\nPlease follow the guidelines in Docs/API-TypeScript-Guidelines.md', 'magenta');
  }
