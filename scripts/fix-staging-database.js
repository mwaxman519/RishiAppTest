...
console.log('Starting database validation and repair for staging environment...');
...
async function validateSchema() {
  ...
  try {
    ...
    if (tables.includes('user_organizations')) {
      ...
      console.log(`Foreign keys on user_organizations: ${fkCheck.rows[0].count}`);
    }
...
async function fixCommonIssues() {
  ...
  try {
    ...
    console.log('Configuring optimized connection pool for staging environment');
    
    // 2. Ensure tables exist through drizzle push
    // (This is just a recommendation - actual push would be done with npm run db:push)
    console.log('Recommended: Run "npm run db:push" to ensure all tables exist with proper schema');
    
    // 3. Check for database connection leaks
    console.log('Checking for potential connection leaks...');
    const poolStatus = await pool.query('SELECT count(*) FROM pg_stat_activity WHERE application_name = $1', ['PostgreSQL JDBC Driver']);
...
async function generateHealthReport() {
  ...
  console.log(`Connection Status: ${connectionSuccess ? 'CONNECTED' : 'FAILED'}`);
  ...
  console.log('\n--- RECOMMENDATIONS ---');
  console.log('1. Verify DATABASE_URL is correctly set in the staging environment');
  console.log('2. Run "npm run db:push" to update schema to latest version');
  console.log('3. Check for database connection handling in auth service endpoints');
  console.log('4. Verify organization service is properly configured to use database');
