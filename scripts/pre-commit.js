...
console.log(`${colors.blue}Running pre-commit TypeScript checks...${colors.reset}`);
...
try {
  ...
  try {
    ...
    console.log(`${colors.yellow}⚠ ESLint found issues but continuing (fix these before PR)${colors.reset}`);
  }

  // Step 3: Run a partial build to catch advanced type issues
  console.log(`\n${colors.cyan}Step 3/3: Partial build check${colors.reset}`);
  console.log(`${colors.yellow}Note: This is a quick check, not a full build${colors.reset}`);
  ...
  try {
    ...
    console.log(`${colors.green}✓ Build check interrupted after time limit (passed initial checks)${colors.reset}`);
  } catch (e) {
    if (e.status === 124) {
      ...
      console.error(`${colors.red}This indicates there are still type errors that would prevent production deployment${colors.reset}`);
      process.exit(1);
    }
  }
  ...
  console.error(`${colors.red}Please fix the issues above before committing.${colors.reset}`);
  process.exit(1);
}