...
function calculateRelativePath(fromFile, toModule) {
  ...
  if (toModule.startsWith('@/lib/')) {
    ...
    targetPath = path.join('app/components', toModule.substring('@/components/'.length));
  ...
  try {
    ...
    let newContent = content.replace(importRegex, (match, importPath) => {
      ...
      if (relativePath) {
        ...
        const afterPath = match.substring(match.indexOf(importPath) + importPath.length + 1); // +1 for the quote
...
async function main() {
  ...
  if (filesToProcess.length === 0) {
    ...
    console.error('Usage: node fix-imports.js path/to/file1.ts path/to/file2.tsx ...');
    process.exit(1);
  }
