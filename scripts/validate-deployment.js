...
async function runProductionValidation(pool) {
  ...
  try {
    ...
    if (testUserCount > 0) {
      console.error(`❌ Found ${testUserCount} test users in production database!`);
      return false;
    }
    ...
    if (testOrgCount > 0) {
      console.error(`❌ Found ${testOrgCount} test organizations in production database!`);
      return false;
    }
    ...
    if (patternUserCount > 0) {
      console.error(`❌ Found ${patternUserCount} users with test email patterns in production!`);
      return false;
    }
    ...
    if (patternOrgCount > 0) {
      console.error(`❌ Found ${patternOrgCount} organizations with test name patterns in production!`);
      return false;
    }
  ...
  try {
    ...
    if (targetEnv === 'production') {
      ...
      if (!productionValid) {
        ...
        console.error('   Run data-sanitizer.js to clean up test data before deploying');
        return false;
      }
    }
...
validateDeployment()
  .then(isValid => {
    if (isValid) {
      console.log(`✅ Deployment validation successful for ${targetEnv} environment`);
      process.exit(0);
    } else {
      console.error(`❌ Deployment validation failed for ${targetEnv} environment`);
      process.exit(1);
    }
  })
  .catch(err => {
