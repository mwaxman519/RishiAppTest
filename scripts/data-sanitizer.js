...
const targetEnv = args.find(arg => !arg.startsWith('--')) || process.env.NODE_ENV || 'development';
const isDryRun = args.includes('--dry-run') || !args.includes('--execute');
const isVerbose = args.includes('--verbose');

console.log(`Data sanitization for ${targetEnv} environment${isDryRun ? ' (DRY RUN)' : ''}`);

// Test data patterns to detect
const testPatterns = {
  emailDomains: ['test.com', 'example.com', 'rishi-test.com', 'test-domain.com'],
      ...
      try {
        ...
        if (conditions.length > 0) {
          ...
          if (testDataResult.rows.length > 0) {
            ...
            // Count by type
            if (tableResults.hasTestDataColumn) {
              tableResults.testDataCount = testDataResult.rows.filter(row => row.is_test_data).length;
            }
            
            if (tableResults.hasEnvironmentColumn) {
              tableResults.wrongEnvironmentCount = testDataResult.rows.filter(row => row.environment !== targetEnv).length;
            }
            
            tableResults.patternMatchCount = testDataResult.rows.length - tableResults.testDataCount - tableResults.wrongEnvironmentCount;
...
async function cleanTestData(pool, testData) {
  if (isDryRun) {
    ...
    console.log(`Total: ${totalItems} items across ${Object.keys(testData).length} tables`);
    return;
  }
  
  if (targetEnv === 'production') {
    console.log('⚠️ EXECUTING DATA SANITIZATION IN PRODUCTION ⚠️');
    console.log('This will permanently remove test data from the production database.');
  }
...
async function runSanitizer() {
  ...
  if (targetEnv === 'production' && !isDryRun) {
    ...
    console.warn('This should only be done if you are absolutely sure what you are doing.');
  ...
  try {
    ...
    for (const table of testDataTables) {
      ...
      if (isVerbose) {
        console.log(`  * ${tableData.testDataCount} explicitly flagged as test data`);
        console.log(`  * ${tableData.wrongEnvironmentCount} with wrong environment`);
    ...
    if (!isDryRun) {
      ...
      if (targetEnv === 'production') {
        console.log('To clean test data from production, run with --execute flag:');
        console.log('  NODE_ENV=production node scripts/data-sanitizer.js --execute');
      }
    }
  } finally {
    await pool.end();
