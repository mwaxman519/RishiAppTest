...
function checkEnvironment() {
  ...
  if (process.env.NEXT_PUBLIC_APP_ENV !== 'production') {
    log(`NEXT_PUBLIC_APP_ENV is set to "${process.env.NEXT_PUBLIC_APP_ENV}" instead of "production"`, 'warning');
...
async function sanitizeData() {
  ...
  const shouldSanitize = await prompt('Do you want to run the data sanitization tool?');
...
function generateAzureFiles() {
  ...
  const staticWebAppConfig = {
    ...
    "globalHeaders": {
      "content-security-policy": "default-src 'self' 'unsafe-inline' 'unsafe-eval' https: data:; object-src 'none'",
...
async function main() {
  ...
  try {
    // Environment check
    if (!checkEnvironment()) {
      log('Environment check failed, please fix the issues before continuing', 'error');
      process.exit(1);
    }
    
    // Sanitize data
    if (!await sanitizeData()) {
      const shouldContinue = await prompt('Data sanitization had issues. Continue anyway?');
      if (!shouldContinue) {
    ...
    log('Your application is ready to be deployed to Azure Static Web Apps.', 'success');
    log('Follow the instructions in .azure/deployment-instructions.md to complete the deployment.', 'info');
