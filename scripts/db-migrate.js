...
console.log(`Running migrations for ${env} environment${isDryRun ? ' (DRY RUN)' : ''}`);
  ...
  try {
    ...
    if (env === 'production') {
      ...
      if (testDataCheck.hasTestData) {
        ...
        console.error(`Found ${testDataCheck.testUserCount} test users and ${testDataCheck.testOrgCount} test organizations`);
        console.error('Run data-sanitizer.js to clean up test data before applying migrations');
        process.exit(1);
      }
    }
