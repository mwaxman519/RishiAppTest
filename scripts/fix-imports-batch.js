...
const replacements = [
  // Lib imports
  { pattern: /@\/lib\/([^'"]+)/g, replacement: (match, p1, offset, string, filepath) => getRelativePath(filepath, `app/lib/${p1}`) },
  
  // Components imports
  { pattern: /@\/components\/([^'"]+)/g, replacement: (match, p1, offset, string, filepath) => getRelativePath(filepath, `app/components/${p1}`) },
  
  // Shared imports
  { pattern: /@\/shared\/([^'"]+)/g, replacement: (match, p1, offset, string, filepath) => getRelativePath(filepath, `shared/${p1}`) },
  
  // Hooks imports
  { pattern: /@\/hooks\/([^'"]+)/g, replacement: (match, p1, offset, string, filepath) => getRelativePath(filepath, `app/hooks/${p1}`) },
  
  // Services imports
  { pattern: /@\/services\/([^'"]+)/g, replacement: (match, p1, offset, string, filepath) => getRelativePath(filepath, `app/services/${p1}`) },
  
  // Types imports
  { pattern: /@\/types\/([^'"]+)/g, replacement: (match, p1, offset, string, filepath) => getRelativePath(filepath, `app/types/${p1}`) },
  
  // Utils imports
  { pattern: /@\/utils\/([^'"]+)/g, replacement: (match, p1, offset, string, filepath) => getRelativePath(filepath, `app/utils/${p1}`) },
  
  // Actions imports
  { pattern: /@\/actions\/([^'"]+)/g, replacement: (match, p1, offset, string, filepath) => getRelativePath(filepath, `app/actions/${p1}`) },
  
  // Root app imports
  { pattern: /@\/([^'"\/]+)/g, replacement: (match, p1, offset, string, filepath) => getRelativePath(filepath, `app/${p1}`) },
];
...
function main() {
  ...
  try {
    ...
    const result = execSync('find app -type f -name "*.ts" -o -name "*.tsx" | xargs grep -l "@/"').toString();
