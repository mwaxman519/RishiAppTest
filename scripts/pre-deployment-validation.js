...
function validateFileCount() {
  ...
  try {
    const tsFiles = execSync("find . -name '*.ts' -o -name '*.tsx' | grep -v node_modules | wc -l", { encoding: 'utf8' });
    const fileCount = parseInt(tsFiles.trim());
    
    log(`Current TypeScript files: ${fileCount}`);
    
    if (fileCount > 1000) {
      console.error(`ERROR: Too many TypeScript files (${fileCount}). Azure SWA limit exceeded.`);
      console.error('Run cleanup script to reduce file count before deployment.');
      process.exit(1);
    }
