...
if (env === 'production' && !isForce) {
  console.error('⚠️ Refusing to seed data in production environment without --force flag');
...
async function seedDatabase() {
  ...
  try {
    ...
    for (const user of data.users) {
      ...
      `, [user.id, user.username, user.password, user.full_name, user.email, user.role, user.active, user.is_test_data, user.environment]);
    ...
    for (const org of data.organizations) {
      ...
      `, [org.id, org.name, org.type, org.tier, org.active, org.is_test_data, org.environment]);
    ...
    if (data.users.length > 0 && data.organizations.length > 0) {
      ...
      console.log(`Linked user ${data.users[0].username} to organization ${data.organizations[0].name}`);
    }
