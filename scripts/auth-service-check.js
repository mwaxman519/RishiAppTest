...
async function checkAuthService() {
  ...
  try {
    ...
    console.log('Registration endpoint available:', registerResponse.status < 500);
    ...
    console.log('Registration endpoint working correctly:', registerTestResponse.status === 400);
  } catch (err) {
  ...
  try {
    ...
    console.log('Login endpoint working correctly:', loginTestResponse.status === 401 || loginTestResponse.status === 400);
  } catch (err) {
  ...
  try {
    ...
    console.log('User endpoint returning correct unauthorized status:', userResponse.status === 401);
  } catch (err) {
  ...
  try {
    ...
    console.log('Organizations endpoint response:', responseText.substring(0, 100) + (responseText.length > 100 ? '...' : ''));
  } catch (err) {
