...
if (fs.existsSync(schemaPath)) {
  ...
  const requiredExports = [
    'items', 'activityTypes', 'activityAssignments', 'organizationUsers',
    'organizationSettings', 'userOrganizationPreferences', 'rolePermissions',
    'bookingComments', 'eventInstances', 'promotionTypes'
  ];
...
if (fs.existsSync(rbacPath)) {
  ...
  const requiredRbacExports = [
    'hasPermission', 'getAllPermissions', 'hasRoutePermission', 'routePermissions'
  ];
    ...
    files.forEach(file => {
      if (file.isDirectory() && !file.name.startsWith('.') && file.name !== 'node_modules') {
        ...
        const problematicImports = [
          { pattern: /from '@\/components\/ui\/Checkbox'/, issue: 'Capital C in Checkbox import' },
          { pattern: /from '.*\/rbac'.*getAllPermissions/, issue: 'getAllPermissions import issue' },
          { pattern: /from '.*schema'.*organizationSettings/, issue: 'organizationSettings import issue' }
        ];
