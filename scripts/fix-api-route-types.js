...
const patterns = {
  findHandler: /export\s+async\s+function\s+(GET|POST|PUT|DELETE|PATCH)\s*\(\s*([^)]*)\)(?:\s*:\s*[^{]+)?\s*(\{\s*)/g,
...
function fixRouteFile(filePath) {
  try {
    ...
    content = content.replace(patterns.findHandler, (match, method, params, opening) => {
      ...
      if (params.includes('{ params }') || params.includes('{params}')) {
        newParams = params.replace(/\{\s*params\s*\}\s*:\s*\{\s*params\s*:\s*\{\s*([^\}]*)\}\s*\}/, 
                                 'context: { params: { $1 } }');
...
function printSummary() {
  ...
  if (changes.changedFileCount === 0) {
    log('✅ No changes needed - all files already follow the correct pattern', 'green');
  } else {
    ...
    if (changes.convertedParamsPattern.length > 0) {
      ...
      changes.convertedParamsPattern.forEach(file => log(`  - ${file}`, 'green'));
    }
    
    if (changes.changedParamsAccess.length > 0) {
      log('\nUpdated params access to use context.params in:', 'yellow');
      changes.changedParamsAccess.forEach(file => log(`  - ${file}`, 'green'));
    }
  }
  
  log('\nPlease review the changes and run check-api-route-types.js to verify', 'magenta');
  log('======================================================', 'cyan');
}
