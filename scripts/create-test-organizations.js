...
async function createTestOrganizations() {
  ...
  try {
    ...
    if (existingOrgMap['Agency Partners Inc.']) {
      console.log('Agency Partners Inc. already exists, using existing organization');
    ...
    if (existingOrgMap['Enterprise Solutions LLC']) {
      console.log('Enterprise Solutions LLC already exists, using existing organization');
    ...
    console.log(`Created organizations: Agency Partners Inc. (ID: ${org1Id}), Enterprise Solutions LLC (ID: ${org2Id})`);
    ...
    if (!superAdminId) {
      try {
        ...
        if (error.code === '23505') { // Unique constraint violation
          const retryQuery = await db.execute(sql`SELECT id FROM users WHERE username = 'superadmin'`);
    ...
    if (!internalAdminId) {
      try {
        ...
        // If insertion fails, try to retrieve the user again
        if (error.code === '23505') { // Unique constraint violation
          const retryQuery = await db.execute(sql`SELECT id FROM users WHERE username = 'internal_admin'`);
          if (retryQuery.rows.length > 0) {
            internalAdminId = retryQuery.rows[0].id;
            console.log(`Found existing Internal Admin user (ID: ${internalAdminId})`);
          } else {
            throw error; // Re-throw if we can't find the user
          }
        } else {
    ...
    console.log(`Created client managers: Agency Manager (ID: ${clientManager1Id}), Enterprise Manager (ID: ${clientManager2Id})`);
    ...
    if (!norcalRegionId) {
      ...
      console.log(`Created region: Northern California (ID: ${norcalRegionId})`);
    } else {
      console.log(`Using existing region: Northern California (ID: ${norcalRegionId})`);
    }
    ...
    if (!socalRegionId) {
      ...
      console.log(`Using existing region: Southern California (ID: ${socalRegionId})`);
    }
    ...
    if (!washingtonRegionId) {
      ...
      console.log(`Using existing region: Washington (ID: ${washingtonRegionId})`);
    }
