...
const optimizations = [
  {
    ...
    optimize: (content) => {
      ...
      return content
        ...
        .replace(/useEffect\(\(\) => \{[\s\S]*?\}, \[.*?\]\);/g, '// useEffect optimized out')
        .replace(/const.*useCallback.*\[[\s\S]*?\];/g, 'const callbackFn = () => {};');
    }
  },
  {
    ...
    optimize: (content) => {
      ...
      return content
        ...
        .replace(/table\.getHeaderGroups\(\)[\s\S]*?}/g, 'headerGroups.map(() => <div key="header">Header</div>)');
    }
  }
];
...
packageJson.scripts['build:optimized'] = 'NEXT_TELEMETRY_DISABLED=1 next build --experimental-build-mode=compile';
