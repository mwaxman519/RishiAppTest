...
console.log(`Deployment process for ${targetEnv} environment${validateOnly ? ' (validation only)' : ''}`);
...
if (targetEnv === 'production' && !skipConfirmation && !validateOnly) {
  ...
  console.log('This action will affect live data and should be done with caution.');
  console.log('');
...
async function runScript(scriptPath, env, ...args) {
  ...
  const command = `${nodeEnv} node ${scriptPath} ${env || ''} ${args.join(' ')}`.trim();
...
async function deploy() {
  ...
  const isValid = await runScript('./scripts/validate-deployment.js', targetEnv);
  ...
  const dryRunResult = await runScript('./scripts/data-sanitizer.js', targetEnv, ...sanitizeArgs);
  ...
  if (targetEnv === 'production') {
    ...
    const sanitizeResult = await runScript('./scripts/data-sanitizer.js', targetEnv, ...sanitizeArgs);
  ...
  try {
    ...
    const { stdout, stderr } = await execPromise(`NODE_ENV=${targetEnv} npm run build`);
    if (stdout) console.log(stdout);
    if (stderr) console.error(stderr);
  } catch (error) {
  ...
  const postDeployValid = await runScript('./scripts/validate-deployment.js', targetEnv);
  
  if (!postDeployValid) {
    console.error('⚠️ Post-deployment validation failed - deployment may be incomplete');
    process.exit(1);
  }
