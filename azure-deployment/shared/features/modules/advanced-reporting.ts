...
export const AdvancedReportingFeature: FeatureModule = {
  ...
  description: 'Generate detailed analytics, custom reports, and data visualizations to better understand your workforce and events.',
  icon: 'bar-chart-2',
  ...
  initializeForOrganization: async (organizationId: string) => {
    console.log(`Initializing advanced reporting for organization ${organizationId}`);
  ...
  onEnable: async (organizationId: string) => {
    console.log(`Enabling advanced reporting for organization ${organizationId}`);
    // Set up scheduled reports
  },
  
  // Run when the feature is disabled
  onDisable: async (organizationId: string) => {
    console.log(`Disabling advanced reporting for organization ${organizationId}`);
