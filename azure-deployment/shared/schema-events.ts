...
export const eventTeamMembers = pgTable("event_team_members", {
  id: uuid("id").defaultRandom().primaryKey(),
  eventInstanceId: uuid("event_instance_id").notNull().references(() => eventInstances.id, { onDelete: "cascade" }),
...
export const activities = pgTable("activities", {
  id: uuid("id").defaultRandom().primaryKey(),
  eventInstanceId: uuid("event_instance_id").references(() => eventInstances.id),
...
export const insertEventStateTransitionSchema = createInsertSchema(eventStateTransitions);
export const selectEventStateTransitionSchema = createSelectSchema(eventStateTransitions);

export type EventStateTransition = typeof eventStateTransitions.$inferSelect;
export type InsertEventStateTransition = typeof eventStateTransitions.$inferInsert;
...
export const extendedInsertEventInstanceSchema = insertEventInstanceSchema.extend({
  status: z.enum([
    'scheduled', 
    'preparation', 
    'in_progress', 
...
export const extendedInsertStaffAssignmentSchema = insertStaffAssignmentSchema.extend({
  status: z.enum([
    'assigned',
    'offered',
    'accepted',
