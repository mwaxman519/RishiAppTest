...
function processNavigationItems(items: NavItem[], rolePrefix: string): NavItem[] {
  return items.map((item, index) => {
    ...
    const pathPart = processedItem.path || processedItem.href || processedItem.label;
...
export function initializeNavigation() {
  ...
  console.log('Navigation structure already defined in shared/navigation-structure.tsx');
  ...
  const processedSuperAdmin = processNavigationItems(superAdminNavigation, 'super-admin');
  const processedInternalAdmin = processNavigationItems(internalAdminNavigation, 'internal-admin');
  const processedFieldManager = processNavigationItems(fieldManagerNavigation, 'field-manager');
  const processedBrandAgent = processNavigationItems(brandAgentNavigation, 'brand-agent');
  const processedClientUser = processNavigationItems(clientUserNavigation, 'client-user');
