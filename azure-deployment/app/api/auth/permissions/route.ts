...
export async function GET(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      if (token) {
        try {
          ...
          console.log('Development mode: JWT verification failed, using default permissions');
        }
      }
      
      return NextResponse.json({
        role: userRole,
        permissions: mockPermissionsByRole[userRole as keyof typeof mockPermissionsByRole] || mockPermissionsByRole.brand_agent
      });
    }
    ...
    try {
      ...
      // Get the user's role
      const [userRole] = await db.select().from(roles).where(eq(roles.name, user.role)).limit(1);
