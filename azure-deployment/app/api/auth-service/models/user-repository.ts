 ...
 */
export async function getUserByUsername(username: string, options: { checkOnly?: boolean } = {}) {
  ...
  if (isNonProduction) {
    ...
    if (Object.keys(testAccounts).includes(username.toLowerCase())) {
      console.log(`[Auth Service] Returning mock user data for test account: ${username}`);
 ...
 */
export async function createUser(userData: any): Promise<{ user: schema.User | null, error?: string }> {
  try {
    ...
    if (process.env.NODE_ENV !== 'production' || 
      ...
      const testUsernames = ['test', 'demo', 'mike', 'user', 'admin', 'john', 'jane'];
      if (testUsernames.includes(userData.username.toLowerCase())) {
        console.log(`[Auth Service] Using simulated user creation for test account: ${userData.username}`);
    ...
    try {
      ...
      if (!user) {
        return { user: null, error: 'Failed to create user: No user returned from database' };
      }
      
      console.log('[Auth Service] User created successfully:', { id: user.id, username: user.username });
      return { user };
    } catch (dbError) {
      ...
      if (errorMessage.includes('password authentication') || 
        ...
        return { user: null, error: 'The registration service is experiencing database authentication issues' };
      }
      
      // Generic database error message for other cases
      return { user: null, error: 'The registration service is experiencing database issues. Please try again later.' };
    }
  } catch (error) {
    ...
    return { user: null, error: 'An unexpected error occurred during registration. Please try again later.' };
  }
}
...
export async function setupUserOrganization(
  ...
  try {
    ...
    if (message.includes('password authentication') || message.includes('neondb_owner')) {
      return { userOrg: null, error: 'The registration service is experiencing database authentication issues' };  
    }
    
    return { userOrg: null, error: 'Database error occurred while setting up organization' };
  }
}
...
export async function getUserPermissions(userId: string, organizationId?: string) {
  try {
    ...
    if (organizationId) {
      conditions.push(eq(schema.userOrganizations.organizationId, organizationId));
    }
