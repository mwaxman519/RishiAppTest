  ...
  try {
    ...
    if (user.role !== 'super_admin') {
      ...
      if (!userOrg && !hasPermission('view:all_organizations', user.role)) {
        return NextResponse.json(
          { error: 'You do not have permission to view users for this organization' },
          { status: 403 }
        );
      }
  ...
  try {
    ...
    if (user.role !== 'super_admin') {
      ...
      if (!userOrg && !hasPermission('manage:all', user.role)) {
        return NextResponse.json(
          { error: 'You do not have permission to remove users from this organization' },
          { status: 403 }
        );
      }
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`DEVELOPMENT MODE: Would remove user ${userId} from organization ${organizationId}`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user.role !== 'super_admin') {
      ...
      if (!userOrg && !hasPermission('manage:all', user.role)) {
        return NextResponse.json(
          { error: 'You do not have permission to add users to this organization' },
          { status: 403 }
        );
      }
    ...
    if (isPrimary) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.log(`DEVELOPMENT MODE: Would update all other organizations for user ${userId} to non-primary`);
      } else {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`DEVELOPMENT MODE: Would add user ${userId} to organization ${organizationId} with role ${role}`);
      return NextResponse.json({ 
        success: true,
        userOrganization: {
          userId,
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!userId || !organizationId || !updates || Object.keys(updates).length === 0) {
      return NextResponse.json(
    ...
    if (user.role !== 'super_admin') {
      ...
      if (!userOrg && !hasPermission('manage:all', user.role)) {
        return NextResponse.json(
          { error: 'You do not have permission to update users in this organization' },
          { status: 403 }
        );
      }
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`DEVELOPMENT MODE: Would update user ${userId} in organization ${organizationId} with:`, updates);
      return NextResponse.json({ 
        success: true,
        userOrganization: {
          ...existingUserOrg,
