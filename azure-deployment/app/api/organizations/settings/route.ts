...
export async function GET(request: NextRequest) {
  try {
    ...
    const canViewSettings = await hasPermission('view:organization_settings', { organizationId });
    if (!canViewSettings) {
      return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const canUpdateSettings = await hasPermission('edit:organization_settings', { organizationId: organization_id });
