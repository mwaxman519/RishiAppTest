...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    console.log("GET /api/availability received with params:", request.nextUrl.search);
    ...
    const status = searchParams.get("status") as 'available' | 'unavailable' | 'tentative' | null;
    ...
    console.log(`Parsed parameters: userId=${userId}, startDate=${startDate}, endDate=${endDate}, status=${status}`);
    ...
    const startDateValid = options.startDate && options.startDate instanceof Date && !isNaN(options.startDate.getTime());
    const endDateValid = options.endDate && options.endDate instanceof Date && !isNaN(options.endDate.getTime());
    ...
    console.log("Query options:", JSON.stringify(options, (key, value) => 
      value instanceof Date ? value.toISOString() : value
    ));

    try {
      console.log("➡️ Calling availabilityService.getAvailabilityBlocks with options");
      ...
      const serviceCallPromise = availabilityService.getAvailabilityBlocks(options);
      ...
      if (!result || !result.success) {
        console.log("⚠️ Service error:", result?.error || "Unknown service error");
      ...
      console.log(`✅ Successfully fetched ${result.data?.length || 0} availability blocks`);
      ...
      // Add more debug info
      console.log(`Returning ${result.data?.length || 0} availability blocks to client`);
      ...
      const errorResponse = {
        success: false,
        error: serviceError instanceof Error ? serviceError.message : "Error fetching availability data from service",
    ...
    const errorResponse = {
      success: false,
      error: error instanceof Error ? error.message : "Unhandled error in availability route",
      data: [] // Include empty data array to prevent client-side errors
    };
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    try {
      ...
      console.log("Received availability creation request:", JSON.stringify(body, null, 2));
    } catch (parseError) {
    ...
    if (isNaN(startDate.getTime())) {
      console.error("Invalid startDate format:", body.startDate || body.start_date);
      return NextResponse.json({ 
        success: false, 
    ...
    const createRequest = {
      ...
      startDate: startDate,
      endDate: endDate,
      status: (body.status as 'available' | 'unavailable' | 'tentative') || 'available',
      isRecurring: body.isRecurring || body.is_recurring || body.recurring || false,
    ...
    console.log("Processed request:", JSON.stringify(createRequest, (k, v) => {
      if (v instanceof Date) return v.toISOString();
      return v;
    }));
    
    try {
      console.log("➡️ Calling availabilityService.createAvailabilityBlock with request");
      ...
      const availabilityService = {
        createAvailabilityBlock: async (data: any) => {
          ...
          await eventBus.publish('availability.created', {
      ...
      // Wrap service call in a timeout to prevent hanging
      const serviceCallPromise = availabilityService.createAvailabilityBlock(createRequest);
      ...
      if (!result || !result.success) {
        console.error("Service error when creating availability:", result?.error);
        // Return standardized error response
        const errorResponse = {
          success: false,
      ...
      const errorResponse = {
        success: false,
        error: serviceError instanceof Error ? serviceError.message : "Failed to create availability block",
        timestamp: new Date().toISOString()
      };
