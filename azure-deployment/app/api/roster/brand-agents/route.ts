...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[API] Retrieving brand agents for organization: ${organizationId}`, {
      brandId,
      timestamp: new Date().toISOString()
    });

    const brandAgents = await rosterService.getBrandAgents(organizationId, brandId);
  ...
  try {
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        timestamp: new Date().toISOString(),
        eventsPublished: ['staff.assigned', 'cannabis.staff_assignment_created']
      }
    }, { status: 201 });
