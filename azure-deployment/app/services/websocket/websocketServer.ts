  ...
  wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (message) => {
      try {
        ...
        console.log(`[WebSocketServer] Received message from ${clientId}:`, data.type);
        ...
        console.error(`[WebSocketServer] Error processing message from ${clientId}:`, error);
        ws.send(JSON.stringify({
          type: 'error',
  ...
  switch (message.type) {
    case 'subscribe':
      ...
      if (Array.isArray(message.events)) {
        webSocketEventSubscriber.updateClientSubscriptions(clientId, message.events);
      }
      break;
    ...
    default:
      console.warn(`[WebSocketServer] Unknown message type from ${clientId}: ${message.type}`);
  }
}