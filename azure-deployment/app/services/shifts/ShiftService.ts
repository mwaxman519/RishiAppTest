...
export class ShiftService {
  ...
  async getShiftById(
    ...
    try {
      ...
      if (!this.hasShiftAccess(shift, requestingUserId, userRole, organizationId)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
  ...
  async updateShift(
    ...
    try {
      ...
      // Check permissions
      if (!this.hasUpdatePermissions(existingShift, updatedBy, userRole, organizationId)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
  ...
  async deleteShift(
    ...
    try {
      ...
      // Check permissions
      if (!this.hasDeletePermissions(shift, deletedBy, userRole, organizationId)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
  ...
  async assignAgentToShift(
    ...
    try {
      ...
      // Create assignment
      const assignment = await this.repository.createAssignment(params, assignedBy);
  ...
  private hasShiftAccess(
    ...
    switch (userRole) {
      case 'brand_agent':
        ...
        const isAssigned = shift.assignments?.some(a => a.agentId === requestingUserId);
