...
export const createShiftSchema = createInsertSchema(shifts, {
  ...
  requiredAgents: z.number().min(1, 'At least 1 agent required').max(50, 'Too many agents'),
  hourlyRate: z.number().positive('Hourly rate must be positive').optional(),
  totalBudget: z.number().positive('Budget must be positive').optional(),
  status: z.enum(['draft', 'open', 'assigned', 'in_progress', 'completed', 'cancelled']).default('draft')
}).omit({
  id: true,
  createdAt: true
}).refine(
...
export const updateShiftSchema = createInsertSchema(shifts, {
  ...
  requiredAgents: z.number().min(1, 'At least 1 agent required').max(50, 'Too many agents').optional(),
  hourlyRate: z.number().positive('Hourly rate must be positive').optional(),
  totalBudget: z.number().positive('Budget must be positive').optional(),
...
export const assignShiftSchema = createInsertSchema(shiftAssignments, {
  ...
  assignmentStatus: z.enum(['assigned', 'confirmed', 'checked_in', 'completed', 'no_show']).default('assigned')
}).omit({
  id: true,
  assignedAt: true
});
