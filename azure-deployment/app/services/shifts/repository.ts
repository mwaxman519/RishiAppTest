...
import { eq, and, gte, lte, or, desc, asc, inArray, isNull, isNotNull } from 'drizzle-orm';
import { 
  ShiftDTO, 
  ShiftAssignmentDTO, 
  CreateShiftParams, 
...
export class ShiftRepository {
    ...
    try {
      ...
      if (filters.agentId) {
        const shiftsWithAgent = await db
          ...
          .where(
            and(
              eq(shiftAssignments.agentId, filters.agentId),
              ...conditions
            )
          )
   ...
   */
  async createAssignment(assignment: ShiftAssignmentParams, assignedBy: string): Promise<ShiftAssignmentDTO> {
    try {
      ...
      const completeAssignment = await this.getAssignmentById(createdAssignment.id);
   ...
   */
  async updateAssignment(id: string, data: Partial<ShiftAssignmentParams>): Promise<ShiftAssignmentDTO> {
    try {
      ...
      const completeAssignment = await this.getAssignmentById(updatedAssignment.id);
      if (!completeAssignment) {
   ...
   */
  async getAgentAssignments(agentId: string, filters: ShiftFilters = {}): Promise<ShiftAssignmentDTO[]> {
    try {
   ...
   */
  private async getAssignmentById(id: string): Promise<ShiftAssignmentDTO | null> {
    try {
      const [result] = await db
        .select({
          assignment: shiftAssignments,
  ...
  private mapToShiftDTO(result: any): ShiftDTO {
    const shift: ShiftDTO = {
      ...
      location: result.location ? {
        ...
        address: `${result.location.address1 || ''} ${result.location.address2 || ''}`.trim()
      } : undefined,
      brand: result.brand ? {
        id: result.brand.id,
