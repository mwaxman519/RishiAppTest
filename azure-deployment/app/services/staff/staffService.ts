...
export class StaffService {
   ...
   */
  async updateStaffMember(id: string, data: UpdateStaffMemberParams): Promise<StaffMember> {
    ...
    if (data.organizationId && data.organizationId !== existingStaff.organizationId) {
      const organization = await organizationService.getOrganizationById(data.organizationId);
      if (!organization) {
        throw new Error(`Organization with ID ${data.organizationId} not found`);
      }
    ...
    if (data.status === StaffStatus.TERMINATED && !data.terminationDate && !existingStaff.terminationDate) {
      throw new Error('Termination date is required when setting status to TERMINATED');
    }
   ...
   */
  async changeStaffStatus(id: string, status: StaffStatus, terminationDate?: string): Promise<StaffMember> {
    // Get the current staff member state
    const existingStaff = await this.repository.findById(id);
    ...
    if (status === StaffStatus.TERMINATED && !terminationDate && !existingStaff.terminationDate) {
      throw new Error('Termination date is required when setting status to TERMINATED');
    }
  ...
  async updateStaffSkill(id: string, data: UpdateStaffSkillParams): Promise<StaffSkill> {
  ...
  async addStaffAvailability(data: CreateAvailabilityParams): Promise<StaffAvailability> {
    // Validate staff member exists
    const staffMember = await this.repository.findById(data.staffId);
    if (!staffMember) {
      throw new Error(`Staff member with ID ${data.staffId} not found`);
  ...
  async updateStaffAvailability(id: string, data: UpdateAvailabilityParams): Promise<StaffAvailability> {
    // Validation logic similar to addition would go here
  ...
  async approveTimeOffRequest(params: ApproveTimeOffRequestParams, reviewerId: string): Promise<TimeOffRequest> {
    // Validate reviewer has permission to approve requests
    // This would be handled by RBAC checks in the API route

    return this.repository.approveTimeOffRequest(params.id, reviewerId, params.notes);
  }

  async rejectTimeOffRequest(params: RejectTimeOffRequestParams, reviewerId: string): Promise<TimeOffRequest> {
    // Validate reviewer has permission to reject requests
    // This would be handled by RBAC checks in the API route

    return this.repository.rejectTimeOffRequest(params.id, reviewerId, params.rejectionReason, params.notes);
  }
  ...
  async isStaffAvailable(
    ...
    const availabilities = await this.repository.getAvailabilityForStaff(staffId);
    
    // Check if available based on regular schedule
    const hasAvailabilityForDay = availabilities.some(a => 
    ...
    const timeOffRequests = await this.repository.getTimeOffRequestsForStaff(staffId);
