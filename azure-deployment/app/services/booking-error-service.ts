...
export const bookingErrorService = {
   ...
   */
  handleError(error: unknown, context: Partial<BookingErrorContext> = {}): BookingError {
    // Convert unknown errors to BookingError format
    const bookingError = this.normalizeError(error, context);
    
    // Log the error with full context
    console.error(`[BookingError][${bookingError.type}] ${bookingError.message}`, {
   ...
   */
  normalizeError(error: unknown, contextOverrides: Partial<BookingErrorContext> = {}): BookingError {
    ...
    const errorMessage = typeof error === 'string' ? error : 'Unknown booking error occurred';
    return new BookingError(
      BookingErrorType.PERSISTENCE_ERROR,
      errorMessage,
