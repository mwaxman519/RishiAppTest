...
import { RosterEvents, type AgentAssignedToBrandEvent, type RosterUpdatedEvent } from "./events";
import { validateBrandAgentAssignment, type AgentSearchCriteria, type BrandAgent } from "./types";

/**
 * Interface defining roster management operations
 */
export interface IRosterService {
  getBrandAgents(organizationId: string, brandId?: string): Promise<BrandAgent[]>;
  assignAgentToBrand(assignment: InsertBrandAgentAssignment, assignedBy: string): Promise<BrandAgentAssignment>;
  ...
  updateAgentSkills(agentId: string, skills: any[], updatedBy: string): Promise<void>;
}
...
export class RosterService implements IRosterService {
  ...
  async getBrandAgents(organizationId: string, brandId?: string): Promise<BrandAgent[]> {
    try {
      const conditions = [
  ...
  async assignAgentToBrand(
    ...
    try {
      ...
      const existingAssignment = await db
        ...
        .where(
          and(
            ...
            eq(brandAgentAssignments.assignmentRole, validatedAssignment.assignmentRole),
            eq(brandAgentAssignments.isActive, true)
          )
        )
        .limit(1);
      ...
      if (existingAssignment.length > 0) {
        throw new Error(`Agent is already assigned to this brand as ${validatedAssignment.assignmentRole}`);
      }
      ...
      await this.eventBus.publish(RosterEvents.AGENT_ASSIGNED_TO_BRAND, assignmentEvent);
  ...
  async removeAgentFromBrand(assignmentId: string, removedBy: string): Promise<void> {
    try {
      ...
      await this.eventBus.publish(RosterEvents.AGENT_REMOVED_FROM_BRAND, removalEvent);
  ...
  async searchAvailableAgents(criteria: AgentSearchCriteria): Promise<BrandAgent[]> {
    try {
      let query = db
        .select({
          id: users.id,
      ...
      if (criteria.organizationId) {
        query = query.where(eq(userOrganizations.organizationId, criteria.organizationId));
      }
  ...
  async updateAgentSkills(agentId: string, skills: any[], updatedBy: string): Promise<void> {
    try {
      await this.auditService.log({
