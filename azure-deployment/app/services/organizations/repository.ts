...
export class OrganizationRepository {
  ...
  async getOrganizationById(id: string): Promise<Organization | null> {
    try {
      ...
      throw new Error(`Failed to fetch organization: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async createOrganization(data: CreateOrganizationParams): Promise<Organization> {
    try {
   ...
   */
  async updateOrganization(id: string, data: UpdateOrganizationParams): Promise<Organization> {
    try {
   ...
   */
  async searchOrganizations(filters: OrganizationFilters): Promise<Organization[]> {
    try {
      ...
      if (filters.parentOrganizationId) {
        query = query.where(eq(organizations.parentOrganizationId, filters.parentOrganizationId));
      }
      ...
      throw new Error(`Failed to search organizations: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async addUserToOrganization(params: OrganizationUserParams): Promise<OrganizationUser> {
    try {
      // Check if the organization exists
      const organization = await this.getOrganizationById(params.organizationId);
      if (!organization) {
        throw new Error(`Organization with ID ${params.organizationId} not found`);
      }
      
      // Check if the user is already in this organization
      const existingUsers = await db.select().from(organizationUsers).where(
        and(
      ...
      const [result] = await db.insert(organizationUsers).values({
      ...
      throw new Error(`Failed to add user to organization: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async updateUserRole(organizationId: string, userId: string, roleId: string): Promise<OrganizationUser> {
    try {
      // Check if the user is in this organization
      const [existingUser] = await db.select().from(organizationUsers).where(
        and(
      ...
      console.error(`Error updating user role in organization ${organizationId}:`, error);
  ...
  async removeUserFromOrganization(organizationId: string, userId: string): Promise<void> {
    try {
      ...
      console.error(`Error removing user from organization ${organizationId}:`, error);
      throw new Error(`Failed to remove user from organization: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async getUserOrganizations(userId: string): Promise<UserOrganization[]> {
    try {
      ...
      throw new Error(`Failed to fetch user organizations: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async getOrganizationUsers(organizationId: string): Promise<OrganizationUser[]> {
    try {
      ...
      throw new Error(`Failed to fetch organization users: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async setDefaultOrganization(userId: string, organizationId: string): Promise<void> {
    try {
      ...
      console.error(`Error setting default organization for user ${userId}:`, error);
      throw new Error(`Failed to set default organization: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
