...
export class SchedulingService {
  ...
  async createSchedule(data: CreateScheduleParams, createdById: string, organizationId: string): Promise<Schedule> {
    ...
    if (event.status === EventStatus.CANCELLED || event.status === EventStatus.REJECTED) {
      throw new Error(`Cannot create schedule for event with status ${event.status}`);
    }
    ...
    if (data.templateId) {
      ...
      newSchedule = await this.repository.createSchedule(data, createdById, organizationId);
  ...
  async updateSchedule(id: string, data: UpdateScheduleParams): Promise<Schedule> {
    ...
    if (data.eventId && data.eventId !== existingSchedule.eventId) {
      ...
      if (event.status === EventStatus.CANCELLED || event.status === EventStatus.REJECTED) {
        throw new Error(`Cannot assign schedule to event with status ${event.status}`);
      }
    }
    ...
    if (data.status) {
      if (data.status === ScheduleStatus.PUBLISHED && existingSchedule.status === ScheduleStatus.DRAFT) {
        throw new Error('Use publishSchedule method to publish a schedule');
      }
      
      if (data.status === ScheduleStatus.FINALIZED && 
          (existingSchedule.status === ScheduleStatus.DRAFT || existingSchedule.status === ScheduleStatus.PUBLISHED)) {
   ...
   * Publish a schedule
   */
  async publishSchedule(params: PublishScheduleParams, publishedById: string): Promise<Schedule> {
    // Get the current schedule state
    ...
    const publishedSchedule = await this.repository.publishSchedule(params.id, publishedById);
   ...
   * Finalize a schedule
   */
  async finalizeSchedule(params: FinalizeScheduleParams, finalizedById: string): Promise<Schedule> {
    // Get the current schedule state
    const existingSchedule = await this.repository.findScheduleById(params.id);
  ...
  async updateShift(id: string, data: UpdateShiftParams): Promise<ScheduleShift> {
    ...
    const schedule = await this.repository.findScheduleById(existingShift.scheduleId);
    if (!schedule) {
   ...
   */
  async getAssignments(filters: AssignmentFilters = {}): Promise<ShiftAssignment[]> {
  ...
  async createAssignment(data: CreateShiftAssignmentParams): Promise<ShiftAssignment> {
    ...
    if (staff.status !== StaffStatus.ACTIVE) {
      throw new Error(`Cannot assign inactive staff member with status ${staff.status}`);
    }
    ...
    const existingAssignments = await this.repository.findAssignmentsForShift(data.shiftId);
    const alreadyAssigned = existingAssignments.some(a => a.staffId === data.staffId);
  ...
  async updateAssignment(id: string, data: UpdateShiftAssignmentParams): Promise<ShiftAssignment> {
    ...
    // Get the schedule to check its status
    const schedule = await this.repository.findScheduleById(existingAssignment.scheduleId);
    if (!schedule) {
      throw new Error(`Schedule with ID ${existingAssignment.scheduleId} not found`);
    }
    ...
    if (data.checkedInAt || data.checkedOutAt) {
      ...
      if (data.checkedOutAt && !data.checkedInAt && !existingAssignment.checkedInAt) {
      ...
      if (data.checkedInAt && !data.checkedOutAt && existingAssignment.checkedOutAt) {
        const checkedInAt = new Date(data.checkedInAt);
        const checkedOutAt = new Date(existingAssignment.checkedOutAt);
        
        if (checkedOutAt <= checkedInAt) {
          throw new Error(`Check-in time must be before existing check-out time`);
        }
      }
      
      // If only check-out provided, validate against existing check-in
      if (!data.checkedInAt && data.checkedOutAt && existingAssignment.checkedInAt) {
        const checkedInAt = new Date(existingAssignment.checkedInAt);
        const checkedOutAt = new Date(data.checkedOutAt);
        
        if (checkedOutAt <= checkedInAt) {
          throw new Error(`Check-out time must be after existing check-in time`);
        }
      }
    }

    // If declining, ensure reason is provided
    if (data.status === 'declined' && !data.declineReason && !existingAssignment.declineReason) {
  ...
  async updateScheduleTemplate(id: string, data: UpdateScheduleTemplateParams): Promise<ScheduleTemplate> {
    ...
    if (data.organizationId && data.organizationId !== existingTemplate.organizationId) {
      const organization = await organizationService.getOrganizationById(data.organizationId);
      if (!organization) {
        throw new Error(`Organization with ID ${data.organizationId} not found`);
      }
   ...
   */
  private async applyTemplateToSchedule(template: ScheduleTemplate, scheduleId: string): Promise<void> {
    // Get the schedule
    const schedule = await this.repository.findScheduleById(scheduleId);
    if (!schedule) {
      throw new Error(`Schedule with ID ${scheduleId} not found`);
    ...
    const shiftTemplates = await this.repository.findShiftTemplates(template.id);
