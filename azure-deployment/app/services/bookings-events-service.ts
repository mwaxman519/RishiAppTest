...
export class BookingsEventsService {
   ...
   */
  async approveBooking(bookingId: string, userId: string, generateEvents: boolean = true) {
    ...
    return await retryExecutor.execute(async () => {
      return db.transaction(async (tx) => {
        ...
        if (generateEvents) {
          const eventIds = await this.generateEventInstances(tx, updatedBooking);
        ...
        try {
          ...
          console.error('Failed to publish booking approval event', { error, bookingId });
   ...
   */
  private async generateEventInstances(tx: any, booking: typeof bookings.$inferSelect) {
   ...
   */
  async assignEventToManager(eventId: string, managerId: string, assignedById: string) {
    const retryExecutor = new RetryExecutor({
      maxRetries: 3,
      retryableErrors: ['deadlock', 'timeout', 'connection']
    });
    ...
    return await retryExecutor.execute(async () => {
      ...
      try {
        ...
        console.error('Failed to publish manager assignment event', { error, eventId });
      }
   ...
   */
  async startEventPreparation(eventId: string, userId: string, tasks?: Array<{
    description: string;
    assignedTo?: string;
    dueDate: string;
  }>) {
   ...
   */
  async markEventReady(eventId: string, userId: string, details: {
