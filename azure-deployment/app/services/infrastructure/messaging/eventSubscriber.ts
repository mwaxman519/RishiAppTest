...
export type EventHandler<E extends AppEvent> = 
  (payload: EventPayload[E], metadata?: EventMetadata) => void | Promise<void>;

// Batch handler for processing multiple events at once
export type BatchEventHandler<E extends AppEvent> = 
  (payloads: EventPayload[E][], metadata?: BatchEventMetadata) => void | Promise<void>;
...
export class AdvancedEventSubscriber<E extends AppEvent = AppEvent> {
  ...
  subscribe(
    ...
    this.eventBus.subscribe(event, (payload) => this.handleEvent(event, payload, subscriptionId));
  ...
  private async handleEvent<E extends AppEvent>(
    ...
    if (subscription.options.filter && !subscription.options.filter(event, payload)) {
      return;
    }
    ...
    if (subscription.isBatchHandler) {
      ...
      if (subscription.batchBuffer.length >= (subscription.options.batchSize || 10)) {
        this.processBatch(subscription);
      }
      // Or set/reset a timeout to process the batch after the timeout period
      else if (subscription.options.batchTimeoutMs && !subscription.batchTimeout) {
  ...
  private async processEvent<E extends AppEvent>(
    ...
    try {
      ...
      subscription.lastError = error instanceof Error ? error : new Error(String(error));
      
      console.error(`Error in event handler (subscription ${subscription.id}):`, error);
   ...
   */
  private async processBatch<E extends AppEvent>(subscription: Subscription<E>): Promise<void> {
    ...
    try {
      ...
      console.error(`Error in batch event handler (subscription ${subscription.id}):`, error);
   ...
   */
  private processBatchIfNeeded<E extends AppEvent>(subscription: Subscription<E>): void {
    if (subscription.isBatchHandler && subscription.batchBuffer && subscription.batchBuffer.length > 0) {
      this.processBatch(subscription);
    }
  }
  ...
  private retryEvent<E extends AppEvent>(
    ...
    setTimeout(async () => {
      try {
        ...
        console.error(`Error in retry ${currentRetry + 1} for subscription ${subscription.id}:`, error);
   ...
   */
  getSubscription(subscriptionId: string): Omit<Subscription<any>, 'handler'> | undefined {
    const subscription = this.subscriptions.get(subscriptionId);
