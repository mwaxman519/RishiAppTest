...
export class DistributedEventBus {
   ...
   */
  unsubscribe(subscriber: EventSubscriber, eventType?: AppEvent | string): void {
    if (eventType) {
  ...
  async publish<T>(
    ...
    console.log(`[EventBus] Publishing event: ${eventType}`, { eventId: event.id });
  ...
  private async executeWithRetry<T>(
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        lastError = error;
        console.warn(`[EventBus] Retry attempt ${attempt + 1} failed for event ${eventType || 'unknown'} (${eventId || 'unknown'}):`, error);
        
        // On last attempt, we should integrate with our error handling strategies
        if (attempt === this.maxRetries - 1 && eventType) {
          console.log(`[EventBus] Last retry attempt failed for event ${eventType} (${eventId || 'unknown'})`);
