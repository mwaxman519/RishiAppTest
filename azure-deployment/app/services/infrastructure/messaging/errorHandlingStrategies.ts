...
export class RetryStrategy implements ErrorHandlingStrategy {
  ...
  async handleError(
    ...
    if (currentRetries >= this.maxRetries) {
      console.log(`[RetryStrategy] Max retries (${this.maxRetries}) exceeded for event ${eventType} (${eventId})`);
      return false; // Can't handle this error anymore
    }
    
    // Calculate delay with exponential backoff and jitter
    const delay = this.initialDelay * Math.pow(2, currentRetries) * (0.75 + Math.random() * 0.5);
    
    console.log(`[RetryStrategy] Retrying event ${eventType} (${eventId}) in ${delay}ms (attempt ${currentRetries + 1}/${this.maxRetries})`);
...
export class DeadLetterQueueStrategy implements ErrorHandlingStrategy {
  ...
  async handleError(
    ...
    if (!event) {
      console.warn(`[DeadLetterQueueStrategy] No event provided in context for ${eventType} (${eventId})`);
      return false;
    }
    ...
    console.log(`[DeadLetterQueueStrategy] Added event ${eventType} (${eventId}) to dead letter queue`);
    
    // In a real implementation, we would persist the DLQ to a database
    // For now, just log the DLQ size
    console.log(`[DeadLetterQueueStrategy] DLQ size for ${eventType}: ${queue.length}`);
...
export class NotificationStrategy implements ErrorHandlingStrategy {
  ...
  constructor() {
    ...
    this.notifyAdmins = async (message, context) => {
      console.log(`[NotificationStrategy] ADMIN NOTIFICATION: ${message}`, context);
    };
  }
