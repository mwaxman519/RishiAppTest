...
// Message states
export type MessageState = 'pending' | 'processing' | 'delivered' | 'failed' | 'dead-letter';
...
export class InMemoryMessageStorage implements MessageStorage {
  ...
  async saveMessage<E extends AppEvent>(message: EventMessage<E>): Promise<void> {
...
export class DatabaseMessageStorage implements MessageStorage {
  async saveMessage<E extends AppEvent>(message: EventMessage<E>): Promise<void> {
    ...
    console.log(`[DB] Storing message ${message.id} for event ${message.event}`);
...
export class RetryableEventBus implements EventPublisher, EventSubscriber {
  ...
  private async processMessages(): Promise<void> {
    ...
    const messages = await this.storage.getMessages('pending', this.config.batchSize);
  ...
  private async processMessage(message: EventMessage): Promise<void> {
    ...
    try {
      ...
      console.error(`Error processing message ${message.id} for event ${message.event}:`, errorMessage);
    }
  }
  ...
  private async retryFailedMessages(): Promise<void> {
    ...
    for (const message of failedMessages) {
      ...
      if (message.retryCount >= message.maxRetries) {
        ...
        console.error(`Message ${message.id} for event ${message.event} moved to dead letter queue after ${message.retryCount} attempts`);
      } else {
        ...
        const nextBackoff = this.calculateBackoffDelay(updatedMessage.retryCount);
        console.log(
  ...
  private calculateBackoffDelay(retryCount: number): number {
    return this.config.retryInitialDelayMs * Math.pow(this.config.retryBackoffFactor, retryCount);
  }
   ...
   */
  private async publishImmediately<E extends AppEvent>(message: EventMessage<E>): Promise<boolean> {
    try {
      // Mark as processing
      await this.storage.updateMessageState(message.id, 'processing');
...
export class DeadLetterQueueHandler {
  ...
  async requeueMessage(id: string): Promise<void> {
    ...
    if (message) {
      ...
      await this.storage.saveMessage(updatedMessage);
      console.log(`Requeued dead letter message ${id} for event ${message.event}`);
    } else {
