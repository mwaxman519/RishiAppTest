...
import { AvailabilityDTO, AvailabilityQueryOptions, AvailabilitiesResponse, 
  AvailabilityResponse, ServiceResponse, ConflictCheckResponse, 
  CreateAvailabilityRequest, UpdateAvailabilityRequest } from './models';
...
class ClientAvailabilityService implements IAvailabilityService {
  async getAvailabilityBlocks(options: AvailabilityQueryOptions): Promise<AvailabilitiesResponse> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch availability blocks: ${response.status} ${response.statusText}`);
      }
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to fetch availability blocks'
      };
    }
  }
  ...
  async getAvailabilityBlockById(id: number): Promise<AvailabilityResponse> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch availability block: ${response.status} ${response.statusText}`);
      }
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to fetch availability block'
      };
    }
  }
  ...
  async createAvailabilityBlock(data: CreateAvailabilityRequest): Promise<AvailabilityResponse> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to create availability block: ${response.status}`);
      }
      
      const responseData = await response.json();
      return {
        success: true,
        data: responseData.success && responseData.data ? responseData.data : responseData
      };
    } catch (error) {
  ...
  async updateAvailabilityBlock(id: number, data: UpdateAvailabilityRequest): Promise<AvailabilityResponse> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || `Failed to update availability block: ${response.status}`);
      }
  ...
  async deleteAvailabilityBlock(id: number, deleteSeries: boolean = false): Promise<ServiceResponse<{success: boolean, count?: number}>> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || `Failed to delete availability block: ${response.status}`);
      }
      ...
      let responseData: {success: boolean, count?: number} = { success: true };
      ...
      console.error(`Error in deleteAvailabilityBlock(${id}, deleteSeries=${deleteSeries}):`, error);
      return {
        success: false,
  ...
  async checkForConflicts(
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || `Failed to check for conflicts: ${response.status}`);
      }
