...
import { AvailabilityBlock, availabilityBlocks, InsertAvailabilityBlock } from '../../../shared/schema';
import { sql, eq, and, or, ne } from 'drizzle-orm';
import { AvailabilityDTO, AvailabilityQueryOptions, AvailabilityConflict } from './models';
...
export class AvailabilityRepository {
   ...
   */
  private async getDaysOfWeekInRange(startDate: Date | string, endDate: Date | string): Promise<number[]> {
  ...
  async findAll(options: AvailabilityQueryOptions): Promise<AvailabilityDTO[]> {
    try {
      console.log('🔍 [AvailabilityRepository.findAll] Starting query with options:',
        JSON.stringify(options, (key, value) => value instanceof Date ? value.toISOString() : value)
      );

      // Validate userId must be a number
      if (!options.userId || isNaN(options.userId)) {
        console.error('❌ [AvailabilityRepository.findAll] Invalid userId:', options.userId);
        return []; // Return empty array instead of throwing
      }

      // Handle potential database issues
      if (!db) {
        console.error('❌ [AvailabilityRepository.findAll] Database instance is not available');
      ...
      if (options.startDate && options.endDate) {
        ...
        if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
          console.error('❌ [AvailabilityRepository.findAll] Invalid date range:', options.startDate, options.endDate);
          return []; // Return empty array instead of throwing
        }

        try {
          // Get days of week that fall in the date range for recurring blocks
          const daysOfWeekInRange = await this.getDaysOfWeekInRange(startDate, endDate);

          console.log(`✅ [AvailabilityRepository.findAll] Days of week in range: ${daysOfWeekInRange.join(', ')}`);
          ...
          console.error('❌ [AvailabilityRepository.findAll] Error processing date range:', dateError);
      ...
      console.log(`ℹ️ [AvailabilityRepository.findAll] Applying ${filters.length} filters to query`);
      ...
      try {
        ...
        const queryPromise = filters.length > 0
          ? db.select({
        ...
        console.log(`✅ [AvailabilityRepository.findAll] Query successful, found ${blocks.length} blocks in ${executionTime}ms`);
        return blocks.map((block: AvailabilityBlock) => this.mapToAvailabilityDTO(block));
      } catch (dbError) {
        console.error('❌ [AvailabilityRepository.findAll] Database query error:', dbError);
        return []; // Return empty array instead of throwing
      }
    } catch (error) {
      console.error('❌ [AvailabilityRepository.findAll] Unhandled error:', error);
  ...
  async create(data: InsertAvailabilityBlock): Promise<AvailabilityDTO> {
    try {
      const [newBlock] = await db.insert(availabilityBlocks).values(data).returning();
  ...
  async update(id: number, data: Partial<InsertAvailabilityBlock>): Promise<AvailabilityDTO | null> {
    try {
  ...
  async delete(id: number): Promise<boolean> {
    try {
      ...
      console.log(`Block to delete: ID=${id}, isRecurring=${block.is_recurring}, recurrenceGroup=${block.recurrence_group}, startDate=${new Date(block.start_date).toISOString()}`);
      
      // More strict checking for recurring blocks to avoid bugs
      const isRecurringBlock = block.is_recurring === true && 
      ...
      if (!isRecurringBlock) {
        ...
        const result = await db.delete(availabilityBlocks).where(eq(availabilityBlocks.id, id));
        console.log(`Deleted non-recurring block ${id}`);
        return true;
      }
      
      // For a recurring block being individually deleted, we need special handling
      console.log(`📅 Deleting single occurrence of recurring block: ${id} in group ${block.recurrence_group}`);
      ...
      console.log(`🔎 Found ${otherInstances.length} other instances in the same recurrence group`);
      ...
      if (recurrenceEndType === 'count' && recurrenceCount && recurrenceCount > 1) {
        // We're deleting one occurrence, so decrement the count
        const newCount = Math.max(1, recurrenceCount - 1);
        console.log(`Updating recurrence count from ${recurrenceCount} to ${newCount} after deleting occurrence`);
        ...
        console.log(`Updated ${updateResult.length} remaining blocks with new count: ${newCount}`);
      }
      
      console.log(`✅ Successfully deleted single occurrence: ${id} from recurrence group ${recurrenceGroup}`);
      ...
      console.log(`After deletion, ${remainingBlocks[0].count} blocks remain in recurrence group ${recurrenceGroup}`);
  ...
  async deleteSeries(id: number): Promise<{success: boolean, count: number}> {
    try {
      ...
      if (!block || !block.is_recurring || !block.recurrence_group) {
        console.warn(`Block ${id} is not recurring or has no recurrence_group, cannot delete series`);
      ...
      console.log(`Deleting all blocks in recurrence group ${recurrenceGroup} for user ${userId}`);
      
      // Delete all blocks with the same recurrence group and user ID
      const result = await db.delete(availabilityBlocks)
        .where(
          and(
            eq(availabilityBlocks.recurrence_group, recurrenceGroup),
      ...
      console.log(`Deleted ${result.length} blocks from series with recurrence group ${recurrenceGroup}`);
  ...
  async findConflicts(
    ...
    try {
      console.log(`[AvailabilityRepository.findConflicts] Checking conflicts for userId=${userId}, startDate=${startDate}, endDate=${endDate}, excludeBlockId=${excludeBlockId || 'none'}`);

      // Validate inputs
      if (!userId || isNaN(userId)) {
        console.error(`[AvailabilityRepository.findConflicts] Invalid userId: ${userId}`);
        return []; // Return empty array instead of throwing
      }

      // Convert string dates to Date objects and normalize to UTC
      const startDateObj = new Date(typeof startDate === 'string' ? startDate : startDate.toISOString());
      const endDateObj = new Date(typeof endDate === 'string' ? endDate : endDate.toISOString());
      ...
      if (!startDateObj || !endDateObj || isNaN(startDateObj.getTime()) || isNaN(endDateObj.getTime())) {
        console.error(`[AvailabilityRepository.findConflicts] Invalid dates: startDate=${startDate}, endDate=${endDate}`);
        return []; // Return empty array instead of throwing
      }
      ...
      try {
        ...
        console.log(`[AvailabilityRepository.findConflicts] Executing query with ${baseFilters.length} filters`);
        ...
        console.log(`[AvailabilityRepository.findConflicts] Found ${conflictingBlocks.length} potential conflicts`);
        ...
        return conflictingBlocks.map((block: AvailabilityBlock) => {
          ...
          // Check if blocks are adjacent (might want to allow this)
            ...
            Math.abs(new Date(startDateObj).getTime() - blockEnd.getTime()) < 1000 ||
            Math.abs(new Date(endDateObj).getTime() - blockStart.getTime()) < 1000
          ) {
        ...
        console.error('[AvailabilityRepository.findConflicts] Database error:', dbError);
        return []; // Return empty array instead of throwing
      }
    } catch (error) {
      console.error('[AvailabilityRepository.findConflicts] Unhandled error:', error);
      return []; // Return empty array instead of throwing to make the API more resilient
    }
  }
}
