  ...
  async calculatePlatformKPIs(
    ...
    try {
      ...
      const kpis: PlatformKPIs = {
        ...
        activeUsers: {
          daily: await this.calculateActiveUsers(organizationId, 'daily', startDate, endDate),
          weekly: await this.calculateActiveUsers(organizationId, 'weekly', startDate, endDate),
          monthly: await this.calculateActiveUsers(organizationId, 'monthly', startDate, endDate)
        },
        
        // Operational metrics
        shiftsCompleted: await this.calculateShiftsCompleted(organizationId, startDate, endDate),
        averageShiftDuration: await this.calculateAverageShiftDuration(organizationId, startDate, endDate),
        shiftCompletionRate: await this.calculateShiftCompletionRate(organizationId, startDate, endDate),
        
        // Financial metrics
        totalExpenses: await this.calculateTotalExpenses(organizationId, startDate, endDate),
        approvedExpenses: await this.calculateApprovedExpenses(organizationId, startDate, endDate),
        pendingExpenses: await this.calculatePendingExpenses(organizationId, startDate, endDate),
        averageExpenseAmount: await this.calculateAverageExpenseAmount(organizationId, startDate, endDate),
        
        // Efficiency metrics
        timeUtilization: await this.calculateTimeUtilization(organizationId, startDate, endDate),
        scheduleAdherence: await this.calculateScheduleAdherence(organizationId, startDate, endDate),
        conflictResolutionTime: await this.calculateConflictResolutionTime(organizationId, startDate, endDate),
        
        // Quality metrics
        customerSatisfactionScore: await this.calculateCustomerSatisfactionScore(organizationId, startDate, endDate),
        taskCompletionRate: await this.calculateTaskCompletionRate(organizationId, startDate, endDate),
        errorRate: await this.calculateErrorRate(organizationId, startDate, endDate)
      };
