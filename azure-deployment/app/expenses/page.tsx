...
interface ExpenseData {
  ...
  eventId?: string | null;
  shiftId?: string | null;
  expenseType: 'mileage' | 'meals' | 'parking' | 'supplies' | 'lodging' | 'transportation' | 'other';
  amount: string;
  currency: string;
  description: string;
  expenseDate: string;
...
export default function ExpensePage() {
  ...
  const submitExpense = async (action: 'draft' | 'submit' = 'submit') => {
    try {
      const expenseData = {
        ...
        ...newExpense,
        mileageData: newExpense.expenseType === 'mileage' ? newExpense.mileageData : null
      };
  ...
  const filteredExpenses = expenses.filter(expense => {
    ...
    if (filters.type !== 'all' && expense.expenseType !== filters.type) return false;
    ...
    <div className="container mx-auto px-4 py-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Dialog open={showMileageCalculator} onOpenChange={setShowMileageCalculator}>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setMileageCalc(prev => ({ ...prev, startLocation: e.target.value }))}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setMileageCalc(prev => ({ ...prev, endLocation: e.target.value }))}
                ...
                {mileageCalc.result && (
                  <div className="p-4 bg-green-50 dark:bg-green-950 rounded-lg border border-green-200 dark:border-green-800">
                    <h4 className="font-semibold text-green-900 dark:text-green-100">Calculation Result</h4>
                    <p className="text-green-700 dark:text-green-300">Distance: {mileageCalc.result.distance} miles</p>
                    <p className="text-green-700 dark:text-green-300">Rate: ${mileageCalc.result.rate} per mile</p>
                    <p className="text-green-700 dark:text-green-300 font-bold">Total: {formatCurrency(mileageCalc.result.amount)}</p>
                  </div>
                )}
              </div>
            </DialogContent>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value: any) => setNewExpense(prev => ({ ...prev, expenseType: value }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setNewExpense(prev => ({
                          ...prev,
                          mileageData: { ...prev.mileageData, startLocation: e.target.value }
                        }))}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setNewExpense(prev => ({
                          ...prev,
                          mileageData: { ...prev.mileageData, endLocation: e.target.value }
                        }))}
                ...
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => submitExpense('draft')} className="flex-1">
                    Save Draft
                  </Button>
                  <Button onClick={() => submitExpense('submit')} className="flex-1">
                    Submit
                  </Button>
                </div>
              </div>
          ...
          {summary && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{summary.pendingApproval}</div>
                  <p className="text-xs text-muted-foreground">
              ...
              {summary && (
                <div className="space-y-4">
                  {Object.entries(summary.byCategory).map(([category, data]) => {
                    const Icon = ExpenseTypeIcons[category as keyof typeof ExpenseTypeIcons] || Receipt;
                    return (
                      <div key={category} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                        <div className="flex items-center gap-3">
                          <Icon className="h-5 w-5 text-muted-foreground" />
                          <div>
                            <p className="font-medium">{ExpenseTypeLabels[category as keyof typeof ExpenseTypeLabels] || category}</p>
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                              ...
                              {expense.agent && (
                                <>
                                  <span>•</span>
                                  <span>{expense.agent.firstName} {expense.agent.lastName}</span>
                                </>
                              )}
                            </div>
                          </div>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    Reports will be generated in PDF format and include all approved expenses for the selected period.
                  </AlertDescription>
                </Alert>
              </div>
            </CardContent>
