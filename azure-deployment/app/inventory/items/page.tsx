...
const inventoryItemsData = [
  {
    ...
    assignedToTemplates: ['Product Demo Standard Kit', 'Basic Retail Support Kit'],
    warranties: {
      hasWarranty: true,
  ...
  },
  {
    ...
    assignedToTemplates: ['Product Demo Standard Kit', 'Street Team Activation Kit'],
    warranties: {
      hasWarranty: false,
      expiryDate: null,
...
const InventoryItemCard = ({ item, onAction }: { item: InventoryItem; onAction: (action: string, itemId: string) => void }) => {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-blue-50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium text-blue-800">Stock Level</span>
            </div>
          ...
          <div className="w-full bg-blue-200 rounded-full h-2">
            <div 
              className={`h-2 rounded-full transition-all duration-300 ${
                getStockPercentage() > 50 ? 'bg-gradient-to-r from-blue-500 to-blue-600' :
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center bg-green-50 rounded-lg p-3">
            <div className="text-lg font-bold text-green-600">${item.unitCost.toFixed(0)}</div>
            <div className="text-xs text-green-700">Unit Cost</div>
          </div>
          <div className="text-center bg-purple-50 rounded-lg p-3">
            <div className="text-lg font-bold text-purple-600">${item.totalValue.toFixed(0)}</div>
        ...
        <div className="flex items-center justify-between bg-gray-50 rounded-lg p-3">
          ...
          <div>
            ...
            <p className={`text-sm ${getUsageFrequencyColor(item.usageFrequency)}`}>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium">{formatLastRestocked(item.lastRestocked)}</span>
          </div>
        </div>
        ...
        <div className="flex space-x-2 pt-2 border-t">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              ...
              <DropdownMenuItem onClick={() => onAction('maintenance', item.id)}>
...
export default function InventoryItemsPage() {
  ...
  const inStockItems = inventoryItemsData.filter(i => i.status === 'in_stock').length;
  const lowStockItems = inventoryItemsData.filter(i => i.status === 'low_stock').length;
  const outOfStockItems = inventoryItemsData.filter(i => i.status === 'out_of_stock').length;
  const totalValue = inventoryItemsData.reduce((sum, i) => sum + i.totalValue, 0);

  const uniqueCategories = [...new Set(inventoryItemsData.map(i => i.category))];
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage individual inventory items, stock levels, and equipment tracking
          </p>
        </div>
        <Button>
