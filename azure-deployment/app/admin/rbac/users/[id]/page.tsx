...
import { ArrowLeft, Check, X, Save, Shield, User, Filter, RefreshCw, AlertCircle } from 'lucide-react';
import { ApplicationFeatures, getAllFeatures, Feature } from '../../../../../shared/rbac/features';
...
export default function UserPermissionsPage({ params }: { params: { id: string } }) {
  ...
  const [dirtyPermissions, setDirtyPermissions] = useState<Record<string, boolean>>({});
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!permissionsResponse.ok) {
          ...
          console.warn("Permission endpoint returned error:", permissionsResponse.status);
          
          // Create permissions based on role for development
          const roleDefinition = allRoles.find(r => r.id === userDataResult.role);
          const allPermissions: UserPermission[] = [];
          
          // Extract all possible permissions from features
          allFeatures.forEach(feature => {
            feature.operations.forEach(op => {
              const hasPermission = roleDefinition?.permissions.includes(op.permission) || false;
              allPermissions.push({
        ...
        toast({
          title: "Error",
          description: err instanceof Error ? err.message : "Failed to load user permissions",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
  ...
  const filteredFeatures = (() => {
    ...
    if (searchQuery) {
      ...
      features = features.filter(feature => 
        ...
        feature.operations.some(op => op.permission.toLowerCase().includes(query))
      );
    }
  ...
  const isPermissionGranted = (permission: string) => {
    ...
    const permissionObj = userData.permissions.find(p => p.permission === permission);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const permissionsToUpdate = Object.entries(dirtyPermissions).map(([permission, granted]) => ({
        permission,
        granted
      }));
      ...
      setUserData(prev => {
        ...
        return {
          ...
          overriddenPermissions: [
            ...
            ...Object.keys(dirtyPermissions).filter(perm => !prev.overriddenPermissions.includes(perm))
          ]
        };
      });
  ...
  const resetToRoleDefaults = () => {
    ...
    userData.permissions.forEach(permission => {
      const shouldBeGranted = roleDefinition.permissions.includes(permission.permission);
  ...
  if (!user || !checkPermission('edit:permissions')) {
    return (
      <SidebarLayout>
        <div className="p-6">
          <h1 className="text-2xl font-bold mb-6 text-gray-900 dark:text-gray-100">User Permissions</h1>
      ...
      <div className="p-6">
        ...
        {loading ? (
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 p-6 rounded">
            <h3 className="text-lg font-semibold text-yellow-800 dark:text-yellow-300 mb-2">User Not Found</h3>
            <p className="text-yellow-700 dark:text-yellow-400">The user you're looking for doesn't exist or has been deleted.</p>
            <button 
              onClick={handleBack}
            ...
            <Tabs defaultValue="by-service" className="mb-6">
              ...
              <TabsContent value="by-service">
                <div className="space-y-6">
                  {Object.entries(featuresByService).map(([service, features]) => (
                    <div key={service} className="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                          ...
                          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {features.flatMap(feature => [
                              // Feature header
                              <tr key={`feature-${feature.id}`} className="bg-gray-50 dark:bg-gray-900/30">
                              ...
                              ...feature.operations.map(operation => (
                                <tr key={`operation-${feature.id}-${operation.id}`} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                  <td className="px-6 py-4">
                                    <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                                      {operation.name}
                                    </div>
                                    <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                      {formatPermission(operation.permission)} <br />
                                      {operation.description}
                                    </div>
                                  </td>
                                  <td className="px-6 py-4 text-center">
                                    <button
                                      onClick={() => togglePermission(operation.permission)}
                                      className={`inline-flex items-center justify-center w-10 h-10 rounded-full ${
                                        isPermissionGranted(operation.permission)
                                  ...
                                  <td className="px-6 py-4">
                                    {isPermissionOverridden(operation.permission) || operation.permission in dirtyPermissions ? (
                              ...
                              ...(feature.subFeatures || []).flatMap(subfeature => [
                                // Subfeature header
                                <tr key={`subfeature-${feature.id}-${subfeature.id}`} className="bg-gray-100 dark:bg-gray-800/50">
                                  <td colSpan={3} className="px-6 py-3 pl-12">
                                    <div className="font-medium text-gray-800 dark:text-gray-200">{subfeature.name}</div>
                                    <div className="text-sm text-gray-500 dark:text-gray-400">{subfeature.description}</div>
                                  </td>
                                </tr>,
                                
                                // Subfeature operations
                                ...subfeature.operations.map(operation => (
                                  <tr key={`operation-${feature.id}-${subfeature.id}-${operation.id}`} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {userData.permissions
                          .filter(p => {
                            ...
                            return p.permission.toLowerCase().includes(searchQuery.toLowerCase());
                          })
                          ...
                          .map(permission => {
                            ...
                            for (const feature of allFeatures) {
                              ...
                              const operation = feature.operations.find(op => op.permission === permission.permission);
                              if (operation) {
                              ...
                              if (feature.subFeatures) {
                                for (const subfeature of feature.subFeatures) {
                                  const subOperation = subfeature.operations.find(op => op.permission === permission.permission);
                                  if (subOperation) {
                            ...
                            return (
                              <tr key={permission.permission} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                <td className="px-6 py-4">
                                  <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                                ...
                                <td className="px-6 py-4 text-center">
                                  <button
                                    onClick={() => togglePermission(permission.permission)}
                                    className={`inline-flex items-center justify-center w-10 h-10 rounded-full ${
                                ...
                                <td className="px-6 py-4">
                                  {isPermissionOverridden(permission.permission) || permission.permission in dirtyPermissions ? (
                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300">
                                      Overridden
                                    </span>
                                  ) : (
                      ...
                      <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        {userData.permissions
                          ...
                          .map(permission => {
                            ...
                            const roleDefinition = allRoles.find(r => r.id === userData.role);
                            const roleDefaultGranted = roleDefinition?.permissions.includes(permission.permission) || false;
                                ...
                                <td className="px-6 py-4">
                                  <button
                                    onClick={() => {
                                      ...
                                      setDirtyPermissions(prev => ({
                                        ...prev,
                                        [permission.permission]: roleDefaultGranted
                                      }));
                                    }}
...
// Helper components
function RoleFeatureAccess({ role, feature, expanded }: { role: any, feature: Feature, expanded: boolean }) {
  // Count how many operations this role has access to
  const totalOperations = feature.operations.length + 
    (feature.subFeatures?.reduce((sum, sub) => sum + sub.operations.length, 0) || 0);
  
  const accessCount = feature.operations.filter(op => 
    role.permissions.includes(op.permission)
  ).length + (feature.subFeatures?.reduce((sum, sub) => 
    sum + sub.operations.filter(op => role.permissions.includes(op.permission)).length, 0
  ) || 0);
  
  const accessPercentage = totalOperations > 0 ? Math.round((accessCount / totalOperations) * 100) : 0;
