...
export default function AdminUsersPage() {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          const userStats = {
            ...
            admins: response.data.filter(u => 
              u.role === 'super_admin' || 
              u.role === 'internal_admin'
            ).length,
            brandAgents: response.data.filter(u => 
              u.role === 'brand_agent'
            ).length,
            clientUsers: response.data.filter(u => 
              u.role === 'client_user' || 
              u.role === 'client_manager'
            ).length,
            internalUsers: response.data.filter(u => 
  ...
  if (!user || !(user.role === 'super_admin' || user.role === 'internal_admin')) {
    return (
      <div className="p-6">
                  ...
                  <div className="px-6 border-b">
                    <TabsList className="mb-px">
                      <TabsTrigger value="all-users" className="relative">
                        All Users
                        <Badge className="ml-2 h-5 px-1.5">{stats.totalUsers}</Badge>
                      </TabsTrigger>
                      <TabsTrigger value="admins">
                        Administrators
                        <Badge className="ml-2 h-5 px-1.5">{stats.admins}</Badge>
                      </TabsTrigger>
                      <TabsTrigger value="brand-agents">
                        Brand Agents
                        <Badge className="ml-2 h-5 px-1.5">{stats.brandAgents}</Badge>
                      </TabsTrigger>
                      <TabsTrigger value="client-users">
                        Client Users
                        <Badge className="ml-2 h-5 px-1.5">{stats.clientUsers}</Badge>
                      </TabsTrigger>
                      <TabsTrigger value="internal-staff">
                        Internal Staff
                        <Badge className="ml-2 h-5 px-1.5">{stats.internalUsers}</Badge>
                      </TabsTrigger>
                      <TabsTrigger value="inactive-users">
                        Inactive Users
                        <Badge className="ml-2 h-5 px-1.5">{stats.inactiveUsers}</Badge>
                      </TabsTrigger>
                    </TabsList>
                  </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-3">
              ...
              <Button variant="outline" onClick={() => router.push('/admin/users/permissions')}>
                <UserCog className="h-4 w-4 mr-2" />
                User Permissions
              </Button>
              <Button variant="outline" onClick={() => router.push('/admin/audit-logs?resource=users')}>
