      ...
      <Tabs defaultValue="all" className="space-y-4">
        ...
        {['all', 'internal', 'client', 'partner'].map((tab) => (
          <TabsContent key={tab} value={tab} className="space-y-4">
                  ...
                  <Card key={organization.id} className="overflow-hidden">
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-start">
                        ...
                        <OrganizationBadge type={organization.type} tier={organization.tier} />
                      </div>
                      <CardDescription>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-medium">{organization.tier || 'N/A'}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Status</p>
                          <p className="font-medium">{organization.active ? 'Active' : 'Inactive'}</p>
                        </div>
                      </div>
                    </CardContent>
                    <CardFooter className="border-t bg-muted/50 px-6 py-3">
                      <div className="flex items-center justify-between w-full">
                        ...
                        <div className="flex gap-2">
                          <Link href={`/admin/organizations/manage?organizationId=${organization.id}`}>
                            <Button variant="outline" size="sm" className="h-8 gap-1">
            ...
            {organizations.filter((org) => tab === 'all' || org.type === tab).length === 0 && (
...
function OrganizationBadge({ type, tier }: { type: string; tier: string | null }) {
