...
import { ChevronLeft, Search, MapPin, Loader2, Plus, Check } from "lucide-react";
  ...
  const onPlaceChanged = useCallback(() => {
    ...
    try {
      ...
      toast({
        ...
        description: 'There was a problem with the location selection. Please try again.',
      });
    }
  }, [form, toast]);
          ...
          if (window.google && window.google.maps) {
            ...
            geocoder.geocode({ location: latLng }, (results, status) => {
              if (status === 'OK' && results && results[0]) {
                ...
                const addressName = result.formatted_address?.split(',')[0] || '';
                form.setValue('name', (result as any).name || addressName || 'New Location');
              } else {
  ...
  const onSubmit = (values: LocationFormValues) => {
    if (!selectedPlace) {
      toast({
        ...
        description: 'Please select a location using either the search tool or manual address entry.',
      });
      return;
    }
    
    // Extract address components
    const city = getAddressComponent(selectedPlace.address_components, 'locality') || 
                getAddressComponent(selectedPlace.address_components, 'sublocality') || '';
    const state = getAddressComponent(selectedPlace.address_components, 'administrative_area_level_1') || '';
    const postalCode = getAddressComponent(selectedPlace.address_components, 'postal_code') || '';
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold tracking-tight">Add New Location (Admin)</h1>
      </div>
      
      <p className="text-muted-foreground">
        As an administrator, you can directly add new locations to the system without requiring approval.
      </p>
      ...
      <Alert className="bg-primary/5 border-primary/20 mb-6">
        ...
        <AlertDescription>
          Search for addresses or businesses, add manual addresses, or use your current location. New locations will be immediately available in the system.
        </AlertDescription>
      </Alert>
            ...
            <CardContent className="space-y-6">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="grid w-full grid-cols-2 mb-4">
                  <TabsTrigger value="google-search">Google Maps Search</TabsTrigger>
                        ...
                        <PlaceAutocompleteAddressPicker
                          onAddressSelect={(addressData) => {
                            ...
                            const placeData: google.maps.places.PlaceResult = {
                              ...
                              geometry: {
                                location: new google.maps.LatLng(addressData.latitude, addressData.longitude),
                                viewport: new google.maps.LatLngBounds(
                                  new google.maps.LatLng(addressData.latitude - 0.01, addressData.longitude - 0.01),
                                  new google.maps.LatLng(addressData.latitude + 0.01, addressData.longitude + 0.01)
                                )
                              },
                              place_id: addressData.place_id,
                              name: addressData.name || addressData.formatted_address.split(',')[0] || 'New Location',
                              types: addressData.address_components.flatMap(comp => comp.types || [])
                            };
                            
                            // Use type assertion to help TypeScript understand this is compatible
                            setSelectedPlace(placeData as google.maps.places.PlaceResult);
                            setIsInfoOpen(true);
                            
                            // Set the place name in the form - ensure we have a non-empty string
                            form.setValue('name', addressData.name || addressData.formatted_address.split(',')[0] || 'New Location');
                            ...
                            const placeTypes = addressData.address_components
                          ...
                          <>
                            ...
                            <div id="marker-container" className="hidden">
                              {(() => {
                                ...
                                if (window.google?.maps?.marker?.AdvancedMarkerElement && mapRef.current) {
                                  ...
                                  pinElement.title = selectedPlace?.name || 'Selected location';
                            ...
                            {isInfoOpen && selectedPlace && (
                              ...
                              >
                                ...
                                >
                                  <h3 className="font-medium">{selectedPlace.name || 'Selected Location'}</h3>
                                  <p className={`text-sm ${mapTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                  ...
                  <ManualAddressEntry 
                    onLocationSelect={(locationData) => {
                      ...
                      const place: google.maps.places.PlaceResult = {
                        ...
                        address_components: [
                          { long_name: locationData.address1, short_name: locationData.address1, types: ['street_number', 'route'] },
                          { long_name: locationData.city, short_name: locationData.city, types: ['locality'] },
                          { long_name: locationData.state, short_name: locationData.state, types: ['administrative_area_level_1'] },
                          { long_name: locationData.zipcode, short_name: locationData.zipcode, types: ['postal_code'] }
                        ],
                        place_id: '',
                        name: locationData.name || formattedAddress.split(',')[0] || 'New Location'
                      };
                      
                      // Add geometry if coordinates are available
                      if (locationData.latitude && locationData.longitude) {
                        place.geometry = {
                          location: new google.maps.LatLng(locationData.latitude, locationData.longitude),
                          viewport: new google.maps.LatLngBounds(
                            new google.maps.LatLng(locationData.latitude - 0.01, locationData.longitude - 0.01),
                            new google.maps.LatLng(locationData.latitude + 0.01, locationData.longitude + 0.01)
                          )
                        };
                      }
                ...
                <div className="space-y-4">
                    ...
                    {selectedPlace.address_components && (
                      <>
                        <div>
                          ...
                          <p>{getAddressComponent(selectedPlace.address_components, 'locality') || 
                             getAddressComponent(selectedPlace.address_components, 'sublocality') || 'N/A'}</p>
                        </div>
                        <div>
                          <h3 className="text-sm font-medium text-muted-foreground">State/Province</h3>
                          <p>{getAddressComponent(selectedPlace.address_components, 'administrative_area_level_1') || 'N/A'}</p>
                        </div>
                        <div>
                          <h3 className="text-sm font-medium text-muted-foreground">Postal Code</h3>
                          <p>{getAddressComponent(selectedPlace.address_components, 'postal_code') || 'N/A'}</p>
                        </div>
                        <div>
                          <h3 className="text-sm font-medium text-muted-foreground">Country</h3>
                          <p>{getAddressComponent(selectedPlace.address_components, 'country') || 'N/A'}</p>
                        </div>
                      </>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-mono text-sm">{selectedLocation ? selectedLocation.lat.toFixed(6) : 'N/A'}</p>
                    </div>
                    <div>
                      <h3 className="text-sm font-medium text-muted-foreground">Longitude</h3>
                      <p className="font-mono text-sm">{selectedLocation ? selectedLocation.lng.toFixed(6) : 'N/A'}</p>
                    </div>
                  </div>
                </div>
              </CardContent>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Optional: Any important details about access, parking, etc.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
