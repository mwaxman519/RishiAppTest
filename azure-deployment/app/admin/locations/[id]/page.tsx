...
export default function AdminLocationDetailPage() {
  ...
  useEffect(() => {
    async function fetchLocationDetails() {
      ...
      try {
        ...
        setError(err.message || 'An error occurred while fetching location details');
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleApprove() {
    try {
      ...
      toast({
        ...
        description: err.message || "An error occurred while approving the location.",
        variant: "destructive"
      });
    } finally {
  ...
  async function handleReject() {
    try {
      ...
      const response = await fetch(`/api/admin/locations/${locationId}/reject`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      toast({
        ...
        description: err.message || "An error occurred while rejecting the location.",
        variant: "destructive"
      });
    } finally {
    ...
    return (
      <div className="p-6">
        ...
        <Alert>
          ...
          <AlertDescription>
            The requested location does not exist or you don't have permission to view it.
          </AlertDescription>
        </Alert>
      </div>
    );
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This action cannot be undone. This will permanently delete this location from the database.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-1">
                    ...
                    <p>
                      {location.city}, {location.state?.name || location.state} {location.zipcode}
                    </p>
                  </div>
                </div>
              ...
              {location.notes && (
                <>
                  ...
                  <div>
                    ...
                    <p className="whitespace-pre-line text-sm">{location.notes}</p>
                  </div>
                </>
              )}
            </CardContent>
              ...
              {location.latitude && location.longitude ? (
                <LocationMap
                  locations={[{
                    id: location.id,
                    name: location.name,
                    address: location.address || `${location.address1 || ''}, ${location.city || ''}, ${location.state?.name || location.state || ''} ${location.zipcode || ''}`,
