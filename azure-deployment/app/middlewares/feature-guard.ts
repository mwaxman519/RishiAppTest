...
import { isFeatureEnabled, isFeatureAvailableForTier } from '../../shared/features/registry';
...
export function createFeatureGuard(config: FeatureGuardConfig) {
  return async function featureGuard(
    ...
    try {
      ...
      const isAvailable = isFeatureAvailableForTier(config.featureId, organization.tier);
      if (!isAvailable) {
        if (config.apiMode) {
          return NextResponse.json({
      ...
      const isEnabled = await isFeatureEnabled(config.featureId, organizationId);
      if (!isEnabled) {
        if (config.apiMode) {
          return NextResponse.json({
...
async function getUserOrganizations(userId: string) {
  ...
  return [{ organizationId: '00000000-0000-0000-0000-000000000001', role: 'user' }];
}