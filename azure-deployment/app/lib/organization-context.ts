...
export function getOrganizationContext(): OrganizationContext | null {
  try {
    ...
    const organizationRole = headersList.get('X-Organization-Role') || undefined;
 ...
 */
export async function getOrganizationHeaderData(req: NextRequest): Promise<OrganizationData | null> {
  try {
 ...
 */
export async function hasOrganizationPermission(permission: string): Promise<boolean> {
  try {
    const context = getOrganizationContext();
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`DEVELOPMENT MODE: Granting permission '${permission}' in organization ${context.organizationId}`);
      return true;
    }
