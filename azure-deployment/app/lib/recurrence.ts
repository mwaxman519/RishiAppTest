 ...
 */
export function parseRecurrencePattern(pattern: string): RecurrencePattern | null {
...
export function generateOccurrences(
  ...
  if (frequency === RecurrenceFrequency.WEEKLY && byday && byday.length > 0) {
    ...
    while (totalOccurrences < maxOccurrences) {
      ...
      if (weekCounter > 520) { // 10 years of weeks
        break;
      }
    }
 ...
 */
export function getRecurrenceDescription(pattern: RecurrencePattern | string): string {
  const recurrencePattern = typeof pattern === 'string' 
    ? parseRecurrencePattern(pattern)
    : pattern;
