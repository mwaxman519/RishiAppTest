 ...
 */
export async function updateRole(id: string, data: UpdateRoleParams): Promise<Role> {
 ...
 * Get user roles
 */
export async function getUserRoles(userId: string, organizationId?: string): Promise<UserRole[]> {
...
export async function getUserPermissions(organizationId?: string): Promise<Permission[]> {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    return Promise.resolve({ permissions: mockPermissions, role: 'super_admin' } as any);
  }
  ...
  try {
    ...
    return Promise.resolve({ permissions: ['view:events', 'view:locations'], role: 'brand_agent' } as any);
  }
}
 ...
 * Assign role to user
 */
export async function assignRoleToUser(params: UserRoleParams): Promise<UserRole> {
  const response = await apiRequest('POST', '/api/rbac/users/roles', params);
  
  // Invalidate user roles cache
  queryClient.invalidateQueries({ queryKey: ['/api/rbac/users', params.userId, 'roles'] });
 ...
 */
export async function removeRoleFromUser(userId: string, roleId: string, organizationId?: string): Promise<void> {
  ...
  // Invalidate user roles cache
  queryClient.invalidateQueries({ queryKey: ['/api/rbac/users', userId, 'roles'] });
  queryClient.invalidateQueries({ queryKey: ['/api/auth/permissions'] });
}
 ...
 */
export async function getOrganizationPermissions(organizationId: string): Promise<OrganizationPermission[]> {
  const response = await apiRequest('GET', `/api/rbac/organizations/${organizationId}/permissions`);
  return response.json();
}
 ...
 */
export async function setOrganizationPermission(params: OrganizationPermissionParams): Promise<OrganizationPermission> {
  const response = await apiRequest('POST', '/api/rbac/organizations/permissions', params);
 ...
 */
export async function removeOrganizationPermission(organizationId: string, permission: string): Promise<void> {
  await apiRequest(
    'DELETE', 
