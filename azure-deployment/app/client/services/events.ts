 ...
 */
export async function updateEvent(id: string, data: UpdateEventParams): Promise<Event> {
...
export async function rejectEvent(id: string, reason: string): Promise<Event> {
  const response = await apiRequest('POST', `/api/events/${id}/reject`, { reason });
...
export async function cancelEvent(id: string, reason?: string): Promise<Event> {
  const response = await apiRequest('POST', `/api/events/${id}/cancel`, { reason });
 ...
 */
export async function createStaffAssignment(data: CreateStaffAssignmentParams): Promise<EventStaffAssignment> {
  const response = await apiRequest('POST', '/api/event-staff', data);
  
  // Invalidate event staff cache
  queryClient.invalidateQueries({ queryKey: ['/api/events', data.eventId, 'staff'] });
  
  return response.json();
}

export async function updateStaffAssignment(id: string, data: UpdateStaffAssignmentParams): Promise<EventStaffAssignment> {
...
export async function deleteStaffAssignment(id: string, eventId: string): Promise<void> {
...
export async function getStaffAssignmentsForEvent(eventId: string): Promise<EventStaffAssignment[]> {
 ...
 */
export async function createChecklistItem(data: CreateChecklistItemParams): Promise<EventChecklistItem> {
  const response = await apiRequest('POST', '/api/event-checklist', data);
  
  // Invalidate event checklist cache
  queryClient.invalidateQueries({ queryKey: ['/api/events', data.eventId, 'checklist'] });
  
  return response.json();
}

export async function updateChecklistItem(id: string, data: UpdateChecklistItemParams): Promise<EventChecklistItem> {
  const response = await apiRequest('PATCH', `/api/event-checklist/${id}`, data);
...
export async function deleteChecklistItem(id: string, eventId: string): Promise<void> {
  await apiRequest('DELETE', `/api/event-checklist/${id}`);
  
  // Invalidate event checklist cache
  queryClient.invalidateQueries({ queryKey: ['/api/events', eventId, 'checklist'] });
}

export async function getChecklistItemsForEvent(eventId: string): Promise<EventChecklistItem[]> {
 ...
 */
export async function getEventsInRange(startDate: string, endDate: string, organizationId?: string): Promise<Event[]> {
