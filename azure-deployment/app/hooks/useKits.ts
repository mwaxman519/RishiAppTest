...
export function useKits(filters?: Record<string, string>) {
  ...
  return useQuery<Kit[]>({
    ...
    queryFn: async () => {
      const response = await apiRequest('GET', `/api/kits?${queryParams.toString()}`);
...
export function useKitTemplates(filters?: Record<string, string>) {
  ...
  return useQuery<KitTemplate[]>({
    ...
    queryFn: async () => {
      const response = await apiRequest('GET', `/api/kits/templates?${queryParams.toString()}`);
      return response.json();
    },
  });
}
...
export function useKitInventory(kitId?: number | string) {
  const queryParams = new URLSearchParams(kitId ? { kitId: kitId.toString() } : {});
  
  return useQuery<KitComponentInventory[]>({
    queryKey: ['/api/kits/inventory', kitId],
    queryFn: async () => {
      const response = await apiRequest('GET', `/api/kits/inventory?${queryParams.toString()}`);
      return response.json();
    },
    enabled: !!kitId,
  });
...
export function useUpdateKitTemplate(id: number | string) {
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      const response = await apiRequest('PUT', `/api/kits/templates/${id}`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/kits/templates'] });
...
export function useUpdateKitInventory() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/kits/inventory', variables.kitId] });
...
export function useApproveKitTemplate(id: number) {
  ...
  return useMutation({
    mutationFn: async (data: ApprovalStatusUpdate) => {
      const response = await apiRequest('PATCH', `/api/kits/templates/${id}/approve`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/kits/templates'] });
...
export function useApproveKit(id: number) {
  ...
  return useMutation({
    mutationFn: async (data: ApprovalStatusUpdate) => {
      const response = await apiRequest('PATCH', `/api/kits/${id}/approve`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/kits'] });
...
export function usePendingApprovals() {
  ...
  const { data: pendingTemplates, isLoading: isLoadingTemplates } = useKitTemplates({ 
