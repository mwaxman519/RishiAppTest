...
import { KitDTO, KitTemplateDTO, CreateKitInstanceRequest } from '../../services/kits/types';
...
export function useKitInstances() {
  ...
  const { 
    ...
    queryFn: async () => {
      try {
        const response = await apiRequest('GET', '/api/kits/instances', null, 2);
        if (!response.ok) {
        ...
        if (!Array.isArray(data)) {
          console.warn('Expected array for kit instances but got:', typeof data);
          return [];
        }
  ...
  const { 
    ...
    queryFn: async () => {
      try {
        const response = await apiRequest('GET', '/api/kits/templates', null, 2);
        if (!response.ok) {
          const errorText = response.statusText || `HTTP Error ${response.status}`;
        ...
        const data = await response.json();
        if (!Array.isArray(data)) {
          console.warn('Expected array for kit templates but got:', typeof data);
          return [];
        }
  ...
  const createKitInstanceMutation = useMutation({
    mutationFn: async (data: CreateKitInstanceRequest) => {
      try {
        const response = await apiRequest('POST', '/api/kits/instances', data, 2); // Allow up to 2 retries
        if (!response.ok) {
