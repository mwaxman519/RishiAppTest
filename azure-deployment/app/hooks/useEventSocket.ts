...
export function useEventSocket(
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      socket.addEventListener('close', (event) => {
        ...
        if (opts.autoReconnect && reconnectAttemptsRef.current < (opts.maxReconnectAttempts || 5)) {
          reconnectAttemptsRef.current += 1;
          const delay = opts.reconnectDelay || 3000;
          
          console.log(`Attempting to reconnect (${reconnectAttemptsRef.current}/${opts.maxReconnectAttempts}) in ${delay}ms`);
          setTimeout(() => {
  ...
  const unsubscribe = useCallback((eventType: AppEvent) => {
    ...
    eventTypesRef.current = eventTypesRef.current.filter(type => type !== eventType);
