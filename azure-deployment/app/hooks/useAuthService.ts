...
export function useAuthService(): AuthServiceClient {
  ...
  async function authRequest<T>(
    ...
    try {
      ...
      const response = await fetch(`/api/auth-service/routes/${endpoint}`, options);
      ...
      if (!response.ok || !result.success) {
        ...
        const errorMessage = result.error?.message || `Request to ${endpoint} failed`;
  ...
  async function login(credentials: LoginCredentials): Promise<UserSession> {
    try {
      ...
      const userData = await authRequest<UserSession>('login', 'POST', credentials);
      return userData;
    } catch (err) {
  ...
  async function register(data: RegisterData): Promise<UserSession> {
    try {
      ...
      try {
        ...
        console.warn('Status check failed, proceeding with registration anyway');
      }
      ...
      if ('registrationPasscode' in safeData) (safeData as any).registrationPasscode = '[REDACTED]';
      ...
      try {
        ...
        if (!response.ok) {
          ...
          try {
            ...
            console.error('Auth service register error: Failed to parse response', parseError);
            
            // Use status code for error message if we can't parse JSON
            if (response.status === 502) {
              errorMessage = 'Registration service is unavailable (Bad Gateway). Please try again later.';
            } else if (response.status === 500) {
              errorMessage = 'Registration service encountered an error. Please try again later.';
            } else {
          ...
          if (errorMessage.includes('database') || 
            ...
            throw new Error('Database error: The registration service is experiencing database issues. Please try again later.');
          }
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        const urlParams = typeof window !== 'undefined' 
          ...
          : null;
        const showUnauthenticated = urlParams?.get('unauthenticated') === 'true';
        ...
        console.log('DEVELOPMENT MODE: Using mock user for session (no HTTP request)');
