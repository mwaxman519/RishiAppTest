...
export function useLocations(filters: LocationFilters = {}) {
  ...
  const { data, isLoading, error } = useQuery<{ data: Location[] }>({
    ...
    queryFn: async () => {
      const response = await apiRequest('GET', `/api/locations?${queryParams.toString()}`);
...
export function useApproveLocation(id: string) {
  ...
  return useMutation({
    mutationFn: async (data: ApprovalData) => {
      ...
      const response = await apiRequest('PUT', `/api/locations/pending`, approvalData);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/locations'] });
...
export function useGeocode() {
  ...
  const geocodeAddress = async (address: string): Promise<GeocodeResult | null> => {
    ...
    try {
      const response = await apiRequest('POST', '/api/locations/geocode', { address });
...
export function useLocationSearch(query: string = '', limit: number = 10) {
  ...
  return useQuery<Location[]>({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest('GET', `/api/locations/search?${queryParams.toString()}`);
      return response.json();
    },
