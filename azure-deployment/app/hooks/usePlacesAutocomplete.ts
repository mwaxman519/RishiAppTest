...
export function usePlacesAutocomplete(options: PlacesAutocompleteOptions = {}): PlacesAutocompleteResult {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to initialize Google Places services'));
    }
  }, [isLoaded, google]);
  
  // Get place predictions with Promise-based API
  const getPlacePredictions = useCallback(async (input: string): Promise<google.maps.places.AutocompletePrediction[]> => {
  ...
  const getPlaceDetails = useCallback(async (placeId: string): Promise<google.maps.places.PlaceResult | null> => {
    ...
    try {
      // Create a promise wrapper around the callback-based API
      return await new Promise<google.maps.places.PlaceResult | null>((resolve, reject) => {
        const request = {
          placeId,
        ...
        placesService.current!.getDetails(
          ...
          (result, status) => {
            if (status !== google.maps.places.PlacesServiceStatus.OK) {
              ...
              reject(new Error(`Places Details API returned status: ${status}`));
              return;
            }
