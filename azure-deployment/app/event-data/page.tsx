...
export default function EventDataPage() {
  ...
  const [filteredSubmissions, setFilteredSubmissions] = useState<EventDataSubmission[]>([]);
    ...
    if (selectedTab !== 'all') {
      filtered = filtered.filter(sub => {
        switch (selectedTab) {
          ...
          case 'completed':
            return ['approved', 'rejected', 'needs_revision'].includes(sub.status);
          default:
            return true;
        }
      });
  ...
  const getTabCounts = () => {
    ...
    const submitted = submissions.filter(s => ['submitted', 'under_review'].includes(s.status)).length;
    const completed = submissions.filter(s => ['approved', 'rejected', 'needs_revision'].includes(s.status)).length;
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading event data submissions...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Event Data Submissions</h1>
          <p className="text-gray-600 mt-1">
            Submit qualitative and quantitative event surveys through Jotforms. 
            Include demo table, shelf images, and additional photos for management review.
          </p>
        </div>
        <Button onClick={handleCreateSubmission} className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Photos Submitted</p>
      ...
      <Card>
        <CardContent className="p-12 text-center">
          ...
          <h3 className="text-xl font-medium text-gray-900 mb-2">No Event Data Found</h3>
          <p className="text-gray-600 mb-6 max-w-md mx-auto">
            Connect with authentic data sources to display real event submissions. 
            Event data will appear here once proper API integration is established.
          </p>
