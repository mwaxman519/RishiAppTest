...
import { ArrowLeft, UserPlus, Calendar, MapPin, Clock, Search, Filter } from 'lucide-react';
...
const availableEvents = [
  {
    ...
    compensation: 350,
    description: 'Major technology conference with product demonstrations and networking.'
  },
  {
    id: 'evt-002',
...
export default function AssignTeamMemberPage({ params }: AssignTeamMemberProps) {
  const { id } = use(params);
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold tracking-tight">Assign to Events</h1>
        ...
        <div className="flex space-x-2">
          ...
          >
            ...
            {isAssigning ? 'Assigning...' : `Assign to ${selectedEvents.length} Event(s)`}
          </Button>
        </div>
      </div>
        ...
        <CardContent>
          ...
          <div className="space-y-4">
            {filteredEvents.map((event) => (
              <div key={event.id} className="flex items-start space-x-4 p-4 border rounded-lg hover:bg-accent/50">
                <Checkbox
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-lg font-bold text-green-600">${event.compensation}</div>
                      <Badge variant={event.status === 'urgent' ? 'destructive' : 'secondary'}>
                        {event.status}
                      </Badge>
                    </div>
                  </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              <p className="font-medium">Selected Events: {selectedEvents.length}</p>
              <p className="text-sm text-muted-foreground">
                Total Compensation: ${selectedEvents.reduce((total, eventId) => {
                  const event = availableEvents.find(e => e.id === eventId);
              ...
              <div className="mt-4">
                ...
                <ul className="space-y-1">
                  {selectedEvents.map(eventId => {
                    const event = availableEvents.find(e => e.id === eventId);
                    return (
                      <li key={eventId} className="text-sm flex justify-between">
