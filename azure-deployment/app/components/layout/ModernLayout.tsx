...
import { ModernNavigationProvider, useNavigation } from '../navigation/ModernNavigationProvider';
...
function ModernLayoutInner({ children }: ModernLayoutProps) {
  ...
  const isFullWidthPage = pathname?.startsWith('/auth/') || 
                         pathname === '/login' || 
                         pathname === '/register' || 
  ...
  return (
    <>
      ...
      {!isMobile ? (
        <ModernSidebar navItems={navItems}>
          {children}
        </ModernSidebar>
      ) : (
