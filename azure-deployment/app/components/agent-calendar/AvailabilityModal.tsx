...
export default function AvailabilityModal({ isOpen, onClose, onSave, selectInfo, userId: propUserId }: AvailabilityModalProps) {
  ...
  const [mergeStrategy, setMergeStrategy] = useState<'merge' | 'override' | undefined>(undefined);
  
  // Use userId from props or fallback to default UUID
  const [userId, setUserId] = useState<string | number>(propUserId || '00000000-0000-0000-0000-000000000001');
  ...
  async function checkForConflicts() {
    ...
    try {
      const response = await fetch(`/api/availability/conflicts?userId=${userId}&startDate=${selectInfo.startStr}&endDate=${selectInfo.endStr}&status=${status}`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.conflicts && data.conflicts.length > 0) {
          const sameStatusConflicts = data.conflicts.filter(
            (conflict: AvailabilityConflict) => conflict.existingBlock.status === status
          );
  ...
  const handleSubmit = () => {
    ...
    onSave(
      ...
      isRecurring && recurrenceEndType === 'date' ? recurrenceEndDate : undefined,
      isRecurring && recurrenceEndType === 'count' ? recurrenceCount : undefined,
      mergeStrategy,
      modifiedStartStr,
      modifiedEndStr
    );
  ...
  const handleClose = () => {
    setStatus('available');
    setTitle('');
    setTitle(status === 'available' ? 'Available' : status === 'unavailable' ? 'Unavailable' : 'Tentative');
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-4 p-3 bg-gray-50 rounded">
          ...
          <div className="font-medium">{startTimeStr} - {endTimeStr}</div>
        </div>
        ...
        {isCheckingConflicts && (
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded">
            <div className="text-sm text-blue-600">Checking for conflicts...</div>
          </div>
        )}
        ...
        {hasConflicts && conflicts.length > 0 && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded">
            ...
            <div className="space-y-2 mb-3">
              {conflicts.slice(0, 3).map((conflict, index) => (
                <div key={index} className="text-xs text-yellow-700 bg-yellow-100 p-2 rounded">
                  <div className="font-medium">{conflict.existingBlock.title}</div>
                  <div>
                    {format(new Date(conflict.existingBlock.startDate), 'MMM d, h:mm a')} - 
                    {format(new Date(conflict.existingBlock.endDate), 'h:mm a')}
                  </div>
                  <div className="capitalize">Status: {conflict.existingBlock.status}</div>
                </div>
              ))}
              {conflicts.length > 3 && (
            ...
            <div>
              <label className="block text-sm font-medium mb-1 text-yellow-800">Resolution Strategy</label>
              <select
                value={mergeStrategy || ''}
                onChange={(e) => setMergeStrategy(e.target.value as 'merge' | 'override' | undefined)}
