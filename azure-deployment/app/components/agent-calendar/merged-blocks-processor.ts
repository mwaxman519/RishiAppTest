...
export function mergeAdjacentBlocks(blocks: Block[]): MergedBlock[] {
  ...
  Object.entries(blocksByDayAndStatus).forEach(([key, blocksInGroup]) => {
    ...
    if (blocksInGroup.length === 1) {
      result.push({...blocksInGroup[0], mergedIds: [blocksInGroup[0].id]} as MergedBlock);
      return;
    }
    ...
    if (status === 'unavailable' && blocksInGroup.length > 1) {
      console.log(`🔄 Merging ${blocksInGroup.length} unavailable blocks into one visual block for ${day}`);
      ...
      // Log the first block
      console.log(`  📌 Initial block: ${new Date(earliestStart).toLocaleTimeString()} - ${new Date(latestEnd).toLocaleTimeString()} (ID: ${blocksInGroup[0].id})`);
      ...
      for (let i = 1; i < blocksInGroup.length; i++) {
        ...
        console.log(`  📌 Block ${i}: ${blockStart.toLocaleTimeString()} - ${blockEnd.toLocaleTimeString()} (ID: ${block.id})`);
        
        if (blockStart < earliestStart) {
          console.log(`    📊 New earliest start: ${blockStart.toLocaleTimeString()}`);
          earliestStart = blockStart;
        }
        if (blockEnd > latestEnd) {
          console.log(`    📊 New latest end: ${blockEnd.toLocaleTimeString()}`);
          latestEnd = blockEnd;
        }
        mergedIds.push(block.id);
      }
      
      console.log(`  🔄 Final merged block: ${earliestStart.toLocaleTimeString()} - ${latestEnd.toLocaleTimeString()}, containing ${mergedIds.length} blocks`);
      ...
      const formattedStartTime = earliestStart.toLocaleTimeString([], {hour: 'numeric', minute:'2-digit'});
      const formattedEndTime = latestEnd.toLocaleTimeString([], {hour: 'numeric', minute:'2-digit'});
      ...
      console.log(`Merged ${blocksInGroup.length} 'unavailable' blocks into one: IDs ${mergedIds.join(', ')}`);
    ...
    for (let i = 1; i < blocksInGroup.length; i++) {
      ...
      if (isAdjacent || isOverlapping) {
        ...
        const currentMergedBlockEnd = new Date(currentMergedBlock.end).getTime();
  ...
  console.log(`Merged blocks: ${blocks.length} original → ${result.length} merged`);
  return result;
}