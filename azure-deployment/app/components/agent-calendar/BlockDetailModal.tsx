...
export default function BlockDetailModal({
  ...
  const [deleteRecurringMode, setDeleteRecurringMode] = useState<'single' | 'series' | null>(null);
  const [isConfirmMergedDeleteOpen, setIsConfirmMergedDeleteOpen] = useState(false);
  ...
  const dayOfWeek = block.dayOfWeek !== undefined ? dayNames[block.dayOfWeek] : '';
  
  // Status display with nicer formatting
  const statusDisplay = block.status.charAt(0).toUpperCase() + block.status.slice(1);
      ...
      <div>
        ...
        <Button
          ...
          onClick={() => {
            ...
            const isMergedBlock = (block.isMerged || 
              (block.mergedIds && block.mergedIds.length > 1) || 
              (block.extendedProps?.mergedIds && block.extendedProps.mergedIds.length > 1));
            
            if (isMergedBlock) {
              // For merged blocks, show a more informative dialog
              const mergedCount = block.mergedIds?.length || block.extendedProps?.mergedIds?.length || 0;
    ...
    <>
      ...
      >
        <div className="space-y-4">
          ...
          {(block.isMerged || (block.extendedProps?.mergedIds && block.extendedProps.mergedIds.length > 1) || (block.mergedIds && block.mergedIds.length > 1)) && (
            <div className="rounded-md bg-violet-50 dark:bg-violet-900/30 p-3 text-violet-800 dark:text-violet-200 mb-4">
              ...
              <p className="text-sm ml-6">
                This is a visual representation of {block.mergedIds?.length || block.extendedProps?.mergedIds?.length || 0} adjacent 
                unavailable time blocks combined into a single view. You can edit or delete individual blocks.
              </p>
            </div>
          )}
            ...
            {((block.mergedIds && block.mergedIds.length > 1) || 
              ...
              <>
                ...
                <div className="rounded-md bg-gray-50 dark:bg-gray-800 p-2 text-sm text-gray-600 dark:text-gray-300 mt-1 mb-2">
                  ...
                  {(block.extendedProps?.mergedBlockData || []).map((subBlock: {id: number, start: Date, end: Date}, index: number) => {
                    const subStart = new Date(subBlock.start).toLocaleTimeString([], {hour: 'numeric', minute:'2-digit'});
                    const subEnd = new Date(subBlock.end).toLocaleTimeString([], {hour: 'numeric', minute:'2-digit'});
                    return (
                  ...
                  {(!block.extendedProps?.mergedBlockData || block.extendedProps.mergedBlockData.length === 0) && 
                    (block.mergedIds || block.extendedProps?.mergedIds || []).map((id: number, index: number) => (
      ...
      <SimpleDialog
        ...
        footer={
          <div className="flex justify-end space-x-3 w-full">
            ...
            <Button
              variant="destructive"
              disabled={isDeleting || (block.isRecurring && !deleteRecurringMode)}
              onClick={() => handleConfirmDelete(deleteRecurringMode === 'series')}
            >
      ...
      >
            ...
            <div className="flex flex-col space-y-3">
              <div 
                className={`p-3 rounded-md cursor-pointer border ${
                  deleteRecurringMode === 'single' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30' : 'border-gray-200 dark:border-gray-700'
                }`}
              ...
              >
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-300">Only remove this specific date and time slot</div>
                  </div>
                </div>
              </div>
              ...
              <div 
                className={`p-3 rounded-md cursor-pointer border ${
                  deleteRecurringMode === 'series' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30' : 'border-gray-200 dark:border-gray-700'
                }`}
              ...
              >
                <div className="flex items-start">
                  ...
                  <div>
                    <div className="font-medium">The entire recurring series</div>
                    <div className="text-sm text-gray-600 dark:text-gray-300">Remove all instances of this recurring block</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <p className="mt-4 text-sm text-red-600 dark:text-red-400">This action cannot be undone.</p>
      </SimpleDialog>
