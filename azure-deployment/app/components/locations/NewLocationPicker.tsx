...
export default function NewLocationPicker({ onAddressSelect, className = '' }: Props) {
  ...
  useEffect(() => {
    if (!selectedLocation || !mapContainerRef.current || !window.google?.maps) return;
  ...
  useEffect(() => {
    ...
    const setupPlaceAutocomplete = () => {
      ...
      try {
        ...
        fallbackInput.placeholder = 'Search for addresses, businesses, or places...';
        fallbackInput.id = 'location-search-input';
        fallbackInput.className = 'w-full h-12 pl-10 pr-3 focus-visible:ring-2 focus-visible:ring-primary rounded-lg shadow-md border border-input bg-background text-foreground';
        
        // Add search icon container
        const searchIconContainer = document.createElement('div');
        searchIconContainer.className = 'pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 z-10';
        const searchIcon = document.createElement('div');
        searchIcon.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-search h-4 w-4 text-muted-foreground"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>';
        ...
        const autocomplete = new window.google.maps.places.Autocomplete(fallbackInput, {
        ...
        autocomplete.addListener('place_changed', () => {
          ...
          const isBusinessOrPlace = place.types && (
  ...
  return (
    <div className={className}>
      ...
      <div className="relative mb-4 max-w-md">
        ...
        <div ref={autocompleteContainerRef} className="place-autocomplete-container relative"></div>
      </div>
