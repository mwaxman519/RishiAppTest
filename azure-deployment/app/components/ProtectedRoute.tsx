...
export function ProtectedRoute({
  ...
  const { hasPermission, hasAllPermissions, isLoading: rbacLoading } = useEnhancedRBAC();
  ...
  useEffect(() => {
    ...
    if (user && !isLoading) {
      ...
      if (permissions && permissions.length > 0 && !hasAllPermissions(permissions, mergedContext)) {
        router.push('/access-denied');
        return;
      }
    }
