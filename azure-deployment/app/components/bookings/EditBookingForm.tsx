...
export default function EditBookingForm({ id }: EditBookingFormProps) {
  ...
  const [initialData, setInitialData] = useState<typeof mockBookingDetails | null>(null);
  ...
  useEffect(() => {
    if (initialData) {
      const startTime = initialData.startDate ? format(initialData.startDate, 'HH:mm') : '09:00';
      const endTime = initialData.endDate ? format(initialData.endDate, 'HH:mm') : '17:00';
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 max-w-3xl">
        ...
        <div className="space-y-6">
          <div>
            <h2 className="text-xl font-semibold tracking-tight">Activity Details</h2>
            <p className="text-muted-foreground text-sm mt-1">
