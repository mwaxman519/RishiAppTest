...
export function BasicMap({ 
  ...
  const showInfoWindow = (location: BasicMapLocation, marker: google.maps.Marker) => {
  ...
  const centerOnLocation = (locationId: string) => {
    ...
    Object.entries(markersRef.current).forEach(([id, marker]) => {
      marker.setAnimation(id === locationId ? 
  ...
  return (
    <LoadScript googleMapsApiKey={apiKey} onLoad={() => console.log("Google Maps script loaded")}>
      <div 
        ref={mapRef} 
        style={{ 
          width: '100%', 
