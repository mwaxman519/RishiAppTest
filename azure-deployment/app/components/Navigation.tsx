...
import { NavItem, USER_TYPES, NAV_ITEM_TYPES } from '@/shared/navigation-constants';
import { 
...
export function Navigation({ children }: NavigationProps) {
  ...
  if (!navigationBuilt) {
    ...
    if (isSuperAdmin) {
      ...
      console.log('Navigation: Super Admin NAV contents:', JSON.stringify(SUPER_ADMIN_NAV.map(item => item.label)));
      console.log('Navigation: Copied primaryNav contents:', JSON.stringify(primaryNav.map(item => item.label)));
      ...
      const brandAgentsSection = primaryNav.find(i => i.label === 'Brand Agents');
      if (brandAgentsSection) {
        console.log('Navigation: Brand Agents section found with', 
          brandAgentsSection.children?.length || 0, 'children');
        
        if (brandAgentsSection.children) {
          const availabilityItem = brandAgentsSection.children.find(c => 
            c.label === 'My Availability');
          
          if (availabilityItem) {
            console.log('Navigation: My Availability item found:', availabilityItem);
          } else {
            console.error('Navigation: My Availability item is missing!');
          }
        }
      } else {
        console.error('Navigation: Brand Agents section is missing from Super Admin nav!');
      }
    } else {
      // Regular role-based navigation
      const userType = userWithRole.userType || '';
      console.log('Navigation: Building regular navigation for user type:', userType);
    ...
    console.log('Navigation: Menu building complete, items in primary nav =', 
  ...
  const userWithRole = user as unknown as ExtendedUser;
  if (navigationBuilt && userWithRole.userType === USER_TYPES.BRAND_AGENT && isMobile) {
    return <MobileNavigation />;
  }
  
  // Log the final navigation items being sent to the Sidebar
  console.log('Navigation: Final navigation items sent to Sidebar:', 
