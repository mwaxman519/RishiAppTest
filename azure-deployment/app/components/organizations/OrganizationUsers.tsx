...
export function OrganizationUsers({ organizationId, organizationName }: OrganizationUsersProps) {
  ...
  const { data: invitationsData, isLoading: isInvitationsLoading } = useQuery<InvitationsResponse>({
    queryKey: [`/api/organizations/invitations?organizationId=${organizationId}`],
    enabled: !!organizationId,
  });
  ...
  const sendInvitationMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/organizations/invitations?organizationId=${organizationId}`] });
    },
    onError: (error: Error) => {
      toast({
        title: 'Error',
  ...
  const cancelInvitationMutation = useMutation({
    mutationFn: async (invitationId: string) => {
      return apiRequest('DELETE', `/api/organizations/invitations?id=${invitationId}`);
    },
    onSuccess: () => {
      toast({
  ...
  const removeUserMutation = useMutation({
    mutationFn: async (userId: string) => {
      return apiRequest('DELETE', `/api/organizations/users?userId=${userId}&organizationId=${organizationId}`);
    },
    onSuccess: () => {
      toast({
        title: 'User Removed',
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/organizations/users?organizationId=${organizationId}`] });
    },
    onError: (error: Error) => {
      toast({
        title: 'Error',
        ...
        <Dialog open={isInviteDialogOpen} onOpenChange={setIsInviteDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Enter the email address and role for the new user. An invitation will be sent via email.
              </DialogDescription>
            </DialogHeader>
            <Form {...form}>
                    ...
                    <TableRow key={user.id}>
                      ...
                      <TableCell className="text-right">
                        <Button
                          ...
                          disabled={user.is_primary || removeUserMutation.isPending}
        ...
        <CardContent>
          {isInvitationsLoading ? (
            ...
            <Table>
              ...
              <TableBody>
                {invitationsData?.invitations && Array.isArray(invitationsData.invitations) && invitationsData.invitations.length > 0 ? (
