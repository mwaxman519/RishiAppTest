...
import { 
  ChevronLeft, ChevronRight, ChevronDown, Menu, LogOut, 
  User, Home, FileText, LayoutDashboard, Users, Calendar, 
  Package, Settings, Lock, Shield, Database, Building, 
  MapPin, Clock, BarChart, ClipboardList, GraduationCap, 
  CreditCard, CheckSquare, Briefcase, Globe, Search, 
  MessageSquare, Bell, HelpCircle, Cog
} from 'lucide-react';
...
export default function ModernSidebar({ navItems, children }: ModernSidebarProps) {
  ...
  const isActiveLink = (item: NavItem): boolean => {
    ...
    if (itemPath !== '/' && pathname.startsWith(itemPath + '/')) {
      ...
      const itemSegments = itemPath.split('/').filter(s => s.length > 0).length;
      const currentSegments = pathname.split('/').filter(s => s.length > 0).length;
          ...
          <Link href="/" className="flex items-center gap-2">
            <div className="flex items-center justify-center w-8 h-8 rounded-md bg-primary text-primary-foreground">
              <svg width="20" height="20" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M21 12H11C10.448 12 10 12.448 10 13V19C10 19.552 10.448 20 11 20H21C21.552 20 22 19.552 22 19V13C22 12.448 21.552 12 21 12Z" fill="white" />
                <path d="M12 9C12 8.448 11.552 8 11 8C10.448 8 10 8.448 10 9V11C10 11.552 10.448 12 11 12C11.552 12 12 11.552 12 11V9Z" fill="white" />
                <path d="M16 9C16 8.448 15.552 8 15 8C14.448 8 14 8.448 14 9V11C14 11.552 14.448 12 15 12C15.552 12 16 11.552 16 11V9Z" fill="white" />
                <path d="M20 9C20 8.448 19.552 8 19 8C18.448 8 18 8.448 18 9V11C18 11.552 18.448 12 19 12C19.552 12 20 11.552 20 11V9Z" fill="white" />
                <path d="M12 21C12 20.448 11.552 20 11 20C10.448 20 10 20.448 10 21V23C10 23.552 10.448 24 11 24C11.552 24 12 23.552 12 23V21Z" fill="white" />
                <path d="M16 21C16 20.448 15.552 20 15 20C14.448 20 14 20.448 14 21V23C14 23.552 14.448 24 15 24C15.552 24 16 23.552 16 23V21Z" fill="white" />
                <path d="M20 21C20 20.448 19.552 20 19 20C18.448 20 18 20.448 18 21V23C18 23.552 18.448 24 19 24C19.552 24 20 23.552 20 23V21Z" fill="white" />
              </svg>
            </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3">
        <ul className="space-y-1">
          {processedNavItems.map((item) => {
            ...
            if (sidebarCollapsed && item.type === NAV_ITEM_TYPES.SECONDARY) return null;
            ...
            if (hasChildren) {
              ...
              return (
                <li key={item.id} className="mb-2">
                  <button
                    type="button"
                    onClick={() => !sidebarCollapsed && toggleSection(sectionKey)}
                    className={cn(
                  ...
                  >
                    <div className="flex items-center">
                      <span className={cn(
                        "text-primary",
                        sidebarCollapsed && "w-6 h-6 flex items-center justify-center"
                      )}>
                      ...
                      {isExpanded && (
                        ...
                        >
                          {item.children && item.children.map((child: NavItem) => {
                            ...
                            return (
                              <li key={`${item.id}-${child.id || child.label}`}>
                                <Link 
                                  href={child.path || child.href || '#'}
                                  className={cn(
                                    "flex items-center rounded-md p-2",
                                    childIsActive ? "bg-primary/10 text-primary" : "hover:bg-muted text-muted-foreground"
                                  )}
                                >
                                  <span className="text-primary">
                                    {typeof child.icon === 'string' ? getIcon(child.icon) : child.icon}
                                  </span>
          ...
          <div className="flex items-center p-2 rounded-lg bg-muted/50 mb-3">
            ...
            <div className="ml-3 overflow-hidden">
              ...
              <p className="text-xs text-muted-foreground truncate">
                {user?.role?.replace(/_/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()) || 'User'}
              </p>
            </div>
          </div>
        )}
