...
const fieldManagers = [
  {
    ...
    joinDate: '2024-03-15',
    specializations: ['Event Management', 'Team Leadership', 'Client Relations'],
  ...
  },
  {
    ...
    joinDate: '2024-01-20',
    specializations: ['Training Programs', 'Quality Assurance', 'Performance Optimization'],
  ...
  },
  {
    ...
    joinDate: '2023-11-10',
    specializations: ['Multi-Site Coordination', 'Brand Activation', 'Crisis Management'],
...
const FieldManagerCard = ({ manager, onAction }: { manager: FieldManager; onAction: (action: string, managerId: string) => void }) => {
          ...
          <div className="flex items-center space-x-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                <DropdownMenuItem onClick={() => onAction('message', manager.id)}>
                  Send Message
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onAction('assign', manager.id)}>
                  Assign Event
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onAction('settings', manager.id)}>
                  Settings
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{manager.teamsManaged}</div>
            <div className="text-xs text-muted-foreground">Teams</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">{manager.activeEvents}</div>
            <div className="text-xs text-muted-foreground">Events</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600">{manager.completionRate}%</div>
...
export default function FieldManagersPage() {
  ...
  const activeManagers = fieldManagers.filter(m => m.status === 'active').length;
  const averageRating = (fieldManagers.reduce((sum, m) => sum + m.rating, 0) / totalManagers).toFixed(1);
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{totalTeams}</p>
              </div>
