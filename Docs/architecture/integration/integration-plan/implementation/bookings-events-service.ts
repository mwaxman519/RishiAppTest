...
export class BookingsEventsService {
   ...
   */
  async approveBooking(bookingId: string, approverId: string, notes?: string): Promise<void> {
    // Start a transaction
    return await db.transaction(async (tx) => {
      ...
      const eventInstanceData: InsertEventInstance[] = eventDates.map(date => ({
  ...
  async startEventPreparation(
    eventId: string,
    startedBy: string,
    preparationTasks?: Array<{description: string, assignedTo?: string, dueDate: string}>
  ): Promise<EventInstance> {
    return await db.transaction(async (tx) => {
    ...
    return await db.transaction(async (tx) => {
      ...
      await eventBus.publish('EVENT_STARTED', {
        ...
        staffPresent: staffPresent.map(staff => ({
          userId: staff.userId,
          role: staff.role,
          checkedInAt: staff.checkInTime?.toISOString() || new Date().toISOString()
        }))
      });
  ...
  async completeEvent(
    ...
    return await db.transaction(async (tx) => {
      ...
      const allCompleted = allEvents.every(event => 
