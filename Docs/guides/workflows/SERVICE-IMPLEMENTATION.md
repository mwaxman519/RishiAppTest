...
import { Location, LocationDTO, CreateLocationParams, UpdateLocationParams } from "./models";
...
export class LocationRepository {
   ...
   */
  async create(data: CreateLocationParams, organizationId: string, requesterId?: string): Promise<LocationDTO> {
...
export class LocationsService {
   ...
   */
  async getAllLocations(filters: Record<string, any> = {}): Promise<LocationDTO[]> {
   ...
   * Create a new location
   */
  async createLocation(data: CreateLocationParams, organizationId: string, requesterId?: string): Promise<LocationDTO> {
    // Validate input data
   ...
   * Update an existing location
   */
  async updateLocation(id: string, data: UpdateLocationParams): Promise<LocationDTO> {
  ...
  async approveLocation({ id }: ApproveLocationParams): Promise<LocationDTO> {
    ...
    if (existingLocation.status !== 'pending') {
      throw new Error(`Cannot approve location with status ${existingLocation.status}`);
    }
   ...
   * Reject a location
   */
  async rejectLocation({ id, reason }: RejectLocationParams): Promise<LocationDTO> {
    // Check if location exists
    const existingLocation = await this.repository.findById(id);
    if (!existingLocation) {
      throw new Error(`Location with ID ${id} not found`);
    ...
    if (existingLocation.status !== 'pending') {
      throw new Error(`Cannot reject location with status ${existingLocation.status}`);
    }
...
export class LocationsClientService {
   ...
   * Reject a location
   */
  async rejectLocation({ id, reason }: { id: string, reason?: string }): Promise<LocationDTO> {
...
export function useLocations(filters = {}) {
  ...
  const approveMutation = useMutation({
    mutationFn: (id: string) => 
      locationsClient.approveLocation(id),
    onSuccess: () => {
      toast({
        title: 'Success',
