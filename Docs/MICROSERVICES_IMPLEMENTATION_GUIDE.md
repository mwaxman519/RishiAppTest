...
*Complete Implementation of Event-Driven Microservices Architecture*
...
export class EventBusService {
  ...
  async publishEvent(event: BusinessEvent): Promise<void> {
    try {
      ...
      console.log(`📤 [EventBus] Publishing event: ${event.type} to ${allHandlers.length} handlers`);
      ...
      results.forEach((result, index) => {
        if (result.status === 'rejected') {
          ...
          console.error(`❌ [EventBus] Handler ${index} failed for event ${event.type}:`, result.reason);
        }
      });
      
      console.log(`✅ [EventBus] Event ${event.type} processed by ${results.filter(r => r.status === 'fulfilled').length}/${allHandlers.length} handlers`);
  ...
  getHealthStatus(): ServiceHealth {
    ...
    return {
      ...
      activeHandlers: Array.from(this.eventHandlers.values()).reduce((sum, handlers) => sum + handlers.length, 0),
      lastActivity: this.eventLog[this.eventLog.length - 1]?.timestamp || new Date(),
...
import { bookings, bookingStaffAssignments, locations, users } from '@shared/schema';
...
export class CannabisBookingService {
  ...
  async createBooking(data: BookingCreateData): Promise<any> {
    try {
      console.log('🌿 [CannabisBookingService] Creating new booking:', data.title);
      ...
      const booking = await db.transaction(async (tx) => {
        const [newBooking] = await tx
          ...
          .values({
            ...
            staffRequirements: data.staffRequirements ? JSON.stringify(data.staffRequirements) : null,
            equipmentRequirements: data.equipmentRequirements ? JSON.stringify(data.equipmentRequirements) : null,
            status: 'draft'
          })
          .returning();
      ...
      await this.eventBus.publishEvent({
        ...
        data: { 
          ...
          locationRequirements: data.locationId ? await this.getLocationRequirements(data.locationId) : null
        },
        timestamp: new Date(),
        ...
        metadata: {
          estimatedDuration: this.calculateDuration(data.scheduledStart, data.scheduledEnd),
          bookingValue: data.budget || 0,
          complexityScore: this.calculateComplexityScore(data)
        }
      });
      
      console.log('✅ [CannabisBookingService] Booking created successfully:', booking.id);
      return booking;
      
    } catch (error) {
      console.error('❌ [CannabisBookingService] Failed to create booking:', error);
      throw error;
    }
  }
  ...
  async updateBookingStatus(
    ...
    try {
      console.log(`🔄 [CannabisBookingService] Updating booking ${bookingId} status to ${status}`);
      ...
      await this.eventBus.publishEvent({
        ...
        data: { 
          ...
          statusTransitionReason: this.getStatusTransitionReason(currentBooking.status, status)
        },
        timestamp: new Date(),
        userId,
      ...
      await this.handleStatusChangeWorkflows(updatedBooking, currentBooking.status, status, userId);
      
      console.log(`✅ [CannabisBookingService] Status updated: ${currentBooking.status} → ${status}`);
      
    } catch (error) {
      console.error('❌ [CannabisBookingService] Failed to update booking status:', error);
      throw error;
    }
  }
  ...
  async assignStaff(
    ...
    try {
      console.log(`👥 [CannabisBookingService] Assigning ${assignments.length} staff to booking ${bookingId}`);
      ...
      await this.eventBus.publishEvent({
        ...
        data: { 
          ...
          totalHourlyRate: assignments.reduce((sum, a) => sum + (a.hourlyRate || 0), 0),
        ...
        correlationId: bookingId,
        metadata: {
          assignmentComplexity: this.calculateAssignmentComplexity(assignments, booking),
          estimatedTotalCost: this.estimateStaffCosts(assignments, booking)
        }
      });
      
      console.log(`✅ [CannabisBookingService] Staff assigned successfully to booking ${bookingId}`);
      
    } catch (error) {
      console.error('❌ [CannabisBookingService] Failed to assign staff:', error);
      throw error;
    }
  }
  ...
  async getBookingsByOrganization(
    ...
    try {
      console.log(`📋 [CannabisBookingService] Getting bookings for organization ${organizationId}`);
      ...
      const bookingsWithStaff = await Promise.all(
        results.map(async (result) => {
          const staffAssignments = await this.getBookingStaffAssignments(result.booking.id);
          return {
            ...result.booking,
      ...
      console.log(`✅ [CannabisBookingService] Retrieved ${bookingsWithStaff.length} bookings`);
      return bookingsWithStaff;
      
    } catch (error) {
      console.error('❌ [CannabisBookingService] Failed to get bookings:', error);
      throw error;
    }
  }
  ...
  async getBookingStatistics(organizationId: string): Promise<any> {
    try {
      ...
      const stats = {
        ...
        activeBookings: allBookings.filter(b => ['approved', 'confirmed', 'in_progress'].includes(b.status)).length,
      ...
      stats.averageBookingValue = stats.totalRevenue / (stats.completedBookings || 1);
      
      return stats;
    } catch (error) {
      console.error('❌ [CannabisBookingService] Failed to get statistics:', error);
      throw error;
    }
  }
  ...
  private async validateSchedulingConflicts(data: BookingCreateData): Promise<void> {
    ...
    if (conflicts.length > 0) {
      throw new Error(`Scheduling conflict detected. Location is already booked during this time.`);
    }
  }

  private determineCannabisOperationType(booking: any): string {
    const title = booking.title.toLowerCase();
    if (title.includes('cultivation') || title.includes('grow')) return 'cultivation';
    if (title.includes('manufacturing') || title.includes('processing')) return 'manufacturing';
    if (title.includes('dispensary') || title.includes('retail')) return 'retail';
  ...
  private async getLocationRequirements(locationId: string): Promise<any> {
    ...
    return location ? {
      ...
      accessibility: location.accessibilityFeatures ? JSON.parse(location.accessibilityFeatures) : []
    } : null;
  }
  ...
  private calculateComplexityScore(data: BookingCreateData): number {
    ...
    const duration = this.calculateDuration(data.scheduledStart, data.scheduledEnd);
  ...
  private getStatusTransitionReason(oldStatus: string, newStatus: string): string {
  ...
  private getTransitionType(oldStatus: string, newStatus: string): string {
    ...
    if (oldStatus === 'draft' && newStatus === 'pending_approval') return 'submission';
    if (oldStatus === 'pending_approval' && newStatus === 'approved') return 'approval';
    return 'progression';
  }

  private assessStatusChangeImpact(booking: any, newStatus: string): string {
    if (newStatus === 'cancelled') {
      const timeUntilStart = new Date(booking.scheduledStart).getTime() - Date.now();
  ...
  private async validateStaffAvailability(
    ...
    console.log(`🔍 [CannabisBookingService] Validating staff availability for ${assignments.length} assignments`);
  }
  ...
  private async validateStaffSkills(
    ...
    console.log(`🎯 [CannabisBookingService] Validating staff skills for ${assignments.length} assignments`);
  }
  ...
  private async getMatchedSkills(assignments: StaffAssignment[], booking: any): Promise<string[]> {
    // Implementation would match staff skills to booking requirements
    return [];
  }

  private calculateAssignmentComplexity(assignments: StaffAssignment[], booking: any): number {
    return assignments.length + (booking.requirements ? 2 : 0);
  }

  private estimateStaffCosts(assignments: StaffAssignment[], booking: any): number {
    const duration = this.calculateDuration(
      new Date(booking.scheduledStart), 
      new Date(booking.scheduledEnd)
    );
    return assignments.reduce((sum, a) => sum + (a.hourlyRate || 25) * duration, 0);
  }
...
export class CircuitBreakerService {
  ...
  async executeWithCircuitBreaker<T>(
    ...
    if (circuit.state === CircuitState.OPEN) {
      if (this.shouldAttemptReset(circuit)) {
        ...
        console.log(`🔄 [CircuitBreaker] ${serviceKey}: Attempting reset (HALF_OPEN)`);
      } else {
    ...
    try {
      ...
      if (circuit.state === CircuitState.HALF_OPEN) {
        ...
        console.log(`✅ [CircuitBreaker] ${serviceKey}: Reset successful (CLOSED)`);
      }
  ...
  private getOrCreateCircuit(serviceKey: string, config?: Partial<CircuitBreakerConfig>) {
  ...
  private shouldAttemptReset(circuit: any): boolean {
    ...
    return Date.now() - circuit.lastFailureTime.getTime() >= circuit.config.resetTimeoutMs;
  }
  ...
  private handleFailure(serviceKey: string, circuit: any, error: any): never {
    ...
    if (circuit.failureCount >= circuit.config.failureThreshold) {
      ...
      console.error(`🚨 [CircuitBreaker] ${serviceKey}: Circuit OPENED after ${circuit.failureCount} failures`);
    }
    
    console.error(`❌ [CircuitBreaker] ${serviceKey}: Operation failed (${circuit.failureCount}/${circuit.config.failureThreshold})`, error);
    throw error;
  }
...
export class HealthMonitorService {
  ...
  async getSystemHealth(): Promise<SystemHealthReport> {
    ...
    const [database, eventBus, circuitBreakers, memory, external] = checks.map(result => 
    ...
    const overall = this.determineOverallHealth([database, eventBus, circuitBreakers, memory, external]);
...
export class BookingEventHandlers {
  ...
  private registerHandlers(): void {
    ...
    this.eventBus.subscribe('BOOKING_CREATED', this.handleBookingCreated.bind(this));
    
    // Handle status changes
    this.eventBus.subscribe('BOOKING_STATUS_CHANGED', this.handleBookingStatusChanged.bind(this));
    
    // Handle staff assignments
    this.eventBus.subscribe('STAFF_ASSIGNED_TO_BOOKING', this.handleStaffAssigned.bind(this));
  ...
  private async handleBookingCreated(event: BusinessEvent): Promise<void> {
    try {
      ...
      console.log(`🌿 [BookingEventHandlers] Processing booking creation: ${booking.title}`);
      ...
      console.error('❌ [BookingEventHandlers] Failed to handle booking creation:', error);
    }
  }

  private async handleBookingStatusChanged(event: BusinessEvent): Promise<void> {
    try {
      const { booking, oldStatus, newStatus, changedBy } = event.data;
      
      console.log(`🔄 [BookingEventHandlers] Processing status change: ${oldStatus} → ${newStatus}`);
      
      // Send status change notifications
      await this.notificationService.sendStatusChangeNotification(booking, oldStatus, newStatus);
      ...
      console.error('❌ [BookingEventHandlers] Failed to handle status change:', error);
    }
  }
  ...
  private async handleStaffAssigned(event: BusinessEvent): Promise<void> {
    try {
      ...
      console.log(`👥 [BookingEventHandlers] Processing staff assignment: ${assignments.length} staff members`);
      ...
      console.error('❌ [BookingEventHandlers] Failed to handle staff assignment:', error);
    }
  }
...
export class ServiceRegistry {
  ...
  public initialize(): void {
    ...
    console.log(`✅ [ServiceRegistry] ${this.services.size} services initialized`);
  }
