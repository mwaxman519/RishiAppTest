...
*Comprehensive Testing Framework for Rishi Platform*
...
const mockEventBus = EventBusService.getInstance() as jest.Mocked<EventBusService>;
    ...
    test('creates cannabis booking with proper event publishing', async () => {
      ...
      expect(mockEventBus.publishEvent).toHaveBeenCalledWith({
        ...
        data: expect.objectContaining({
          ...
          requiredStaffSkills: ['cannabis_product_knowledge', 'customer_service']
        }),
  ...
  describe('GET /api/bookings', () => {
    test('returns cannabis bookings for organization', async () => {
      ...
      expect(data.bookings[0]).toMatchObject({
        ...
        status: expect.stringMatching(/^(draft|approved|in_progress|completed)$/),
        organizationId: 'org-123'
      });
    });
...
describe('Cannabis Database Operations', () => {
  ...
  test('creates complete cannabis booking workflow', async () => {
    ...
    const [booking] = await db.insert(bookings).values({
      ...
      description: 'Educational workshop on cannabis products and consumption methods',
      organizationId: organization.id,
      createdBy: user.id,
...
test.describe('Cannabis Booking Management Workflow', () => {
  ...
  test('creates complete cannabis booking workflow', async ({ page }) => {
    ...
    await page.fill('[data-testid=booking-title]', 'Cannabis Product Education Event');
    await page.fill('[data-testid=booking-description]', 
    ...
    await page.selectOption('[data-testid=skill-select]', 'cannabis_product_knowledge');
    await page.click('[data-testid=add-skill-requirement]');
  ...
  test('manages cannabis staff assignments', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid=assigned-staff-list]')).toContainText('3 staff members assigned');
    await expect(page.locator('[data-testid=total-cost-estimate]')).toContainText('$720'); // 8 hours * (35+30+25)
  });
  ...
  test('handles cannabis compliance requirements', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid=booking-status]')).toContainText('Compliance Verified');
  });
  ...
  test('generates cannabis industry reports', async ({ page }) => {
    ...
    await page.selectOption('[data-testid=report-type]', 'cannabis-operations-summary');
    ...
    await expect(page.locator('[data-testid=report-title]')).toContainText('Cannabis Operations Summary');
    ...
    expect(download.suggestedFilename()).toMatch(/cannabis-operations-summary.*\.pdf/);
  });
});
```
...
describe('Cannabis Booking Performance Benchmarks', () => {
  ...
  test('cannabis staff assignment performance', async () => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      await service.assignStaff('test-booking-id', [
    ...
    test('sanitizes cannabis product descriptions', async () => {
      const response = await request(app)
        ...
        .send({
          ...
          description: '<script>alert("xss")</script>Cannabis product information',
          organizationId: 'org-123'
        });

      expect(response.status).toBe(201);
      expect(response.body.description).not.toContain('<script>');
      expect(response.body.description).toContain('Cannabis product information');
    });
  });
...
export class CannabisTestDataFactory {
  ...
  static createCannabisLocation(organizationId: string, overrides: Partial<Location> = {}): Location {
    const locationTypes = ['dispensary', 'cultivation_facility', 'manufacturing_facility', 'testing_lab'];
  ...
  static createCannabisBooking(
    ...
    return {
      ...
      status: faker.helpers.arrayElement(['draft', 'approved', 'in_progress', 'completed']),
...
export class CannabisTestReporter {
  ...
  private static filterCannabisTests(testFiles: string[]): string[] {
    return testFiles.filter(file => {
      ...
      return content.includes('cannabis') || 
...
console.log(`  Booking Creation: ${metrics.performanceBenchmarks.bookingCreation}ms`);
console.log(`  Staff Assignment: ${metrics.performanceBenchmarks.staffAssignment}ms`);
console.log(`Security Tests: ${metrics.securityTests.authenticationTests + metrics.securityTests.authorizationTests + metrics.securityTests.inputValidationTests} total`);
```
