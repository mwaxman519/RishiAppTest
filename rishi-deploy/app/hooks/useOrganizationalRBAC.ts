...
import { hasOrganizationalPermission, loadOrganizationSettings, getDefaultOrganizationSettings } from '@/lib/rbac/organizational-permissions';

export function useOrganizationalRBAC(organizationId?: string) {
  const { user } = useAuth();
  const [organizationSettings, setOrganizationSettings] = useState<Record<string, boolean>>(
