...
const ScheduleEventCard = ({ event, onAction }: { event: ScheduleEvent; onAction: (action: string, eventId: string) => void }) => {
    ...
    <Card className="hover:shadow-lg transition-all duration-200">
      <CardHeader className="pb-4">
        <div className="flex items-start justify-between">
          ...
          <Badge className={`${getStatusColor(event.status)} border flex items-center gap-1`}>
        ...
        <div className="bg-blue-50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium text-blue-800">Staffing Status</span>
            </div>
          ...
          <div className="w-full bg-blue-200 rounded-full h-2">
            <div 
              ...
              style={{ width: `${Math.min((event.assignedStaff / event.requiredStaff) * 100, 100)}%` }}
            />
          </div>
        </div>
          ...
          <div className="space-y-2 max-h-32 overflow-y-auto">
            {event.assignments.map((assignment) => (
              <div key={assignment.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                ...
                <Badge className={`${getAssignmentStatusColor(assignment.status)} text-xs`}>
                  {assignment.status}
                </Badge>
              </div>
            ))}
...
export default function StaffSchedulePage() {
  ...
  const fullyStaffedEvents = scheduleData.filter(e => e.status === 'fully_staffed').length;
  const needsStaffEvents = scheduleData.filter(e => e.status === 'needs_staff').length;
  const totalAssignments = scheduleData.reduce((sum, e) => sum + e.assignedStaff, 0);
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Fully Staffed</p>
                <p className="text-2xl font-bold text-green-600">{fullyStaffedEvents}</p>
              </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Needs Staff</p>
                <p className="text-2xl font-bold text-orange-600">{needsStaffEvents}</p>
              </div>
      ...
      {filteredEvents.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="mt-4 text-lg font-medium">No scheduled events found</h3>
