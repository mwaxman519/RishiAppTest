...
function getDatabaseUrl(): string {
  ...
  if (!process.env.DATABASE_URL) {
    console.error("[Auth Service] DATABASE_URL must be set. Did you forget to provision a database?");
...
if (connectionString) {
  try {
    ...
    console.log(`[Auth Service] Successfully parsed database connection info for user: ${user}, host: ${host}`);
  } catch (parseError) {
    console.error('[Auth Service] Error parsing database connection string:', parseError);
  }
}
...
export async function testConnection() {
  ...
  console.log(`[Auth Service] Testing database connection with config:`, diagnosticInfo);
  ...
  while (retries <= maxRetries) {
    try {
      ...
      try {
        ...
        console.log(`[Auth Service] Database connection test successful after ${retries} retries`);
      ...
      let errorDetails = lastError instanceof Error ? lastError.message : String(lastError);
      ...
      if (errorDetails.includes('password authentication failed') ||
        ...
        console.error(`[Auth Service] Authentication error detected. This is likely due to incorrect credentials for environment: ${env}`);
        console.error(`[Auth Service] Recommendation: Check that database credentials match the Replit environment`);
      }
      
      console.error(`[Auth Service] Database connection test failed (attempt ${retries + 1}/${maxRetries + 1}):`, errorDetails);
