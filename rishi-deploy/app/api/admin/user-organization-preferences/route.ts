...
import { hasEnhancedPermission, createPermissionContext } from '../../../lib/rbac-enhanced';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (data.is_default) {
      await db.update(userOrganizationPreferences)
        .set({ is_default: false })
        .where(and(
          eq(userOrganizationPreferences.user_id, data.user_id),
          ne(userOrganizationPreferences.organization_id, data.organization_id)
        ));
    }
    ...
    if (existingPreference) {
      const updated = await db.update(userOrganizationPreferences)
        .set({ 
    ...
    const result = await db.insert(userOrganizationPreferences).values({
