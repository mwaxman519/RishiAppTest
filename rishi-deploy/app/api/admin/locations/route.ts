...
export async function POST(req: Request) {
  try {
    ...
    const insertResult = await db.insert(locations).values(locationData).returning();
    ...
    if (location) {
      try {
        ...
        const { eventBus } = await import('../../../services/infrastructure/messaging/eventBus');
...
export async function GET(req: Request) {
  try {
    ...
    const locations_data = result.map(location => ({
      ...
      longitude: location.geoLng ? parseFloat(location.geoLng.toString()) : null,
    ...
    console.log('Returning transformed location data for map component:', 
