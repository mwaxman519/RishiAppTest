...
import { userOrganizationPreferences, userOrganizations, organizations } from '@shared/schema';
    ...
    if (organizationId) {
      const preference = await db.query.userOrganizationPreferences.findFirst({
        where: and(
          ...
          eq(userOrganizationPreferences.organizationId, parseInt(organizationId))
        ),
      });

      if (!preference) {
        // Create a default preference if none exists
        const newPreference = await db.insert(userOrganizationPreferences).values({
          userId: user.id,
      ...
      if (preference.notificationSettings) {
        try {
          preference.notificationSettings = JSON.parse(preference.notificationSettings);
        } catch (e) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { organizationId, theme, dashboardLayout, notificationSettings } = validation.data;
    ...
    const existingPreference = await db.query.userOrganizationPreferences.findFirst({
      where: and(
        eq(userOrganizationPreferences.userId, user.id),
    ...
    if (notificationSettings) preferenceData.notificationSettings = JSON.stringify(notificationSettings);

    let updatedPreference;

    // Create or update preference
    if (existingPreference) {
      updatedPreference = await db.update(userOrganizationPreferences)
        .set(preferenceData)
        .where(and(
          eq(userOrganizationPreferences.userId, user.id),
          eq(userOrganizationPreferences.organizationId, organizationId)
      ...
      updatedPreference = await db.insert(userOrganizationPreferences)
        .values({
          userId: user.id,
    ...
    if (updatedPreference[0].notificationSettings) {
      try {
        updatedPreference[0].notificationSettings = JSON.parse(updatedPreference[0].notificationSettings);
      } catch (e) {
        console.warn("Failed to parse notification settings JSON:", e);
      }
    }
