...
export async function GET(request: NextRequest) {
  try {
    ...
    const canViewBranding = await hasPermission('view:organization_branding', { organizationId });
    if (!canViewBranding) {
      return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const canUpdateBranding = await hasPermission('edit:organization_branding', { organizationId: organization_id });
    ...
    if (!canCustomize) {
      return NextResponse.json(
        { error: 'This organization tier does not support branding customization' },
        { status: 403 }
      );
    }
