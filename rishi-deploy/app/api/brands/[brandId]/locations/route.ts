...
import { brandLocations, brands, locations } from '../../../../../shared/schema';
...
export async function GET(
  ...
  try {
    ...
    const brandExists = await db.select({ id: brands.id })
      .from(brands)
      .where(
        and(
          eq(brands.id, brandId),
          eq(brands.clientId, organizationId)
        )
      )
      .limit(1);
...
export async function POST(
  ...
  try {
    ...
    const locationExists = await db.select({ id: locations.id, status: locations.status })
      .from(locations)
    ...
    const existingBrandLocation = await db.select({ id: brandLocations.id })
      .from(brandLocations)
      .where(
        and(
