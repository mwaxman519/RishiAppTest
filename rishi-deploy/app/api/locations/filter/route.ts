...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (skipPending) {
        filteredLocations = filteredLocations.filter(loc => 
          loc.status === 'active' || 
          (loc.status !== 'pending' && loc.status !== 'rejected' && loc.approved === true)
        );
      }
      ...
      if (filters.search) {
        ...
        filteredLocations = filteredLocations.filter(location => 
          ...
          (location.zipCode && location.zipCode.toLowerCase().includes(searchLower))
        );
      }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to filter locations" },
      { status: 500 }
    );
  }
}