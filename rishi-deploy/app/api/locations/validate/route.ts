...
export async function GET(request: NextRequest) {
  try {
    ...
    // City
    locationData.city = addressComponents.find((component: any) => 
      component.types.includes('locality'))?.long_name || '';
    
    // State
    locationData.state = addressComponents.find((component: any) => 
      component.types.includes('administrative_area_level_1'))?.short_name || '';
    
    // ZIP Code
    locationData.zipcode = addressComponents.find((component: any) => 
