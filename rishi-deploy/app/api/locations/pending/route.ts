...
import { AppEvent, LocationApprovalPayload, LocationRejectionPayload } from "../../../../services/infrastructure/messaging/eventTypes";
...
export async function PUT(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const [location] = await db.select().from(schema.locations).where(eq(schema.locations.id, body.id));
    ...
    if (isDevelopment) {
      try {
        ...
        async function getOrCreateTestUserId() {
          ...
          const testUsers = await db.select().from(schema.users).where(eq(schema.users.username, 'testuser'));
    ...
    try {
      ...
      console.log(`WebSocket notification sent for location ${body.id} ${newStatus}`);
    } catch (eventError) {
