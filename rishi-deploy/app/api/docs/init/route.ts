...
function copyDir(src: string, dest: string): void {
  ...
  try {
    ...
    console.error(`[DOCS INIT] Error copying directory from ${src} to ${dest}:`, error);
  }
}
...
function findDocsDirectory(): string | null {
  ...
  console.log('[DOCS INIT] Searching for Docs directory in all potential locations...');
      ...
      if (existsSync(dirPath) && statSync(dirPath).isDirectory()) {
        ...
        try {
          ...
          if (files.length > 0) {
            ...
            console.log(`[DOCS INIT] - Contains ${files.length} entries (${mdFiles.length} markdown files, ${directories.length} subdirectories)`);
            console.log(`[DOCS INIT] - First few items: ${files.slice(0, 5).join(', ')}`);
          ...
          console.error(`[DOCS INIT] Directory exists but cannot be read: ${dirPath} - ${readErr instanceof Error ? readErr.message : 'Unknown error'}`);
        }
      }
    } catch (err) {
  ...
  if (validDirectories.length > 0) {
    ...
    console.log(`[DOCS INIT] - Contains ${fileCount} files (${mdCount} markdown files)`);
  ...
  if (emptyDirectories.length > 0) {
    ...
    console.log(`[DOCS INIT] No populated directories found. Using empty directory: ${firstEmptyDir}`);
    return firstEmptyDir;
  }
  
  console.error('[DOCS INIT] Could not find a valid Docs directory in any location');
  ...
  try {
    ...
    console.error(`[DOCS INIT] Failed to create new Docs directory: ${createErr instanceof Error ? createErr.message : 'Unknown error'}`);
    return null;
  }
}
...
function createSampleDocumentation(targetDir: string): boolean {
  console.log(`[DOCS INIT] Creating minimal documentation samples in ${targetDir}`);
...
`;
    ...
    console.log(`[DOCS INIT] Successfully created sample documentation with ${directories.length + 1} files`);
    return true;
  } catch (error) {
    console.error(`[DOCS INIT] Error creating sample documentation: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return false;
  }
}
...
async function initDocs(forceInit: boolean = false) {
  ...
  if (forceInit && sourceDir) {
    console.log('[DOCS INIT] Force initialization enabled - creating fresh documentation');
    
    try {
      // Create sample documentation in the source directory
      if (createSampleDocumentation(sourceDir)) {
        console.log('[DOCS INIT] Created fresh sample documentation due to force flag');
        isCreateMode = true;
      }
    } catch (err) {
      console.error(`[DOCS INIT] Error creating fresh documentation: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }
  ...
  if (!sourceDir) {
    ...
    try {
      ...
      if (createSampleDocumentation(newDir)) {
        console.log('[DOCS INIT] Created sample documentation. Will use this as source.');
        workingSourceDir = newDir;
        isCreateMode = true;
      } else {
        console.error('[DOCS INIT] Failed to create sample documentation. Process aborted.');
        return false;
      }
    } catch (err) {
      console.error(`[DOCS INIT] Failed to create directory: ${err instanceof Error ? err.message : 'Unknown error'}`);
      return false;
    }
  } else {
    ...
    try {
      ...
      if (files.length === 0) {
        console.warn(`[DOCS INIT] Source directory is empty. Creating sample content.`);
        if (createSampleDocumentation(workingSourceDir)) {
          console.log('[DOCS INIT] Created sample documentation in source directory.');
          isCreateMode = true;
        }
      }
    } catch (err) {
      console.error(`[DOCS INIT] Error checking source directory: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }
  ...
  console.log(`[DOCS INIT] Will copy from ${workingSourceDir} to ${targetDirs.length} locations`);
    ...
    try {
      ...
      if (!existsSync(parentDir)) {
        try {
          ...
          console.warn(`[DOCS INIT] Could not create parent directory ${parentDir}: ${mkdirErr instanceof Error ? mkdirErr.message : 'Unknown error'}`);
        }
      }
      
      // Safety check for source directory
      if (workingSourceDir === '') {
        console.error(`[DOCS INIT] Cannot copy from empty source directory to ${targetDir}`);
        failCount++;
        continue;
      }
      ...
      try {
        ...
        if (existsSync(targetDir)) {
          try {
            ...
            if (files.length > 0) {
              console.log(`[DOCS INIT] ✓ Successfully copied to ${targetDir} (${files.length} items)`);
              successCount++;
            } else {
              console.error(`[DOCS INIT] ✗ Copy completed but target directory is empty: ${targetDir}`);
              failCount++;
            }
          } catch (readErr) {
            console.error(`[DOCS INIT] ✗ Cannot verify copy, directory not readable: ${targetDir}`);
            failCount++;
          }
        } else {
          console.error(`[DOCS INIT] ✗ Target directory does not exist after copy: ${targetDir}`);
          failCount++;
        }
      } catch (copyError) {
        console.error(`[DOCS INIT] ✗ Error during copy process: ${copyError instanceof Error ? copyError.message : 'Unknown error'}`);
        failCount++;
      }
    } catch (error) {
      console.error(`[DOCS INIT] ✗ Error copying to ${targetDir}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      failCount++;
    }
  }
  ...
  if (successCount > 0) {
    console.log('[DOCS INIT] ✅ Documentation successfully copied to at least one location');
    
    if (isCreateMode) {
      console.log('[DOCS INIT] NOTE: Sample documentation was created since no actual docs were found');
    }
    
    // Success
    return true;
  } else {
    console.error('[DOCS INIT] ❌ Documentation copy process failed completely');
    return false;
  }
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log(`[DOCS INIT API] Running in ${isProduction ? 'PRODUCTION' : 'DEVELOPMENT'} mode${isEmergency ? ' (EMERGENCY MODE)' : ''}${forceInit ? ' (FORCE REINITIALIZATION)' : ''}`);
    
    // Get the current docs dir before initialization to compare
    const beforeDocsDir = await getDocsDirectory();
    console.log(`[DOCS INIT API] Current docs directory before initialization: ${beforeDocsDir ?? 'Not found'}`);
    
    if (beforeDocsDir) {
      try {
        const beforeFiles = await fs.readdir(beforeDocsDir);
        console.log(`[DOCS INIT API] BEFORE: Directory contains ${beforeFiles.length} items`);
        console.log(`[DOCS INIT API] BEFORE: First few items: ${beforeFiles.slice(0, 5).join(', ')}`);
      } catch (err) {
        console.error(`[DOCS INIT API] Cannot read docs directory: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }
    }
    ...
    if (isProduction || isEmergency) {
      try {
        ...
        const publicDirExists = await fs.access(publicDocsDir).then(() => true).catch(() => false);
        if (publicDirExists) {
          console.log(`[DOCS INIT API] Found public/Docs directory - copying to main Docs directory`);
          try {
            ...
            for (const entry of entries) {
              try {
                ...
                console.error(`[DOCS INIT API] Error copying ${entry.name}: ${e instanceof Error ? e.message : 'Unknown error'}`);
              }
            }
            
            console.log(`[DOCS INIT API] Copied ${copyCount} items from public/Docs to main Docs directory`);
          } catch (e) {
            console.error(`[DOCS INIT API] Error copying from public/Docs: ${e instanceof Error ? e.message : 'Unknown error'}`);
          }
        } else {
          console.log(`[DOCS INIT API] public/Docs directory not found - skipping public directory copy`);
        }
      } catch (e) {
        console.error(`[DOCS INIT API] Error during public directory check: ${e instanceof Error ? e.message : 'Unknown error'}`);
      }
    }
    
    // Always run initialization process, but log differently
    console.log(`[DOCS INIT API] Running initialization in ${isProduction ? 'PRODUCTION' : 'DEVELOPMENT'} mode${forceInit ? ' with FORCE flag' : ''}`);
    ...
    if (result) {
      ...
      console.log(`[DOCS INIT API] Current docs directory after initialization: ${docsDirPath}`);
      
      // Check if the directory is readable and print detailed information
      if (docsDir) {
        try {
          const files = await fs.readdir(docsDir);
          console.log(`[DOCS INIT API] AFTER: Docs directory contains ${files.length} items`);
          console.log(`[DOCS INIT API] AFTER: First few items: ${files.slice(0, 5).join(', ')}`);
          ...
          if (isProduction) {
            ...
            console.log(`[DOCS INIT API] Found ${allDirectories.length} subdirectories: ${allDirectories.join(', ')}`);
            ...
            for (const keyFile of keyFilesToCheck) {
              ...
              console.log(`[DOCS INIT API] Key file ${keyFile}: ${keyFileExists ? 'EXISTS' : 'MISSING'}`);
            }
          }
        } catch (err) {
          console.error(`[DOCS INIT API] Cannot read docs directory: ${err instanceof Error ? err.message : 'Unknown error'}`);
        ...
        console.error('[DOCS INIT API] No docs directory was found to read from after initialization');
      }
      ...
      return NextResponse.json({
        ...
        filesCount: docsDir ? (await fs.readdir(docsDir).catch(() => [])).length : 0
      });
    } else {
      return NextResponse.json(
...
export async function POST(request: Request) {
  console.log('[DOCS INIT API] Documentation initialization started via POST request');
  ...
  try {
    ...
    console.log(`[DOCS INIT API] POST request${forceInit ? ' with FORCE flag' : ''}`);
    ...
    if (initSuccess) {
      ...
      const docsDir = await getDocsDirectory();
      console.log(`[DOCS INIT API] Docs directory after initialization: ${docsDir ?? 'Not found'}`);
      
      // Check for key files to verify initialization worked
      if (docsDir) {
        try {
          const filesAfter = await fs.readdir(docsDir);
          console.log(`[DOCS INIT API] AFTER: Directory contains ${filesAfter.length} items`);
          console.log(`[DOCS INIT API] AFTER: First few items: ${filesAfter.slice(0, 5).join(', ')}`);
