...
import { bookings, bookingComments, insertBookingCommentSchema } from '../../../../../shared/schema';
...
export async function GET(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Check permissions
    const isAdmin = ['super_admin', 'internal_admin', 'internal_field_manager'].includes(session.user.role);
    const isInClientOrg = booking.clientOrganizationId === session.user.organizationId;
    ...
    let query = db.select().from(bookingComments)
