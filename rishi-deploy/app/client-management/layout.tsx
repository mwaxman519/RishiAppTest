        ...
        <div className="flex-1">
          <nav className="flex space-x-1" aria-label="Breadcrumb">
            <div className="flex items-center text-sm">
              <Link href="/" className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200">
                Home
              </Link>
              ...
              {pathname !== '/client-management' && (
                <>
                  <ChevronRight className="h-4 w-4 mx-1 text-gray-400" />
                  <span className="text-gray-900 dark:text-gray-200 font-medium">
                    {allowedNavItems.find(item => pathname.startsWith(item.href))?.title || 'Details'}
                  </span>
                </>
              )}
            </div>
        ...
        {allowedNavItems.map((item) => (
          <Link 
            ...
            className={cn(
              ...
              pathname === item.href || 
              (item.href !== '/client-management' && pathname.startsWith(item.href))
