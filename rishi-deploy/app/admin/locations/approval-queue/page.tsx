...
export default function LocationApprovalQueuePage() {
  ...
  const { data: hasPermission, error: permissionError } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch('/api/permissions/check?permission=manage:locations');
      if (!res.ok) {
  ...
  useEffect(() => {
    ...
    if (hasPermission === false) {
      toast({
        title: 'Access Denied',
        description: 'You do not have permission to access the location approval queue.',
        variant: 'destructive'
      });
      router.push('/dashboard');
    }
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Review and manage locations that require administrator approval before being published
        </p>
      </div>
