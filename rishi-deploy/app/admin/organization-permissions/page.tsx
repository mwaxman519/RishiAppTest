...
export default function OrganizationPermissionsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchAvailablePermissions() {
      try {
        ...
        toast({
          title: 'Error',
          description: 'Failed to load available permissions. Please try again.',
          variant: 'destructive'
        });
      }
    }
  ...
  useEffect(() => {
    ...
    async function fetchOrgPermissions() {
      try {
        setLoading(true);
        const response = await fetch(`/api/rbac/organization-permissions?organizationId=${selectedOrgId}`);
        if (!response.ok) {
        ...
        toast({
          title: 'Error',
          description: 'Failed to load organization permissions. Please try again.',
          variant: 'destructive'
        });
        setLoading(false);
      }
  ...
  const addCustomPermission = () => {
    ...
    toast({
      title: 'Success',
      description: 'Custom permission added. Don\'t forget to save your changes.',
      variant: 'default'
    });
  };
  ...
  return (
    <PermissionGuard permission="admin:permissions">
      <div className="container mx-auto py-8">
        <h1 className="text-3xl font-bold mb-8">Organization Permissions Management</h1>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="w-[150px] text-center">Allowed</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {loading ? (
                          ...
                          orgPermissions.map((perm) => {
                            ...
                            const permDetails = permissions.find(p => p.name === perm.name);
                  ...
                  <div className="space-y-6">
                    <div className="flex items-end gap-2">
                      <div className="flex-1 space-y-1.5">
                        <Label htmlFor="new-permission">Add Custom Permission</Label>
                        <Input
                          id="new-permission"
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {loading ? (
                            ...
                            orgPermissions
                              .filter(perm => !permissions.some(p => p.name === perm.name))
                              .map((perm) => (
                                <TableRow key={perm.name}>
                                  <TableCell className="font-medium">{perm.name}</TableCell>
                                  <TableCell className="text-center">
                          ...
                          {!loading && 
                           orgPermissions.filter(perm => !permissions.some(p => p.name === perm.name)).length === 0 && (
                            <TableRow>
                              <TableCell colSpan={2} className="text-center py-4">
