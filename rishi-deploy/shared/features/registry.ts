 ...
 */
export function isFeatureEnabled(featureId: string, organizationTier?: string): boolean {
  ...
  if (organizationTier) {
    return isFeatureAvailableForTier(featureId, organizationTier) && feature.enabled;
  }
 ...
 */
export function isFeatureAvailableForTier(featureId: string, organizationTier: string): boolean {
  const feature = FeatureModuleRegistry[featureId];
  if (!feature) {
    return false;
  }
 ...
 */
export function getFeaturesForTier(organizationTier: OrganizationTier): FeatureModule[] {
 ...
 */
export function initializeOrganizationFeatures(organizationId: string, tier: OrganizationTier): {
  ...
  console.log(`Initialized ${enabledFeatures.length} features for organization ${organizationId}`);
 ...
 */
export function validateFeatureDependencies(featureId: string, enabledFeatures: string[]): boolean {
  const feature = FeatureModuleRegistry[featureId];
