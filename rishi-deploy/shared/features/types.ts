...
export abstract class BaseFeatureModule {
  ...
  async initializeForOrganization(organizationId: string): Promise<void> {
    console.log(`Initializing feature ${this.id} for organization ${organizationId}`);
  }
...
export interface IFeatureRegistry {
  ...
  getFeatureStatus(featureId: string, organizationId: string): Promise<FeatureStatus | null>;
...
export class FeatureModuleRegistry implements IFeatureRegistry {
  ...
  async isFeatureEnabled(featureId: string, organizationId: string): Promise<boolean> {
    // In a real implementation, this would check the database
    // For simplicity, we'll always return true in development
    return true;
  }
  
  async enableFeature(featureId: string, organizationId: string): Promise<void> {
    const module = this.getModule(featureId);
    if (!module) {
    ...
    console.log(`Enabled feature ${featureId} for organization ${organizationId}`);
  }
  
  async disableFeature(featureId: string, organizationId: string): Promise<void> {
    const module = this.getModule(featureId);
    if (!module) {
      throw new Error(`Feature module with ID ${featureId} not found`);
    }
    ...
    console.log(`Disabled feature ${featureId} for organization ${organizationId}`);
  }
  
  async getFeatureStatus(featureId: string, organizationId: string): Promise<FeatureStatus | null> {
  ...
  async getOrganizationFeatures(organizationId: string): Promise<FeatureStatus[]> {
    // In a real implementation, this would fetch from the database
