...
export class CircuitBreakerService {
  ...
  async execute<T>(operation: () => Promise<T>, fallback?: () => Promise<T>): Promise<T> {
    if (this.state === CircuitState.OPEN) {
      if (this.shouldAttemptReset()) {
        ...
        console.warn(`Circuit breaker ${this.serviceName} is OPEN, using fallback`);
        if (fallback) {
          return await fallback();
        }
        throw new Error(`Circuit breaker ${this.serviceName} is OPEN and no fallback provided`);
      }
    }

    if (this.state === CircuitState.HALF_OPEN && this.halfOpenCalls >= this.config.halfOpenMaxCalls) {
      console.warn(`Circuit breaker ${this.serviceName} HALF_OPEN call limit reached`);
      if (fallback) {
        return await fallback();
      }
      throw new Error(`Circuit breaker ${this.serviceName} HALF_OPEN call limit exceeded`);
    }
  ...
  private onFailure(): void {
    ...
    if (this.state === CircuitState.HALF_OPEN) {
      ...
      console.warn(`Circuit breaker ${this.serviceName} failed in HALF_OPEN, returning to OPEN`);
      return;
    }

    if (this.state === CircuitState.CLOSED && this.metrics.failedCalls >= this.config.failureThreshold) {
      this.state = CircuitState.OPEN;
      console.error(`Circuit breaker ${this.serviceName} opened due to ${this.metrics.failedCalls} failures`);
    }
  }
  ...
  setState(newState: CircuitState): void {
    console.log(`Circuit breaker ${this.serviceName} state manually changed from ${this.state} to ${newState}`);
    this.state = newState;
...
export class CircuitBreakerRegistry {
  ...
  getOrCreate(serviceName: string, config?: CircuitBreakerConfig): CircuitBreakerService {
    if (!this.circuitBreakers.has(serviceName)) {
      this.circuitBreakers.set(serviceName, new CircuitBreakerService(serviceName, config));
    }
  ...
  getHealthSummary() {
    ...
    const openBreakers = breakers.filter(b => b.getStatus().state === CircuitState.OPEN);
