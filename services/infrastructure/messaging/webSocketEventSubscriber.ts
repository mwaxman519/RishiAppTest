...
export class WebSocketEventSubscriber {
   ...
   */
  addConnection(connectionId: string, socket: WebSocket, userId: string, organizationId?: string): void {
   ...
   * @param organizationId Organization ID associated with the connection (optional)
   */
  removeConnection(connectionId: string, userId?: string, organizationId?: string): void {
  ...
  subscribe(connectionId: string, eventTypes: string[]): void {
    ...
    if (subscriptionSet) {
      ...
      console.log(`Connection ${connectionId} subscribed to events: ${eventTypes.join(', ')}`);
    } else {
  ...
  unsubscribe(connectionId: string, eventTypes: string[]): void {
    ...
    if (subscriptionSet) {
      ...
      console.log(`Connection ${connectionId} unsubscribed from events: ${eventTypes.join(', ')}`);
    } else {
   ...
   */
  publish(event: EventMessage, targetUserId?: string, targetOrgId?: string): void {
    ...
    if (targetOrgId && this.organizationSubscriptions.has(targetOrgId)) {
      const connectionIds = this.organizationSubscriptions.get(targetOrgId) || new Set();
   ...
   */
  private sendToConnection(connectionId: string, message: string, eventType: string): void {
    const socket = this.connections.get(connectionId);
    const subscriptions = this.subscriptions.get(connectionId);
    
    if (socket && subscriptions && (subscriptions.has(eventType) || subscriptions.has('*'))) {
      if (socket.readyState === WebSocket.OPEN) {
        socket.send(message);
        console.log(`Event sent to connection ${connectionId}: ${eventType}`);
      } else {
        console.warn(`Cannot send to connection ${connectionId}: Socket not open`);
      }
    }
  }
