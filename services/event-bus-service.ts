...
export class EventBusService {
  ...
  async publish(eventType: string, data: any, metadata?: any): Promise<boolean> {
    ...
    try {
      ...
      if (failures > 0) {
        console.warn(`${failures}/${results.length} handlers failed for event ${eventType}`);
      }
  ...
  private async executeHandlerSafely(handler: EventHandler, event: EventMessage): Promise<void> {
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error('Handler timeout')), this.publishTimeout);
    });
  ...
  private addToEventStore(event: EventMessage): void {
    ...
    if (this.eventStore.length > this.maxEventStoreSize) {
      this.eventStore.splice(0, this.eventStore.length - this.maxEventStoreSize);
    }
  }
  ...
  private async executeHandler(handler: EventHandler, event: EventMessage): Promise<void> {
    try {
