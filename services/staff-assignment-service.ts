...
export interface StaffAssignment {
  ...
  cannabisExpertiseLevel: 'beginner' | 'intermediate' | 'experienced' | 'expert';
...
export class StaffAssignmentService {
  ...
  async assignStaffToBooking(
    ...
    for (const staffId of staffIds) {
      ...
      await this.eventBus.publish('staff.assigned', {
        assignmentId,
        bookingId,
        staffId,
        assignedBy,
  ...
  async checkInStaff(
    ...
    await this.eventBus.publish('staff.checked_in', {
      checkInId,
      assignmentId,
      staffId: checkedInBy,
  ...
  async checkOutStaff(
    ...
    await this.eventBus.publish('staff.checked_out', {
      checkOutId,
      checkInId,
      staffId: checkedOutBy,
    ...
    await this.eventBus.publish('event.data_submitted', {
      staffId: checkedOutBy,
  ...
  async findAvailableCannabisStaff(
    ...
    await this.eventBus.publish('staff.availability_queried', {
      queryId: uuidv4(),
  ...
  async getAssignmentById(assignmentId: string): Promise<StaffAssignment | null> {
  ...
  async updateAssignmentStatus(
    ...
    await this.eventBus.publish('staff.assignment_status_changed', {
      assignmentId,
      newStatus,
      updatedBy,
      reason,
  ...
  async getStaffPerformanceMetrics(
    ...
    await this.eventBus.publish('staff.performance_metrics_queried', {
      staffId,
      startDate: startDate.toISOString(),
      endDate: endDate.toISOString(),
      metrics,
