...
export class HealthMonitorService {
  ...
  private async checkCircuitBreakers(): Promise<void> {
    try {
      ...
      this.updateHealthCheck('circuitBreakers', {
        ...
        status: healthSummary.healthStatus === 'healthy' ? 'healthy' : 'degraded',
        lastCheck: new Date(),
        details: {
  ...
  private async checkCannabisServices(): Promise<void> {
    try {
      ...
      this.updateHealthCheck('cannabis', {
        ...
        status: 'unhealthy',
        lastCheck: new Date(),
        error: error instanceof Error ? error.message : 'Cannabis services check failed'
      });
    }
  }
  ...
  async getSystemHealth(): Promise<SystemHealth> {
    ...
    return {
      ...
      checks,
      dependencies: {
        database: this.healthChecks.get('database') || this.createUnknownCheck('Database'),
        eventBus: this.healthChecks.get('eventBus') || this.createUnknownCheck('Event Bus'),
        circuitBreakers: this.healthChecks.get('circuitBreakers') || this.createUnknownCheck('Circuit Breakers'),
        memory: this.healthChecks.get('memory') || this.createUnknownCheck('Memory Usage'),
        cannabis: this.healthChecks.get('cannabis') || this.createUnknownCheck('Cannabis Services')
      }
    };
  }
