...
import { CannabisBooking, CreateCannabisBookingRequest, BookingEvent, BookingEventType } from '../shared/types/cannabis-booking';
...
export class CannabisBookingService {
  ...
  async updateBookingStatus(
    ...
    await db
      ...
      .set({
        ...
        stage: newStage,
        updatedAt: now,
        ...(newStatus === 'approved' && { approvedById: updatedBy, approvedAt: now }),
        ...(newStatus === 'rejected' && { rejectedById: updatedBy, rejectedAt: now, rejectionReason: reason }),
        ...(newStatus === 'canceled' && { canceledById: updatedBy, canceledAt: now, cancelReason: reason })
      })
      .where(eq(bookings.id, bookingId));
  ...
  async performOperationalCheck(
    ...
    if (booking.assignedStaff.length < booking.operationalRequirements.staffRequirements.totalStaffNeeded) {
