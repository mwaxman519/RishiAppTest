...
export default function AdminAvailabilityView({
  ...
  const [personalView, setPersonalView] = useState<'calendar' | 'list'>('calendar');
  ...
  return (
    <div>
      ...
      {activeTab === 'personal' && (
        <div>
          ...
          {personalView === 'calendar' && (
            <div className="mb-6">
              <div className="mb-6">
                <h2 className="text-lg font-semibold mb-4">My Availability Calendar</h2>
                <p className="text-gray-600 mb-4">
                  Manage your availability by selecting dates on the calendar. You can add, edit, or remove availability blocks.
                </p>
          ...
          {personalView === 'list' && (
            <div>
              ...
              <div>
                <h2 className="text-lg font-semibold mb-4">My Availability Blocks</h2>
                {userAvailabilityBlocks.length > 0 ? (
                      ...
                      <tbody>
                        {userAvailabilityBlocks.map((block) => (
                          <tr key={block.id} className="border-b">
                            <td className="p-2 border">{block.title || 'Untitled'}</td>
                            <td className="p-2 border">{new Date(block.startDate).toLocaleString()}</td>
                            <td className="p-2 border">{new Date(block.endDate).toLocaleString()}</td>
                            <td className="p-2 border">{block.status}</td>
                            <td className="p-2 border">{block.isRecurring ? 'Yes' : 'No'}</td>
                  ...
                  <p className="text-gray-500">No availability blocks found. Add some using the form above.</p>
                )}
              </div>
            </div>
          )}
      ...
      {activeTab === 'team' && (
        <div>
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-semibold">Organization-Wide Availability</h2>
            <div className="flex space-x-2">
              <Link 
          ...
          <p className="text-gray-600 mb-6">
            View and manage availability for all agents in the organization. Use the team view for more detailed options.
          </p>
          
          {/* Team availability summary */}
          <div className="bg-gray-50 p-6 rounded-lg mb-6">
            <h3 className="font-medium mb-4">Organization Availability Overview</h3>
            
            {/* This would typically be a component that shows a summary of organization-wide availability */}
            <p className="text-gray-500">Organization-wide availability overview will be displayed here.</p>
      ...
      {activeTab === 'system' && (
        <div>
          <h2 className="text-lg font-semibold mb-6">Availability System Settings</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white border rounded-lg p-6">
              <h3 className="text-md font-medium mb-4">Default Availability Rules</h3>
            ...
            <div className="bg-white border rounded-lg p-6">
              <h3 className="text-md font-medium mb-4">Working Hours Configuration</h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
