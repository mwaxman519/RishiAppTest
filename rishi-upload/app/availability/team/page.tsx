  ...
  useEffect(() => {
    const loadTeamMembers = async () => {
      ...
      try {
        ...
        const agentsResponse = await userService.getUsersByRole(USER_ROLES.BRAND_AGENT);
        ...
        if (agentsResponse.success && agentsResponse.data) {
          ...
          if (isAtLeastRole(USER_ROLES.INTERNAL_ADMIN)) {
            ...
            const managersResponse = await userService.getUsersByRole(USER_ROLES.INTERNAL_FIELD_MANAGER);
            
            if (managersResponse.success && managersResponse.data) {
              const combinedTeam = [...agentsResponse.data, ...managersResponse.data];
              console.log(`[team/page.tsx] Setting combined team (${combinedTeam.length} members)`);
              setAgents(combinedTeam);
            }
          } else {
            console.log(`[team/page.tsx] Setting agents only (${agentsResponse.data.length} members)`);
          }
        } else if (!agentsResponse.success) {
          console.error(`[team/page.tsx] Error fetching agents: ${agentsResponse.error}`);
        }
      } catch (error) {
        ...
        <>
          ...
          {isAtLeastRole(USER_ROLES.INTERNAL_FIELD_MANAGER) && (
            <div className="mb-6 bg-gray-50 p-4 rounded">
              <h2 className="text-lg font-semibold mb-2">Team Management Tools</h2>
          ...
          <div className="mb-6">
            ...
            {agents.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {agents.map((agent) => (
                  <div key={agent.id} className="border rounded-lg p-4 bg-white hover:shadow-md transition">
                    <div className="flex items-center mb-4">
                      <div className="w-12 h-12 bg-gray-300 rounded-full flex items-center justify-center text-gray-600 mr-4">
                        {agent.profileImage ? (
                          ...
                          <span>{(agent.fullName || agent.username).charAt(0).toUpperCase()}</span>
                        )}
                      </div>
                      <div>
                        <div className="font-medium">{agent.fullName || agent.username}</div>
              ...
              <p className="text-gray-500">No agents found. Please add agents to view team availability.</p>
            )}
          </div>
