      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="bg-white border rounded-lg p-6 mb-6">
            ...
            <p className="text-gray-500 mb-4">This calendar shows all team members' availability in a consolidated view.</p>
                  ...
                  <div key={member.id} className="flex items-center p-2 border rounded">
                    <div className="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center text-gray-600 mr-3">
                      {member.profileImage ? (
                        ...
                        <span>{(member.fullName || member.username).charAt(0).toUpperCase()}</span>
                      )}
                    </div>
                    <div>
                      <div className="font-medium">{member.fullName || member.username}</div>
                      <div className="text-xs text-gray-500">
                        {member.role === USER_ROLES.BRAND_AGENT ? 'Agent' : 'Manager'}
                      </div>
                    </div>
                  </div>
                ))}
