...
export function useAvailability(userId: number, startDate: string, endDate: string) {
  ...
  const fetchAvailability = useCallback(
    debounce(async () => {
      ...
      try {
        ...
        console.log(`useAvailability: Fetching for user ${userId} from ${startDate} to ${endDate}`);
        ...
        console.log(`useAvailability: Received ${Array.isArray(data) ? data.length : 0} blocks`);
        ...
        setError(err instanceof Error ? err.message : 'Unknown error fetching availability');
        setAvailability([]);
      } finally {
