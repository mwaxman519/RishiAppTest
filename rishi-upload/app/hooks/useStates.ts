...
export function useStates() {
  ...
  const fetchStates = async () => {
    try {
      ...
      try {
        ...
        console.warn('Error parsing API response, using mock data:', parseError);
        setStates(MOCK_STATES);
      }
    } catch (error) {
      console.error('Error fetching states:', error);
      setError(error instanceof Error ? error : new Error('Failed to fetch states'));
  ...
  const findStateByAbbreviation = (abbreviation: string): State | undefined => {
    return states.find(state => 
      state.code.toLowerCase() === abbreviation.toLowerCase()
    );
  };
