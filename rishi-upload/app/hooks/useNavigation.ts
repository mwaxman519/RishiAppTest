...
import { BRAND_AGENT_NAV, BRAND_AGENT_MOBILE_NAV } from '../components/navigation/BrandAgentNav';
...
export function useNavigation() {
  const { user } = useAuth();
  const [mobileNavItems, setMobileNavItems] = useState<NavItem[]>(BRAND_AGENT_MOBILE_NAV);
  ...
  useEffect(() => {
    ...
    } else if (role === 'internal_field_manager' || role === 'field_coordinator') {
  ...
  const isFieldManager = user?.role === 'internal_field_manager' || user?.role === 'field_coordinator';
  const isBrandAgent = user?.role === 'brand_agent';
  const isClientUser = user?.role === 'client_manager' || user?.role === 'client_user';
