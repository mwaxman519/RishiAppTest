...
export default function SystemRBACDefaultsPage() {
  ...
  const [organizationOverrides, setOrganizationOverrides] = useState<OrganizationOverride[]>([]);
        ...
        const defaults: SystemRBACDefault[] = [
          {
            ...
            description: 'Default setting for whether brand agents can view all organizational events or only their assigned ones',
            category: 'Events',
          ...
          },
          {
            ...
            description: 'Default setting for brand agent availability management permissions',
            category: 'Events',
          ...
          },
          {
            ...
            description: 'Default setting for field coordinator approval permissions',
            category: 'Events',
            value: data.defaults.field_coordinators_approve_assignments !== false
          },
          {
            id: 'client_users_create_events',
            name: 'Client Users Can Create Events',
            description: 'Default setting for client user event creation permissions',
            category: 'Events',
          ...
          },
          {
            ...
            description: 'Automatically assign available brand agents to new events',
            category: 'Events',
          ...
          },
          {
            ...
            description: 'Send notifications when users approach overtime hours',
            category: 'System',
  ...
  const updateSystemDefault = (settingId: string, value: boolean) => {
    setSystemDefaults(prev => prev.map(setting => 
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'Events': return <Shield className="h-4 w-4" />;
  ...
  const settingsByCategory = systemDefaults.reduce((acc, setting) => {
  ...
  return (
    <div className="container mx-auto p-6 max-w-6xl">
      ...
      <p className="text-muted-foreground mb-8">
        Configure system-wide RBAC defaults that apply to all organizations. Organizations can override these settings individually.
      </p>
          ...
          {Object.entries(settingsByCategory).map(([category, categorySettings]) => (
            <Card key={category}>
              <CardHeader>
                ...
                <CardDescription>
                  System-wide defaults for {category.toLowerCase()}-related permissions
                </CardDescription>
              </CardHeader>
                  ...
                  <div key={setting.id}>
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(checked) => updateSystemDefault(setting.id, checked)}
                      />
                    </div>
                ...
                <div className="space-y-4">
                  {organizationOverrides.map(org => (
                    <div key={org.organizationId} className="border rounded-lg p-4">
                      <h3 className="font-medium mb-2">{org.organizationName}</h3>
                      <div className="text-sm text-muted-foreground">
                        <strong>Overrides:</strong>
                        <ul className="mt-1 ml-4 list-disc">
                          {Object.entries(org.overrides).map(([key, value]) => (
                            <li key={key}>
                              {systemDefaults.find(s => s.id === key)?.name}: {value ? 'Enabled' : 'Disabled'}
                            </li>
                          ))}
                        </ul>
                      </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                These defaults apply to all new organizations. Existing organizations retain their current settings unless manually updated. 
                Organizations can override these defaults through their individual RBAC settings.
              </p>
            </div>
          </div>
        </CardContent>
