...
export default function AdminLocationsPage() {
  ...
  const filteredLocations = locationsData?.filter(location => {
    ...
    const stateMatch = !advancedFilters.states.length || advancedFilters.states.includes(location.state);
    const cityMatch = !advancedFilters.cities.length || advancedFilters.cities.includes(location.city);
    const zipMatch = !advancedFilters.zipCodes.length || (location.zipCode && advancedFilters.zipCodes.includes(location.zipCode));
    const statusMatch = !advancedFilters.status.length || advancedFilters.status.includes(location.status);
    const typeMatch = !advancedFilters.locationType.length || 
      (location.locationType && advancedFilters.locationType.includes(location.locationType));
    
    return searchMatch && stateMatch && cityMatch && zipMatch && statusMatch && typeMatch;
  }) || [];

  // Calculate count statistics
  const pendingCount = filteredLocations.filter(loc => loc.status === 'pending').length;
  const approvedCount = filteredLocations.filter(loc => loc.status === 'approved').length;
  const rejectedCount = filteredLocations.filter(loc => loc.status === 'rejected').length;
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to approve locations',
        variant: 'destructive',
      });
    }
  };
      ...
      <div className="container mx-auto py-8 px-4 space-y-6">
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          ...
          <div className="flex gap-2 ml-auto">
            <Badge variant="outline" className="rounded-md bg-amber-900/10 border-amber-500/20 text-amber-400">
              <Clock className="mr-1 h-3 w-3" />
              Pending: {pendingCount}
            </Badge>
            
            <Badge variant="outline" className="rounded-md bg-green-900/10 border-green-500/20 text-green-400">
              <ChartBar className="mr-1 h-3 w-3" />
              Approved: {approvedCount}
            </Badge>
            
            <Badge variant="outline" className="rounded-md bg-blue-900/10 border-blue-500/20 text-blue-400">
        ...
        {selectedLocations.size > 0 && (
          <div className="bg-blue-900/30 border border-blue-500/30 rounded-md p-3 flex justify-between items-center">
            <div className="text-blue-200">
              <span className="font-semibold">{selectedLocations.size}</span> locations selected
            </div>
              ...
              <CardContent className="p-0">
                {locationsLoading ? (
                  ...
                  <div className="p-8 text-center text-red-400">
                    <p>Error loading locations: {locationsError instanceof Error ? locationsError.message : 'Unknown error'}</p>
                    <Button 
                      variant="outline" 
                      onClick={() => refetch()} 
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="bg-muted/50">
                          ...
                          <th className="py-3 px-4 text-left font-medium text-sm">Added</th>
                        ...
                        {filteredLocations.length === 0 ? (
                          ...
                          filteredLocations.map((location) => (
                            ...
                            >
                              ...
                              <td className="py-3 px-4">
                                <div className="text-sm">{location.city}, {location.state}</div>
                                <div className="text-xs text-muted-foreground">{location.zipCode}</div>
                              </td>
                              <td className="py-3 px-4">
                                {location.locationType && (
                                  <span className={`px-2 py-0.5 rounded-full text-xs ${getTypeColor(location.locationType)}`}>
                                    {location.locationType}
                                  </span>
                                )}
                              </td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <Card className="h-[75vh] md:h-[650px] overflow-hidden">
                <CardHeader className="p-2 pb-1 border-b">
                  <CardTitle className="text-sm font-semibold">Locations ({filteredLocations.length})</CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                        ...
                        {filteredLocations.map((location) => (
                          <div 
                            ...
                            className={`py-2 px-3 cursor-pointer hover:bg-muted/50 transition-colors text-xs ${
                              selectedLocationId === location.id ? "bg-primary/10 border-l-2 border-l-primary" : ""
                            }`}
                            onClick={() => setSelectedLocationId(location.id)}
                          >
                            <div className="font-semibold truncate">{location.name}</div>
                            <div className="text-muted-foreground truncate text-xs">{location.address}</div>
                            <div className="flex items-center gap-1 mt-1">
                              <span className="text-[10px] text-muted-foreground">
                                {location.city}, {location.state}
                              </span>
                              {location.status && (
                                <span className={`inline-block ml-auto px-1.5 py-0.5 text-[10px] rounded-sm 
                                ...
                                `}>
                                  {location.status === 'active' ? 'approved' : location.status}
                                </span>
                              )}
                            </div>
                          </div>
        ...
        <div className="text-xs text-gray-500 mt-4 text-center">
          <p>Locations are global entities that can be shared across organizations.</p>
        </div>
      </div>
    </div>
  );
