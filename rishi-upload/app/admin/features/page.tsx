...
import { LucideIcon, Server, BarChart2, Layers, AlertTriangle, RefreshCw } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchFeatures = async () => {
      try {
        ...
        const response = await fetch(`/api/features/list?organizationId=${currentOrganization.id}`);
        ...
        toast({
          ...
          description: 'There was a problem loading the feature list. Please try again.',
        });
      } finally {
  ...
  const initializeFeatures = async () => {
    try {
      ...
      if (currentOrganization) {
        const featuresResponse = await fetch(`/api/features/list?organizationId=${currentOrganization.id}`);
      ...
      toast({
        ...
        description: 'There was a problem initializing the feature system. Please try again.',
      });
    }
  };
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500">
            ...
            {currentOrganization.tier && ` (${currentOrganization.tier.replace('_', ' ')})`}
          </p>
        </div>
        ...
        <AlertDialog open={showInitDialog} onOpenChange={setShowInitDialog}>
          ...
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={initializeFeatures}>Continue</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
      ...
      {features.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <p className="text-gray-500 mt-2">
            Either no features are registered or the feature list couldn't be loaded.
          </p>
          <Button variant="outline" className="mt-4" onClick={() => setShowInitDialog(true)}>
            Initialize Features
          </Button>
        </div>
      ) : (
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div>
                      <Switch
                        ...
                        disabled={!feature.userConfigurable || saving[feature.id]}
                      />
                    </div>
                  </div>
                    ...
                    <div className="mt-4">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {feature.dependencies.map(dep => (
                          <span key={dep} className="text-xs bg-gray-100 rounded-full px-2 py-0.5">
                            {dep}
                          </span>
                        ))}
                      </div>
