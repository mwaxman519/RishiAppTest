...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      const statesArray = Array.from(states).map(state => ({ id: state, name: state }));
      const citiesArray = Array.from(cities).map(city => ({ id: city, name: city }));
      const zipCodesArray = Array.from(zipCodes).map(zipCode => ({ id: zipCode, name: zipCode }));
      const locationTypesArray = Array.from(locationTypes).map(type => ({ id: type, name: type }));
      const statusesArray = Array.from(statuses).map(status => ({ id: status, name: status }));
      ...
      const pendingLocations = locations.filter(loc => loc.status === 'pending').length;
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to fetch location metadata" },
      { status: 500 }
    );
  }
}