...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      const locationsWithStates = await db.select({
      ...
      const activeLocations = locationsWithStates
        .filter(item => 
          ...
          (item.location.status !== 'pending' && item.location.status !== 'rejected')
        )
        .map(item => ({
          id: item.location.id,
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      const [location] = await db.insert(schema.locations).values(locationData).returning();
      
      // Publish location creation event
      const eventBus = new EventBusService();
      await eventBus.publish('location.created', {
