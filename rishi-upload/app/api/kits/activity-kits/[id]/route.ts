...
export async function GET(
  ...
  try {
    ...
    if (!activityKit) {
      return NextResponse.json({ error: "Activity kit assignment not found" }, { status: 404 });
    }

    return NextResponse.json(activityKit);
  } catch (error) {
    console.error(`Error fetching activity kit assignment with ID ${params.id}:`, error);
...
export async function PATCH(
  ...
  try {
    ...
    console.error(`Error updating activity kit assignment with ID ${params.id}:`, error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "An unknown error occurred" },
      { status: 400 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error(`Error deleting activity kit assignment with ID ${params.id}:`, error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "An unknown error occurred" },
      { status: 400 }
    );
