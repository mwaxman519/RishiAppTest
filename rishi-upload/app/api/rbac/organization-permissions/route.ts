...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('User from getCurrentUser:', user ? 'User found' : 'No user found');
    if (!user) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      const filteredResults = permissions.length > 0
        ? Object.fromEntries(
    ...
    try {
      ...
      const orgPermissions = await db.select()
        .from(organizationPermissions)
        .where(eq(organizationPermissions.organizationId, parseInt(organizationId)));
      ...
      if (permissions.length > 0) {
        for (const permission of permissions) {
          ...
          const orgPermission = orgPermissions.find(p => p.permissionName === permission);
        ...
        for (const orgPermission of orgPermissions) {
          permissionResults[orgPermission.permissionName] = orgPermission.allowed;
        }
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log('DEVELOPMENT MODE: Using mock response for organization permissions update');
      
      // Create mock results based on the request
      const mockResults = Object.entries(permissions).map(([permissionName, allowed], index) => ({
        id: index + 1,
    ...
    try {
      ...
      if (!hasAdminAccess) {
        return NextResponse.json({ 
          error: 'You do not have permission to modify organization permissions' 
        }, { status: 403 });
      }
      ...
      console.error('Database error in organization permissions update:', dbError);
