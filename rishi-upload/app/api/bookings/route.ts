...
const CreateCannabisBookingSchema = z.object({
  ...
  cannabisBookingType: z.enum(['product_demo', 'promotional_event', 'party_staffing', 'staff_training']),
  ...
  targetAudience: z.enum(['medical_patients', 'recreational_consumers', 'industry_professionals', 'general_public']),
  ...
  minimumExperience: z.enum(['entry_level', 'intermediate', 'experienced', 'expert']),
...
const mockBookings: Booking[] = [
  {
    ...
    description: "Quarterly training session for the marketing team focusing on new product features.",
    ...
    notes: "Lunch will be provided. Require training materials for all attendees.",
  ...
  },
  {
    ...
    notes: "Multiple breakout sessions and main presentation. VIP guests expected.",
    promotionTypeId: null,
    kitTemplateId: null,
    staffCount: 15,
    requiresTraining: true,
  ...
  },
  {
    ...
    description: "Exclusive dinner with top clients to discuss partnership opportunities.",
    clientOrganizationId: "org-1",
    locationId: "loc-1",
    activityTypeId: "act-4",
  ...
  },
  {
    ...
    description: "Recurring monthly training for field staff on new procedures.",
    clientOrganizationId: "org-3",
    locationId: "loc-3",
    activityTypeId: "act-1",
...
export async function GET(request: NextRequest) {
  ...
  if (organizationId) {
    ...
    filteredBookings = filteredBookings.filter(b => orgIds.includes(b.clientOrganizationId));
  }
  
  if (startDate) {
    const filterStartDate = new Date(startDate);
    filteredBookings = filteredBookings.filter(b => new Date(b.startDate) >= filterStartDate);
  }
  
  if (endDate) {
    const filterEndDate = new Date(endDate);
    filteredBookings = filteredBookings.filter(b => new Date(b.endDate) <= filterEndDate);
  }
