...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const token = authHeader?.split(' ')[1] || request.cookies.get('auth_token')?.value;
  ...
  if (userData.role !== USER_ROLES.INTERNAL_FIELD_MANAGER && userData.role !== USER_ROLES.INTERNAL_ADMIN) {
    return NextResponse.json({
      success: false,
  ...
  try {
    ...
    const endDate = endDateParam ? new Date(endDateParam) : new Date(Date.now() + 14 * 24 * 60 * 60 * 1000); // Default to 2 weeks from now
