...
export async function POST(request: NextRequest) {
  try {
    ...
    const organizationId = (session.user as any).organizationId || '00000000-0000-0000-0000-000000000001';
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        agentId: validatedData.agentId,
        assignedBy: userId,
        eventsPublished: ['staff.assigned', 'cannabis.shift_assignment_created'],
  ...
  try {
    ...
    if (!result.success) {
      const status = result.code === 'NOT_FOUND' ? 404 : 
                    result.code === 'UNASSIGNMENT_PERMISSION_DENIED' ? 403 : 400;
      return NextResponse.json(
