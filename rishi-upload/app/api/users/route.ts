...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    try {
      ...
      console.log('Mapped to service model:', { ...createUserRequest, password: '[REDACTED]' });
      
      // Call the service to create the user
      console.log('Calling userService.createUser');
      const result = await userService.createUser(createUserRequest);
      console.log('Service result:', { success: result.success, error: result.error, data: result.data ? 'User data returned' : null });
      
      if (!result.success) {
        const status = result.error === 'Username already exists' ? 409 : 500;
        console.log(`Returning error response with status ${status}:`, result.error);
        return NextResponse.json(
          { error: result.error },
          { status }
        );
