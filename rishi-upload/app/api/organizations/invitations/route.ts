...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('Database error fetching organization invitations:', dbError);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`DEVELOPMENT MODE: Would create invitation for ${email} to organization ${organizationId} with role ${role}`);
      ...
      console.log(`Would send email to ${email} with token ${token} (this would be a URL in production)`);
    ...
    try {
      ...
      const inviterId = typeof user.id === 'string' ? parseInt(user.id) : user.id;
      
      const invitation = await db.insert(organizationInvitations)
        .values({
          email,
          role,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const invitedById = typeof invitation.invitedById === 'string' ? parseInt(invitation.invitedById) : invitation.invitedById;
    ...
    if (user.role !== 'super_admin' && invitedById !== userId) {
      ...
      if (!userOrg && !hasPermission('manage:all', user.role)) {
        return NextResponse.json(
          { error: 'You do not have permission to cancel invitations for this organization' },
          { status: 403 }
        );
      }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (existingUserOrg) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.log(`DEVELOPMENT MODE: User is already a member of this organization`);
      } else {
        // In production, update invitation status
        await db.update(organizationInvitations)
          .set({ status: 'accepted' })
      ...
      return NextResponse.json(
        { error: 'You are already a member of this organization', alreadyMember: true },
        { status: 400 }
      );
    }
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`DEVELOPMENT MODE: Would add user ${user.id} to organization ${invitation.organizationId} with role ${invitation.role}`);
      console.log(`DEVELOPMENT MODE: Would mark invitation ${invitation.id} as accepted`);
    ...
    try {
      ...
      const userOrg = await db.insert(userOrganizations)
        .values({
          userId: user.id,
