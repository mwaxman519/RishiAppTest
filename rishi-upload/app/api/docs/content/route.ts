...
export async function GET(request: Request) {
  try {
    ...
    console.log(`[DOCS CONTENT API] Received request for document path: ${docPath}`);
    console.log(`[DOCS CONTENT API] Environment: ${process.env.NODE_ENV || 'unknown'}`);
    console.log(`[DOCS CONTENT API] Current working directory: ${process.cwd()}`);
    ...
    console.log(`[DOCS CONTENT API] Found ${docsDirectories.length} documentation directories`);
    
    // Get all doc files directly for full diagnostic info
    const allDocFiles = await listAllDocumentFiles(docsDirectories);
    console.log(`[DOCS CONTENT API] Found ${allDocFiles.length} total doc files across all directories`);
    
    if (allDocFiles.length > 0) {
      console.log(`[DOCS CONTENT API] First few doc files: ${allDocFiles.slice(0, 5).join(', ')}`);
    }
    ...
    if (!document) {
      console.error(`[DOCS CONTENT API] Document not found for path: ${normalizedPath}`);
      ...
      return NextResponse.json({
        ...
        availableFiles: allDocFiles.length > 20 ? allDocFiles.slice(0, 20) : allDocFiles,
    ...
    console.log(`[DOCS CONTENT API] Successfully retrieved document for path: ${normalizedPath}`);
    console.log(`[DOCS CONTENT API] Document title: ${document.metadata.title}`);
    console.log(`[DOCS CONTENT API] Document content length: ${document.content.length} characters`);
...
async function listAllDocumentFiles(directories: string[]): Promise<string[]> {
  ...
  for (const dir of directories) {
    try {
      ...
      console.error(`[DOCS CONTENT API] Error traversing directory ${dir}:`, error);
    }
  }
  ...
  async function traverseDirectory(dir: string, relativePath: string) {
    try {
      ...
      console.error(`[DOCS CONTENT API] Error reading directory ${dir}:`, error);
    }
  }
    ...
    try {
      ...
      if (exists) {
        ...
        if (stats.isDirectory()) {
          ...
          console.log(`[DOCS CONTENT API] Found directory ${dir} with ${files.length} files`);
          foundDirs.push(dir);
        }
      }
    } catch (e) {
 ...
 */
async function getDocumentContent(docPath: string): Promise<{content: string, metadata: DocMetadata} | null> {
  ...
  console.log(`[DOCS CONTENT] Looking for document: ${normalizedPath} (original: ${docPath})`);
  
  // Get the main docs directory from the utility function
  const mainDocsDirectory = await getDocsDirectory();
  console.log(`[DOCS CONTENT] Main docs directory from utility: ${mainDocsDirectory}`);
  ...
  console.log(`[DOCS CONTENT] Will search in ${uniqueRootDirs.length} unique root directories`);
  ...
  const hasExtension = normalizedPath.endsWith('.md') || normalizedPath.endsWith('.mdx');
          ...
          for (const readmePath of readmeVariations) {
            try {
              ...
              if (readmeStat.isFile()) {
                console.log(`[DOCS CONTENT] ✅ Found directory index file: ${readmePath}`);
  ...
  if (!foundFile || !fullFilePath) {
    console.log(`[DOCS CONTENT] ❌ Document not found after checking all possible locations: ${normalizedPath}`);
    console.log(`[DOCS CONTENT] Checked these root directories:`, uniqueRootDirs);
    return null;
  }
  ...
  try {
    ...
    console.log(`[DOCS CONTENT] Successfully read file, size: ${content.length} bytes`);
    
    // Extract title from content
    const title = getDocTitle(content) || path.basename(filePath, path.extname(filePath));
    ...
    if (frontmatterMatch && frontmatterMatch[1]) {
      ...
      if (tagsArrayMatch && tagsArrayMatch[1]) {
        tags = tagsArrayMatch[1].split(',').map(tag => tag.trim().replace(/["']/g, ''));
    ...
    throw new Error(`Failed to read document ${docPath} (resolved to ${fullFilePath}): ${error instanceof Error ? error.message : String(error)}`);
  }
}
