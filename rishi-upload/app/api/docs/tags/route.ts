 ...
 */
function getDocMetadata(content: string): { title: string | null, tags: string[] | undefined } {
  // Look for # Title
...
async function getAllDocs(): Promise<DocInfo[]> {
  ...
  async function traverseDir(dirPath: string, relativePath: string = '') {
    ...
    for (const entry of entries) {
      ...
      const docRelativePath = relativePath ? `${relativePath}/${entry.name}` : entry.name;
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          docs.push({
            ...
            title: typeof title === 'string' ? title : entry.name.replace(/\.(md|mdx)$/, '').replace(/-/g, ' '),
  ...
  return docs.sort((a, b) => b.lastModified.getTime() - a.lastModified.getTime());
}