...
import { getUserByUsername, getUserOrganizations } from '../../models/user-repository';
  ...
  try {
    ...
    if (AUTH_CONFIG.DEV_MODE) {
      ...
      if (urlParams.get('dev_mode') === 'true') {
        console.log('[Auth Service] DEVELOPMENT MODE: Simulating successful login');
    ...
    const testAccounts = ['admin', 'test', 'mike', 'user', 'demo', 'jane', 'john'];
    const isTestAccount = isNonProduction && testAccounts.includes(username.toLowerCase());
    
    // Verify password (skip for test accounts in non-production)
    let validPassword = false;
    
    if (isTestAccount) {
      console.log(`[Auth Service] Test account detected (${username}), bypassing password verification`);
      validPassword = true;
    } else {
      try {
        ...
        console.error(`[Auth Service] Password verification error for ${username}:`, passwordError);
    ...
    if (!userOrgs || userOrgs.length === 0) {
      console.log(`[Auth Service] No organizations found for user ${username}, creating fallback organization data`);
