...
import { errorResponse, responseWithAuthCookie, successResponse } from '../../utils/response';
import { 
  createUser, 
  getUserByEmail, 
  getUserByUsername,
...
const registerSchema = z.object({
  ...
  password: z.string().min(AUTH_CONFIG.MIN_PASSWORD_LENGTH, 
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      try {
        ...
        if (requestBody.username) {
          console.log(`[Auth Service] DEVELOPMENT MODE: Using mock registration for test account: ${requestBody.username}`);
          
          // Verify registration passcode
          if (requestBody.registrationPasscode !== AUTH_CONFIG.REGISTRATION_PASSCODE) {
            console.log(`[Auth Service] Invalid registration passcode: ${requestBody.registrationPasscode}`);
        ...
        console.log('[Auth Service] Unable to use mock registration flow:', parseError);
    ...
    if (process.env.NODE_ENV !== 'development') {
      try {
        ...
        console.log(`[Auth Service] Running in ${environment} environment (isReplit=${isReplit})`);
        
        // For Replit staging environment, we need additional validation
        if (isReplit && environment === 'staging') {
          console.log('[Auth Service] Replit staging environment detected, testing database connection');
          
          // Run connection test to verify database is accessible
          const connectionStatus = await testConnection();
          
          if (!connectionStatus.connected) {
            console.error('[Auth Service] Database connection test failed in Replit staging environment:', 
          ...
          console.log('[Auth Service] Database connection verified in Replit staging environment');
        } else {
          console.log(`[Auth Service] Database connection verified in ${environment} environment`);
        }
      } catch (dbError) {
    ...
    if (!AUTH_CONFIG.ALLOW_SELF_REGISTRATION) {
      ...
      if (!internalAuthToken || internalAuthToken !== AUTH_CONFIG.INTERNAL_AUTH_TOKEN) {
        return errorResponse(
    ...
    if (AUTH_CONFIG.DEV_MODE) {
      ...
      if (urlParams.get('dev_mode') === 'true') {
        console.log('[Auth Service] DEVELOPMENT MODE: Simulating successful registration');
    ...
    if (createOrganization && !AUTH_CONFIG.ALLOW_ORG_CREATION && !isInternalManagementRequest) {
      return errorResponse(
    ...
    if (role === 'super_admin') {
      console.log(`[Auth Service] Setting up organization for super_admin user ${username}`);
    ...
    // Handle normal organization assignment for non-super_admin users
    else if (isInternalManagementRequest && createOrganization && organizationName) {
      // Create new organization and assign user (only for internal management users)
      console.log(`[Auth Service] Creating new organization "${organizationName}" for user ${username}`);
      setupOrgResult = await setupUserOrganization(userId, {
        createNewOrg: true,
      ...
      console.log(`[Auth Service] Assigning user ${username} to organization ${organizationId}`);
      setupOrgResult = await setupUserOrganization(userId, {
      ...
      console.log(`[Auth Service] Assigning user ${username} to default organization`);
      setupOrgResult = await setupUserOrganization(userId, {
        useDefaultOrg: true,
        role
      });
    ...
    if (!setupOrgResult.userOrg) {
      const errorMessage = setupOrgResult.error || 'Unknown error during organization assignment';
      console.warn(`[Auth Service] Failed to assign user ${username} to an organization: ${errorMessage}`);
    ...
    if (error instanceof Error) {
      ...
      if (errMsg.includes('connection') || errMsg.includes('connect') || 
          errMsg.includes('timeout') || errMsg.includes('unable to connect')) {
      ...
      // Authentication error with database - specific handling for Replit staging
        ...
        if (isReplit && env === 'production') {
          errorMessage = 'Database authentication error in Replit environment. This might be a configuration issue with the staging database.';
      ...
      // Replit neondb_owner errors
        ...
        errorMessage = 'Replit database configuration error. Please verify database environment variables are correctly set.';
      ...
      // Constraint violation 
        ...
        if (errMsg.includes('username')) {
          ...
          errorMessage = 'Registration failed due to a conflict. Please try different information.';
