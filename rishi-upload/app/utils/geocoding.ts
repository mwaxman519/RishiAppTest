 ...
 */
export async function enhanceLocationWithGeocoding<T extends Partial<LocationDTO>>(location: T): Promise<T> {
  try {
    ...
    if (result.address_components) {
      ...
      const streetNumber = findAddressComponent(result.address_components, 'street_number')?.long_name;
      const route = findAddressComponent(result.address_components, 'route')?.long_name;
      ...
      // City
      const locality = findAddressComponent(result.address_components, 'locality');
      if (locality) {
        updated.city = locality.long_name;
      }
      
      // State
      const administrativeArea = findAddressComponent(result.address_components, 'administrative_area_level_1');
      ...
      // Postal code
      const postalCode = findAddressComponent(result.address_components, 'postal_code');
      if (postalCode) {
 ...
 */
function findAddressComponent(components: AddressComponent[], type: string): AddressComponent | undefined {
