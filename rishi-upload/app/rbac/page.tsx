...
export default function RBACVisualizationPage() {
  const [activeTab, setActiveTab] = useState<'routes' | 'permissions'>('routes');
  const [selectedRole, setSelectedRole] = useState<string>(USER_ROLES.SUPER_ADMIN);
            ...
            >
              {allRoles.map((role: string) => (
                <option key={role} value={role}>
                  {role.split('_').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                </option>
              ))}
            </select>
          </div>
          ...
          {activeTab === 'routes' ? (
            <div>
              <h2 className="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-200">
                Routes Access for {selectedRole.split('_').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
              </h2>
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {allRoutes.map((route: string) => {
                      ...
                      const hasAccess = requiredPermissions.length === 0 || requiredPermissions.some((perm: string) => userPermissions.includes(perm));
                      ...
                      return (
                        <tr key={route}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                            {Array.isArray(requiredPermissions) ? requiredPermissions.join(', ') : 'None'}
                          </td>
            ...
            <div>
              <h2 className="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-200">
                Permissions for {selectedRole.split('_').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
              </h2>
              ...
              <thead className="bg-gray-50 dark:bg-gray-900">
                <tr>
                  ...
                  {allPermissions.filter(p => 
                    ...
                    <th key={permission} className="px-3 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                      {permission}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {allRoles.map((role: string) => (
                  <tr key={role}>
                    ...
                    {allPermissions.filter((p: string) => 
                      p.startsWith('read:') || 
                      p.startsWith('create:') || 
                      p.startsWith('update:') || 
                      p.startsWith('delete:') ||
