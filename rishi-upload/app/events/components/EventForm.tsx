...
const formSchema = z.object({
  ...
  numberOfOccurrences: z.string().transform(val => parseInt(val) || 0).optional(),
});
    ...
    onError: (error: any) => {
      if (error.message === 'location_conflict' && error.cause) {
        ...
        toast({
          ...
          description: 'The selected location is not available for the chosen time.',
          variant: 'destructive'
        });
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select 
                    onValueChange={field.onChange} 
                    defaultValue={field.value}
                    disabled={isLoadingLocations || !form.watch('organizationId')}
                  >
                    <FormControl>
                      <SelectTrigger>
            ...
            {locationConflict && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  ...
                  {locationConflict.conflicts && locationConflict.conflicts.length > 0 && (
                    <div className="mt-2">
                      <p className="font-semibold">Conflicting Events:</p>
                      <ul className="list-disc pl-5 mt-1">
                        {locationConflict.conflicts.map((conflict: any, index: number) => (
                          <li key={index}>
                            {conflict.title} ({format(new Date(conflict.startDateTime), 'PPp')} - {format(new Date(conflict.endDateTime), 'p')})
                          </li>
                        ))}
                      </ul>
                    </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Any additional notes about the event" {...field} />
                  </FormControl>
                  <FormDescription>
