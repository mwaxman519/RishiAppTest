...
export interface EventBusAdapter {
  ...
  subscribe(eventName: string, callback: (data: any) => void | Promise<void>): () => void;
...
export class InMemoryEventBusAdapter implements EventBusAdapter {
  private events: Map<string, Array<(data: any) => void | Promise<void>>> = new Map();
...
export class AzureServiceBusAdapter implements EventBusAdapter {
  ...
  private localSubscribers: Map<string, Array<(data: any) => void | Promise<void>>> = new Map();

  subscribe(eventName: string, callback: (data: any) => void | Promise<void>): () => void {
    // In production, this would register a handler with Azure Service Bus
    console.log(`[AZURE] Subscribing to event ${eventName} from Azure Service Bus`);
...
export function createEventBusAdapter(): EventBusAdapter {
  ...
  switch (config.eventBusType) {
    ...
    case 'aws':
      console.log('AWS event bus adapter not yet implemented, falling back to in-memory');
