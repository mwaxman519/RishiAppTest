...
export function hasEnhancedPermission(
  ...
  for (const rolePerm of rolePermissions) {
    ...
    if (permissionCovers(rolePerm, permission)) {
      ...
      if (roleFeature.scope === PERMISSION_SCOPES.ORGANIZATION && !context.organizationId) {
      ...
      if (roleFeature.scope === PERMISSION_SCOPES.REGION && 
      ...
      if (roleFeature.scope === PERMISSION_SCOPES.OWNED && !context.resourceOwnerId) {
        continue;
      }
...
export function hasRoutePermission(
  ...
  for (const [routePath, permission] of Object.entries(routePermissions)) {
    ...
    for (let i = 0; i < routeSegments.length; i++) {
      ...
      if (segment && typeof segment === 'string' && segment.startsWith('[') && segment.endsWith(']')) {
        continue;
      }
