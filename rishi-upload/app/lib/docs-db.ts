...
if (!process.env.DATABASE_URL) {
  console.warn('DATABASE_URL is not set. Documentation database caching will be disabled.');
}
 ...
 */
export async function cacheDocumentInDb(path: string, content: string, metadata: any) {
...
export async function getCachedDocumentFromDb(path: string): Promise<{
  ...
  try {
    ...
    if (isStale) {
      console.log(`[DOCS DB] Cache stale for ${path}, last updated ${updated_at}`);
