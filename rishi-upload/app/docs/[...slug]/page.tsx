...
export default async function DocPage({ params }: DocPageProps) {
  ...
  if (docPath === 'business/user-requirements' || docPath === 'business/user-requirements.md') {
  ...
  try {
    ...
    if (DOC_PATH_REDIRECTS[normalizedPath]) {
      ...
      console.log(`[DOCS PAGE] Redirecting from ${normalizedPath} to ${redirectTarget}`);
      // Return redirect instead of using it directly
    ...
    if (!docTree || Object.keys(docTree).length === 0) {
      throw new Error('Documentation tree is empty or invalid - no documentation files were found');
    }
    
    console.log(`[DOCS PAGE] Successfully loaded document tree with ${Object.keys(docTree).length} root entries`);
    
    // Add debugging info for the path
    console.log(`[DOCS PAGE DEBUG] Requesting document with path: '${docPath}'`);
    ...
    if (!doc) {
      ...
      try {
        ...
        if (possibleMatches.length > 0) {
          console.error(`[DOCS PAGE] Similar documents found: ${possibleMatches.join(', ')}`);
        }
      } catch (matchError) {
        console.error(`[DOCS PAGE] Error checking similar documents: ${matchError}`);
      }
    ...
    console.error(`[DOCS PAGE] Error details:`, error instanceof Error ? error.message : String(error));
...
export async function generateStaticParams() {
  try {
    ...
    console.log('[DOCS generateStaticParams] Starting to traverse document tree');
    
    // Traverse the tree to build paths
    traverseTree(docTree);
    
    // Log the result
    console.log(`[DOCS generateStaticParams] Generated ${paths.length} static paths`);
    ...
    console.error('[DOCS generateStaticParams] Error generating paths during build:', error);
