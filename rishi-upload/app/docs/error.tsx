...
import { AlertTriangle, Info, RefreshCw, FolderTree, FileSearch } from 'lucide-react';
...
export default function DocsErrorComponent({
  ...
  const reinitializeDocumentation = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(`Documentation reinitialized: ${data.message || 'Success'}`, 'success');
        ...
        showToast(`Failed to reinitialize: ${response.status} ${response.statusText}`, 'error');
      }
    } catch (err) {
      showToast(`Error during reinitialization: ${err instanceof Error ? err.message : 'Unknown error'}`, 'error');
    } finally {
  ...
  const forceEmergencyDocs = async () => {
    try {
      ...
      if (response.ok) {
        showToast('Emergency documentation setup initiated. Reinitializing...', 'success');
        // Then reinitialize
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] p-4">
      <div className="w-full max-w-3xl bg-gray-50 dark:bg-gray-900 border border-red-200 dark:border-red-900 rounded-lg shadow-sm p-6">
        ...
        <div className="mb-4">
          <p className="mb-2">An error occurred while loading the documentation system:</p>
          ...
          {initializationSuccess && (
            <div className="bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-800 p-3 rounded-md mt-2">
              <p className="text-green-700 dark:text-green-400">
                Reinitialization successful! The page should refresh momentarily...
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col space-y-3">
          ...
          <Link href="/" className="flex items-center justify-center px-4 py-2 rounded-md font-medium
          ...
          {detailsVisible && (
            <div className="mt-4 bg-gray-100 dark:bg-gray-800 rounded p-3 overflow-auto max-h-64">
              ...
              {errorDigest && (
                <>
                  <h3 className="text-sm font-semibold mt-3 mb-2">Error Digest:</h3>
                  <code className="text-xs">{errorDigest}</code>
                </>
              )}
              
              <h3 className="text-sm font-semibold mt-3 mb-2">Runtime Information:</h3>
