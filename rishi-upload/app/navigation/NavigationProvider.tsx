    ...
    if (!initialized) {
      try {
        ...
        console.log('Navigation initialized successfully:', 
          'Super Admin Navigation Items:', superAdminNavigation.length, 
          'Field Manager Navigation Items:', fieldManagerNavigation.length
        );

        // Inspect some items for debugging
        if (superAdminNavigation && superAdminNavigation.length > 0) {
          console.log('First Super Admin item:', superAdminNavigation[0]?.label || 'No label');
        }
        if (fieldManagerNavigation && fieldManagerNavigation.length > 0) {
          console.log('First Field Manager item:', fieldManagerNavigation[0]?.label || 'No label');
        }
      } catch (error) {
