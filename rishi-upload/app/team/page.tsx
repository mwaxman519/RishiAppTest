...
import { Users, UserPlus, Search, Filter, MoreVertical, Mail, Phone, MapPin, Star, TrendingUp, Calendar, Award, Building, Target, Activity, Briefcase, Clock } from 'lucide-react';
...
const generateTeamMembers = () => {
  const firstNames = ['Sarah', 'Michael', 'Emily', 'David', 'Jessica', 'Ryan', 'Ashley', 'Kevin', 'Amanda', 'Brandon', 'Stephanie', 'Tyler', 'Rachel', 'Justin', 'Nicole', 'Matthew', 'Lauren', 'Daniel', 'Megan', 'Christopher', 'Hannah', 'Andrew', 'Brittany', 'Joshua', 'Samantha', 'Nicholas', 'Elizabeth', 'Anthony', 'Taylor', 'William'];
  const lastNames = ['Johnson', 'Chen', 'Rodriguez', 'Park', 'Smith', 'Williams', 'Brown', 'Davis', 'Miller', 'Wilson', 'Moore', 'Taylor', 'Anderson', 'Thomas', 'Jackson', 'White', 'Harris', 'Martin', 'Thompson', 'Garcia', 'Martinez', 'Robinson', 'Clark', 'Lewis', 'Lee', 'Walker', 'Hall', 'Allen', 'Young', 'King'];
  const cities = ['San Francisco, CA', 'Los Angeles, CA', 'Chicago, IL', 'Seattle, WA', 'New York, NY', 'Boston, MA', 'Austin, TX', 'Denver, CO', 'Portland, OR', 'Miami, FL'];
  const specialties = [
  ...
  for (let i = 0; i < 150; i++) {
    ...
    teamMembers.push({
      ...
      status: Math.random() > 0.8 ? (Math.random() > 0.5 ? 'inactive' : 'on_leave') : 'active',
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-gray-600 dark:text-gray-400">Total Members</p>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{teamMembers.filter(m => m.status === 'active').length}</p>
                </div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-gray-600 dark:text-gray-400">Total Events</p>
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{teamMembers.reduce((sum, m) => sum + m.eventsCompleted, 0)}</p>
                </div>
          ...
          <CardContent className="p-0">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Select>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="lead-brand-agent">Lead Brand Agent</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="divide-y divide-gray-200 dark:divide-gray-600">
              ...
              {teamMembers.slice(0, 20).map((member) => (
                <div key={member.id} className="grid grid-cols-12 gap-4 px-6 py-4 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  ...
                  <div className="col-span-1 flex items-center">
                    <Badge 
                      ...
                      className="text-xs"
                    >
                      {member.status === 'active' ? 'Active' : member.status === 'inactive' ? 'Inactive' : 'On Leave'}
                    </Badge>
                  </div>
                  ...
                  <div className="col-span-1 flex items-center">
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{member.eventsCompleted}</span>
                  </div>
