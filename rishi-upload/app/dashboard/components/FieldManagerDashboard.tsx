        ...
        <div className="relative bg-gradient-to-r from-indigo-600 via-purple-600 to-blue-600 rounded-3xl p-8 text-white overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-4 lg:space-y-0">
              <div>
                <h1 className="text-4xl font-bold mb-2">Field Manager Dashboard</h1>
                <p className="text-indigo-100 text-lg">Coordinate teams and optimize operational excellence</p>
              </div>
          ...
          <Card className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white border-0 shadow-xl">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-emerald-100 font-medium">Team Performance</p>
              ...
              <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <CardDescription>Monitor staffing and event readiness</CardDescription>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="space-y-4">
                  {upcomingEvents.map((event) => (
                    <div key={event.id} className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 bg-white border border-gray-100 rounded-xl p-4">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <Badge 
                          className={`${
                            event.status === 'fully_staffed' ? 'bg-green-100 text-green-800 border-green-200' :
                            event.status === 'understaffed' ? 'bg-red-100 text-red-800 border-red-200' :
                            'bg-yellow-100 text-yellow-800 border-yellow-200'
                          } px-3 py-1`}
                        >
                          {event.status === 'fully_staffed' ? 'FULLY STAFFED' : 
                           event.status === 'understaffed' ? 'UNDERSTAFFED' : 'PENDING'}
                        </Badge>
                      </div>
                      ...
                      <div className="bg-blue-50 rounded-lg p-3">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm font-medium text-blue-800">Staff Assignment</span>
                          </div>
                        ...
                        <div className="w-full bg-blue-200 rounded-full h-2">
                          <div 
                            className={`h-2 rounded-full transition-all duration-300 ${
                              event.status === 'fully_staffed' ? 'bg-gradient-to-r from-green-500 to-emerald-500' :
                              event.status === 'understaffed' ? 'bg-gradient-to-r from-red-500 to-orange-500' :
                              'bg-gradient-to-r from-yellow-500 to-orange-500'
                            }`}
                            style={{ width: `${(event.staffAssigned / event.staffRequired) * 100}%` }}
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    <div key={member.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                      ...
                      <Badge 
                        className={`${
                          ...
                          member.status === 'on_event' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <div key={request.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-600">From: {request.from}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm">
            ...
            <CardContent className="p-6 space-y-3">
              ...
              <Button variant="outline" className="w-full justify-start border-gray-200 hover:border-emerald-500 hover:text-emerald-600">
                <Users className="h-4 w-4 mr-2" />
                Assign Staff
              </Button>
              <Button variant="outline" className="w-full justify-start border-gray-200 hover:border-purple-500 hover:text-purple-600">
                <Calendar className="h-4 w-4 mr-2" />
                Schedule Review
              </Button>
              <Button variant="outline" className="w-full justify-start border-gray-200 hover:border-orange-500 hover:text-orange-600">
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-purple-600">{'< 2hrs'}</span>
                </div>
              </div>
            </CardContent>
          </Card>
