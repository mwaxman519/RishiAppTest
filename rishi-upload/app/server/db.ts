...
export const db = {
  ...
  execute: async (sql: any) => {
    ...
    if (sql.toString().toLowerCase().includes('select')) {
      ...
      return {
        rows: [
          { id: 'mock-location-1', name: 'Location 1', address: '123 Main St', city: 'Austin', state: 'TX' },
          { id: 'mock-location-2', name: 'Location 2', address: '456 Oak St', city: 'Dallas', state: 'TX' }
        ]
      };
      ...
      return {
        rows: [
          { 
            ...
            name: sql.values.find((v: any) => typeof v === 'string') || 'New Location',
            type: 'venue',
