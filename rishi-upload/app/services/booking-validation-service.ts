...
export const bookingSchema = z.object({
  ...
  startTime: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, "Start time must be in 24-hour format (HH:MM)"),
  endTime: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, "End time must be in 24-hour format (HH:MM)"),
  timezone: z.string().min(1, "Timezone is required"),
  locationId: z.string().uuid("Valid location ID is required"),
  activityType: z.string().min(1, "Activity type is required"),
  notes: z.string().optional(),
  isRecurring: z.boolean().default(false),
  attendeeCount: z.number().min(1, "At least one attendee is required").optional(),
});

// Extended schema for recurring bookings
export const recurringBookingSchema = bookingSchema.extend({
  recurringFrequency: z.nativeEnum(RecurrenceFrequency, {
    errorMap: () => ({ message: "Please select a valid frequency" }),
  }).optional(),
  recurringCount: z.number().min(2, "Must have at least 2 occurrences").max(52, "Maximum 52 occurrences allowed").optional(),
  recurringExceptions: z.array(z.date()).optional(),
});

// Validation for booking form values
export const bookingFormSchema = recurringBookingSchema.refine(
  (data) => {
  ...
  (data) => {
    ...
    if (data.isRecurring) {
      return data.recurringFrequency !== undefined && data.recurringCount !== undefined;
    }
    return true;
  },
  {
    message: "Recurring frequency and count are required for recurring bookings",
