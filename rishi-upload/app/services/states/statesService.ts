...
export class StatesService {
  ...
  async getActiveStates(): Promise<State[]> {
    try {
      if (process.env.NODE_ENV === 'development') {
        console.log("DEVELOPMENT MODE: Using mock active states data for testing");
  ...
  async getStateById(id: string): Promise<State | null> {
    try {
      if (process.env.NODE_ENV === 'development') {
        console.log("DEVELOPMENT MODE: Using mock state by ID data for testing");
  ...
  async getStateByCode(code: string): Promise<State | null> {
    try {
      if (process.env.NODE_ENV === 'development') {
        console.log("DEVELOPMENT MODE: Using mock state by code data for testing");
        
        // Return a specific state from mock data
        const allStates = await this.getAllStates();
        return allStates.find(state => state.code.toUpperCase() === code.toUpperCase()) || null;
      }
   ...
   * Update a state
   */
  async updateState(id: string, stateData: UpdateStateDto): Promise<State | null> {
    ...
    try {
      if (process.env.NODE_ENV === 'development') {
        ...
        regions.forEach(region => {
          region.states = allStates.filter(state => state.regionId === region.id);
        });
   ...
   * Update a region
   */
  async updateRegion(id: string, regionData: UpdateRegionDto): Promise<Region | null> {
  ...
  async initializeDefaultStatesData(): Promise<void> {
    try {
      ...
      // Create regions
      const regions = [
        { name: 'Northeast', code: 'NE', description: 'Northeastern United States' },
        { name: 'Southeast', code: 'SE', description: 'Southeastern United States' },
        { name: 'Midwest', code: 'MW', description: 'Midwestern United States' },
        { name: 'Southwest', code: 'SW', description: 'Southwestern United States' },
        { name: 'West', code: 'W', description: 'Western United States' },
        { name: 'Territories', code: 'TERR', description: 'United States Territories' }
      ];
