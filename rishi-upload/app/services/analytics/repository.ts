...
export class AnalyticsRepository {
    ...
    try {
      const dashboards = Array.from(this.dashboards.values())
        .filter(dashboard => {
          ...
          if (filters?.isPublic !== undefined && dashboard.isPublic !== filters.isPublic) return false;
          if (filters?.createdBy && dashboard.createdBy !== filters.createdBy) return false;
  ...
  async findDashboardById(id: string): Promise<AnalyticsServiceResponse<Dashboard>> {
    try {
  ...
  async createDashboard(
    data: CreateDashboardInput & { createdBy: string; createdAt: Date; updatedAt: Date }
  ): Promise<AnalyticsServiceResponse<Dashboard>> {
    try {
  ...
  async deleteDashboard(id: string): Promise<AnalyticsServiceResponse<boolean>> {
    try {
  ...
  async findReportTemplateById(id: string): Promise<AnalyticsServiceResponse<ReportTemplate>> {
    try {
  ...
  async getRealTimeMetrics(
    ...
    try {
      const metrics = Array.from(this.realTimeMetrics.values())
        .filter(metric => {
          return metric.organizationId === organizationId &&
  ...
  async exportData(
    ...
    config: ExportConfig,
    organizationId: string
  ): Promise<AnalyticsServiceResponse<{ downloadUrl: string; fileName: string; recordCount: number }>> {
    try {
