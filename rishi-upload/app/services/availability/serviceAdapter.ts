 ...
 */
export class LocalAvailabilityServiceAdapter implements ServiceAdapter<IAvailabilityService> {
  ...
  getService(): IAvailabilityService {
    throw new Error('Direct service access is only available server-side. Use API routes instead.');
  }
}
 ...
 */
export class RemoteAvailabilityServiceAdapter implements ServiceAdapter<IAvailabilityService> {
  constructor(private baseUrl: string, private apiClient: ApiClient) {}
    ...
    return {
      ...
      async createAvailabilityBlock(data: CreateAvailabilityRequest): Promise<AvailabilityResponse> {
        try {
          ...
          const response = await apiClient.post<AvailabilityResponse>(url, data);
      ...
      async deleteAvailabilityBlock(id: number, deleteSeries: boolean = false): Promise<ServiceResponse<{success: boolean, count?: number}>> {
        try {
          ...
          const response = await apiClient.delete<ServiceResponse<{success: boolean, count?: number}>>(url);
