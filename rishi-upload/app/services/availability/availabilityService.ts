...
class AvailabilityService {
  ...
  async getAvailabilityBlocks(options: AvailabilityQueryOptions): Promise<AvailabilitiesResponse> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to retrieve availability blocks'
      };
    }
  }
  ...
  async getAvailabilityBlockById(id: number): Promise<AvailabilityResponse> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to retrieve availability block'
      };
    }
  }
   ...
   */
  async createAvailabilityBlock(data: CreateAvailabilityRequest): Promise<AvailabilityResponse> {
    try {
      console.log('Creating availability block with data:', JSON.stringify(data, null, 2));
      ...
      if (data.isRecurring) {
        ...
        if (data.recurrenceEndType === 'count' && data.recurrenceCount && data.recurrenceCount > 0) {
          // Count-based recurrence: create a specific number of instances
          occurrencesToCreate = data.recurrenceCount;
          console.log(`Creating ${occurrencesToCreate} occurrences based on count`);
        } 
        else if (data.recurrenceEndType === 'date' && data.recurrenceEndDate) {
          ...
          const daysBetween = Math.ceil((endDateLimit.getTime() - firstDate.getTime()) / (1000 * 60 * 60 * 24));
          ...
          console.log(`Creating occurrences until date: ${endDateLimit.toISOString()} (estimated: ${occurrencesToCreate} occurrences)`);
        }
          ...
          console.log(`Creating ${occurrencesToCreate} occurrences for "never" end type (safety limit)`);
        }
        ...
        if (occurrencesToCreate > MAX_OCCURRENCES) {
          console.warn(`Limiting occurrences from ${occurrencesToCreate} to ${MAX_OCCURRENCES}`);
        ...
        for (let i = 0; i < occurrencesToCreate; i++) {
          try {
            ...
            const currentEndDate = new Date(currentDate.getTime() + timeDurationMs);
            ...
            if (endDateLimit && currentDate > endDateLimit) {
              console.log(`Stopping at occurrence ${i+1}/${occurrencesToCreate} - exceeds end date limit`);
              break;
            }
            
            console.log(`Creating occurrence ${i+1}/${occurrencesToCreate}: ${currentDate.toISOString()} to ${currentEndDate.toISOString()}`);
            ...
            const currentBlockData = {
              ...
              recurrence_end_date: data.recurrenceEndDate ? new Date(data.recurrenceEndDate) : null
            };
            
            // Create this occurrence
            const newBlock = await availabilityRepository.create(currentBlockData);
            if (newBlock) {
              console.log(`✅ Successfully created occurrence ${i+1} with ID ${newBlock.id} on ${dateKey}`);
        ...
        console.log(`Successfully created ${actualOccurrencesCreated} of ${occurrencesToCreate} planned occurrences`);
        
        // Return the first block created
        if (createdBlockIds.length > 0) {
          console.log(`Created ${createdBlockIds.length} blocks, returning the first one ID ${createdBlockIds[0]}`);
          try {
            const firstBlock = await availabilityRepository.findById(createdBlockIds[0]);
            if (firstBlock) {
        ...
        if (createdBlockIds.length > 0) {
          console.log(`Created ${createdBlockIds.length} blocks but couldn't fetch the first one. Returning partial success.`);
          return {
            success: true,
            data: {
      ...
      const validationResult = insertAvailabilityBlockSchema.safeParse(blockData);
      ...
      if (conflicts.length > 0 && conflicts.some(c => c.conflictType !== 'adjacent')) {
        // We have real conflicts (not just adjacent blocks)
        
        // First, check if we need to handle a merge or override case
        const blocksThatNeedAction = conflicts.filter(c => c.conflictType !== 'adjacent');
        ...
        if (blocksThatNeedAction.length > 0) {
          ...
          for (const conflict of blocksThatNeedAction) {
            ...
            if (existingBlock.status === data.status) {
              ...
              const updatedBlock = await availabilityRepository.findById(existingBlock.id);
              return {
                success: true,
                data: updatedBlock!
              };
              ...
              await availabilityRepository.update(existingBlock.id, {
                ...
                start_date: typeof existingBlock.startDate === 'string' ? new Date(existingBlock.startDate) : existingBlock.startDate,
                end_date: typeof existingBlock.endDate === 'string' ? new Date(existingBlock.endDate) : existingBlock.endDate
              });
      ...
      const newBlock = await availabilityRepository.create(validationResult.data);
   ...
   */
  async updateAvailabilityBlock(id: number, data: UpdateAvailabilityRequest): Promise<AvailabilityResponse> {
    try {
      ...
      const isPartOfRecurringGroup = existingBlock.recurrenceGroup && existingBlock.isRecurring;
      ...
      if (data.recurrenceEndType !== undefined) updateData.recurrence_end_type = data.recurrenceEndType;
      if (data.recurrenceCount !== undefined) updateData.recurrence_count = data.recurrenceCount;
          ...
          if (blocksThatNeedAction.length > 0) {
            ...
            for (const conflict of blocksThatNeedAction) {
              ...
              if (conflictBlock.status === currentStatus) {
                ...
                const mergedStart = newStart < conflictStart ? newStart : conflictStart;
                ...
                // Case 2: Current block is in the middle of conflict block - split into two blocks
                  ...
                  await availabilityRepository.create({
                    ...
                    day_of_week: typeof conflictBlock.dayOfWeek === 'number' ? conflictBlock.dayOfWeek : undefined
                  });
                }
                // Case 3: Current block overlaps the beginning of conflict block
                else if (newStart <= conflictStart && newEnd < conflictEnd && newEnd > conflictStart) {
                ...
                else if (newStart > conflictStart && newStart < conflictEnd && newEnd >= conflictEnd) {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to update availability block'
      };
    }
  }
   ...
   */
  async deleteAvailabilityBlock(id: number, deleteSeries: boolean = false): Promise<ServiceResponse<{success: boolean, count?: number}>> {
    try {
      console.log(`⏳ Starting deletion process for block ID ${id}, deleteSeries=${deleteSeries}`);
      ...
      const isRecurring = existingBlock.isRecurring === true && existingBlock.recurrenceGroup !== null && existingBlock.recurrenceGroup !== undefined;
      
      // Log all properties to help debug
      console.log(`Block ID ${id} properties: isRecurring=${existingBlock.isRecurring}, recurrenceGroup=${existingBlock.recurrenceGroup}`);
      console.log(`Final determination: isRecurring=${isRecurring}, deleteSeries=${deleteSeries}`);
      
      // CASE 1: Delete entire series
      if (deleteSeries && isRecurring) {
        console.log(`🗑️ Deleting entire recurrence series for block ID ${id} (group: ${existingBlock.recurrenceGroup})`);
        
        // Delete all blocks in the series
        const seriesResult = await availabilityRepository.deleteSeries(id);
        
        if (!seriesResult.success) {
          console.error(`❌ Failed to delete recurring series for block ID ${id}`);
          return {
            success: false,
        ...
        console.log(`✅ Successfully deleted ${seriesResult.count} blocks in recurrence series`);
      ...
      // CASE 2: Delete single instance from recurring series
        ...
        console.log(`🗑️ Deleting single occurrence from recurring series, block ID ${id}, group ${existingBlock.recurrenceGroup}`);
        
        // Delete the single block with recurrence handling
        const result = await availabilityRepository.delete(id);
        
        if (!result) {
          console.error(`❌ Failed to delete single occurrence from recurring series, block ID ${id}`);
          return {
            success: false,
        ...
        console.log(`✅ Successfully deleted single occurrence from recurring series, block ID ${id}`);
      ...
      // CASE 3: Delete regular non-recurring block 
        ...
        console.log(`🗑️ Deleting single non-recurring block ID ${id} (isRecurring=${existingBlock.isRecurring})`);
  ...
  async checkForConflicts(
    ...
    try {
      ...
      const significantConflicts = conflicts.filter(c => c.conflictType !== 'adjacent');
