...
export function initializeWebSocketService(httpServer: Server): void {
  ...
  try {
    ...
    if (!eventBus.hasSubscriber(webSocketEventSubscriber)) {
      ...
      console.log('[WebSocketService] WebSocket event subscriber registered with event bus');
    }
    
    console.log('[WebSocketService] WebSocket service successfully initialized');
  } catch (error) {
    console.error('[WebSocketService] Failed to initialize WebSocket service:', error);
