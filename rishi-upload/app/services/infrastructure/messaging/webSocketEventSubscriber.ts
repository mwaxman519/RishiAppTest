...
export class WebSocketEventSubscriber implements EventSubscriber {
  ...
  async handleEvent<T>(event: Event<T>): Promise<void> {
    console.log(`[WebSocketEventSubscriber] Broadcasting event: ${event.type}`, { eventId: event.id });
    
    try {
      if (this.clients.size === 0) {
        console.log(`[WebSocketEventSubscriber] No connected clients, skipping broadcast`);
        return;
      }
      ...
      for (const client of this.clients.values()) {
        if (this.shouldDeliverToClient(client, event)) {
          try {
            ...
            console.error(`[WebSocketEventSubscriber] Error sending to client ${client.id}:`, sendError);
            // We may want to remove this client if sending consistently fails
          }
        }
      }
      
      console.log(`[WebSocketEventSubscriber] Event broadcast complete. Delivered to ${deliveryCount}/${this.clients.size} clients`);
    } catch (error) {
      console.error(`[WebSocketEventSubscriber] Error broadcasting event:`, error);
      throw error;
    }
  }
  ...
  registerClient(
    ...
    console.log(`[WebSocketEventSubscriber] Client registered: ${clientId} with ${subscriptions.size} subscriptions`);
  }
  ...
  updateClientSubscriptions(clientId: string, subscriptions: string[]): boolean {
    ...
    console.log(`[WebSocketEventSubscriber] Updated subscriptions for client ${clientId}: ${subscriptions.length} events`);
    return true;
  }
   ...
   */
  private shouldDeliverToClient<T>(client: WebSocketClient, event: Event<T>): boolean {
   ...
   */
  getStats(): { clientCount: number, activeSubscriptions: Record<string, number> } {
    ...
    for (const client of this.clients.values()) {
      for (const eventType of client.subscriptions) {
        activeSubscriptions[eventType] = (activeSubscriptions[eventType] || 0) + 1;
      }
    }
