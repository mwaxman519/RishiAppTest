...
export class CircuitBreaker {
  ...
  async execute<T>(operation: () => Promise<T>): Promise<CircuitBreakerResult<T>> {
    ...
    if (this.state === CircuitState.OPEN) {
      const timeSinceLastStateChange = new Date().getTime() - this.lastStateChange.getTime();
  ...
  private transitionToOpen(): void {
    ...
    console.log(`Circuit breaker [${this.name}] OPENED due to failures. Last error: ${this.lastError?.message}`);
  ...
  private transitionToHalfOpen(): void {
    ...
    console.log(`Circuit breaker [${this.name}] in HALF-OPEN state, testing service recovery`);
  }
  ...
  private transitionToClosed(): void {
    ...
    console.log(`Circuit breaker [${this.name}] CLOSED, normal operations resumed`);
  }
}
