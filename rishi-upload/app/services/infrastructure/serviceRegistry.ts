...
export function getServiceAdapter<T extends object>(serviceId: string, localInstance: T): ServiceAdapter<T> {
  ...
  if (!config) {
    console.warn(`No configuration found for service '${serviceId}'. Defaulting to local.`);
  ...
  if (config.type === 'local') {
    ...
    if (!config.url) {
      throw new Error(`Remote service '${serviceId}' is missing a URL configuration.`);
    }
 ...
 * @returns A service adapter that provides access to the service
 */
export function getAsyncServiceAdapter<T extends object>(serviceId: string): ServiceAdapter<T> {
  const config = serviceConfigurations[serviceId];
  
  if (!config) {
    console.warn(`No configuration found for service '${serviceId}'. Defaulting to remote.`);
  ...
  if (config.type === 'remote') {
    ...
    return {
      getService(): T {
        throw new Error('This service must be accessed through an API route. Direct client access is not supported.');
      }
    } as ServiceAdapter<T>;
  }
}
  ...
  return async () => {
    ...
    try {
      if (serviceId === 'availability') {
        ...
        const { LocalAvailabilityServiceAdapter } = await import('../availability/serviceAdapter');
      ...
      throw new Error(`No async import handler defined for service '${serviceId}'`);
    } catch (error) {
      console.error(`Failed to asynchronously import service '${serviceId}':`, error);
      throw error;
    }
  };
}