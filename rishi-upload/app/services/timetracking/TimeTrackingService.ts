...
export class TimeTrackingService {
  ...
  initializeSampleData(): void {
    ...
    console.log('TimeTrackingService: Sample data initialization (database-backed)');
  }

  /**
   * Get all time entries with filtering (alias for compatibility)
   */
  async getAllTimeEntries(organizationId: string, filters: Partial<TimeEntryFilters> = {}): Promise<any[]> {
   ...
   * Clock in agent
   */
  async clockIn(agentId: string, clockInData?: Partial<ClockInData>): Promise<any> {
    ...
    try {
      ...
      const [timeEntry] = await db
        ...
        .values({
          ...
          locationIn: fullClockInData.location ? JSON.stringify(fullClockInData.location) : null,
          status: 'active',
   ...
   * Clock out agent
   */
  async clockOut(agentId: string, clockOutData?: Partial<ClockOutData>): Promise<any> {
    try {
      ...
      if (existingEntry.hourlyRate) {
        totalPay = Number((totalHours * Number(existingEntry.hourlyRate)).toFixed(2));
      }
      ...
      const [updatedEntry] = await db
        ...
        .set({
          ...
          locationOut: clockOutData?.location ? JSON.stringify(clockOutData.location) : null,
          status: 'completed',
  ...
  async validateLocation(
    ...
    try {
      ...
      if (!shift || !shift.location) {
        return { valid: true, message: "No shift location to validate against" };
      }
  ...
  async getTimeTrackingAnalytics(
    ...
    try {
      ...
      const averageShiftLength = entries.length > 0 ? totalHours / entries.length : 0;
