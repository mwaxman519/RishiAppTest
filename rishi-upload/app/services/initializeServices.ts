...
import { locationEventBus, connectLocationEventBusToWebSockets, setupLocationEventLogging } from './infrastructure/messaging/locationEvents';
...
export class ErrorHandler {
  ...
  registerStrategy(strategy: any) {
    ...
    console.log(`[ErrorHandler] Registered strategy: ${strategy.constructor.name}`);
  }
  
  async handleError(error: Error, eventType: string, context: Record<string, any>) {
    console.log(`[ErrorHandler] Processing error for event ${eventType} with ${this.strategies.length} strategies`);
    ...
    for (const strategy of this.strategies) {
      if (strategy.getApplicableEvents().includes(eventType)) {
        try {
          ...
          if (result) {
            handled = true;
            console.log(`[ErrorHandler] Error handled by ${strategy.constructor.name}`);
          }
        } catch (strategyError) {
          console.error(`[ErrorHandler] Strategy ${strategy.constructor.name} failed:`, strategyError);
        }
      }
    }
...
export function initializeServices() {
  ...
  try {
    ...
    console.log('[Services] Location event bus connected to WebSocket publisher');
