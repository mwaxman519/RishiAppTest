...
export class RBACService {
  ...
  async getUserPermissions(userId: string, organizationId?: string): Promise<Permission[]> {
    ...
    const rolePromises = userRoles.map(userRole => this.getRoleById(userRole.roleId));
    ...
    if (organizationId) {
      const orgPermissions = await this.getOrganizationPermissions(organizationId);
   ...
   */
  async hasPermission(userId: string, params: PermissionCheckParams): Promise<boolean> {
    ...
    if (scope) {
      ...
      if (scope === PermissionScope.ORGANIZATION && organizationId) {
        ...
        if (userPermissions.includes(`${action}:${resource}:${PermissionScope.ORGANIZATION}`)) {
          return true;
        }
      }
      
      // Handle owned resources
      if (scope === PermissionScope.OWNED && params.resourceOwnerId === userId) {
        if (userPermissions.includes(`${action}:${resource}:${PermissionScope.OWNED}`)) {
          return true;
        }
      }
      
      // Handle assigned resources
      if (scope === PermissionScope.ASSIGNED && params.resourceAssigneeId === userId) {
        if (userPermissions.includes(`${action}:${resource}:${PermissionScope.ASSIGNED}`)) {
          return true;
        }
      }
  ...
  async removeOrganizationPermission(organizationId: string, permission: string): Promise<void> {
    return this.repository.removeOrganizationPermission(organizationId, permission);
  }
}