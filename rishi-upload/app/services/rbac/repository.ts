...
export class RBACRepository {
  ...
  async assignRoleToUser(params: UserRoleParams): Promise<UserRole> {
    try {
      console.log("DEVELOPMENT MODE: Using mock user role assignment for testing");
  ...
  async removeRoleFromUser(userId: string, roleId: string, organizationId?: string): Promise<void> {
    try {
      ...
      console.log(`Mock removing role ${roleId} from user ${userId} ${organizationId ? 'in organization ' + organizationId : 'globally'}`);
  ...
  async getOrganizationPermissions(organizationId: string): Promise<OrganizationPermission[]> {
    try {
      ...
      throw new Error(`Failed to fetch organization permissions: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async setOrganizationPermission(params: OrganizationPermissionParams): Promise<OrganizationPermission> {
    try {
      ...
      if (existingPermissions.length > 0) {
        ...
        const [result] = await db.update(organizationPermissions)
          .set({
        ...
        const [result] = await db.insert(organizationPermissions)
          .values({
            organizationId: params.organizationId,
      ...
      throw new Error(`Failed to set organization permission: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async removeOrganizationPermission(organizationId: string, permission: string): Promise<void> {
    try {
      await db.delete(organizationPermissions)
        .where(
          and(
      ...
      console.error(`Error removing permission ${permission} from organization ${organizationId}:`, error);
      throw new Error(`Failed to remove organization permission: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  private mapOrganizationPermissionToDTO(orgPermission: any): OrganizationPermission {
    return {
