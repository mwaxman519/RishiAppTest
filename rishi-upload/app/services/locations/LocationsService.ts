...
export class LocationsService {
  ...
  async getLocationById(
    ...
    try {
      ...
      if (!this.hasLocationAccess(location, requestingUserId, userRole, organizationId)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
      ...
      console.error(`LocationsService.getLocationById error for ID ${id}:`, error);
      return {
        success: false,
  ...
  async createLocation(
    ...
    try {
      ...
      // Create location
      const location = await this.repository.create(validatedData, organizationId, createdBy);
  ...
  async updateLocation(
    ...
    try {
      ...
      // Check permissions
      if (!this.hasLocationAccess(existingLocation, updatedBy, userRole, organizationId)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
      ...
      console.error(`LocationsService.updateLocation error for ID ${id}:`, error);
      return {
        success: false,
  ...
  async deleteLocation(
    ...
    try {
      ...
      // Check permissions
      if (!this.hasLocationAccess(existingLocation, deletedBy, userRole, organizationId)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
      ...
      console.error(`LocationsService.deleteLocation error for ID ${id}:`, error);
      return {
        success: false,
  ...
  async approveLocation(
    ...
    try {
      ...
      const existingLocation = await this.repository.findById(approval.locationId);
      if (!existingLocation) {
        return {
          success: false,
          error: 'Location not found',
      ...
      const updatedLocation = await this.repository.update(approval.locationId, {
  ...
  async rejectLocation(
    ...
    try {
      ...
      // Get existing location
      const existingLocation = await this.repository.findById(rejection.locationId);
      if (!existingLocation) {
        return {
          success: false,
          error: 'Location not found',
      ...
      // Update location status
      const updatedLocation = await this.repository.update(rejection.locationId, {
  ...
  private hasApprovalPermissions(userRole: string): boolean {
    return ['internal_field_manager', 'organization_admin', 'super_admin'].includes(userRole);
  }
}