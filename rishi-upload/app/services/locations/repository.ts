...
import { locations, states, users, organizations } from '../../../shared/schema';
import { eq, and, or, desc } from "drizzle-orm";
import { LocationDTO, CreateLocationParams, UpdateLocationParams, LocationStatus, LocationType } from "./models";
...
export class LocationRepository {
  ...
  async findAll(filters: Record<string, any> = {}): Promise<LocationDTO[]> {
    try {
      ...
      throw new Error(`Failed to find locations: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async findById(id: string): Promise<LocationDTO | null> {
    try {
      ...
      throw new Error(`Failed to find location: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private mapToDTO(row: any): LocationDTO {
    ...
    const latitude = row.locations.latitude ? parseFloat(row.locations.latitude) : undefined;
    const longitude = row.locations.longitude ? parseFloat(row.locations.longitude) : undefined;
