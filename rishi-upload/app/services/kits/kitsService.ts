...
export class KitsService {
   ...
   */
  async createTemplate(params: CreateKitTemplateParams, userId: string): Promise<KitTemplateDTO> {
   ...
   */
  async updateTemplate(id: string, params: UpdateKitTemplateParams, userId: string): Promise<KitTemplateDTO> {
    // Validate parameters using zod schema
    ...
    if (template.approval_status === KitTemplateStatus.APPROVED && 
      ...
      throw new Error('Cannot update an approved template except for its active status');
    }
    
    // Only allow admins/managers to change approval status
    if (params.approval_status && params.approval_status !== template.approval_status) {
   ...
   */
  async approveTemplate(params: ApproveKitTemplateParams, userId: string): Promise<KitTemplateDTO> {
   ...
   */
  async rejectTemplate(params: RejectKitTemplateParams, userId: string): Promise<KitTemplateDTO> {
  ...
  async createKit(params: CreateKitParams, userId: string): Promise<KitDTO> {
    ...
    if (template.approval_status !== KitTemplateStatus.APPROVED) {
      throw new Error('Cannot create a kit from a template that is not approved');
    }
   ...
   */
  async updateKit(id: string, params: UpdateKitParams, userId: string): Promise<KitDTO> {
    // Validate parameters using zod schema
    ...
    if (kit.active && kit.status === KitStatus.ACTIVE && 
        !params.status) {
   ...
   */
  async getAllActivityKits(filters?: Record<string, any>): Promise<ActivityKitDTO[]> {
   ...
   */
  async createActivityKit(params: CreateActivityKitParams): Promise<ActivityKitDTO> {
    // Validate parameters using zod schema
    ...
    if (!validatedParams.kit_template_id && !validatedParams.kit_instance_id) {
      throw new Error('Either kit_template_id or kit_instance_id must be provided');
    }
   ...
   */
  async updateActivityKit(id: string, params: UpdateActivityKitParams): Promise<ActivityKitDTO> {
    // Validate parameters using zod schema
