...
export class KitRepository {
  ...
  async findAllTemplates(filters: Record<string, any> = {}): Promise<KitTemplateDTO[]> {
    try {
      ...
      if (filters.organizationId) {
        queryFilters.push(eq(kitTemplates.organization_id, filters.organizationId));
      }
      ...
      throw new Error(`Failed to find kit templates: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async findTemplateById(id: number | string): Promise<KitTemplateDTO | null> {
    try {
      ...
      throw new Error(`Failed to find kit template: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async createTemplate(data: CreateKitTemplateParams, userId: string): Promise<KitTemplateDTO> {
    try {
      const [template] = await db.insert(kitTemplates)
        .values({
          ...data,
      ...
      throw new Error(`Failed to create kit template: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async updateTemplate(id: number | string, data: UpdateKitTemplateParams): Promise<KitTemplateDTO> {
    try {
      // For UUID fields, we don't need to convert to number
      const templateId = id.toString();
      ...
      if (data.organization_id) updateData.organization_id = data.organization_id;
      ...
      if (data.approval_status) updateData.approval_status = data.approval_status;
      if (data.approval_notes) updateData.approval_notes = data.approval_notes;
      
      await db.update(kitTemplates)
        .set(updateData)
        .where(eq(kitTemplates.id, templateId));
      ...
      throw new Error(`Failed to update kit template: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async approveTemplate(id: number | string, userId: string, notes?: string): Promise<KitTemplateDTO> {
    try {
      // For UUID fields, we don't need to convert to number
      const templateId = id.toString();
      ...
      throw new Error(`Failed to approve kit template: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async rejectTemplate(id: number | string, userId: string, reason: string): Promise<KitTemplateDTO> {
    try {
      // For UUID fields, we don't need to convert to number
      const templateId = id.toString();
      ...
      throw new Error(`Failed to reject kit template: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async findAllKits(filters: Record<string, any> = {}): Promise<KitDTO[]> {
    try {
      ...
      const kitsData = await db.select()
        .from(kits)
      ...
      throw new Error(`Failed to find kits: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async findKitById(id: string): Promise<KitDTO | null> {
    try {
      ...
      throw new Error(`Failed to find kit: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async createKit(data: CreateKitParams, userId: string): Promise<KitDTO> {
    try {
      ...
      const result = await db.insert(kits)
        .values({
          name: data.name,
      ...
      const kit = result[0];
      if (!kit) {
        throw new Error('Failed to create kit: No kit was returned after insertion');
      }

      return this.findKitById(kit.id) as Promise<KitDTO>;
    } catch (error) {
      console.error("Error creating kit:", error);
      throw new Error(`Failed to create kit: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async updateKit(id: string, data: UpdateKitParams): Promise<KitDTO> {
    try {
      ...
      throw new Error(`Failed to update kit: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   * Approve a kit
   */
  async approveKit(id: string, userId: string, notes?: string): Promise<KitDTO> {
    try {
      await db.update(kits)
        .set({
      ...
      throw new Error(`Failed to approve kit: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async rejectKit(id: string, userId: string, reason: string): Promise<KitDTO> {
    try {
      ...
      throw new Error(`Failed to reject kit: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async findAllActivityKits(filters: Record<string, any> = {}): Promise<ActivityKitDTO[]> {
    try {
      // Build query filters
      const queryFilters = [];
      ...
      if (filters.kit_template_id) {
        queryFilters.push(eq(activityKits.kit_template_id, filters.kit_template_id));
      }
      
      if (filters.kit_instance_id) {
        queryFilters.push(eq(activityKits.kit_instance_id, filters.kit_instance_id));
      }
      ...
      if (filters.assigned_to_id) {
        queryFilters.push(eq(activityKits.assigned_to_id, filters.assigned_to_id));
      }

      // Execute query with relations
      const activityKitsData = await db.select()
        .from(activityKits)
        .leftJoin(activities, eq(activityKits.activity_id, activities.id))
        .leftJoin(kitTemplates, eq(activityKits.kit_template_id, kitTemplates.id))
      ...
      throw new Error(`Failed to find activity kits: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async findActivityKitById(id: string): Promise<ActivityKitDTO | null> {
    try {
      const [activityKitData] = await db.select()
        .from(activityKits)
        .leftJoin(activities, eq(activityKits.activity_id, activities.id))
        .leftJoin(kitTemplates, eq(activityKits.kit_template_id, kitTemplates.id))
        .leftJoin(kits, eq(activityKits.kit_instance_id, kits.id))
      ...
      throw new Error(`Failed to find activity kit: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async createActivityKit(data: any): Promise<ActivityKitDTO> {
    try {
      ...
      const result = await db.insert(activityKits)
        .values({
          id,
          ...data
        })
      ...
      if (!activityKit) {
        throw new Error('Failed to create activity kit: No activity kit was returned after insertion');
      }

      return this.findActivityKitById(activityKit.id) as Promise<ActivityKitDTO>;
    } catch (error) {
      console.error("Error creating activity kit:", error);
      throw new Error(`Failed to create activity kit: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async updateActivityKit(id: string, data: any): Promise<ActivityKitDTO> {
    try {
      ...
      throw new Error(`Failed to update activity kit: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async deleteActivityKit(id: string): Promise<void> {
    try {
      await db.delete(activityKits)
        .where(eq(activityKits.id, id));
    } catch (error) {
      console.error(`Error deleting activity kit with ID ${id}:`, error);
      throw new Error(`Failed to delete activity kit: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private mapToTemplateDTO(row: any): KitTemplateDTO {
    ...
    return {
      ...
      organization_id: row.organization_id || row.kit_templates?.organization_id,
      ...
      approval_status: row.approval_status || row.kit_templates?.approval_status,
      ...
      approval_date: (row.approval_date || row.kit_templates?.approval_date)?.toISOString(),
      approval_notes: row.approval_notes || row.kit_templates?.approval_notes,
      requested_by_id: row.requested_by_id || row.kit_templates?.requested_by_id,
      ...
      created_at: (row.created_at || row.kit_templates?.created_at)?.toISOString(),
      updated_at: (row.updated_at || row.kit_templates?.updated_at)?.toISOString()
    };
  }
  ...
  private mapToActivityKitDTO(row: any): ActivityKitDTO {
    ...
    return {
      ...
      kitInstance: kitData ? this.mapToKitDTO({ kits: kitData, kit_templates: row.kit_templates }) : undefined,
