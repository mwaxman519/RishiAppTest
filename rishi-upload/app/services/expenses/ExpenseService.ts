...
export class ExpenseService {
  ...
  async getExpenses(
    ...
    userRole: string,
    organizationId: string
  ): Promise<ServiceResponse<{ expenses: ExpenseData[]; total: number; page: number; limit: number }>> {
    try {
      // Validate filters
  ...
  async getExpenseById(
    ...
    try {
      ...
      if (!this.hasExpenseAccess(result.data, requestingUserId, userRole, organizationId)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
  ...
  async updateExpense(
    ...
    try {
      ...
      // Check permissions
      if (!this.hasExpenseAccess(existingExpense, updatedBy, userRole, organizationId)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
  ...
  async processApproval(
    ...
    try {
      ...
      // Get existing expense
      const existingResult = await this.repository.findById(validatedApproval.expenseId);
      if (!existingResult.success || !existingResult.data) {
        return existingResult;
      }
      ...
      // Prepare update data
      const updateData = {
        status: validatedApproval.approved ? 'approved' as const : 'rejected' as const,
        approvedBy: approverUserId,
        approvedAt: new Date(),
        rejectionReason: validatedApproval.approved ? null : validatedApproval.rejectionReason
      };

      // Perform update
      const result = await this.repository.update(validatedApproval.expenseId, updateData);
  ...
  async deleteExpense(
    ...
    try {
      ...
      // Check permissions
      if (!this.hasExpenseAccess(existingExpense, deletedBy, userRole, organizationId)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
