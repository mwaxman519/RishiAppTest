 ...
 */
export const brandAgentAssignmentSchema = z.object({
  userId: z.string().uuid("Invalid user ID"),
  brandId: z.string().uuid("Invalid brand ID"),
  assignmentRole: z.enum(['primary', 'backup', 'seasonal', 'temporary'], {
    errorMap: () => ({ message: "Role must be primary, backup, seasonal, or temporary" })
  }),
 ...
 */
export function validateBrandAgentAssignment(assignment: any): InsertBrandAgentAssignment {
...
export const agentSkillSchema = z.object({
  ...
  skillType: z.enum(['certification', 'language', 'specialty', 'equipment'], {
    errorMap: () => ({ message: "Skill type must be certification, language, specialty, or equipment" })
  }),
  skillName: z.string().min(1, "Skill name is required").max(200, "Skill name too long"),
...
export interface AssignmentConflict {
  agentId: string;
  agentName: string;
  conflictType: 'existing_assignment' | 'skill_mismatch' | 'territory_unavailable';
...
export interface RosterOptimizationSuggestion {
  type: 'coverage_gap' | 'overallocation' | 'skill_improvement' | 'territory_rebalance';
