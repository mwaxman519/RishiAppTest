...
export class EventsService {
   ...
   * Create a new event
   */
  async createEvent(data: CreateEventParams, createdById: string): Promise<Event> {
  ...
  async updateEvent(id: string, data: UpdateEventParams): Promise<Event> {
    ...
    if (
      ...
      throw new Error(`Cannot update event with status ${existingEvent.status}`);
    }
  ...
  async deleteEvent(id: string): Promise<void> {
    ...
    if (existingEvent.status !== EventStatus.DRAFT) {
      throw new Error(`Cannot delete event with status ${existingEvent.status}`);
    }
  ...
  async submitEventForApproval(id: string): Promise<Event> {
    ...
    if (existingEvent.status !== EventStatus.DRAFT) {
      throw new Error(`Cannot submit event with status ${existingEvent.status} for approval`);
    }
   ...
   * Approve an event
   */
  async approveEvent(params: ApproveEventParams, reviewerId: string): Promise<Event> {
    // Get the current event state
    ...
    if (existingEvent.status !== EventStatus.PENDING_APPROVAL) {
      throw new Error(`Cannot approve event with status ${existingEvent.status}`);
    }
   ...
   * Reject an event
   */
  async rejectEvent(params: RejectEventParams, reviewerId: string): Promise<Event> {
    // Get the current event state
    const existingEvent = await this.repository.findById(params.id);
    ...
    if (existingEvent.status !== EventStatus.PENDING_APPROVAL) {
      throw new Error(`Cannot reject event with status ${existingEvent.status}`);
    }
  ...
  async cancelEvent(id: string, reason?: string): Promise<Event> {
    ...
    if (
      ...
      throw new Error(`Cannot cancel event with status ${existingEvent.status}`);
    }
  ...
  async completeEvent(id: string): Promise<Event> {
    ...
    if (existingEvent.status !== EventStatus.IN_PROGRESS) {
      throw new Error(`Cannot complete event with status ${existingEvent.status}`);
    }
  ...
  async finalizeEvent(id: string): Promise<Event> {
    ...
    if (existingEvent.status !== EventStatus.COMPLETED) {
      throw new Error(`Cannot finalize event with status ${existingEvent.status}`);
    }
  ...
  async createChecklistItem(data: CreateChecklistItemParams): Promise<EventChecklistItem> {
    ...
    if (data.dueDate) {
      ...
      if (dueDate > eventEndDate) {
        throw new Error('Checklist item due date cannot be after event end date');
      }
    }
