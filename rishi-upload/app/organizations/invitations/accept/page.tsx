...
function AcceptInvitationPage() {
  ...
  const acceptInvitationMutation = useMutation({
    mutationFn: async (token: string) => {
      return apiRequest('POST', '/api/organizations/invitations/accept', { token });
    },
    onSuccess: () => {
      setStatus('success');
      toast({
    ...
    onError: (error: Error) => {
      setStatus('error');
      setErrorMessage(error.message || 'There was an error accepting the invitation.');
      toast({
        title: 'Error',
  ...
  const rejectInvitationMutation = useMutation({
    mutationFn: async (token: string) => {
      return apiRequest('POST', '/api/organizations/invitations/reject', { token });
    },
    onSuccess: () => {
      toast({
        title: 'Invitation Rejected',
        description: 'You have declined the invitation to join the organization.',
      });
      router.push('/');
    },
    onError: (error: Error) => {
      toast({
        title: 'Error',
        description: error.message || 'There was an error rejecting the invitation.',
        variant: 'destructive',
      });
    },
  });
  ...
  useEffect(() => {
    if (!isAuthLoading && !user) {
      router.push(`/auth/login?redirectUrl=${encodeURIComponent(window.location.href)}`);
    }
          ...
          <CardContent>
            ...
            {status === 'error' && (
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-center text-gray-600 dark:text-gray-400 mb-4">
                  {errorMessage || 'There was an error processing your invitation.'}
                </p>
              </div>
            )}
          </CardContent>
