...
interface UserRoleEditorProps {
  user: UserProfile;
  onSave: (userData: { role: UserRole; customPermissions?: Permission[] }) => Promise<void>;
  disabled?: boolean;
}
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center">
            <button
              ...
              `}
            >
              {isCustomizing ? 'Using Custom Permissions' : 'Use Role Default Permissions'}
            </button>
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-end">
          <button
            type="button"
            onClick={handleSave}
            disabled={disabled || isSaving || selectedRole === user.role && !isCustomizing}
            className={`
              px-4 py-2 bg-blue-600 text-white rounded-md
              ${(disabled || isSaving || (selectedRole === user.role && !isCustomizing)) 
