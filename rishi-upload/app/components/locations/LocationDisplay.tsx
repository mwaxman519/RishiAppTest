...
export default function LocationDisplay({
  ...
  useEffect(() => {
    ...
    if (isLoaded && !mapInitialized && showMap && location.latitude && location.longitude) {
      try {
        // Initialize the map
    ...
    <Card className="w-full dark:bg-gray-800 dark:text-gray-100">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div>
          ...
          <CardDescription className="dark:text-gray-300">{formattedAddress()}</CardDescription>
        </div>
        
        {statusBadge && (
          <Badge variant={
            statusBadge === 'Approved' ? 'success' : 
        ...
        {showMap && (
          <div className="w-full rounded-md overflow-hidden h-[200px] relative dark:bg-gray-700">
            ...
            <div 
              ...
              style={{ display: (!mapInitialized || mapError) ? 'none' : 'block' }}
            />
          </div>
        )}
