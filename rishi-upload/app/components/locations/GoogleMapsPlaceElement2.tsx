...
export default function GoogleMapsPlaceElement({ 
  ...
  useEffect(() => {
          ...
          (result: any, status: any) => {
            ...
            if (status === window.google.maps.places.PlacesServiceStatus.OK && result) {
              ...
              if (result.geometry && result.geometry.location) {
                ...
                console.log('🔍 Created address data to pass to parent:', addressData);
    ...
    function setupGooglePlaceAutocomplete() {
      console.log('🔍 Setting up Google PlaceAutocompleteElement with containerRef state:', !!containerRef.current);
      ...
      const checkContainerAndSetup = () => {
        ...
        try {
          ...
          if (!window.google.maps.places.PlaceAutocompleteElement) {
            console.error('🔍 PlaceAutocompleteElement constructor is not available');
            setIsLoading(false);
            toast({
              title: 'API Support Error',
              description: 'Your Google Maps API key does not have access to the PlaceAutocompleteElement beta feature',
              variant: 'destructive'
            });
            return;
          }
          ...
          try {
            ...
            element.addEventListener('gmp-placeselect', (event: any) => {
              console.log('🎯 PlaceAutocompleteElement selection event triggered!', event);
              ...
              try {
                ...
                if (event.detail && event.detail.place && event.detail.place.id) {
                  ...
                  console.log('🎯 About to call handlePlaceSelection with ID:', placeId);
            ...
            console.error('🔍 API Error creating PlaceAutocompleteElement:', apiError);
            ...
            if (errorMessage.includes('places.googleapis.com') && 
              ...
              toast({
                ...
                description: 'Your API key does not have access to the Places API v1. Contact your administrator to enable it.',
                variant: 'destructive'
              });
            } else {
              toast({
          ...
          console.error('🔍 Error setting up Google PlaceAutocompleteElement:', error);
          setIsLoading(false);
          toast({
    ...
    if (window.google && window.google.maps) {
      console.log('🔍 Google Maps already loaded, checking for PlaceAutocompleteElement support');
      
      // Check if the places library exists
      if (window.google.maps.places) {
        console.log('🔍 Places library found, checking for PlaceAutocompleteElement');
        ...
        if (window.google.maps.places.PlaceAutocompleteElement) {
          ...
          console.error('🔍 PlaceAutocompleteElement not available - you need beta API enabled');
          toast({
            title: 'API Support Error',
            description: 'Your Google Maps API key does not have access to the PlaceAutocompleteElement beta feature',
            variant: 'destructive'
  ...
  return (
    <div className={`${className} relative`}>
      ...
      <div className="relative min-h-[45px] border rounded-md mb-2">
        ...
        <div 
          ref={containerRef}
          className="relative w-full min-h-[40px]"
        />
