...
import { GoogleMapsProvider, useGoogleMaps } from '@/components/maps/GoogleMapsContext';
...
export function AddLocationModal({ open, onOpenChange }: AddLocationModalProps) {
  ...
  const handlePlaceSelected = useCallback((placeData: LocationData) => {
    try {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            ...
            const response = await fetch(`/api/maps/geocode?lat=${latLng.lat}&lng=${latLng.lng}`);
            ...
            if (data.results && data.results.length > 0) {
              ...
              const placeData: LocationData = {
                ...
                latitude: data.results[0].geometry?.location?.latitude || latLng.lat,
                longitude: data.results[0].geometry?.location?.longitude || latLng.lng,
  ...
  const onSubmit = (values: LocationFormValues) => {
    ...
    const city = selectedPlace.addressComponents?.find((c: {types: string[]}) => 
      c.types.includes('locality'))?.longText || 
      selectedPlace.addressComponents?.find((c: {types: string[]}) => 
      c.types.includes('sublocality'))?.longText || '';

    const state = selectedPlace.addressComponents?.find((c: {types: string[]}) => 
      c.types.includes('administrative_area_level_1'))?.longText || '';
      
    const postalCode = selectedPlace.addressComponents?.find((c: {types: string[]}) => 
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Search for a location and provide additional details to add it to the system.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6">
                    ...
                    <>
                      ...
                      <div id="marker-container" className="hidden">
                        {(() => {
                          ...
                          if (window.google?.maps?.marker?.AdvancedMarkerElement) {
                            ...
                            pinElement.title = selectedPlace?.displayName || 'Selected location';
                            ...
                            setTimeout(() => {
                              if (mapRef.current) {
                                markerRef.current = new window.google.maps.marker.AdvancedMarkerElement({
                                  ...
                                  content: pinElement,
                                  title: selectedPlace?.displayName || 'Selected location'
                                });
                              }
                            }, 0);
                          }
                      ...
                      {isInfoOpen && selectedPlace && (
                        ...
                        >
                          ...
                          >
                            <h3 className="font-medium">{selectedPlace.displayName || 'Selected Location'}</h3>
            ...
            {selectedPlace && (
              <div className="text-sm space-y-2 border rounded-md p-3 bg-muted/20">
                ...
                {selectedPlace.addressComponents && selectedPlace.addressComponents.length > 0 && (
                  <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t">
                    <div>
                      <span className="text-muted-foreground">City: </span>
                      <span className="font-medium">
                        {selectedPlace.addressComponents.find((c: {types: string[]}) => c.types.includes('locality'))?.longText || 'N/A'}
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">State: </span>
                      <span className="font-medium">
                        {selectedPlace.addressComponents.find((c: {types: string[]}) => c.types.includes('administrative_area_level_1'))?.longText || 'N/A'}
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Postal Code: </span>
                      <span className="font-medium">
                        {selectedPlace.addressComponents.find((c: {types: string[]}) => c.types.includes('postal_code'))?.longText || 'N/A'}
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Country: </span>
                      <span className="font-medium">
                        {selectedPlace.addressComponents.find((c: {types: string[]}) => c.types.includes('country'))?.longText || 'N/A'}
                      </span>
                    </div>
                  </div>
                )}
