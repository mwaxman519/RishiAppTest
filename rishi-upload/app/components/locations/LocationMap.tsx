...
export function LocationMap({
  ...
  }, [isLoaded, defaultCenter, zoom, clickable, onMapClick, disableDefaultUI, interactive, mapId]);
  ...
  useEffect(() => {
    ...
    locations.forEach(location => {
      ...
      try {
        ...
        markerElement.style.backgroundColor = isSelected ? '#ef4444' : markerColor;
        ...
        if (marker) {
          ...
          marker.addListener('gmp-click', () => {
            ...
            if (showInfoWindow && infoWindowRef.current && googleMapRef.current) {
        ...
        if (!window.google?.maps?.marker) {
          setMapError('Advanced marker library is required but was not loaded correctly. Please check your Google Maps API configuration and ensure the "marker" library is included.');
        }
      }
    });
  ...
  }, [locations, mapInitialized, selectedLocationId, markerColor, showInfoWindow, onSelectLocation, clusterMarkers]);
      ...
      >
        <div className="flex flex-col items-center justify-center p-4 text-center max-w-md">
          ...
          <p className="text-sm text-muted-foreground">
            {loadError?.message || mapError || "There was a problem loading Google Maps."}
          </p>
        </div>
      </div>
    );
  ...
  // Main map render
  return (
    <div className={cn("relative rounded-md overflow-hidden", className)} style={{ height, width }}>
      {/* Map container */}
      <div 
        ref={mapRef} 
