...
const AppLogo = () => (
  <svg width="24" height="24" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M16 2C8.268 2 2 8.268 2 16C2 23.732 8.268 30 16 30C23.732 30 30 23.732 30 16C30 8.268 23.732 2 16 2Z" fill="#5A36CC" />
  ...
  <div className="min-h-screen flex flex-col bg-white dark:bg-gray-900">
    ...
    <div className="sticky bottom-0 bg-white dark:bg-gray-900 border-t z-40 shadow-lg h-16 flex items-center justify-center">
      <div className="flex gap-8">
        {[1,2,3,4,5].map(i => (
          <div key={i} className="w-6 h-6 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></div>
        ))}
      </div>
    </div>
  </div>
...
export default function ResponsiveLayout({ children }: ResponsiveLayoutProps) {
  ...
  const isFullWidthPage = pathname?.startsWith('/auth/') || pathname === '/login' || pathname === '/register';
  
  // Media query hook (only works on client side)
  const isDesktop = useMediaQuery('(min-width: 1024px)');
  
  // Check if the URL has the unauthenticated parameter for testing
  const hasUnauthenticatedParam = mounted && typeof window !== 'undefined' 
    ? new URLSearchParams(window.location.search).get('unauthenticated') === 'true'
    : false;
