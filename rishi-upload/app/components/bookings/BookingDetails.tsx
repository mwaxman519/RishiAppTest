...
const mockBookingDetails = {
  ...
  description: 'Showcase our new product lineup with interactive demonstrations for mall visitors.',
  activityType: 'event',
  ...
  specialInstructions: 'Ensure all demo units are fully charged. Bring extra product literature.',
  assignments: [
    {
      id: '1',
      userId: '456',
...
// Status badge mapper
const activityStatusMapper: Record<string, { label: string, variant: "default" | "secondary" | "destructive" | "outline" }> = {
...
const activityTypeMapper: Record<string, { label: string, icon: React.ReactNode }> = {
  event: { 
    label: 'Event', 
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight break-words">{booking.title}</h1>
          <div className="flex flex-wrap gap-2 mt-2">
            <Badge 
              variant={activityStatusMapper[booking.activityStatus]?.variant || 'default'}
              className="px-2 py-1"
            >
              {activityStatusMapper[booking.activityStatus]?.label || booking.activityStatus}
            </Badge>
            <Badge variant="outline" className="flex items-center px-2 py-1">
              {activityTypeMapper[booking.activityType]?.icon}
              {activityTypeMapper[booking.activityType]?.label || booking.activityType}
            </Badge>
            <Badge variant="outline" className="flex items-center px-2 py-1 capitalize">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {canEdit && (
                <DropdownMenuItem onClick={() => router.push(`/bookings/${id}/edit`)}>
                  <Edit className="h-4 w-4 mr-2" />
                  Edit Booking
                </DropdownMenuItem>
              )}
              <DropdownMenuItem onClick={() => {/* Add to calendar functionality */}}>
      ...
      <Tabs defaultValue="details" className="w-full">
        <TabsList className="grid grid-cols-3 max-w-md">
          ...
          <TabsTrigger value="staff">Staff ({booking.assignments.length})</TabsTrigger>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      {booking.startDate && (
                        <div className="font-medium">
                          ...
                          {booking.endDate && booking.endDate.toDateString() !== booking.startDate.toDateString() && 
                            ` to ${formatDate(booking.endDate)}`
                          }
                        </div>
                      )}
                      {!booking.allDay && booking.startDate && booking.endDate && (
                ...
                {booking.activityType === 'event' && booking.promotionType && (
                  <div className="space-y-3">
                    <h3 className="text-sm font-medium text-muted-foreground">Promotion Type</h3>
                    <div className="flex items-start gap-3">
                ...
                {booking.kitTemplate && (
                  <div className="space-y-3">
                    <h3 className="text-sm font-medium text-muted-foreground">Kit Template</h3>
                    <div className="flex items-start gap-3">
                      <Package className="h-5 w-5 text-muted-foreground flex-shrink-0 mt-0.5" />
                      <div>
                        <div className="font-medium">{booking.kitTemplate.name}</div>
                        <div className="text-sm text-muted-foreground">
                ...
                <div className="space-y-3">
                  <h3 className="text-sm font-medium text-muted-foreground">Required Staff</h3>
                  <div className="flex items-start gap-3">
                    <Users className="h-5 w-5 text-muted-foreground flex-shrink-0 mt-0.5" />
                    <div className="font-medium">{booking.requiredStaffCount} staff members</div>
                  </div>
                </div>
              ...
              {booking.assignments.length > 0 ? (
                <div className="space-y-4">
                  {booking.assignments.map((assignment) => (
                    <div key={assignment.id} className="flex items-center justify-between p-3 bg-muted/40 rounded-md">
                      <div className="flex items-center gap-3">
                        <Avatar>
                          <AvatarImage src={assignment.userAvatar || undefined} />
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground capitalize">{assignment.role}</div>
                        </div>
                      </div>
                      <Badge variant={assignment.status === 'accepted' ? 'default' : 'outline'}>
                        {assignment.status === 'accepted' ? 'Accepted' : 'Pending'}
                      </Badge>
                    </div>
                  ))}
                </div>
        ...
        <TabsContent value="history" className="mt-6 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Timeline of changes and approvals for this booking</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                  ...
                  <div key={item.id} className="flex gap-4">
                    ...
                    <div className="flex-1 pb-4">
                      <div className="flex flex-col gap-1">
                        ...
                        {item.notes && (
                          <div className="text-sm mt-1 text-muted-foreground">{item.notes}</div>
                        )}
                      </div>
                    </div>
                  </div>
      ...
      <Dialog open={showApproveDialog} onOpenChange={setShowApproveDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will approve the activity and notify the assigned staff. Are you sure you want to proceed?
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
      ...
      <Dialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Please provide a reason for rejecting this activity. This information will be shared with the creator.
            </DialogDescription>
          </DialogHeader>
