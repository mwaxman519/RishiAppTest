...
import { X, Search, Home, FileText, Folder, ChevronRight, ExternalLink } from 'lucide-react';
...
export function MobileNav({ tree, isOpen, onClose, currentPath }: MobileNavProps) {
  ...
  const getFlattenedItems = (tree: DocTree, basePath = '/docs') => {
    ...
    Object.entries(tree).forEach(([key, value]) => {
      if (value === null) {
        // It's a file
        const displayTitle = key === 'README.md' || key === 'README.mdx' 
          ? 'Overview' 
          : key.replace(/\.[^/.]+$/, '');
        
        const filePath = key === 'README.md' || key === 'README.mdx'
          ? basePath
  ...
  const getTopSections = () => {
    return Object.entries(tree)
      ...
      .map(([key, value]) => ({
        title: key,
        hasReadme: Object.keys(value as DocTree).some(k => k === 'README.md' || k === 'README.mdx'),
        path: `/docs/${key}`
      }))
  ...
  const getActiveSectionFiles = () => {
    ...
    const sectionTree = Object.entries(tree).find(([key]) => key === activeSection);
    if (!sectionTree) return [];
    
    const [, value] = sectionTree;
    
    return Object.entries(value as DocTree)
      .filter(([key, val]) => val === null && key !== 'README.md' && key !== 'README.mdx')
      .map(([key]) => ({
        title: key.replace(/\.[^/.]+$/, ''),
    ...
    >
      ...
      <div className="sticky top-0 z-10 bg-gradient-to-r from-purple-600 to-teal-500 text-white p-4 shadow-md">
        <div className="flex items-center justify-between">
          <Link href="/docs" className="flex items-center space-x-2" onClick={onClose}>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  <span className="font-medium text-gray-800 dark:text-gray-200">Home</span>
                </Link>
                <Link
