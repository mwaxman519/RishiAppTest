...
export default function AgentCalendar({ 
  ...
  const fetchAvailability = async () => {
    ...
    try {
      ...
      console.log(`Fetching availability for user ${userIdString} from ${formattedStartDate} to ${formattedEndDate}`);
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch availability data (Status: ${response.status})`);
      }
      ...
      const standardizedBlocks = availabilityBlocks.map((block: any) => {
        ...
        return {
          ...
          recurrencePattern: block.recurrencePattern || block.recurrence_pattern,
      ...
      console.log(`Merged ${standardizedBlocks.length} blocks into ${mergedBlocks.length} blocks`);
      
      // Log any merged unavailable blocks for debugging
      const mergedUnavailable = mergedBlocks.filter(block => 
        block.isMerged && block.status === 'unavailable');
      if (mergedUnavailable.length > 0) {
        console.log(`Found ${mergedUnavailable.length} merged unavailable blocks`);
        mergedUnavailable.forEach(block => {
          console.log(`Merged block: ${new Date(block.start).toLocaleTimeString()} - ${new Date(block.end).toLocaleTimeString()}, IDs: ${block.mergedIds?.join(', ')}`);
        });
      }
      
      // Debug all blocks to understand the structure
      console.log("Block structure analysis:");
      const sampleBlocks = mergedBlocks.slice(0, Math.min(5, mergedBlocks.length));
      ...
      const calendarEvents = mergedBlocks.map((block: any) => {
        ...
        let displayTitle = block.isRecurring ? `${block.title} 🔄` : block.title;
        ...
        const blockStart = block.displayStart?.toISOString() || block.start.toISOString();
        const blockEnd = block.displayEnd?.toISOString() || block.end.toISOString();
        ...
        if (block.status === 'unavailable') {
          const startTime = new Date(blockStart).toLocaleTimeString([], {hour: 'numeric', minute: '2-digit'});
          const endTime = new Date(blockEnd).toLocaleTimeString([], {hour: 'numeric', minute: '2-digit'});
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch availability data';
  ...
  useEffect(() => {
    ...
    if (calendarRef.current) {
      console.log('AgentCalendar: Sidebar state changed, updating calendar size');
      setTimeout(() => {
        if (calendarRef.current) {
          const calendarApi = calendarRef.current.getApi();
          calendarApi.updateSize();
  ...
  useEffect(() => {
    if (deletedSeriesGroups.length > 0 && calendarRef.current) {
      console.log(`Cleaning up ${deletedSeriesGroups.length} deleted series from UI`);
      const calendarApi = calendarRef.current.getApi();
  ...
  const throttledFetch = useCallback(async (url: string, options?: RequestInit) => {
    ...
    if (!globalThrottle) {
      try {
        ...
        if (response.status === 502) {
          console.warn(`Received 502 Bad Gateway for ${url}, returning empty data array to prevent errors`);
          return {
      ...
      return new Promise<Response>((resolve) => {
        const executeRequest = async () => {
          try {
            ...
            if (response.status === 502) {
              console.warn(`Received 502 Bad Gateway for queued request to ${url}, returning empty data array`);
              const emptyResponse = {
                ok: true,
                status: 200,
                json: () => Promise.resolve({ success: true, data: [] }),
    ...
    try {
      ...
      while (retries >= 0) {
        try {
          ...
          if (response.status === 502 && retries > 0) {
            console.warn(`Received 502 for ${url}, retrying after delay (${retries} retries left)`);
      ...
      if (!response) {
        console.error(`No response object created for ${url}, returning empty data`);
        return {
          ok: true,
          status: 200,
          json: () => Promise.resolve({ success: true, data: [] }),
      ...
      if (response.status === 502) {
        console.warn(`All retries failed with 502 for ${url}, returning empty data`);
        return {
  ...
  useEffect(() => {
    ...
    async function fetchAvailability() {
      ...
      try {
        ...
        console.log(`AgentCalendar: Fetching for user ${userIdString} from ${formattedStartDate} to ${formattedEndDate}`);
        ...
        if (responseData.success === true && Array.isArray(responseData.data)) {
          ...
          availabilityBlocks = responseData.data;
          console.log(`AgentCalendar: Received ${availabilityBlocks.length} availability blocks (standardized format)`);
        } else if (responseData.success === false) {
          // New format - error response
          console.error(`AgentCalendar: API returned error: ${responseData.error}`);
          ...
          availabilityBlocks = responseData;
          console.log(`AgentCalendar: Received ${availabilityBlocks.length} availability blocks (legacy format)`);
        } else {
          console.error("AgentCalendar: Unexpected response format", responseData);
        }
        ...
        console.log(`AgentCalendar: Using ${availabilityBlocks.length} blocks directly from the API (client-side expansion disabled)`);
        ...
        const blocksForMerge = expandedBlocks.map((block: any) => {
          return {
            ...
            recurrenceGroup: block.recurrenceGroup || block.recurrence_group,
            dayOfWeek: block.dayOfWeek || block.day_of_week,
            title: block.title || (block.status === 'unavailable' ? 'Unavailable' : 'Available'),
        ...
        console.log(`Merged blocks: ${blocksForMerge.length} original → ${mergedBlocks.length} after merging`);
        ...
        const calendarEvents = mergedBlocks.map((block: any) => {
          ...
          const formattedStartTime = new Date(block.start).toLocaleTimeString([], {hour: 'numeric', minute:'2-digit'});
          const formattedEndTime = new Date(block.end).toLocaleTimeString([], {hour: 'numeric', minute:'2-digit'});
          
          // Add time information to merged blocks
          let displayTitle = isRecurring ? `${block.title || 'Available'} 🔄` : block.title || 'Available';
          ...
          const eventId = block._forceId || (isVirtual ? block._virtualId : `block-${block.id}`);
          ...
          return {
            ...
            dataAttributes: {
              ...
              'data-day': new Date(block.startDate || block.start_date).getDay() // Add day of week for alignment
            }
          };
        });
  ...
  }, [userIdString, formattedStartDate, formattedEndDate, onError, onLoadingChange, throttledFetch]);
  ...
  function getStatusColor(status: string): string {
    switch(status) {
      case 'unavailable': return '#DC2626'; // red
      default: return '#00A8A8'; // teal for available
    }
  }
  
  // Show styled alert instead of browser alert
  const showAlert = (message: string, title: string = 'Notification', variant: 'info' | 'warning' | 'error' | 'success' = 'info') => {
  ...
  const showConfirm = (message: string, callback: () => void, title: string = 'Confirm Action') => {
  ...
  const handleModalSave = useCallback(async (
    ...
    try {
      ...
      const requestBody = {
        ...
        recurrenceEndDate: isRecurring && recurrenceEndType === 'date' ? recurrenceEndDate : undefined,
        recurrence_end_date: isRecurring && recurrenceEndType === 'date' ? recurrenceEndDate : undefined, // Include both formats for compatibility
        recurrenceCount: isRecurring && recurrenceEndType === 'count' ? recurrenceCount : undefined,
        recurrence_count: isRecurring && recurrenceEndType === 'count' ? recurrenceCount : undefined // Include both formats for compatibility
      };
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          throw new Error('You are not authorized to create availability blocks');
        } else if (response.status === 403) {
          throw new Error('You do not have permission to create availability blocks');
        } else if (response.status === 400) {
          const errorData = await response.json();
          throw new Error(errorData.error || 'Invalid availability block data');
        } else if (response.status === 500) {
          throw new Error('Server error while creating availability block');
        } else {
          const errorData = await response.json();
          throw new Error(errorData.error || `Failed to create availability block (Status: ${response.status})`);
        }
      }
      ...
      if (responseData.success && responseData.data) {
        ...
        throw new Error('Invalid response format from server after creating availability block');
      }

      if (!newBlock || !newBlock.id) {
        throw new Error('Invalid response from server after creating availability block');
      }
      ...
      if (calendarApi) {
        ...
        if (isRecurring) {
          console.log('Creating recurring instances immediately for newly created block:', newBlock.id);
          ...
          const startDate = new Date(modifiedStartStr || selectedTimeSlot.startStr);
          ...
          console.log(`Created ${occurrencesToGenerate} additional instances for recurring block ${newBlock.id}`);
          ...
          console.log("Refreshing calendar data to show all recurring instances from the server");
  ...
  }, [userId, selectedTimeSlot, calendarRef, setNewBlockStatus, setError, fetchAvailability]);
  ...
  const handleDateSelect = useCallback(async (selectInfo: DateSelectArg) => {
    ...
    try {
      ...
      // @ts-ignore
      const isMonthViewSelection = (selectInfo.view && selectInfo.view.type === 'dayGridMonth');
      ...
      if (isMonthViewSelection) {
        ...
        console.log(`Month view selection detected: ${startDate.toLocaleString()} - ${endDate.toLocaleString()}`);
        ...
        if (isSameDay) {
          ...
          console.log(`Single day selection modified to work hours: ${startDate.toLocaleString()} - ${endDate.toLocaleString()}`);
        } else {
          ...
          console.log(`Multi-day selection modified to be inclusive: ${startDate.toLocaleString()} - ${endDate.toLocaleString()}`);
        }
  ...
  const handleEventClick = useCallback(async (clickInfo: EventClickArg) => {
    ...
    if (isMerged) {
      console.log(`Clicked on merged event with ${mergedIds.length} blocks:`, mergedIds);
  ...
  const handleDeleteBlock = useCallback(async (blockId: number, deleteEntireSeries: boolean): Promise<void> => {
    console.log(`Deleting availability block ${blockId}, deleteEntireSeries=${deleteEntireSeries}`);
    ...
    try {
      ...
      console.log(`Preparing to update calendar state for block deletion: ${blockId}`);
...
Do you want to continue?`;
        ...
        setConfirmCallback(() => {
          ...
          console.log("User confirmed deletion of merged block, deleting all related blocks");
          ...
          setCalendarData(prevData => {
            const filteredData = prevData.filter(event => {
              ...
              if (event.extendedProps?.mergedIds && 
                ...
                const foundMatch = event.extendedProps.mergedIds.some((idValue: any) => Number(idValue) === blockId);
                if (foundMatch) {
                  return false;
                }
              }
            ...
            console.log(`Filtered from ${prevData.length} to ${filteredData.length} events`);
            return filteredData;
          });
          ...
          (async () => {
            for (const deleteRequest of deleteRequests) {
              try {
                ...
                if (!response.ok) {
                  console.warn(`Failed to delete one of the blocks in merged group: ${response.status}`);
                }
              } catch (err) {
            ...
            console.log(`Successfully deleted merged block containing ${mergedIds.length} blocks`);
      ...
      if (deleteEntireSeries && selectedEvent?.isRecurring && selectedEvent?.recurrenceGroup) {
        ...
        console.log(`Filtering out events with recurrenceGroup: ${recurrenceGroup}`);
        ...
        setCalendarData(prevData => {
          const filteredData = prevData.filter(event => {
            ...
            const eventRecurrenceGroup = 
              event.extendedProps?.recurrenceGroup || 
        ...
        setCalendarData(prevData => {
          const filteredData = prevData.filter(event => {
            ...
            const eventBlockId = event.extendedProps?.blockId || event.blockId;
      ...
      if (!response.ok) {
        // Handle specific error cases
        if (response.status === 401) {
          throw new Error('You are not authorized to delete this availability block');
        } else if (response.status === 403) {
          throw new Error('You do not have permission to delete this availability block');
          ...
          try {
            const errorData = await response.json();
            throw new Error(errorData.error || `Failed to delete availability block (Status: ${response.status})`);
          } catch (parseError) {
            throw new Error(`Failed to delete availability block (Status: ${response.status})`);
          }
        }
      }

      console.log(`Successfully deleted availability block ${blockId}${deleteEntireSeries ? ' and all recurring instances' : ''}`);
      ...
      if (deleteEntireSeries && selectedEvent?.isRecurring) {
        try {
          ...
          console.warn('Non-critical error processing deletion response:', parseError);
        }
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error deleting availability block';
      setError(errorMessage);
  ...
  }, [selectedEvent, calendarRef, calendarData, setCalendarData, setLoading, setError, setDeletedSeriesGroups]);
  ...
  return (
    <div className={`${theme === 'dark' ? 'bg-gray-800 text-white dark' : 'bg-white'} rounded-lg shadow p-4 h-full w-full transition-all duration-300 ease-in-out overflow-hidden`}>
      {loading && !calendarData.length ? (
        <div className="flex justify-center items-center h-64">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-teal-500"></div>
        </div>
      ) : (
        <div className={`${theme === 'dark' ? 'fc-theme-dark dark' : ''} calendar-container w-full ${sidebarCollapsed ? 'sidebar-collapsed' : 'sidebar-expanded'}`}>
          ...
          <FullCalendar
            ref={(el) => {
              ...
              if (fullCalendarRef) {
                if (typeof fullCalendarRef === 'function') {
                  ...
                  (fullCalendarRef as React.MutableRefObject<FullCalendar | null>).current = el;
                }
              }
            }}
            ...
            selectAllow={(selectInfo) => {
              ...
              const duration = selectInfo.end.valueOf() - selectInfo.start.valueOf();
              ...
              const isAllDaySelection = startDate.getHours() === 0 && 
            ...
            eventContent={(info) => {
              ...
              const showTime = info.view && info.view.type && info.view.type.includes('timeGrid');
              return {
      ...
      <ConflictDialog
        ...
        onProceed={(mergeStrategy) => {
          console.log(`Proceeding with conflict resolution strategy: ${mergeStrategy}`);
