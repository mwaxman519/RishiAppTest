...
import { getGoogleMapsLoaderConfig, getGoogleMapsOptions } from '@/config/google-maps';
...
export function SimplifiedLocationMap({
  ...
  useEffect(() => {
      ...
      if ('marker' in google.maps && 'AdvancedMarkerElement' in google.maps.marker) {
        ...
        markerElement.innerHTML = `
          <div class="flex items-center justify-center w-8 h-8 text-white rounded-full ${
            location.status === 'pending' ? 'bg-amber-500' :
            location.status === 'rejected' ? 'bg-red-500' :
            location.status === 'inactive' ? 'bg-slate-500' :
            'bg-emerald-500'
        ...
        marker = new google.maps.Marker({
          ...
          icon: {
            ...
            fillColor: location.status === 'pending' ? '#f59e0b' :
                      location.status === 'rejected' ? '#ef4444' : 
                      location.status === 'inactive' ? '#94a3b8' : '#10b981',
            fillOpacity: 1,
            strokeWeight: 2,
            strokeColor: '#ffffff',
            scale: 10
          ...
          <div class="p-2 max-w-[250px]">
            ...
            <div class="mt-1 text-xs">
              <span class="inline-block px-1.5 py-0.5 rounded-full ${
                ...
                location.status === 'inactive' ? 'bg-slate-100 text-slate-800' :
    ...
    if (selectedLocationId) {
      const markerToSelect = newMarkers.find(m => m.location.id === selectedLocationId);
      if (markerToSelect) {
        selectMarker(markerToSelect.marker, markerToSelect.location, markerToSelect.infoWindow);
      }
    }
        ...
        <Card className="p-4 max-w-md">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-2">
              {mapError || 'Failed to load location data. Please try again later.'}
            </p>
            <Button 
              variant="outline" 
              size="sm"
