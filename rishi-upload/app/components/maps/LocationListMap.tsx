...
// Map Container Style
const getContainerStyle = (width: string | number = '100%', height: number = 500) => ({
  width,
  height: `${height}px`,
  borderRadius: '0.5rem'
});
...
export function LocationListMap({ 
  ...
  const handleMarkerClick = (location: MapLocation) => {
    ...
    if (onMarkerClick) {
      ...
      if (mapRef.current && isValidCoordinate(location.latitude, location.longitude)) {
        const latLng = {
          lat: Number(location.latitude),
          lng: Number(location.longitude)
        };
  ...
  return (
    ...
    >
      ...
      {infoOpen && selectedLocation && isValidCoordinate(selectedLocation.latitude, selectedLocation.longitude) && (
        <InfoWindow
          position={{ 
        ...
        >
          ...
          >
            ...
            {selectedLocation.locationType && (
              <div className={`text-xs mt-1 ${mapTheme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
