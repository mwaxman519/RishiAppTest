...
import { pgTable, text, varchar, timestamp, boolean, integer, uuid, jsonb, decimal, pgEnum } from 'drizzle-orm/pg-core';
...
export const brandAgentAssignments = pgTable('brand_agent_assignments', {
  id: uuid('id').primaryKey().defaultRandom(),
  brandAgentId: uuid('brand_agent_id').notNull().references(() => users.id),
...
export const kits = pgTable('kits', {
  id: uuid('id').primaryKey().defaultRandom(),
  template_id: uuid('template_id').notNull().references(() => kitTemplates.id),
  name: varchar('name', { length: 255 }).notNull(),
...
export const insertBrandAgentAssignmentSchema = createInsertSchema(brandAgentAssignments).omit({
  id: true,
  assignedAt: true
});
...
export type InsertBrandAgentAssignment = z.infer<typeof insertBrandAgentAssignmentSchema>;
...
export const insertBrandLocationSchema = createInsertSchema(brandLocations).omit({
  id: true,
  createdAt: true
});
...
export const userOrganizationPreferences = pgTable('user_organization_preferences', {
  id: integer('id').primaryKey(),
  user_id: uuid('user_id').notNull(),
  organization_id: uuid('organization_id').notNull(),
...
export const insertBookingCommentSchema = createInsertSchema(bookingComments).omit({
  id: true,
  created_at: true
});

export const insertEventInstanceSchema = createInsertSchema(eventInstances).omit({
  id: true,
  created_at: true
});
...
export const insertUserOrganizationPreferencesSchema = createInsertSchema(userOrganizationPreferences).omit({
  id: true,
  created_at: true,
  updated_at: true
});
...
export const insertOrganizationRegionSchema = createInsertSchema(organizationRegions).omit({
  id: true,
  created_at: true
});
...
export type UserOrganizationPreferences = typeof userOrganizationPreferences.$inferSelect;
export type InsertUserOrganizationPreferences = z.infer<typeof insertUserOrganizationPreferencesSchema>;
...
export const rolePermissions = {
  ...
  internal_admin: [
    ...
    'approve:bookings', 'reject:bookings'
  ],
  internal_field_manager: [
...
export const insertOrganizationPermissionSchema = createInsertSchema(organizationPermissions).omit({
  id: true,
  createdAt: true,
  updatedAt: true
});
...
// Insert schemas for missing tables
export const insertKitComponentInventorySchema = createInsertSchema(kitComponentInventory).omit({
  id: true,
  createdAt: true,
  updatedAt: true
});
...
export type InsertKitComponentInventory = z.infer<typeof insertKitComponentInventorySchema>;
