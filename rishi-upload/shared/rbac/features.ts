...
export function permissionCovers(
  ...
  if (covering.action === 'manage' && 
    ...
    if (covering.scope) {
      ...
      if (covering.scope === PERMISSION_SCOPES.ORGANIZATION) {
        return [PERMISSION_SCOPES.ORGANIZATION, PERMISSION_SCOPES.OWNED, PERMISSION_SCOPES.ASSIGNED].includes(target.scope as any);
      }
      
      // Region scope covers region, owned, and assigned
      if (covering.scope === PERMISSION_SCOPES.REGION) {
        return [PERMISSION_SCOPES.REGION, PERMISSION_SCOPES.OWNED, PERMISSION_SCOPES.ASSIGNED].includes(target.scope as any);
      }
      
      // Owned scope covers only owned and assigned
      if (covering.scope === PERMISSION_SCOPES.OWNED) {
        return [PERMISSION_SCOPES.OWNED, PERMISSION_SCOPES.ASSIGNED].includes(target.scope as any);
      }
  ...
  if (covering.action === target.action && covering.resource === target.resource) {
