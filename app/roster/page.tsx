...
export default function RosterManagementPage() {
  ...
  const [selectedOrganization, setSelectedOrganization] = useState<string>('00000000-0000-0000-0000-000000000001');
  ...
  const { data: brandAgents = [], isLoading, error } = useQuery({
  ...
  const rosterMetrics = {
    ...
    activeAssignments: brandAgents.reduce((sum: number, agent: BrandAgent) => 
      sum + agent.brandAssignments.length, 0),
    skillsCoverage: brandAgents.reduce((sum: number, agent: BrandAgent) => 
      sum + agent.skills.length, 0)
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Brand Agent Roster</h1>
          ...
          <Dialog open={showAssignmentDialog} onOpenChange={setShowAssignmentDialog}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a new brand agent assignment with role and territory specifications.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setNewAssignment(prev => ({ ...prev, assignmentRole: value }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewAssignment(prev => ({ ...prev, endDate: e.target.value || undefined }))}
                  />
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{rosterMetrics.totalAgents}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{rosterMetrics.activeAssignments}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{rosterMetrics.skillsCoverage}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">Failed to load brand agents</p>
            </div>
            ...
            <div className="space-y-4">
              {filteredAgents.map((agent: BrandAgent) => (
                <div key={agent.id} className="flex items-center space-x-4 p-4 border rounded-lg hover:bg-gray-50">
                  <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center">
                    <span className="text-blue-600 font-semibold">
                      {agent.firstName?.charAt(0) || agent.email.charAt(0).toUpperCase()}
                    </span>
                  </div>
