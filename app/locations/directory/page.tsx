...
import { LocationList, type Location } from '@/components/locations/LocationList';
  ...
  useEffect(() => {
    if (locationsError) {
      toast({
        ...
        description: locationsError instanceof Error 
  ...
  return (
    <div className="container mx-auto py-8 space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Location Directory</h1>
      ...
      {metadataData && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="text-3xl font-bold">{metadataData.stats.total}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
            ...
            <CardContent>
              <div className="text-3xl font-bold">{metadataData.stats.active}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
            ...
            <CardContent>
              <div className="text-3xl font-bold">{metadataData.metadata.states.length}</div>
            </CardContent>
          </Card>
        </div>
      )}
      ...
      <Tabs defaultValue="list" value={activeView} onValueChange={setActiveView} className="space-y-4">
        ...
        <TabsContent value="map" className="space-y-4">
          {isLocationsLoading ? (
            <div className="flex justify-center items-center h-64">
              <Loader2 className="h-8 w-8 animate-spin text-primary" />
              <span className="ml-2 text-muted-foreground">Loading locations map...</span>
            </div>
          ) : locationsError ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              <XCircle className="h-8 w-8 text-destructive mb-2" />
