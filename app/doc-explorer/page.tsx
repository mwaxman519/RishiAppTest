...
export default function DocExplorerPage() {
  ...
  async function fetchDocPath(path: string) {
    try {
      ...
      const response = await fetch(`/api/doc-path?path=${encodeURIComponent(path)}`);
    ...
    if (!docData.exists) {
      return (
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-md text-amber-700">
          ...
          <p className="text-sm">The requested documentation path was not found.</p>
      ...
      return (
        <div className="space-y-4">
          <div className="flex items-center space-x-2 text-sm mb-4">
            ...
            {currentPath.split('/').filter(Boolean).map((segment, index, segments) => (
              <React.Fragment key={index}>
                <button
                  onClick={() => fetchDocPath(segments.slice(0, index + 1).join('/'))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {docData.files && docData.files.map((file: string, index: number) => {
              const isDirectory = !file.includes('.');
              
              return (
                <div key={index} className="flex items-center">
                  <button
                    onClick={() => isDirectory ? navigateToDirectory(file) : fetchDocPath(`${currentPath}/${file}`)}
