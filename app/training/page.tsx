...
import { GraduationCap, Play, BookOpen, CheckCircle, Clock, Award, Star, Download } from 'lucide-react';
...
export default function TrainingPage() {
  const completedTraining = [...requiredTraining, ...optionalTraining].filter(t => t.status === 'completed');
  const overallProgress = Math.round((completedTraining.length / (requiredTraining.length + optionalTraining.length)) * 100);
                      ...
                      <div>
                        ...
                        {cert.status === 'active' ? (
                          <div className="text-sm text-muted-foreground">
                            Earned: {cert.earnedDate} • Expires: {cert.expiryDate}
                          </div>
                        ) : (
                          <div className="text-sm text-muted-foreground">
