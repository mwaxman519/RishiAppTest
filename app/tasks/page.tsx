...
interface Task {
  ...
  title: string;
  description: string;
  type: 'event_report' | 'mileage_submission' | 'clock_in_out' | 'training_required' | 'logistics_kit' | 'shadowing' | 'personnel_update' | 'photo_submission' | 'compliance_check';
...
export default function TasksPage() {
    ...
    if (selectedTab !== 'all') {
      filtered = filtered.filter(task => {
        switch (selectedTab) {
          ...
          case 'overdue':
            return task.status === 'overdue' || (task.status !== 'completed' && isOverdue(task.dueDate));
          default:
            return true;
        }
      });
  ...
  const getTabCounts = () => {
    ...
    const overdue = tasks.filter(t => t.status === 'overdue' || (t.status !== 'completed' && isOverdue(t.dueDate))).length;
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage assigned tasks including reports, mileage, training, logistics, shadowing, and personnel updates.
            Tasks can be assigned by Client Users, Field Managers, and Internal Admins.
          </p>
        </div>
        <Button>
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-gray-600 mb-6 max-w-md mx-auto">
                Connect with authentic data sources to display real task assignments. 
                Tasks will appear here once proper API integration is established.
              </p>
