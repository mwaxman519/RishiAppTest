 ...
 */
export async function createOrganization(data: CreateOrganizationParams): Promise<Organization> {
 ...
 */
export async function updateOrganization(id: string, data: UpdateOrganizationParams): Promise<Organization> {
 ...
 */
export async function searchOrganizations(filters: OrganizationFilters): Promise<Organization[]> {
  ...
  const response = await apiRequest('GET', `/api/organizations/search?${params.toString()}`);
  return response.json();
}
 ...
 */
export async function addUserToOrganization(params: OrganizationUserParams): Promise<OrganizationUser> {
  const response = await apiRequest('POST', '/api/organizations/users', params);
  
  // Invalidate organization users cache
  queryClient.invalidateQueries({ queryKey: ['/api/organizations', params.organizationId, 'users'] });
 ...
 */
export async function updateUserRole(organizationId: string, userId: string, roleId: string): Promise<OrganizationUser> {
  const response = await apiRequest('PATCH', `/api/organizations/${organizationId}/users/${userId}`, { roleId });
  
  // Invalidate organization users cache
  queryClient.invalidateQueries({ queryKey: ['/api/organizations', organizationId, 'users'] });
  queryClient.invalidateQueries({ queryKey: ['/api/organizations/user'] });
...
export async function removeUserFromOrganization(organizationId: string, userId: string): Promise<void> {
  await apiRequest('DELETE', `/api/organizations/${organizationId}/users/${userId}`);
...
export async function getOrganizationUsers(organizationId: string): Promise<OrganizationUser[]> {
  const response = await apiRequest('GET', `/api/organizations/${organizationId}/users`);
  return response.json();
}
 ...
 */
export async function switchOrganization(organizationId: string): Promise<UserOrganization> {
  const response = await apiRequest('POST', '/api/auth/switch-organization', { organizationId });
