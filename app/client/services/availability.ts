...
export class AvailabilityServiceClient {
   ...
   */
  async getAvailabilityWindows(params: AvailabilityQueryParams): Promise<AvailabilityWindow[]> {
    try {
   ...
   */
  async getUserAvailability(userId: string, startDate: string, endDate: string): Promise<AvailabilityTimeSlot[]> {
    try {
  ...
  async getAvailabilityWindowById(id: string): Promise<AvailabilityWindow> {
    try {
      ...
      throw new ApiError(`Failed to fetch availability window with ID ${id}`, 500);
    }
  }
   ...
   */
  async createAvailabilityWindow(userId: string, availabilityData: AvailabilityWindowDTO): Promise<AvailabilityWindow> {
    try {
   ...
   */
  async updateAvailabilityWindow(id: string, availabilityData: Partial<AvailabilityWindowDTO>): Promise<AvailabilityWindow> {
    try {
      const response = await fetch(`/api/availability/window/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      throw new ApiError(`Failed to update availability window with ID ${id}`, 500);
    }
  }
  ...
  async deleteAvailabilityWindow(id: string): Promise<boolean> {
    try {
      ...
      throw new ApiError(`Failed to delete availability window with ID ${id}`, 500);
    }
  }
   ...
   */
  async checkStaffAvailability(userIds: string[], startDate: string, endDate: string): Promise<Record<string, boolean>> {
    try {
