...
export class LocationsClientService {
  ...
  async getAllLocations(filters = {}): Promise<LocationDTO[]> {
    try {
      ...
      const response = await api.get(`/api/locations${queryString ? `?${queryString}` : ''}`);
   ...
   * Reject a location
   */
  async rejectLocation({ id, reason }: { id: string, reason?: string | undefined }): Promise<LocationDTO> {
    try {
