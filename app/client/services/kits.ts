...
class KitsClientService {
   ...
   */
  async getAllTemplates(filters: Record<string, any> = {}): Promise<KitTemplateDTO[]> {
  ...
  async getTemplateById(id: number): Promise<KitTemplateDTO> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || `Failed to fetch kit template with ID ${id}`);
    }
   ...
   */
  async updateTemplate(id: number, data: UpdateKitTemplateParams): Promise<KitTemplateDTO> {
    const response = await fetch(`${this.apiUrl}/templates/${id}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || `Failed to update kit template with ID ${id}`);
    }
   ...
   */
  async approveTemplate(params: ApproveKitTemplateParams): Promise<KitTemplateDTO> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || `Failed to approve kit template with ID ${id}`);
    }
   ...
   */
  async rejectTemplate(params: RejectKitTemplateParams): Promise<KitTemplateDTO> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || `Failed to reject kit template with ID ${id}`);
    }
   ...
   */
  async getActivityKits(filters: Record<string, any> = {}): Promise<ActivityKitDTO[]> {
    const queryParams = new URLSearchParams();
    Object.entries(filters).forEach(([key, value]) => {
      if (value !== undefined && value !== null) {
        queryParams.append(key, String(value));
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || 'Failed to fetch activity kit assignments');
    }
  ...
  async getActivityKitById(id: string): Promise<ActivityKitDTO> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || `Failed to fetch activity kit with ID ${id}`);
    }
   ...
   */
  async createActivityKit(data: CreateActivityKitParams): Promise<ActivityKitDTO> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || 'Failed to create activity kit assignment');
    }
   ...
   */
  async updateActivityKit(id: string, data: UpdateActivityKitParams): Promise<ActivityKitDTO> {
    const response = await fetch(`${this.apiUrl}/activity-kits/${id}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || `Failed to update activity kit with ID ${id}`);
    }
  ...
  async deleteActivityKit(id: string): Promise<void> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || `Failed to delete activity kit with ID ${id}`);
    }
  }
}
