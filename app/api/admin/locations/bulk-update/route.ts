...
export async function POST(req: NextRequest) {
  ...
  const userHasPermission = hasPermission(session, { action: 'update', resource: 'locations' });
  ...
  try {
    ...
    const eventPromises = locationIds.map(locationId => {
      const locationBefore = locationsBeforeUpdate.find((loc: { id: string; status?: string; locationType?: string }) => loc.id === locationId);
      ...
      if (updates.locationType && locationBefore?.locationType !== updates.locationType) {
    ...
    return NextResponse.json(
      { error: 'Failed to update locations', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}