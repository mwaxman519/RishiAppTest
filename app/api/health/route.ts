...
// Updated for Azure Static Web Apps deployment - triggers workflow
...
export async function ready(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ status: 'not ready', error: 'Health check failed' }, { status: 503 });
  }
}
...
export async function live(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ status: isAlive ? 'alive' : 'dead' }, { 
