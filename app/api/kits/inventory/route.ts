...
import { z } from "zod";
import { db } from '@db';
import { kitComponentInventory, insertKitComponentInventorySchema, kits } from '@shared/schema';
  ...
  try {
    ...
    if (lowStock === 'true') {
      ...
      query = query.where(and(
        ...
        kitComponentInventory.quantity.lte(kitComponentInventory.lowThreshold)
      ));
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existingInventory) {
      ...
      const [updated] = await db.update(kitComponentInventory)
        .set({
      ...
      const [inserted] = await db.insert(kitComponentInventory)
        .values({
          ...validatedData,
          lastRestockedDate: validatedData.lastRestockedDate || new Date()
        })
