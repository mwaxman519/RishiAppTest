...
import { AvailabilityConflict, AvailabilityDTO, ConflictCheckResponse } from '../../../services/availability/models';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const excludeBlockId = excludeBlockIdStr ? parseInt(excludeBlockIdStr) : undefined;
    
    if (isNaN(userIdNum) || (excludeBlockIdStr && isNaN(excludeBlockId as number))) {
      const errorResponse: ConflictCheckResponse = {
        success: false,
    ...
    try {
      console.log(`[conflicts/route GET] Checking conflicts for userId=${userIdNum}, startDate=${startDate}, endDate=${endDate}`);
      ...
      console.log(`[conflicts/route GET] Found ${conflictingBlocks.length} potential conflicts`);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    try {
      ...
      console.log(`[conflicts/route POST] Checking conflicts for userId=${userIdNum}, startDate=${startDate}, endDate=${endDate}`);
      ...
      console.log(`[conflicts/route POST] Found ${conflictingBlocks.length} potential conflicts`);
