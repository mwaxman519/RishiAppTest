...
export async function GET() {
  ...
  for (const dir of directoriesToCheck) {
    try {
      ...
      const stats = exists ? { 
        ...
        fileCount: exists && statSync(dir).isDirectory() ? readdirSync(dir).length : 0,
        firstFewFiles: exists && statSync(dir).isDirectory() ? readdirSync(dir).slice(0, 5) : []
      } : null;
  ...
  const testDocumentPaths = [
    ...
    path.join(process.cwd(), 'public', 'Docs', 'test-folder', 'production-test-doc.md'),
    path.join(process.cwd(), '.next', 'standalone', 'Docs', 'test-folder', 'production-test-doc.md'),
  ...
  for (const filePath of testDocumentPaths) {
    try {
      ...
      const content = exists ? (await fs.readFile(filePath, 'utf-8')).substring(0, 100) + '...' : null;
      const stats = exists ? { 
  ...
  for (const scriptPath of documentationScripts) {
    try {
      ...
      const stats = exists ? { 
        ...
        isExecutable: exists && statSync(scriptPath).mode & 0o111 ? true : false
      } : null;
  ...
  results.environmentVariables = {
    ...
    DATABASE_URL: process.env.DATABASE_URL ? 'Exists (not displayed for security)' : 'Not set'
  };
