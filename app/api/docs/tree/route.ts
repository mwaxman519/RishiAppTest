...
export async function GET() {
  try {
    ...
    console.log(`[DOCS TREE API] Successfully retrieved document tree with ${Object.keys(tree).length} root entries`);
    ...
    console.error('[DOCS TREE API] Error details:', error instanceof Error ? error.stack : 'Unknown error');
 ...
 */
async function getDocTree(dir?: string, currentPath: string = '/'): Promise<DocTree> {
  // If dir is not provided, get the docs directory
  if (!dir) {
    dir = await getDocsDirectory();
    console.log(`[DOCS TREE] Starting tree scan at root docs directory: ${dir}`);
  }
  ...
  try {
    ...
    console.log(`[DOCS TREE] Found ${allItems.length} items in ${dir} (path: ${currentPath})`);
    
    const filteredItems = allItems.filter(item => !item.startsWith('.'));
    console.log(`[DOCS TREE] After filtering hidden items: ${filteredItems.length} items remain`);
    ...
    console.log(`[DOCS TREE] Directory ${dir} contains ${directories.length} directories and ${files.length} markdown files`);
    ...
    if (dir.includes('api/endpoints') || currentPath.includes('api/endpoints')) {
      console.log(`[DOCS TREE] 🔍 ENDPOINTS DIRECTORY SCAN - ${dir}`);
      console.log(`[DOCS TREE] 🔍 Found ${files.length} endpoint files: ${files.join(', ')}`);
    }
    ...
    console.log(`[DOCS TREE] Tree for ${dir} contains ${Object.keys(tree).length} entries`);
    ...
    if (currentPath === '/' && Object.keys(tree).length < 3) {
      ...
      if (Object.keys(tree).length === 0) {
        throw new Error(`Empty document tree - no documents found in directory: ${dir}`);
      }
    }
    
    // Log warning for endpoints directory without fallback
    if (currentPath.includes('/api/endpoints') && files.length < 3) {
      console.warn(`[DOCS TREE] ⚠️ Endpoints directory has very few files (${files.length})`);
    ...
    console.error(`[DOCS TREE] ❌ Error building document tree for ${dir}:`, detailedError);
    throw detailedError;
  }
}
