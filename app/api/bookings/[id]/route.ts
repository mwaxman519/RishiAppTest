...
export async function GET(
  ...
  try {
    ...
    if (!isAdmin && session.user.organizationId) {
      conditions.push(eq(bookings.clientOrganizationId, session.user.organizationId));
    }
...
export async function PATCH(
  ...
  try {
    ...
    if (!existingBooking) {
      return NextResponse.json({ error: "Booking not found or you don't have permission to update it" }, { status: 404 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!existingBooking) {
      return NextResponse.json({ error: "Booking not found or you don't have permission to delete it" }, { status: 404 });
    }
