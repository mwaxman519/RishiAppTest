...
import { organizations, regions, organizationRegions } from '../../../../../shared/schema';
  ...
  try {
    ...
    if (user.id === '123e4567-e89b-12d3-a456-426614174000') {
      ...
      return NextResponse.json({
        regions: [
          { id: 1, name: 'Northern California', code: 'CA-N', type: 'state', is_primary: true },
          { id: 2, name: 'Southern California', code: 'CA-S', type: 'state', is_primary: false },
        ]
      });
    }
    ...
    return NextResponse.json({ error: 'Failed to fetch organization regions' }, { status: 500 });
  }
}