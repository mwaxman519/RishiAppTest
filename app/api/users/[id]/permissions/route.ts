...
export async function GET(
  ...
  try {
    ...
    const userRecordResult = await db.select()
      .from(users)
    ...
    if (!roleDefinition) {
      return NextResponse.json({ error: "Invalid role configuration" }, { status: 500 });
    }
    ...
    const permissions = Object.entries(permissionsMap).map(([permission, granted]) => ({
      permission,
      granted
    }));
