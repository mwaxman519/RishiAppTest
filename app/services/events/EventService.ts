...
import { db } from '../../db';
import { events, eventAssignments, activities, activityAssignments, eventStateTransitions } from '@/app/lib/schema';
...
export class EventService {
   ...
   */
  async getEvents(options: EventQueryOptions, organizationContext: OrganizationContext) {
    try {
      ...
      if (organizationContext.restrictToOrganization) {
        query = query.where(eq(systemEvents.organizationId, organizationContext.organizationId));
      }
   ...
   * Respects organization boundaries based on context
   */
  async getEventById(eventId: string, organizationContext: OrganizationContext) {
    try {
      let query = db.select()
        .from(events)
   ...
   */
  async createEvent(eventData: any, organizationContext: OrganizationContext, currentUserId: string) {
    try {
      ...
      // Create the event
      const [createdEvent] = await db.insert(systemEvents).values({
        ...eventData,
        createdById: currentUserId,
        // Ensure organization is set from context if not provided
        organizationId: eventData.organizationId || organizationContext.organizationId
      }).returning();
   ...
   * Enforces organization boundaries based on context
   */
  async updateEvent(eventId: string, eventData: any, organizationContext: OrganizationContext) {
    try {
      // Get the existing event to check permissions
      const existingEvent = await this.getEventById(eventId, organizationContext);
   ...
   * Enforces organization boundaries based on context
   */
  async cancelEvent(eventId: string, reason: string, organizationContext: OrganizationContext, currentUserId: string) {
    try {
      // Get the existing event to check permissions
      const existingEvent = await this.getEventById(eventId, organizationContext);
      ...
      return await db.transaction(async (tx) => {
        // Update event status
        const [cancelledEvent] = await tx.update(events)
          .set({
            status: 'canceled',
            updatedAt: new Date()
          })
        ...
        const eventActivities = await tx.select({ id: activities.id })
          .from(activities)
          .where(eq(activities.eventId, eventId));
        ...
        if (eventActivities.length > 0) {
          ...
          await tx.update(activityAssignments)
            .set({
              status: 'canceled',
              updatedAt: new Date()
            })
    ...
    try {
      ...
      return await db.transaction(async (tx) => {
        ...
        for (const assignment of staffAssignments) {
          ...
          const [eventAssignment] = await tx.insert(eventAssignments)
            .values({
              eventId,
          ...
          // Get all activities for this event
          const eventActivities = await tx.select()
            .from(activities)
            .where(eq(activities.eventId, eventId));
          ...
          if (assignment.activityIds && assignment.activityIds.length > 0) {
            ...
            const validActivityIds = assignment.activityIds.filter(id => eventActivityIds.has(id));
            
            // Create activity assignments
            if (validActivityIds.length > 0) {
              await tx.insert(activityAssignments)
                .values(
   ...
   * Enforces organization boundaries based on context
   */
  async getEventStaff(eventId: string, organizationContext: OrganizationContext) {
    try {
      // Get the existing event to check permissions
  ...
  async checkStaffAvailability(
    ...
    try {
      ...
      let query = db.select()
        .from(events)
        .innerJoin(eventAssignments, eq(systemEvents.id, eventAssignments.eventId))
        .where(
          and(
