...
import { mapDatabaseUserToCredentials, mapDatabaseUserToProfile } from '../../lib/user-repository-helpers';
...
export async function createUser(userData: CreateUserRequest): Promise<UserResponse> {
  try {
    console.log('[userService] createUser - Starting with username:', userData.username);
    ...
    try {
      ...
      console.log('[userService] User created successfully:', { id: newUser.id, username: newUser.username });
    ...
    return { 
      success: false, 
      error: error instanceof Error ? `Failed to create user: ${error.message}` : 'Failed to create user' 
    };
  }
}
 ...
 * Update a user
 */
export async function updateUser(id: string, userData: UpdateUserRequest): Promise<UserResponse> {
  try {
...
export async function getUsersByRole(role: UserRole): Promise<UsersResponse> {
  try {
    ...
    const filteredUsers = workers.filter((user: UserProfile) => user.role === role);
    console.log(`[userService] Found ${filteredUsers.length} users with role ${role}`);
 ...
 */
export async function getUserWithCredentials(username: string): Promise<ServiceResponse<UserWithCredentials>> {
  try {
