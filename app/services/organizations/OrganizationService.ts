...
export class OrganizationService {
  ...
  async getAllOrganizations(
    ...
    try {
      ...
      const organizations = await this.repository.findAll(accessControlledFilters);
  ...
  async getOrganizationById(
    ...
    try {
      ...
      if (!this.hasOrganizationAccess(organization, requestingUserId, userRole)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
      ...
      console.error(`OrganizationService.getOrganizationById error for ID ${id}:`, error);
      return {
        success: false,
  ...
  async updateOrganization(
    ...
    try {
      ...
      // Check permissions
      if (!this.hasUpdatePermissions(existingOrganization, updatedBy, userRole)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
      ...
      // Update organization
      const updatedOrganization = await this.repository.update(id, validatedData);
      ...
      console.error(`OrganizationService.updateOrganization error for ID ${id}:`, error);
      return {
        success: false,
  ...
  async deleteOrganization(
    ...
    try {
      ...
      // Check permissions
      if (!this.hasDeletePermissions(existingOrganization, deletedBy, userRole)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
      ...
      console.error(`OrganizationService.deleteOrganization error for ID ${id}:`, error);
      return {
        success: false,
