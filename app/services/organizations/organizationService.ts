...
export class OrganizationService {
  ...
  async updateOrganization(id: string, data: UpdateOrganizationParams): Promise<Organization> {
    ...
    if (data.type === OrganizationType.CLIENT && !data.tier && !existingOrg.tier) {
      throw new Error('Client tier is required for client organizations');
    }
  ...
  async addUserToOrganization(params: OrganizationUserParams): Promise<OrganizationUser> {
    const organization = await this.repository.getOrganizationById(params.organizationId);
    ...
    if (params.isDefault) {
      await this.repository.setDefaultOrganization(params.userId, params.organizationId);
    }
  ...
  async setDefaultOrganization(userId: string, organizationId: string): Promise<void> {
    ...
    if (!isMember) {
      throw new Error(`User is not a member of organization with ID ${organizationId}`);
    }
   ...
   */
  async getUserDefaultOrganization(userId: string): Promise<UserOrganization | null> {
    const userOrgs = await this.repository.getUserOrganizations(userId);
   ...
   */
  async switchUserOrganization(userId: string, organizationId: string): Promise<UserOrganization> {
