...
export interface ExpenseEvent {
  id: string;
  type: 'expense.submitted' | 'expense.approved' | 'expense.rejected' | 'expense.updated' | 'expense.deleted';
  payload: any;
  timestamp: Date;
  userId: string;
...
class ExpenseEventService extends EventEmitter {
  ...
  private async notifyApprovalService(event: ExpenseSubmittedEvent): Promise<void> {
    ...
    console.log(`Notifying approval service for expense: ${event.payload.expenseId}`);
  }

  private async notifyPayrollService(event: ExpenseApprovedEvent): Promise<void> {
    // Add to payroll processing
    console.log(`Notifying payroll service for approved expense: ${event.payload.expenseId}`);
  }
