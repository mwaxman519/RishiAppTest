...
import type { ExpenseData, ExpenseFilters, ExpenseSummary, ServiceResponse } from './models';
...
export class ExpenseRepository {
   ...
   */
  async findMany(filters: ExpenseFilters): Promise<ServiceResponse<{ expenses: ExpenseData[]; total: number; page: number; limit: number }>> {
    try {
   ...
   * Update expense
   */
  async update(id: string, updateData: Partial<any>): Promise<ServiceResponse<ExpenseData>> {
    try {
   ...
   */
  async getSummary(filters: Partial<ExpenseFilters>): Promise<ServiceResponse<ExpenseSummary>> {
    try {
      const conditions: any[] = [];
  ...
  async exists(id: string): Promise<boolean> {
    try {
      ...
      console.error('[ExpenseRepository] Error checking expense existence:', error);
      return false;
    }
  }
   ...
   */
  async findByAgentId(agentId: string, filters?: Partial<ExpenseFilters>): Promise<ServiceResponse<ExpenseData[]>> {
