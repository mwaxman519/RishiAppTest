...
export class LocalEventBus implements EventPublisher, EventSubscriber {
  ...
  async publish<E extends AppEvent>(event: E, payload: EventPayload[E]): Promise<boolean> {
    try {
      ...
      if (event === 'availability.created' || event === 'availability.updated') {
        ...
        if (availabilityPayload && 
          ...
          availabilityPayload.startDate = availabilityPayload.startDate.toISOString();
        }
        if (availabilityPayload && 
            'endDate' in availabilityPayload && 
            availabilityPayload.endDate instanceof Date) {
          availabilityPayload.endDate = availabilityPayload.endDate.toISOString();
        }
      }
