...
export class ServiceHealthMonitor {
  ...
  private alertConfig: AlertConfig = {
    ...
    handler: (serviceId, status, message) => {
      console.warn(`HEALTH ALERT: Service ${serviceId} is ${status}: ${message}`);
    },
  ...
  registerService(
    ...
    if (service.circuitBreaker) {
      ...
      this.updateHealthFromCircuit(service.id, service.circuitBreaker.getState());
    }
   ...
   */
  private startHealthCheck(serviceId: string, options: HealthCheckOptions): void {
   ...
   */
  private async checkServiceHealth(serviceId: string, options: HealthCheckOptions): Promise<void> {
      ...
      if (service.healthCheck) {
        ...
        isHealthy = await Promise.race([
          ...
          new Promise<boolean>((_, reject) => {
            setTimeout(() => reject(new Error('Health check timed out')), options.timeout);
          })
        ]);
   ...
   */
  private updateHealthFromCircuit(serviceId: string, circuitState: CircuitState): void {
    const health = this.healthStatus.get(serviceId);
    if (!health) return;
  ...
  private updateHealthStatus(serviceId: string, status: HealthStatus): void {
    ...
    if (this.alertConfig.enabled && 
      ...
      this.sendAlert(serviceId, status, health.message || 'Service health status changed');
    }
  }
   ...
   */
  private isStatusDegradation(oldStatus: HealthStatus, newStatus: HealthStatus): boolean {
   ...
   */
  private sendAlert(serviceId: string, status: HealthStatus, message: string): void {
  ...
  getSystemHealth(): { 
    ...
    const degradedCount = Array.from(this.healthStatus.values())
      .filter(health => health.status === HealthStatus.DEGRADED).length;
    
    const unhealthyCount = Array.from(this.healthStatus.values())
      .filter(health => health.status === HealthStatus.UNHEALTHY).length;
    
    const unknownCount = Array.from(this.healthStatus.values())
      .filter(health => health.status === HealthStatus.UNKNOWN).length;
   ...
   */
  setServiceStatus(serviceId: string, status: HealthStatus, message?: string): void {
    const health = this.healthStatus.get(serviceId);
    if (!health) return;
    ...
    if (this.alertConfig.enabled && 
        this.isStatusDegradation(oldStatus, status)) {
      this.sendAlert(serviceId, status, health.message || 'Service health status manually changed');
    }
  }
