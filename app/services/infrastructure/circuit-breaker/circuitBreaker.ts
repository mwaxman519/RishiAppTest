...
export class CircuitBreaker<T = any> {
  ...
  constructor(
    ...
    if (options.monitorInterval) {
      this.monitorInterval = setInterval(() => this.checkHealth(), options.monitorInterval);
    }
  }
  ...
  async execute(...args: any[]): Promise<T> {
    ...
    if (this.state === CircuitState.OPEN) {
      ...
      if (this.fallback) {
        ...
        const error = new Error(`Circuit for service '${this.service}' is OPEN`);
        return this.fallback(error, ...args);
      }
      
      throw new Error(`Service '${this.service}' is unavailable (circuit OPEN)`);
    }
    ...
    try {
      ...
      if (this.options.timeout) {
        result = await Promise.race([
          ...
          new Promise<T>((_, reject) => {
            setTimeout(() => reject(new Error(`Service '${this.service}' timed out after ${this.options.timeout}ms`)), 
      ...
      // Use fallback if provided
      if (this.fallback) {
        return this.fallback(error instanceof Error ? error : new Error(String(error)), ...args);
      }
  ...
  private transitionTo(newState: CircuitState): void {
    ...
    console.log(`Circuit '${this.service}' changed from ${oldState} to ${newState}`);
  }
