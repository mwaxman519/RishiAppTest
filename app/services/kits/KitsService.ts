...
export class KitsService {
  ...
  async createKit(
    ...
    try {
      ...
      // Create kit
      const kit = await this.repository.create(validatedData, organizationId, createdBy);
      
      // Publish kit created event
      await kitEventPublisher.publishKitCreated(
        kit,
        createdBy,
        organizationId
      );
  ...
  async updateKit(
    ...
    try {
      ...
      // Check permissions
      if (!this.hasUpdatePermissions(existingKit, updatedBy, userRole, organizationId)) {
        return {
          success: false,
          error: 'Access denied',
          code: 'ACCESS_DENIED'
  ...
  async checkoutKit(
    ...
    try {
      ...
      if (![KitStatus.AVAILABLE, KitStatus.ASSIGNED].includes(existingKit.status)) {
        return {
          success: false,
  ...
  async checkinKit(
    ...
    try {
      ...
      const newStatus = checkin.condition === 'damaged' ? KitStatus.DAMAGED : KitStatus.AVAILABLE;
  ...
  private hasKitAccess(
    ...
    switch (userRole) {
      case 'brand_agent':
        ...
        return kit.assignedTo === requestingUserId || kit.status === KitStatus.AVAILABLE;
