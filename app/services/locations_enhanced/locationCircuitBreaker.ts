...
export class LocationCircuitBreaker {
  ...
  async executeGeocodingOperation<T>(operation: () => Promise<T>): Promise<T> {
    const circuit = circuitBreakerRegistry.get(LOCATION_SERVICE_CIRCUITS.GEOCODING);
    if (!circuit) {
  ...
  async executeCreateOperation<T>(operation: () => Promise<T>): Promise<T> {
    const circuit = circuitBreakerRegistry.get(LOCATION_SERVICE_CIRCUITS.CREATE);
    if (!circuit) {
  ...
  async executeUpdateOperation<T>(operation: () => Promise<T>): Promise<T> {
    const circuit = circuitBreakerRegistry.get(LOCATION_SERVICE_CIRCUITS.UPDATE);
    if (!circuit) {
  ...
  async executeApprovalOperation<T>(operation: () => Promise<T>): Promise<T> {
    const circuit = circuitBreakerRegistry.get(LOCATION_SERVICE_CIRCUITS.APPROVAL);
    if (!circuit) {
  ...
  async executeDeleteOperation<T>(operation: () => Promise<T>): Promise<T> {
    const circuit = circuitBreakerRegistry.get(LOCATION_SERVICE_CIRCUITS.DELETE);
    if (!circuit) {
  ...
  getCircuitStatus(): Record<string, any> {
    return Object.values(LOCATION_SERVICE_CIRCUITS).reduce((status, circuitName) => {
