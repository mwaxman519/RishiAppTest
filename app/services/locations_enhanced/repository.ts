...
import { LocationDTO, CreateLocationParams, UpdateLocationParams } from './models';
...
export class LocationRepository extends BaseLocationRepository {
  ...
  async updateGeocodingMetadata(
    ...
    try {
      ...
      console.error(`Error updating geocoding metadata for location ${id}:`, error);
      throw new Error(`Failed to update geocoding metadata: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
      ...
      try {
        ...
        if (placeIdMatch || formattedAddressMatch || accuracyMatch) {
          baseDTO.geocodingData = {
            ...
            formattedAddress: formattedAddressMatch ? formattedAddressMatch[1].trim() : null,
            geocodingAccuracy: accuracyMatch ? accuracyMatch[1].trim() : null,
            lastGeocodedAt: lastGeocodedMatch ? lastGeocodedMatch[1].trim() : null
          };
        }
      } catch (error) {
