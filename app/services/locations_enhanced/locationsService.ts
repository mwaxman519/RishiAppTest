...
export class EnhancedLocationsService extends BaseLocationsService {
    ...
    try {
      ...
      if (data.validateAddress) {
        await locationCircuitBreaker.executeGeocodingOperation(async () => {
          ...
          if (!isValid) {
            throw new Error('Invalid address: Could not geocode the provided address');
          }
      ...
      const createdLocation = await locationCircuitBreaker.executeCreateOperation(async () => {
      ...
      try {
        ...
        console.error(`Warning: Could not geocode location ${createdLocation.id}:`, geocodeError);
  ...
  async updateLocation(
    ...
    try {
      ...
      if (addressChanged && data.validateAddress) {
        ...
        const currentLocation = await locationCircuitBreaker.executeFetchOperation(async () => {
        ...
        const mergedAddress = this.getMergedAddress(currentLocation, addressData);
      ...
      const updatedLocation = await locationCircuitBreaker.executeUpdateOperation(async () => {
        return await super.updateLocation(id, baseData as any) as LocationDTO;
      });
      ...
      if (addressChanged) {
        try {
          ...
          console.error(`Warning: Could not geocode updated location ${id}:`, geocodeError);
      ...
      console.error(`EnhancedLocationsService.updateLocation error for ID ${id}:`, error);
      throw error;
    }
  }
  ...
  private async geocodeAndUpdateLocation(
    ...
    // Geocode the address
    const geocodeResult = await this.geocodingService.getSimpleGeocodingResult(fullAddress);
    ...
    const updateData: Pick<UpdateLocationParams, 'latitude' | 'longitude' | 'address1'> = {
  ...
  private getMergedAddress(
    ...
    const address2 = updates.address2 !== undefined ? updates.address2 : current.address2;
    
    return {
      address1: updates.address1 !== undefined ? updates.address1 : current.address1,
      ...(address2 ? { address2 } : {}),
      city: updates.city !== undefined ? updates.city : current.city,
      stateId: updates.stateId !== undefined ? updates.stateId : current.stateId,
      zipcode: updates.zipcode !== undefined ? updates.zipcode : current.zipcode
    };
  }
  ...
  private formatFullAddress(address: Record<string, any>): string {
    ...
    parts.push(`${address.city}, ${address.stateId || ''} ${address.zipcode || ''}`);
   ...
   */
  private extractBaseLocationData(data: UpdateLocationParams): Partial<UpdateLocationParams> {
    // Explicitly pick only the fields we need without enhanced service fields
    const {
      name, type, address1, address2, city, stateId, zipcode,
      phone, email, website, contactName, contactEmail, contactPhone, notes,
      latitude, longitude, status, rejectionReason
    } = data;
    ...
    if (rejectionReason !== undefined) baseData.rejectionReason = rejectionReason;
  ...
  async approveLocation(params: ApproveLocationParams): Promise<LocationDTO> {
    try {
      ...
      const existingLocation = await locationCircuitBreaker.executeFetchOperation(async () => {
      ...
      const approvedLocation = await locationCircuitBreaker.executeApprovalOperation(async () => {
      ...
      console.error(`EnhancedLocationsService.approveLocation error for ID ${params.id}:`, error);
      throw error;
    }
  }
  ...
  async rejectLocation(params: RejectLocationParams): Promise<LocationDTO> {
    try {
      ...
      const rejectedLocation = await locationCircuitBreaker.executeApprovalOperation(async () => {
      ...
      console.error(`EnhancedLocationsService.rejectLocation error for ID ${params.id}:`, error);
      throw error;
    }
  }
  ...
  async deleteLocation(id: string, requesterId?: string): Promise<void> {
    try {
      ...
      const locationToDelete = await locationCircuitBreaker.executeFetchOperation(async () => {
        const location = await this.getLocationById(id);
        if (!location) {
          throw new Error(`Location with ID ${id} not found`);
        }
      ...
      console.error(`EnhancedLocationsService.deleteLocation error for ID ${id}:`, error);
      throw error;
    }
  }
}