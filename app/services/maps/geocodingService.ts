...
export class GeocodingService {
  ...
  constructor() {
    ...
    if (!this.apiKey) {
      console.warn('No Google Maps API key found. Geocoding features will be unavailable.');
    }
  }
  ...
  private extractLocationName(result: any): string {
    ...
    if (result.types && 
      ...
      const premiseComponent = result.address_components?.find(
        (c: any) => c.types.includes('premise') || c.types.includes('point_of_interest')
      );
   ...
   */
  async getSimpleGeocodingResult(address: string): Promise<SimpleGeocodingResult | null> {
    const result = await this.geocodeAddress(address);
