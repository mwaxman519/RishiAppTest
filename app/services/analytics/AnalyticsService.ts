...
export class AnalyticsService {
  ...
  async getDashboards(
    ...
    try {
      ...
      const roleFilters = this.applyRoleBasedDashboardFiltering(filters || {}, userRole, userId);
      
      const result = await this.repository.findDashboards(organizationId, roleFilters);
  ...
  async getDashboardById(
    ...
    try {
      ...
      if (!this.hasDashboardAccess(dashboard, userId, userRole, organizationId)) {
        return { success: false, error: 'Access denied', code: 'DASHBOARD_ACCESS_DENIED' };
      }
  ...
  async updateDashboard(
    ...
    try {
      ...
      const existingResult = await this.repository.findDashboardById(dashboardId);
      ...
      if (!this.hasUpdateDashboardPermissions(existingDashboard, updatedBy, userRole, organizationId)) {
        return { success: false, error: 'Access denied', code: 'DASHBOARD_UPDATE_DENIED' };
      }
  ...
  async deleteDashboard(
    ...
    try {
      ...
      if (!this.hasDeleteDashboardPermissions(dashboard, deletedBy, userRole, organizationId)) {
        return { success: false, error: 'Access denied', code: 'DASHBOARD_DELETE_DENIED' };
      }
  ...
  async getRealTimeMetrics(
    ...
    try {
      const result = await this.repository.getRealTimeMetrics(organizationId, metricIds, timePeriod);
  ...
  async generateReport(
    ...
    try {
      ...
      const templateResult = await this.repository.findReportTemplateById(templateId);
      
      if (!templateResult.success || !templateResult.data) {
        return { success: false, error: 'Report template not found', code: 'TEMPLATE_NOT_FOUND' };
      }
      ...
      if (!reportResult.success || !reportResult.data) {
        return { success: false, error: reportResult.error, code: reportResult.code };
      }
  ...
  async exportData(
    ...
    userId: string,
    organizationId: string
  ): Promise<AnalyticsServiceResponse<{ downloadUrl: string; fileName: string }>> {
    try {
      const exportResult = await this.repository.exportData(query, config, organizationId);
      
      if (!exportResult.success || !exportResult.data) {
        return { success: false, error: exportResult.error, code: exportResult.code };
      }
  ...
  private hasDashboardAccess(
    ...
    // Organization check
    if (dashboard.organizationId !== organizationId && userRole !== 'super_admin') {
      return false;
    }
  ...
  private hasCreateDashboardPermissions(userRole: string): boolean {
    return ['super_admin', 'organization_admin', 'internal_field_manager'].includes(userRole);
  }
  ...
  private hasUpdateDashboardPermissions(
    ...
    if (dashboard.sharedWith?.some(share => share.userId === userId && share.permission === 'edit')) {
      return true;
    }
