...
import { ReportTemplate, GeneratedReport, ReportStatus, AnalyticsServiceResponse } from '../models';
...
export class ReportGenerator {
  ...
  async scheduleReport(template: ReportTemplate): Promise<boolean> {
    ...
    console.log(`[Report Scheduler] Scheduled ${template.name} for ${template.schedule.frequency}`);
  ...
  async cancelScheduledReport(templateId: string): Promise<boolean> {
    ...
    console.log(`[Report Scheduler] Cancelled schedule for template ${templateId}`);
