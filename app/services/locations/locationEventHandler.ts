...
export class LocationEventHandler implements EventSubscriber {
  ...
  async handleEvent<T>(event: Event<T>): Promise<void> {
    console.log(`[LocationEventHandler] Handling event: ${event.type}`, { eventId: event.id });
    
    try {
      switch (event.type) {
        case AppEvent.LOCATION_CREATED:
          await this.handleLocationCreated(event as Event<LocationCreatedPayload>);
          break;
        case AppEvent.LOCATION_UPDATED:
          await this.handleLocationUpdated(event as Event<LocationUpdatedPayload>);
          break;
        case AppEvent.LOCATION_APPROVED:
          await this.handleLocationApproved(event as Event<LocationApprovalPayload>);
          break;
        case AppEvent.LOCATION_REJECTED:
          await this.handleLocationRejected(event as Event<LocationRejectionPayload>);
          break;
        case AppEvent.LOCATION_DELETED:
          await this.handleLocationDeleted(event as Event<LocationDeletedPayload>);
          break;
        default:
          console.warn(`[LocationEventHandler] Unhandled event type: ${event.type}`);
      }
    } catch (error) {
      console.error(`[LocationEventHandler] Error handling event ${event.type}:`, error);
      throw error;
    }
  }
   ...
   */
  private async handleLocationCreated(event: Event<LocationCreatedPayload>): Promise<void> {
    const payload = event.payload;
    console.log(`[LocationEventHandler] Processing location created event for location: ${payload.locationId}`);
    ...
    try {
      ...
      console.log(`[LocationEventHandler] Successfully processed location created event for location: ${payload.locationId}`);
    } catch (error) {
      console.error(`[LocationEventHandler] Error handling location created event:`, error);
      throw error;
    }
  }
   ...
   */
  private async handleLocationUpdated(event: Event<LocationUpdatedPayload>): Promise<void> {
    const payload = event.payload;
    console.log(`[LocationEventHandler] Processing location updated event for location: ${payload.locationId}`);
    ...
    try {
      ...
      console.log(`[LocationEventHandler] Successfully processed location updated event for location: ${payload.locationId}`);
    } catch (error) {
      console.error(`[LocationEventHandler] Error handling location updated event:`, error);
      throw error;
    }
  }
   ...
   */
  private async handleLocationApproved(event: Event<LocationApprovalPayload>): Promise<void> {
    const payload = event.payload;
    console.log(`[LocationEventHandler] Processing location approved event for location: ${payload.locationId}`);
    ...
    try {
      ...
      if (payload.submittedById) {
        try {
          ...
          console.error(`[LocationEventHandler] Error sending approval notification:`, notificationError);
      ...
      console.log(`[LocationEventHandler] Successfully processed location approved event for location: ${payload.locationId}`);
    } catch (error) {
      console.error(`[LocationEventHandler] Error handling location approved event:`, error);
      throw error;
    }
  }
   ...
   */
  private async handleLocationRejected(event: Event<LocationRejectionPayload>): Promise<void> {
    const payload = event.payload;
    console.log(`[LocationEventHandler] Processing location rejected event for location: ${payload.locationId}`);
    ...
    try {
      ...
      if (payload.submittedById) {
        try {
          ...
          console.error(`[LocationEventHandler] Error sending rejection notification:`, notificationError);
          // Continue processing even if notification fails
        }
      }
      
      console.log(`[LocationEventHandler] Successfully processed location rejected event for location: ${payload.locationId}`);
    } catch (error) {
      console.error(`[LocationEventHandler] Error handling location rejected event:`, error);
      throw error;
    }
  }
   ...
   */
  private async handleLocationDeleted(event: Event<LocationDeletedPayload>): Promise<void> {
    const payload = event.payload;
    console.log(`[LocationEventHandler] Processing location deleted event for location: ${payload.locationId}`);
    ...
    try {
      ...
      console.log(`[LocationEventHandler] Successfully processed location deleted event for location: ${payload.locationId}`);
    } catch (error) {
      console.error(`[LocationEventHandler] Error handling location deleted event:`, error);
      throw error;
    }
  }
  ...
  private async sendApprovalNotification(
    ...
    console.log(`[LocationEventHandler] Sending approval notification for location: ${locationId} to user: ${userId}`);
    ...
    try {
      ...
      console.log(`[LocationEventHandler] Approval notification sent to user: ${userId}`);
    } catch (error) {
      console.error(`[LocationEventHandler] Failed to send approval notification:`, error);
  ...
  private async sendRejectionNotification(
    ...
    console.log(`[LocationEventHandler] Sending rejection notification for location: ${locationId} to user: ${userId}`);
    ...
    try {
      ...
      console.log(`[LocationEventHandler] Rejection notification sent to user: ${userId}`);
    } catch (error) {
      console.error(`[LocationEventHandler] Failed to send rejection notification:`, error);
      // Don't rethrow to prevent event processing from failing
    }
  }
