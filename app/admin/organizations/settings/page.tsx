...
const settingsFormSchema = z.object({
  ...
  notification_email: z.string().email({ message: 'Please enter a valid email address' }).optional().or(z.literal('')),
  ...
  billing_contact_email: z.string().email({ message: 'Please enter a valid email address' }).optional().or(z.literal('')),
...
function OrganizationSettingsPage() {
  ...
  useEffect(() => {
    if (settingsData?.settings && organizationId) {
      form.reset({
        ...
        billing_contact_email: settingsData.settings.billing_contact_email || '',
  ...
  const saveSettingsMutation = useMutation({
    mutationFn: async (data: SettingsFormValues) => {
      const response = await apiRequest('POST', '/api/organizations/settings', data);
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/organizations/settings?organizationId=${organizationId}`] });
    },
    onError: (error: Error) => {
      toast({
        title: 'Error saving settings',
        description: error.message || 'There was an error saving the organization settings.',
        variant: 'destructive',
      });
    },
  });
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure billing contact information for invoices and financial communications.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <FormField
