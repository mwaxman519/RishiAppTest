...
const brandingFormSchema = z.object({
  ...
  primary_color: z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, { message: 'Must be a valid hex color code' }),
  secondary_color: z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, { message: 'Must be a valid hex color code' }),
  accent_color: z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, { message: 'Must be a valid hex color code' }),
...
function OrganizationBrandingPage() {
  ...
  const saveBrandingMutation = useMutation({
    mutationFn: async (data: BrandingFormValues) => {
      const response = await apiRequest('POST', '/api/organizations/branding', data);
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/organizations/branding?organizationId=${organizationId}`] });
    },
    onError: (error: Error) => {
      toast({
        title: 'Error saving branding',
        description: error.message || 'There was an error saving the organization branding.',
        variant: 'destructive',
      });
    },
  });
    ...
    <div className="container mx-auto py-6">
      ...
      {!canCustomize && (
        <Alert className="mb-6">
          ...
          <AlertDescription>
            Branding customization is only available for Tier 3 organizations or with special permissions.
            {organization?.tier !== 'tier_3' && ' Consider upgrading to Tier 3 to access this feature.'}
          </AlertDescription>
        </Alert>
      )}
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure the visual appearance of the organization in the platform.
              </CardDescription>
            </CardHeader>
          ...
          <div className="p-4 border rounded-lg bg-muted/50">
            ...
            <div 
              ...
              style={{
                ...
                '--secondary-color': form.watch('secondary_color') || '#675EA9',
                '--accent-color': form.watch('accent_color') || '#2DD4BF',
              } as React.CSSProperties}
            >
              <div 
                className="h-12 w-full mb-4 bg-[var(--primary-color)]" 
                style={{ fontFamily: form.watch('font_family') || 'Inter, sans-serif' }}
              >
                <div className="h-full flex items-center px-4 text-white font-medium">
                  {form.watch('logo_url') ? (
                    <img 
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = 'data:image/svg+xml;charset=utf-8,%3Csvg xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22 viewBox%3D%220 0 100 100%22%3E%3Crect fill%3D%22%23CCC%22 width%3D%22100%22 height%3D%22100%22%2F%3E%3Cpath fill%3D%22%23FFF%22 d%3D%22M34 46h32v8H34z%22%2F%3E%3C%2Fsvg%3E';
                      }}
                    />
                  ) : (
