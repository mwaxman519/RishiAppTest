...
const ROLE_PERMISSIONS = {
  super_admin: PERMISSION_CATEGORIES.flatMap(cat => cat.permissions.map(p => p.id)),
  internal_admin: PERMISSION_CATEGORIES.flatMap(cat => 
    cat.permissions
      .filter(p => !['edit:system_config', 'edit:permissions', 'delete:organizations'].includes(p.id))
      .map(p => p.id)
  ),
...
export default function UserPermissionsPage() {
  ...
  const [rolePermissions, setRolePermissions] = useState<Record<string, string[]>>(ROLE_PERMISSIONS);
  ...
  const togglePermission = (permissionId: string) => {
    setRolePermissions(prev => {
      ...
      if (newPermissions[selectedRole].includes(permissionId)) {
        ...
        newPermissions[selectedRole] = newPermissions[selectedRole].filter(id => id !== permissionId);
      } else {
        // Add the permission
        newPermissions[selectedRole] = [...newPermissions[selectedRole], permissionId];
      }
        ...
        <CardContent>
          ...
          >
            <TabsList className="mb-4">
              ...
              <TabsTrigger value="internal_field_manager">Field Manager</TabsTrigger>
              <TabsTrigger value="field_coordinator">Field Coordinator</TabsTrigger>
          ...
          <div className="mt-8 flex justify-end">
            <Button 
              ...
              disabled={!hasChanges || isSubmitting || selectedRole === 'super_admin'}
            >
              {isSubmitting ? (
                <>
