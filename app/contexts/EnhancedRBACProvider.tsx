...
import { PermissionContext, hasEnhancedPermission, hasAllEnhancedPermissions, hasSomeEnhancedPermissions } from '../lib/rbac-enhanced';
...
interface EnhancedRBACContextType {
  ...
  hasSomePermissions: (permissions: string[], context?: PermissionContext) => boolean;
...
export function EnhancedRBACProvider({ 
  ...
  const hasAllPermissions = useCallback(
    (permissionsToCheck: string[], context: PermissionContext = {}): boolean => {
      if (!user?.role) return false;
      ...
      return hasAllEnhancedPermissions(permissionsToCheck, user.role, fullContext);
    },
    [user?.role, user?.organizationId, user?.regionIds]
  );
  ...
  const hasSomePermissions = useCallback(
    (permissionsToCheck: string[], context: PermissionContext = {}): boolean => {
      ...
      return hasSomeEnhancedPermissions(permissionsToCheck, user.role, fullContext);
    },
    [user?.role, user?.organizationId, user?.regionIds]
  );
...
export function useEnhancedRBAC() {
  ...
  if (context === undefined) {
    throw new Error('useEnhancedRBAC must be used within a EnhancedRBACProvider');
  }
