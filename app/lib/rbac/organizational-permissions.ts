...
import { UserRole, getRolePermissions, permissionMatches } from '@/shared/rbac-roles';
...
export async function hasOrganizationalPermission({
  ...
  if (userRole === 'brand_agent') {
    return checkBrandAgentPermissions(permission, organizationSettings, rolePermissions);
  }
 ...
 */
export async function loadOrganizationSettings(organizationId: string): Promise<Record<string, boolean>> {
  try {
