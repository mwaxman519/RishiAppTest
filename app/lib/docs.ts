...
function debugLog(message: string, ...args: any[]) {
  if (process.env.NODE_ENV !== 'production' || process.env.DEBUG_DOCS === 'true') {
  ...
  function processDirectory(currentDir: string) {
    try {
      ...
      for (const entry of entries) {
        ...
        } else if (entry.isFile() && MARKDOWN_EXTENSIONS.includes(path.extname(entry.name).toLowerCase())) {
...
function buildDocTree(dir: string, baseDir: string = DOCS_DIRECTORY): DocTree {
  ...
  for (const entry of files) {
    ...
    if (entry.name.startsWith('.')) continue;
    if (!MARKDOWN_EXTENSIONS.includes(path.extname(entry.name).toLowerCase())) continue;
 ...
 */
export async function getRecentDocuments(limit: number = 5): Promise<DocInfo[]> {
  try {
    debugLog(`[DOCS] Scanning for recent documents (limit: ${limit})`);
    const docsInfo: DocInfo[] = await getAllDocs();
    
    // Sort by last modified date, most recent first
    const sortedDocs = [...docsInfo].sort((a, b) => b.lastModified.getTime() - a.lastModified.getTime());
 ...
 */
function getAllDocsFromDirectory(dir: string, basePath: string = ''): DocInfo[] {
 ...
 */
function extractDocInfo(content: string, filename: string): { title: string; excerpt: string } {
  ...
  if (!excerpt) {
    ...
    if (excerptMatch && excerptMatch[1] && typeof excerptMatch[1] === 'string') {
...
export async function getDocumentByPath(docPath: string): Promise<DocContent | null> {
  try {
    ...
    console.log(`[DOCS REDIRECT DEBUG] Processing document request for path: '${docPath}', normalized to: '${normalizedPath}'`);
    ...
    if (PATH_REDIRECTS[normalizedPath]) {
      ...
      console.log(`[DOCS REDIRECT] Redirecting from '${normalizedPath}' to '${PATH_REDIRECTS[normalizedPath]}'`);
      
      // Get the document from the target path
      const redirectedDocument = await getDocumentByPath(PATH_REDIRECTS[normalizedPath]);
      
      // Log result of redirection
      if (redirectedDocument) {
        console.log(`[DOCS REDIRECT] Successfully retrieved redirected document with title: '${redirectedDocument.metadata.title}'`);
      } else {
    ...
    debugLog(`[DOCS FETCH] Requesting content for: ${docPath} (normalized: ${normalizedPath})`);
    
    // Check if we have a valid cached version
    if (contentCache[cacheKey] && (currentTime - contentCache[cacheKey].timestamp) < CONTENT_CACHE_TTL) {
      // In production, just use the cache
      if (process.env.NODE_ENV === 'production' && !process.env.DEBUG_DOCS) {
        debugLog(`[DOCS FETCH] Using cached content for ${normalizedPath} (production mode)`);
        return {
      ...
      try {
        ...
        if (stats.mtime.getTime() === contentCache[cacheKey].mtime) {
          debugLog(`[DOCS FETCH] Using cached content for ${normalizedPath} (unchanged)`);
          return {
            content: contentCache[cacheKey].content,
            metadata: contentCache[cacheKey].metadata
          };
        } else {
          debugLog(`[DOCS FETCH] File has been modified, recompiling: ${cachedFilePath}`);
        }
      } catch (err) {
        debugLog(`[DOCS FETCH] Error checking file modification, will recompile: ${err}`);
      }
    }
    ...
    for (const ext of MARKDOWN_EXTENSIONS) {
      const candidatePath = path.join(DOCS_DIRECTORY, `${normalizedPath}${ext}`);
    ...
    if (!filePath) {
      for (const ext of MARKDOWN_EXTENSIONS) {
        const indexPath = path.join(DOCS_DIRECTORY, normalizedPath, `index${ext}`);
    ...
    if (!filePath) {
      for (const ext of MARKDOWN_EXTENSIONS) {
        const readmePath = path.join(DOCS_DIRECTORY, normalizedPath, `README${ext}`);
    ...
    if (!filePath) {
      ...
      console.error(`[DOCS FETCH] Tried these paths: ${pathVariations.join(', ')}`);
      return null;
    }
    ...
    try {
      ...
      debugLog(`[DOCS FETCH] Using fallback plain text rendering for ${docPath}`);
...
export async function getDocumentsByTag(tag: string): Promise<DocInfo[]> {
  ...
  if (tagCache[normalizedTag] && (currentTime - tagCache[normalizedTag].timestamp) < CACHE_TTL) {
  ...
  try {
    ...
    for (const doc of allDocs) {
      try {
        ...
        debugLog(`[DOCS] Error processing document ${doc.path} for tags:`, error);
    ...
    debugLog(`[DOCS] Found ${results.length} documents with tag: "${normalizedTag}"`);
    return results;
  } catch (error) {
    ...
    if (tagCache[normalizedTag]) {
      console.log(`[DOCS] Returning cached tag results as fallback after error for "${normalizedTag}"`);
      return tagCache[normalizedTag].docs;
    }
...
export function traceDocumentPath(docPath: string): string[] {
  ...
  try {
    ...
    // Normalize the path
    const normalizedPath = docPath.replace(/^\/+|\/+$/g, '').replace(/\.(md|mdx)$/i, '');
    traces.push(`Normalized path: ${normalizedPath}`);
    
    // Check if path is in redirects
    if (PATH_REDIRECTS[normalizedPath]) {
      traces.push(`Found redirect: ${normalizedPath} -> ${PATH_REDIRECTS[normalizedPath]}`);
    } else {
    ...
    if (!pathVariations.some(p => fs.existsSync(p))) {
      traces.push(`No matching file found. Tried: ${pathVariations.join(', ')}`);
    }
