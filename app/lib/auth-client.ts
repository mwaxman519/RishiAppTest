...
export async function hasPermission(permission: string, organizationId?: string): Promise<boolean> {
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`DEV MODE: Permission check for ${permission}${organizationId ? ` in org ${organizationId}` : ''} => true`);
      return true;
    }

    // Real implementation would call the API
    const response = await fetch(`/api/permissions/check?permission=${permission}${organizationId ? `&organizationId=${organizationId}` : ''}`);
