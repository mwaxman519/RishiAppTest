...
export async function getTeamMembers(): Promise<UsersResponse> {
  try {
    ...
    if (hasPermission('view:managers', currentUser.role)) {
      console.log('[availability.ts] getTeamMembers - User has admin permission, fetching managers');
      
      const managersResponse = await getUsersByRole(USER_ROLES.INTERNAL_FIELD_MANAGER);
      
      if (agentsResponse.success && managersResponse.success && 
          agentsResponse.data && managersResponse.data) {
        // Combine agents and managers
        const combinedTeam = [...agentsResponse.data, ...managersResponse.data];
        console.log(`[availability.ts] getTeamMembers - Returning combined team (${combinedTeam.length} members)`);
    ...
    console.log(`[availability.ts] getTeamMembers - Returning agents only (${agentsResponse.data?.length || 0} members)`);
    return agentsResponse;
    
  } catch (error) {
    console.error('[availability.ts] Error in getTeamMembers server action:', error);
    return { 
      success: false, 
