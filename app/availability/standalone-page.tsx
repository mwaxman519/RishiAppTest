...
export default function StandaloneAvailabilityPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'available' | 'unavailable' | 'tentative'>('all');
    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 401) {
            throw new Error("You are not authorized to view this availability data");
          } else if (response.status === 403) {
            throw new Error("You do not have permission to view this availability data");
        ...
        if (fetchError instanceof Error && (
          ...
          console.warn("Network error while fetching availability, returning empty data");
          setMyBlocks([]);
          return;
        }
      ...
      if (isAtLeastRole(USER_ROLES.INTERNAL_FIELD_MANAGER)) {
        try {
          ...
          if (!teamResponse.ok) {
            console.error('Failed to fetch team availability:', teamResponse.status);
            setTeamBlocks([]);
          } else {
  ...
  const handleDeleteBlock = async (blockId: number) => {
    if (window.confirm('Are you sure you want to delete this availability block?')) {
      try {
        ...
        setMyBlocks(currentBlocks => currentBlocks.filter(block => block.id !== blockId));
  ...
  const filteredBlocks = myBlocks.filter(block => {
    ...
    const matchesSearch = 
      ...
      format(new Date(block.startDate), 'MMM dd, yyyy').toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <div className="p-6 w-full">
      <h1 className="text-2xl font-bold mb-6">Availability Management (Simplified View)</h1>
      ...
      {isLoading ? (
        ...
        <div className="w-full">
          ...
          <p className="mb-6 bg-blue-50 border-l-4 border-blue-400 p-4 rounded text-blue-700">
            Due to technical issues, we're showing a simplified view of your availability. 
            To add, edit, or manage your availability blocks, please use the Calendar tab.
          </p>
          
          <div>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
              <div className="flex items-center">
                <h2 className="text-lg font-semibold">My Availability Blocks</h2>
                <div className="ml-4">
                  <ThemeToggle />
                </div>
              </div>
            ...
            {isLoading ? (
                ...
                <table className="w-full border-collapse dark:border-gray-700">
                  <thead>
                    <tr className="bg-gray-100 dark:bg-gray-800">
                      <th className="p-2 text-left border dark:border-gray-700 dark:text-gray-200">Title</th>
                      <th className="p-2 text-left border dark:border-gray-700 dark:text-gray-200">Start Date</th>
                      <th className="p-2 text-left border dark:border-gray-700 dark:text-gray-200">End Date</th>
                      <th className="p-2 text-left border dark:border-gray-700 dark:text-gray-200">Status</th>
                      <th className="p-2 text-left border dark:border-gray-700 dark:text-gray-200">Recurring</th>
                      <th className="p-2 text-left border dark:border-gray-700 dark:text-gray-200">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredBlocks.map((block) => (
                      <tr key={block.id} className="border-b hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700 dark:text-white">
                        <td className="p-2 border dark:border-gray-700">{block.title || 'Untitled'}</td>
                        <td className="p-2 border dark:border-gray-700">
                          {block.startDate instanceof Date 
                            ? format(block.startDate, 'MMM dd, yyyy h:mm a')
                            : format(new Date(block.startDate), 'MMM dd, yyyy h:mm a')}
                        </td>
                        <td className="p-2 border dark:border-gray-700">
                          {block.endDate instanceof Date 
                            ? format(block.endDate, 'MMM dd, yyyy h:mm a')
                            : format(new Date(block.endDate), 'MMM dd, yyyy h:mm a')}
                        </td>
                        <td className="p-2 border dark:border-gray-700">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                            ${block.status === 'available' ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' : 
                              block.status === 'unavailable' ? 'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100' : 
              ...
              <div className="text-center py-8 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">No availability blocks found</h3>
