...
export function TableOfContents({ tree, className, onLinkClick }: TableOfContentsProps) {
  ...
  return (
    <nav className={cn("text-sm", className)}>
      <TreeNode tree={tree} level={0} basePath={basePath} onLinkClick={handleLinkClick} />
    </nav>
  );
}
...
function TreeNode({ tree, level, basePath, onLinkClick }: TreeNodeProps) {
  ...
  let readmeEntry = entries.find(([key]) => key === 'README.md' || key === 'README.mdx' || key === 'README');
  ...
  const directories = entries
    .filter(([key, value]) => value !== null && key !== 'README.md' && key !== 'README.mdx' && key !== 'README')
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`TreeNode at ${basePath}: Found ${directories.length} directories and ${files.length} files`);
  }
  
  // If we have no content, try to reinitialize (client-side only)
  if (directories.length === 0 && files.length === 0 && typeof window !== 'undefined') {
    console.warn(`[DOCS TOC] Empty tree at ${basePath}, attempting to reinitialize...`);
    // Try to initialize docs in the background - fetch directly (don't await)
    fetch('/api/docs/init').then(response => {
      if (response.ok) {
        console.log('[DOCS TOC] Reinitialization succeeded, refreshing page...');
  ...
  const handleClick: MouseEventHandler<HTMLAnchorElement> | undefined = 
    onLinkClick ? (e) => { onLinkClick(); } : undefined;
  ...
  return (
    <ul className={cn(
          ...
          {handleClick ? (
            <Link
              ...
              className={cn(
                ...
                pathname === basePath && "bg-gradient-to-r from-purple-100 to-teal-50 text-purple-700 dark:from-purple-900/20 dark:to-teal-900/10 dark:text-purple-300"
              )}
              onClick={handleClick}
            >
      ...
      {files.map(([key]) => {
        ...
        if (process.env.NODE_ENV !== 'production') {
          console.log(`[TOC] Creating file link: ${key} at path: ${currentPath}`);
        }
            ...
            {handleClick ? (
              <Link
                ...
                className={cn(
                  "flex items-center gap-2 py-1 px-2 rounded-md text-sm",
                  "text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-800/50",
                  isActive && "bg-gradient-to-r from-teal-100 to-purple-50 text-teal-700 dark:from-teal-900/20 dark:to-purple-900/10 dark:text-teal-300"
                )}
                onClick={handleClick}
              >
                <File className="h-4 w-4 flex-shrink-0 text-gray-400" />
...
function DirectoryNode({ name, value, level, basePath, currentPath, isActive, onLinkClick }: DirectoryNodeProps) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`Directory ${name} at path ${basePath} has files:`, Object.keys(value));
  }
  
  // Ensure directory value is valid
  if (!value || typeof value !== 'object') {
    if (process.env.NODE_ENV !== 'production') {
      console.error(`Invalid directory value for ${name} at ${basePath}:`, value);
    }
    return null;
  }
  
  // Check if this directory has a README.md or README.mdx file
  const hasReadme = Object.keys(value).some(key => key === 'README.md' || key === 'README.mdx' || key === 'README');
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`DirectoryNode ${name}: Current path = ${cleanCurrentPath}, pathname = ${pathname}`);
  }
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`[TOC] Rendering directory node: ${name}, path: ${cleanCurrentPath}`);
  }
  ...
  return (
    <li>
      ...
      {isOpen && (
        <TreeNode tree={value} level={level} basePath={cleanCurrentPath.replace(/\.md$/, '')} onLinkClick={onLinkClick} />
      )}
    </li>
  );
}