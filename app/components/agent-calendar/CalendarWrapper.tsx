...
function CalendarWrapper({ userId, viewOnly = false, debug = false }: CalendarWrapperProps) {
  ...
  console.log('CalendarWrapper: Rendering with userId:', userId, 'shouldRender:', shouldRender, 'isLoading:', isLoading);
  ...
  return (
    <div className="calendar-wrapper w-full">
          ...
          <ErrorBoundary
            ...
            errorComponent={(error, reset) => (
              <div className="p-4 border border-red-300 rounded bg-red-50 text-red-700 dark:bg-red-900/30 dark:border-red-800 dark:text-red-200">
                ...
                <p className="mb-4">There was a problem loading the calendar component.</p>
      ...
      {(!shouldRender || isLoading) && (
        <div className={`flex justify-center items-center h-64 w-full ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow p-4`}>
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-teal-500"></div>
          {false && <div className="ml-3 text-sm text-gray-600 dark:text-gray-300">Loading calendar (userId: {userId})</div>}
        </div>
      )}
