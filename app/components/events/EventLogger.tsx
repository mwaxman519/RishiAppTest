...
export function EventLogger({ 
  ...
  const displayEvents = filter 
    ? events.filter(e => filter.includes(e.type as AppEvent)).slice(0, maxEvents)
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            {isConnected ? (
              <Badge variant="outline" className="flex gap-1 items-center bg-green-500/10 text-green-500 border-green-500/20">
                <Wifi className="h-3 w-3" />
                <span>Connected</span>
              </Badge>
            ) : (
              <Badge variant="outline" className="flex gap-1 items-center bg-red-500/10 text-red-500 border-red-500/20">
