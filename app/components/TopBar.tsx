...
const TopBar: React.FC<TopBarProps> = ({ items }) => {
  ...
  const { currentOrganization, userOrganizations, switchOrganization } = useOrganizationContext();
      ...
      <div className="organization-selector">
        {currentOrganization && (
          <div className="current-organization">
            ...
            <button className="org-switcher" onClick={() => toggleDropdown('organizations')}>
                ...
                >
                  {item.icon && (Icons as any)[item.icon] && (
                    <span className="action-icon">
                      {React.createElement((Icons as any)[item.icon], { size: 20 })}
                    </span>
                  )}
                  <span className="action-label">{item.label}</span>
                </button>
                {openDropdown === item.path && renderDropdownItems(item.children)}
              </div>
            ) : (
          ...
          {openDropdown === 'user' && (
            <ul className="dropdown-menu user-menu">
              <li className="dropdown-item">
                <Link href="/profile" onClick={closeDropdowns}>Profile</Link>
              </li>
              <li className="dropdown-item">
                <Link href="/settings" onClick={closeDropdowns}>Settings</Link>
              </li>
              <li className="dropdown-item">
                <Link href="/api/auth/logout" onClick={closeDropdowns}>Logout</Link>
              </li>
            </ul>
          )}
        </div>
