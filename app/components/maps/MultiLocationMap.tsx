...
export function MultiLocationMap({ 
  ...
  const getMapCenter = useCallback(() => {
    ...
    if (selectedLocationId) {
      ...
      console.log("Finding center for selected location:", selectedLocationId, selectedLocation);
      ...
      if (selectedLocation && 
        ...
        console.log("Using selected location for center:", selectedLocation.latitude, selectedLocation.longitude);
        return { 
    ...
    if (validLocations.length === 1) {
      console.log("Single valid location, using it for center:", validLocations[0].latitude, validLocations[0].longitude);
      return { 
  ...
  const onLoad = (mapInstance: google.maps.Map): void => {
    ...
    // Setup idle listener
    idleListenerRef.current = google.maps.event.addListener(mapInstance, 'idle', () => {
      ...
      if (selectedLocationId && initialRenderRef.current) {
        ...
        if (loc && isValidCoordinate(loc.latitude, loc.longitude)) {
          console.log("Initial render with selection, centering on:", loc.name);
          setTimeout(() => {
  ...
  useEffect(() => {
    ...
    if (selectedLocationId) {
      ...
      const selectedLocation = locationMapRef.current[selectedLocationId] || 
      ...
      if (selectedLocation && isValidCoordinate(selectedLocation.latitude, selectedLocation.longitude)) {
        ...
        if (map) {
          ...
          console.log("⭐ Centering map on coordinates:", selectedLocation.latitude, selectedLocation.longitude);
  ...
  const getMarkerColor = (location: MapLocation): string => {
    ...
    if (location.locationType) {
      const normalizedType = location.locationType.toLowerCase().replace('_', '');
  ...
  // Handle error state
  if (loadError) {
    return <div className="p-4 text-red-500">Error loading Google Maps: {loadError.message}</div>;
  }
  ...
  return (
    ...
    >
      ...
      {locations.map(location => (
        <div key={location.id}>
          {activeInfoWindow === location.id && isValidCoordinate(location.latitude, location.longitude) && (
            <InfoWindow
              position={{
                lat: location.latitude,
                lng: location.longitude
