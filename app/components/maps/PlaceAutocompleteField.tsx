...
interface PlaceAutocompleteFieldProps {
  ...
  className?: string;
  disabled?: boolean;
  type?: 'default' | 'geocode' | 'address' | 'establishment' | 'regions' | 'cities';
}
...
export function PlaceAutocompleteField({
  ...
  const debugConsole = {
    ...
    error: (...args: any[]) => console.error('[PlaceAutocompleteField]', ...args),
  ...
  useEffect(() => {
    ...
    if (!isLoaded || !inputRef.current) {
      debugConsole.log('Waiting for Maps API to load or input to be available...');
      return;
    }
    ...
    if (!autocompleteRef.current) {
      try {
        ...
        autocomplete.setAttribute('data-input-id', inputRef.current.id || 'unknown-input');
        ...
        handlePlaceChanged = (event: any) => {
          ...
          try {
            ...
            setInputValue(locationData.displayName || locationData.formattedAddress);
      ...
      if (autocompleteRef.current) {
        try {
          ...
          if (handlePlaceChanged) {
            autocompleteRef.current.removeEventListener('place-changed', handlePlaceChanged);
          }
