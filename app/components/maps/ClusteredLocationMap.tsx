...
interface ClusteredLocationMapProps {
  ...
  onFilterChange?: (filter: {type?: string; status?: string; state?: string}) => void;
  resetFilter?: boolean;
  mapZoom?: number;
...
export function ClusteredLocationMap({
  ...
  const { isLoaded: isMapsApiLoaded, loadError, mapId } = useContext(GoogleMapsContext);
  ...
  const handleFilter = (filterType: 'type' | 'status' | 'state', value: string) => {
  ...
  const setupClusterer = useCallback(() => {
    ...
    const markers: google.maps.marker.AdvancedMarkerElement[] = locations.map((loc, index) => {
      ...
      markerElement.className = "flex items-center justify-center w-8 h-8 rounded-full shadow-lg transition-all duration-200 cursor-pointer hover:scale-110";
      ...
      const clustererOptions = {
        ...
        renderer: {
          render: ({ count, position }) => {
            ...
            clusterElement.className = "flex items-center justify-center rounded-full";
            ...
            clusterElement.style.backgroundColor = resolvedTheme === 'dark' ? '#4f46e5' : '#4338ca'; // indigo
            clusterElement.style.color = "white";
            clusterElement.style.border = "3px solid white";
            clusterElement.style.boxShadow = "0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)";
  ...
  const typeOptions = Array.from(new Set(locations.map(loc => loc.type).filter(Boolean)));
  const stateOptions = Array.from(new Set(locations.map(loc => loc.state).filter(Boolean)));
  const statusOptions = Array.from(new Set(locations.map(loc => loc.status).filter(Boolean)));
  ...
  const renderLocationDetail = () => {
    ...
    return (
      <div className="p-4">
        <h3 className={`text-lg font-medium ${mapTheme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
          ...
          {selectedLocation.status && (
            <Badge 
              ...
              style={{
                ...
                color: statusColors[selectedLocation.status] || statusColors.default,
        ...
        {selectedLocation.address && (
          <p className={`text-sm ${mapTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'} my-1`}>
            {selectedLocation.address}
          </p>
        )}
        ...
        <p className={`text-xs font-mono ${mapTheme === 'dark' ? 'text-gray-400' : 'text-gray-500'} my-1`}>
        ...
        <div className="mt-3 flex space-x-2">
          <Button
            size="sm"
            variant="default"
            onClick={() => onViewLocationDetail && onViewLocationDetail(selectedLocation.id)}
          >
            View Details
          </Button>
          <Button
    ...
    return (
      <div className="absolute top-4 left-4 z-10 flex flex-wrap gap-2 max-w-[70%]">
        {Object.entries(activeFilters).map(([key, value]) => (
          ...
          >
            {key}: {value}
            <span className="ml-1 font-bold text-muted-foreground hover:text-foreground">×</span>
          </Badge>
        ))}
            ...
            <div className="flex flex-wrap gap-1">
              {statusOptions.map(status => (
                <Badge
                  ...
                  className="cursor-pointer"
                  style={activeFilters.status === status ? {
          ...
          <div className="mb-2">
            ...
            <div className="flex flex-wrap gap-1">
              {stateOptions.map(state => (
                <Badge
                  key={`state-${state}`}
                  variant={activeFilters.state === state ? 'default' : 'outline'}
                  className="cursor-pointer"
                  onClick={() => handleFilter('state', state)}
                >
                  {state} {stats?.byState?.[state] && `(${stats.byState[state]})`}
                </Badge>
              ))}
            </div>
          </div>
    ...
    >
      ...
      {loadError && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 z-20 p-4 text-center">
          <div className="text-destructive mb-2">Failed to load Google Maps</div>
