...
export default function DynamicPlacesAutocomplete({
  ...
  useEffect(() => {
    ...
    window.gm_authFailure = () => {
      ...
      setError('Google Maps authentication failed. Your API key may be invalid or restricted.');
    };
    ...
    script.onerror = (error) => {
      ...
      setError('Failed to load Google Maps script. Check your internet connection and API key.');
    };
  ...
  const initializePlacesAutocomplete = () => {
    try {
      ...
      setError(`Error initializing Places Autocomplete: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
  ...
  return (
    <div className="relative w-full">
      ...
      {!loaded && !error && (
        <div className="text-gray-500 text-sm mt-1">Loading Places Autocomplete...</div>
      )}
    </div>
  );
}
