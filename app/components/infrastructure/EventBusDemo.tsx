...
import { AlertCircle, Check, Clock, RefreshCw, Server, Activity, Wifi, WifiOff } from "lucide-react";
...
export function EventBusDemo() {
  ...
  const { isConnected, events, reconnect } = useEventSocket(selectedEventTypes, {
    onConnect: () => {
      toast({
  ...
  const toggleFeature = (feature: keyof typeof features) => {
    ...
    fetch(`/api/infrastructure/features/${feature}`, {
      ...
      toast({
        ...
        description: data.message || `Successfully updated ${feature} setting`,
        variant: "default"
      });
    }).catch(err => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      setMetrics(prev => ({
        ...
        avgLatency: Math.max(5, Math.min(50, prev.avgLatency + (Math.random() * 4 - 2))),
        activeCircuitBreakers: Math.floor(Math.random() * 2),
        connectedClients: isConnected ? prev.connectedClients + (Math.random() > 0.7 ? 1 : 0) : 0
      }));
    }, 5000);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Event Bus Infrastructure</h2>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={reconnect} disabled={isConnected}>
        ...
        <TabsContent value="overview" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                How the distributed event bus works with circuit breakers and error handling
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="rounded-md bg-muted p-4">
                  <h3 className="font-semibold mb-2">Event Distribution Channels</h3>
                  <p className="text-sm text-muted-foreground mb-3">
                    Messages are distributed across multiple channels for maximum reliability
                  </p>
                  
                  <div className="grid grid-cols-3 gap-2">
                    <div className="bg-background rounded p-3 text-center">
                      <div className="font-semibold">Local</div>
                      <div className="text-xs text-muted-foreground">In-process handlers</div>
                    </div>
                    <div className="bg-background rounded p-3 text-center">
                      <div className="font-semibold">HTTP</div>
                      <div className="text-xs text-muted-foreground">Service-to-service calls</div>
                    </div>
                    <div className="bg-background rounded p-3 text-center">
                      <div className="font-semibold">WebSocket</div>
                      <div className="text-xs text-muted-foreground">Real-time clients</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-md bg-muted p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <div className="bg-background rounded p-3 text-center">
                      <div className="font-semibold text-yellow-600">Half-Open</div>
                      <div className="text-xs text-muted-foreground">Testing recovery</div>
                    </div>
                    <div className="bg-background rounded p-3 text-center">
                      <div className="font-semibold text-red-600">Open</div>
                      <div className="text-xs text-muted-foreground">Fast failure</div>
                    </div>
                  </div>
                </div>
                
                <div className="rounded-md bg-muted p-4">
                  <h3 className="font-semibold mb-2">Error Handling Strategies</h3>
                  <p className="text-sm text-muted-foreground mb-3">
                    Customized error handling based on error type and event criticality
                  </p>
                  
                  <div className="space-y-2">
                    <div className="bg-background rounded p-3">
                      <div className="font-semibold">Exponential Backoff with Jitter</div>
                      <div className="text-xs text-muted-foreground">
                        Progressively increases retry delays to prevent thundering herd
                      </div>
                    </div>
                    <div className="bg-background rounded p-3">
                      <div className="font-semibold">Categorized Error Handling</div>
                      <div className="text-xs text-muted-foreground">
                        Different strategies for transient vs. permanent errors
                      </div>
                    </div>
                    <div className="bg-background rounded p-3">
                      <div className="font-semibold">Event-Specific Retry Policies</div>
                      <div className="text-xs text-muted-foreground">
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    The event bus is operating normally with {metrics.activeCircuitBreakers} active circuit breakers.
                ...
                <div className="space-y-4">
                  <div className="rounded-md bg-muted p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-background rounded p-3">
                        <div className="font-semibold">Enhanced Distributed Event Bus</div>
                        <div className="text-xs text-muted-foreground">
                          Multi-channel distribution with WebSocket and HTTP support
                        </div>
                      </div>
                      <div className="bg-background rounded p-3">
                      ...
                      <div className="bg-background rounded p-3">
                        <div className="font-semibold">Error Handling Strategies</div>
                        <div className="text-xs text-muted-foreground">
                      ...
                      <div key={index} className="rounded-md border p-4">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <Badge variant="outline">
                            {new Date(event.metadata?.timestamp || Date.now()).toLocaleTimeString()}
                          </Badge>
                        </div>
              ...
              <Sheet>
                ...
                <SheetContent>
                  ...
                  <div className="py-4">
                    ...
                    <div className="mt-4">
                      <h4 className="text-sm font-semibold mb-2">Active Subscriptions</h4>
                      <div className="space-y-2">
                        {selectedEventTypes.map(eventType => (
                          <div key={eventType} className="flex items-center justify-between rounded-md border px-4 py-2">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Isolate failing components to prevent cascading failures
                        </p>
                      </div>
                      <Switch
                        id="circuitBreaker"
                        checked={features.circuitBreakerEnabled}
                        onCheckedChange={() => toggleFeature('circuitBreakerEnabled')}
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        ...
                        onCheckedChange={() => toggleFeature('websocketEnabled')}
                      />
                    </div>
                  </div>
                </div>
