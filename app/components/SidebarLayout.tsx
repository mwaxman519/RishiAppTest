...
import { filterLinksByPermission, hasMyAvailability, createMyAvailabilityLink } from '@/utils/navigation-utils';
import { getNavigationForRole, getPlatformAdminNav } from '@/shared/navigation-structure';
import { ThemeToggle } from './ui/theme-toggle';
import { ChevronLeft, ChevronRight, ChevronDown, Menu, X, LogOut, User, Home, FileText, LayoutDashboard, Users, Calendar, Package, Settings, Lock, Shield, Database, Building, MapPin, Clock, BarChart, ClipboardList, GraduationCap, Award, CreditCard, MessageSquare, CheckSquare, Briefcase, UserCog } from 'lucide-react';
...
import { getSuperAdminNavigation, isNavItemActive } from './navigation/SuperAdminNavigation';
...
const formatString = (str: string): string => {
  return str.replace(/_/g, ' ').replace(/\b\w/g, (letter: string) => letter.toUpperCase());
};
...
export default function SidebarLayout({ children }: SidebarLayoutProps) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (urlParams.get('unauthenticated') === 'true') {
        console.log("SidebarLayout: URL parameter indicates unauthenticated state");
      }
    }
  }, []);
  ...
  if (typeof window === 'undefined' && !user) {
    console.log("### SidebarLayout STATIC GENERATION ### Providing fallback layout");
    return <div className="min-h-screen bg-gray-50 dark:bg-gray-900">{children}</div>;
  }
  
  console.log("### SidebarLayout RENDERED ### isSuperAdmin =", isSuperAdmin, "role =", user?.role);

  // Public links available to all users
  const publicLinks = [
    { href: '/', label: 'Dashboard', icon: <LayoutDashboard size={20} />, type: NAV_ITEM_TYPES.PRIMARY },
    { href: '/docs', label: 'Documentation', icon: <FileText size={20} />, type: NAV_ITEM_TYPES.PRIMARY },
  ];
  ...
  const adminLinks = protectedLinks.filter(link => 
    link.href.startsWith('/admin') || link.label.includes('Admin')
  );
  
  const regularLinks = protectedLinks.filter(link => 
    !link.href.startsWith('/admin') && !link.label.includes('Admin')
  );
  ...
  console.log("ROLE-BASED NAVIGATION LOADED:", 
    Array.isArray(roleBasedNavigation) ? roleBasedNavigation.map(item => item.label) : 'No items'
  );
  ...
  const mainNavigationItems = roleBasedNavigation.filter(item => 
    item && item.path && !item.path.includes('/admin') && 
    !item.label?.includes('Administration') &&
    item.label !== 'Platform Administration'
  );
  
  // Get admin-specific navigation sections
  const adminPortalNavItems = roleBasedNavigation.filter(item => 
    item && (
  ...
  const platformAdminNavItems = isSuperAdmin && typeof getPlatformAdminNav === 'function' 
    ? getPlatformAdminNav() 
    : [];
  ...
  const filteredPublicLinks = publicLinks.filter(
    publicItem => !mainNavigationItems.some(navItem => 
      (publicItem.href === navItem.path || publicItem.href === navItem.href) ||
  ...
  const links = [
    ...
    ...(isSuperAdmin ? [] : 
      filterLinksByPermission(regularLinks, user ? { id: user.id, role: user.role || 'client_user' } : null, userRole, (permission: string) => {
    ...
    <div className="flex h-screen overflow-hidden bg-[rgb(var(--background))]">
      ...
      <aside className={`hidden lg:flex sidebar h-screen flex-col transition-all duration-300 ${sidebarCollapsed ? 'w-20 sidebar-collapsed' : 'w-64 sidebar-expanded'}`}>
        ...
        {!sidebarCollapsed ? (
          ...
          <div className="flex justify-center py-3 border-b border-[rgb(var(--sidebar-border))]">
            ...
            >
              <Building size={20} className="text-[rgb(var(--sidebar-muted-foreground))]" />
            </button>
          </div>
        )}
          ...
          <ul className="space-y-1">
              ...
              <li className="mt-4">
                ...
                >
                  <div className={`flex items-center ${sidebarCollapsed ? 'justify-center' : ''}`}>
                    <Briefcase size={20} className="flex-shrink-0 text-green-500" />
                    <span className={`ml-3 font-medium transition-opacity duration-300 ${sidebarCollapsed ? 'hidden' : 'opacity-100'}`}>
                      Field Manager
                    </span>
                  </div>
                  {!sidebarCollapsed && (
                ...
                <div className={`overflow-hidden transition-all duration-200 ${fieldManagerOpen ? 'max-h-[800px] opacity-100 pt-2' : 'max-h-0 opacity-0'}`}>
                  <ul className="pl-4 space-y-1">
                    {(() => {
                      // Use the already imported getNavigationForRole function
                      const fieldManagerNav = getNavigationForRole('internal_field_manager');
                        ...
                        return (
                          <li key={`field-manager-${index}-${item.label}`}>
                            ...
                            >
                              ...
                              <span className={`ml-3 transition-opacity duration-300 ${sidebarCollapsed ? 'opacity-0 hidden' : 'opacity-100'}`}>{item.label}</span>
                            </Link>
                          </li>
                        );
                      });
            ...
            {isSuperAdmin && (
              <li className="mt-1">
                ...
                <div className={`overflow-hidden transition-all duration-200 ${brandAgentsOpen ? 'max-h-[800px] opacity-100 pt-2' : 'max-h-0 opacity-0'}`}>
                  <ul className="pl-4 space-y-1">
                    {(() => {
                      // Use the already imported getNavigationForRole function
            ...
            {isSuperAdmin && (
              <li className="mt-1">
                ...
                >
                  <div className={`flex items-center ${sidebarCollapsed ? 'justify-center' : ''}`}>
                    <Building size={20} className="flex-shrink-0 text-orange-500" />
                    <span className={`ml-3 font-medium transition-opacity duration-300 ${sidebarCollapsed ? 'hidden' : 'opacity-100'}`}>
                      Client Users
                    </span>
                  </div>
                ...
                <div className={`overflow-hidden transition-all duration-200 ${clientUsersOpen ? 'max-h-[800px] opacity-100 pt-2' : 'max-h-0 opacity-0'}`}>
                  <ul className="pl-4 space-y-1">
                    {(() => {
                      // Use the already imported getNavigationForRole function
            ...
            {isSuperAdmin && (
              <li className="mt-1">
                ...
                <div className={`overflow-hidden transition-all duration-200 ${internalAdminOpen ? 'max-h-[800px] opacity-100 pt-2' : 'max-h-0 opacity-0'}`}>
                  <ul className="pl-4 space-y-1">
                    {(() => {
                      // Use the already imported getNavigationForRole function
                      const internalAdminNav = getNavigationForRole('internal_admin');
                      
                      return internalAdminNav.map((item, index) => {
                        // Handle sections differently than regular links
                        if (item.type === NAV_ITEM_TYPES.SECTION && item.children) {
                            ...
                            <li key={sectionId} className="mt-2">
                              ...
                              <ul className="mt-1 space-y-1">
                                {item.children.map((child, childIndex) => {
                                  ...
                                  return (
                                    <li key={`${sectionId}-child-${childIndex}`}>
                                      <Link
                                        href={childPath}
                                        className={`flex items-center px-4 py-2 rounded-md transition-colors ${
                                          isActive 
                                      ...
                                      >
                                        ...
                                        <span className={`ml-3 transition-opacity duration-300 ${sidebarCollapsed ? 'opacity-0 hidden' : 'opacity-100'}`}>{child.label}</span>
                                      </Link>
                                    </li>
                                  );
                                })}
            ...
            {(isSuperAdmin || adminPortalNavItems.length > 0 || platformAdminNavItems.length > 0) && (
              <li className="mt-4">
                {/* Admin Header */}
                <button 
                ...
                <div className={`overflow-hidden transition-all duration-200 ${adminPortalOpen ? 'max-h-[1000px] opacity-100 pt-2' : 'max-h-0 opacity-0'}`}>
                  <ul className="pl-4 space-y-1">
                        ...
                        <ul className="mt-1 mb-4 space-y-1">
                          {platformAdminNavItems.map((link) => {
                            ...
                            const isActive = pathname === linkPath || 
                                          (linkPath !== '/' && pathname?.startsWith(linkPath));
                            return (
                              <li key={link.id || `platform-admin-${link.label}-${Math.random()}`}>
                                <Link
                                  href={linkPath}
                                  className={`flex items-center px-4 py-2 rounded-md transition-colors ${
                                    isActive 
                    ...
                    {adminPortalNavItems.map((section, sectionIndex) => (
                      <li key={`admin-section-${sectionIndex}-${section.label || section.path}`}>
                        <div className="px-4 py-2 text-xs font-semibold text-[rgb(var(--sidebar-muted-foreground))] uppercase tracking-wider">
                          {section.label}
                        </div>
                        <ul className="mt-1 mb-4 space-y-1">
                          {section.children?.map((link: any, linkIndex: number) => {
                            const isActive = pathname === (link.path || link.href) || 
                                          pathname?.startsWith(link.path || link.href || '');
                            return (
                              <li key={`admin-link-${sectionIndex}-${linkIndex}-${link.path || link.href || link.label}`}>
                                <Link
                        ...
                        <ul className="mt-1 mb-4 space-y-1">
                          {adminLinks.filter(link => {
                            ...
                            const isActive = pathname === linkPath || 
                                          (linkPath !== '/' && pathname?.startsWith(linkPath + '/'));
                            return (
                              <li key={link.id || `admin-link-${link.label}-${Math.random()}`}>
                                <Link
                                  href={linkPath}
                                  className={`flex items-center px-4 py-2 rounded-md transition-colors ${
                                    isActive 
                  ...
                  <div className={`ml-3 overflow-hidden transition-opacity duration-300 ${sidebarCollapsed ? 'opacity-0 w-0' : 'opacity-100'}`}>
                    ...
                    <div className="flex items-center">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-[rgba(var(--primary),0.15)] text-[rgb(var(--primary))]">
                        {(user.role || 'client_user').replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <ul className="space-y-1">
            {links.map((link) => {
              ...
              const isActive = pathname === linkPath || 
                             (linkPath !== '/' && pathname?.startsWith(linkPath));
              return (
                <li key={link.id || `mobile-nav-${link.label}-${Math.random()}`}>
                  <Link
                    href={linkPath}
            ...
            {(isSuperAdmin || adminPortalNavItems.length > 0 || platformAdminNavItems.length > 0) && (
              <li className="mt-4">
                ...
                {/* Admin Submenu - Combined All Admin Sections */}
                <div className={`overflow-hidden transition-all duration-200 ${adminPortalOpen ? 'max-h-[1500px] opacity-100 pt-2' : 'max-h-0 opacity-0'}`}>
                  <ul className="pl-4">
                    {/* Platform Admin Items (For Super Admins) */}
                    {isSuperAdmin && platformAdminNavItems.length > 0 && (
                      <li className="mb-3">
                        ...
                        <ul className="space-y-1 mb-2">
                          {platformAdminNavItems.map((link) => {
                            ...
                            const isActive = pathname === linkPath || 
                                          (linkPath !== '/' && pathname?.startsWith(linkPath));
                            return (
                              <li key={link.id || `client-nav-${link.label}-${Math.random()}`}>
                                <Link
                                  href={linkPath}
                                  className={`flex items-center px-4 py-2 rounded-md transition-colors ${
                                    isActive 
                      ...
                      <li className="mb-3">
                        ...
                        <ul className="space-y-1 mb-2">
                          {adminLinks.filter(link => {
                            ...
                            return (
                              <li key={`extra-nav-${link.label}-${link.href}`}>
                                <Link
                                  href={link.href}
                                  className={`flex items-center px-4 py-2 rounded-md transition-colors ${
                                    isActive 
            ...
            {isSuperAdmin && brandAgentLinks.length > 0 && (
              <li className="mt-4">
                ...
                <div className={`overflow-hidden transition-all duration-200 ${brandAgentsOpen ? 'max-h-96 opacity-100 pt-2' : 'max-h-0 opacity-0'}`}>
                  <ul className="pl-4 space-y-1">
                    {brandAgentLinks.filter(link => {
            ...
            {isSuperAdmin && clientUserLinks.length > 0 && (
              <li className="mt-4">
                ...
                >
                  <div className="flex items-center">
                    <Building size={20} className="flex-shrink-0 text-amber-500" />
                ...
                <div className={`overflow-hidden transition-all duration-200 ${clientUsersOpen ? 'max-h-96 opacity-100 pt-2' : 'max-h-0 opacity-0'}`}>
                  <ul className="pl-4 space-y-1">
                    {clientUserLinks.filter(link => {
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 bg-[rgb(var(--sidebar-background))]/95 backdrop-blur-sm border-t border-[rgb(var(--sidebar-border))] lg:hidden">
        <div className="flex items-center justify-around py-2">
          ...
          {links.slice(0, isSuperAdmin ? 2 : Math.min(links.length, 5)).map((link) => {
            // Ensure we have a path by checking both href and path properties
            const linkPath = link.href || link.path || '/';
            const isActive = pathname === linkPath || 
                          (linkPath !== '/' && pathname?.startsWith(linkPath));
      ...
      {/* Main content */}
      <div className={`flex-1 flex flex-col overflow-hidden pb-16 lg:pb-0 ${sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'} transition-all duration-300`}>
