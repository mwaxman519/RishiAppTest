...
import { serviceHealthMonitor, HealthStatus, ServiceHealth } from '@/services/infrastructure/circuit-breaker/serviceHealthMonitor';
  ...
  const getCircuitStateLabel = (state?: CircuitState) => {
    ...
    switch (state) {
      ...
      case CircuitState.HALF_OPEN:
        return <Badge variant="outline" className="bg-amber-100 text-amber-800 border-amber-200">Half-Open</Badge>;
      ...
      <CardContent className="pb-0">
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-medium">{health.responseTime ? formatDuration(health.responseTime) : 'N/A'}</p>
          </div>
          <div>
...
const SystemHealthSummary = ({ 
  ...
  const unknownCount = services.filter(s => s.status === HealthStatus.UNKNOWN).length;
    ...
    <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <div className="text-3xl font-bold text-red-500">{unhealthyCount}</div>
            <div className="text-sm text-muted-foreground">Unhealthy</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-gray-400">{unknownCount}</div>
...
export function ServiceHealthDashboard() {
  ...
  const filteredServices = Object.entries(health).filter(([_, serviceHealth]) => {
    if (activeTab === 'all') return true;
    if (activeTab === 'healthy') return serviceHealth.status === HealthStatus.HEALTHY;
    if (activeTab === 'degraded') return serviceHealth.status === HealthStatus.DEGRADED;
    if (activeTab === 'unhealthy') return serviceHealth.status === HealthStatus.UNHEALTHY;
    return true;
  });
        ...
        <TabsContent value="all" className="mt-6">
          {loading && Object.keys(health).length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="mt-2 text-muted-foreground">Loading service health data...</p>
            </div>
          ) : filteredServices.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-muted-foreground">No services registered for monitoring</p>
            </div>
          ) : (
