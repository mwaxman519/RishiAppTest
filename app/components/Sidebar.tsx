...
const Sidebar: React.FC<SidebarProps> = ({ items: propItems }) => {
  ...
  if (brandAgentsSection) {
    console.log('Brand Agents section found:', JSON.stringify(brandAgentsSection, null, 2));
    console.log('Brand Agents children count:', brandAgentsSection.children?.length || 0);
    if (brandAgentsSection.children && brandAgentsSection.children.length > 0) {
      console.log('Brand Agents children labels:', brandAgentsSection.children.map(c => c.label));
    }
  } else {
        ...
        {hasChildren && (
          <div className={`nav-section-children ${isOpen ? 'open' : ''}`}>
            ...
            {item.label === 'Brand Agents' ? (
              <>
                {console.log('Sidebar: Brand Agents children rendering:', 
                  JSON.stringify(item.children?.map(c => ({ label: c.label, path: c.path })), null, 2))}
              </>
            ) : null}
            {item.children?.map(child => {
              const childPath = (child.href || child.path || '/') as string;
              if (item.label === 'Brand Agents' && child.label === 'My Availability') {
                console.log('Sidebar: Rendering My Availability item with path:', childPath);
              }
              return (
                <Link
                  key={childPath}
